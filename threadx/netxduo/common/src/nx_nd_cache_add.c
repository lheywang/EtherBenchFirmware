...
/*                                                                        */
/**************************************************************************/
UINT _nx_nd_cache_add(NX_IP *ip_ptr, ULONG *dest_ip, NX_INTERFACE *if_ptr, CHAR *mac, INT IsStatic, INT status,
                      NXD_IPV6_ADDRESS *iface_address, ND_CACHE_ENTRY **nd_cache_entry) {
    ...
    *nd_cache_entry = entry;
    ...
    if (entry->nx_nd_cache_nd_status == ND_CACHE_STATE_REACHABLE) {
        /* Check mac address.*/
        if ((copy_from[0] == copy_to[0]) && (copy_from[1] == copy_to[1]) && (copy_from[2] == copy_to[2])) {
