...
/*                                                                        */
/**************************************************************************/
static UINT _nx_udp_socket_driver_send(NX_UDP_SOCKET *socket_ptr, NX_PACKET *packet_ptr, NXD_ADDRESS *ip_src_address,
...
#ifndef NX_DISABLE_IPV4
        if (ip_dst_address->nxd_ip_version == NX_IP_VERSION_V4) {
            _nx_ip_header_add(ip_ptr, packet_ptr, ip_src_address->nxd_ip_address.v4, ip_dst_address->nxd_ip_address.v4,
                              socket_ptr->nx_udp_socket_type_of_service, socket_ptr->nx_udp_socket_time_to_live,
...
#ifdef FEATURE_NX_IPV6
        if (ip_dst_address->nxd_ip_version == NX_IP_VERSION_V6) {
            if (_nx_ipv6_header_add(ip_ptr, &packet_ptr, NX_PROTOCOL_UDP, packet_ptr->nx_packet_length,
                                    ip_ptr->nx_ipv6_hop_limit, ip_src_address->nxd_ip_address.v6,
    ...
    status = interface_ptr->nx_interface_tcpip_offload_handler(
        ip_ptr, interface_ptr, socket_ptr, NX_TCPIP_OFFLOAD_UDP_SOCKET_SEND, packet_ptr, ip_src_address, ip_dst_address,
...
UINT _nxd_udp_socket_send(NX_UDP_SOCKET *socket_ptr, NX_PACKET *packet_ptr, NXD_ADDRESS *ip_address, UINT port) {
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_UDP_SOCKET_SEND, socket_ptr, packet_ptr, packet_ptr->nx_packet_length,
                            ip_address_log, NX_TRACE_UDP_EVENTS, 0, 0);
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_UDP_SEND, ip_ptr, socket_ptr, packet_ptr,
                            udp_header_ptr->nx_udp_header_word_0, NX_TRACE_INTERNAL_EVENTS, 0, 0);
    ...
    */
    ...
    if ((!socket_ptr->nx_udp_socket_disable_checksum) || (ip_address->nxd_ip_version == NX_IP_VERSION_V6)) {
#ifdef NX_ENABLE_INTERFACE_CAPABILITY
        ...
        */
        {
            ...
            checksum = _nx_ip_checksum_compute(packet_ptr, NX_PROTOCOL_UDP, (UINT)packet_ptr->nx_packet_length,
    ...
    if (ip_address->nxd_ip_version == NX_IP_VERSION_V4) {
        ...
        _nx_ip_packet_send(ip_ptr, packet_ptr, ip_address->nxd_ip_address.v4, socket_ptr->nx_udp_socket_type_of_service,
                           socket_ptr->nx_udp_socket_time_to_live, NX_IP_UDP, socket_ptr->nx_udp_socket_fragment_enable,
                           next_hop_address);
    }
#endif /* NX_DISABLE_IPV4 */
    ...
    if (ip_address->nxd_ip_version == NX_IP_VERSION_V6) {
        ...
        _nx_ipv6_packet_send(ip_ptr, packet_ptr, NX_PROTOCOL_UDP, packet_ptr->nx_packet_length,
