...
VOID _nx_ipv6_packet_send(NX_IP *ip_ptr, NX_PACKET *packet_ptr, ULONG protocol, ULONG payload_size, ULONG hop_limit,
    ...
    /* Add IPv6 header. */
    if (_nx_ipv6_header_add(ip_ptr, &packet_ptr, protocol, payload_size, hop_limit, src_address, dest_address,
                            &fragment) != NX_SUCCESS) {
        ...
        if ((dest_address[0] & (ULONG)0xFF000000) == (ULONG)0xFF000000) {
            ...
            if (status != NX_SUCCESS) {
                ...
                if (_nxd_ipv6_search_onlink(ip_ptr, dest_address)) {
                    ...
                    status = _nx_icmpv6_dest_table_add(ip_ptr, dest_address, &dest_entry_ptr, next_hop_address,
                ...
                if (NDCacheEntry->nx_nd_cache_packet_waiting_head == NX_NULL) {
                    ...
                    if (ip_ptr->nx_ip_icmpv6_packet_process) {
                        ...
                        /* Note that the 2nd last parameter sendUnicast is set to Zero. In
                           ...
                           _nx_icmpv6_send_ns. */
                        _nx_icmpv6_send_ns(ip_ptr, next_hop_address, 1,
                                           packet_ptr->nx_packet_address.nx_packet_ipv6_address_ptr, 0, NDCacheEntry);
