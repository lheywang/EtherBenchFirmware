...
static UINT _nx_tcp_server_socket_driver_unlisten(NX_IP *ip_ptr, NX_TCP_SOCKET *socket_ptr, UINT port) {
    ...
    for (i = 0; i < NX_MAX_IP_INTERFACES; i++) {
        ...
        interface_ptr->nx_interface_tcpip_offload_handler(ip_ptr, interface_ptr, socket_ptr,
                                                          NX_TCPIP_OFFLOAD_TCP_SERVER_SOCKET_UNLISTEN, NX_NULL, NX_NULL,
                                                          NX_NULL, port, NX_NULL, NX_NO_WAIT);
    }
            ...
            if (listen_ptr->nx_tcp_listen_port == port) {
                ...
                if (socket_ptr) {
                    ...
                    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_STATE_CHANGE, ip_ptr, socket_ptr,
                                            socket_ptr->nx_tcp_socket_state, NX_TCP_CLOSED, NX_TRACE_INTERNAL_EVENTS, 0,
                                            0);
                ...
                if (listen_ptr == listen_ptr->nx_tcp_listen_next) {
                    ...
                    (listen_ptr->nx_tcp_listen_next)->nx_tcp_listen_previous = listen_ptr->nx_tcp_listen_previous;
                    (listen_ptr->nx_tcp_listen_previous)->nx_tcp_listen_next = listen_ptr->nx_tcp_listen_next;
