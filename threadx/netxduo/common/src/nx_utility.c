    ...
    *number = 0;
    ...
    while (i < string_length) {
        ...
        if ((input_string[i] >= '0') && (input_string[i] <= '9')) {
            ...
            if (((*number == 429496729) && (input_string[i] > '5')) || (*number >= 429496730)) {
                return (NX_OVERFLOW);
            }
    ...
    while (j < name_size) {
        ...
        if (step == 0) {
            ...
            if ((i + 1) < input_name_size) {
                base64name[j++] =
                    (UCHAR)_nx_utility_base64_array[((((UCHAR)name[i]) & 0x3) << 4) | (((UCHAR)name[i + 1]) >> 4)];
            } else {
            ...
            if ((i + 1) < input_name_size) {
                base64name[j++] =
                    (UCHAR)_nx_utility_base64_array[((((UCHAR)name[i]) & 0xF) << 2) | (((UCHAR)name[i + 1]) >> 6)];
            } else {
...
UINT _nx_utility_base64_decode(UCHAR *base64name, UINT base64name_size, UCHAR *name, UINT name_size,
    ...
    while ((j < base64name_size) && (base64name[i]) && (base64name[i] != '=')) {
        ...
        else if ((base64name[i + 1] == '+') || (base64name[i + 1] == '-')) /* Base64 URL.  */
            value2 = 62;
        else if ((base64name[i + 1] == '/') || (base64name[i + 1] == '_')) /* Base64 URL.  */
            value2 = 63;
        else
            value2 = 0;
