...
VOID _nx_ip_forward_packet_process(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
    ...
    if (_nx_ip_route_find(ip_ptr, ip_header_ptr->nx_ip_header_destination_ip, &outgoing_interface, &next_hop_address) ==
        NX_SUCCESS) {
        ...
        if (compute_checksum) {
            ...
            ip_header_ptr->nx_ip_header_word_2 =
                ((ip_header_ptr->nx_ip_header_word_2 & 0xFFFF0000) | ((~checksum) & NX_LOWER_16_MASK));
#ifdef NX_ENABLE_INTERFACE_CAPABILITY
        } else {
        ...
        if (outgoing_interface->nx_interface_ip_address == destination_ip) {
            ...
            if (packet_ptr->nx_packet_length <=
                ...
                if ((ip_ptr->nx_ip_fragment_processing) && (!(fragment_bit & NX_DONT_FRAGMENT))) {
                    ...
                    _nx_ip_fragment_forward_packet(ip_ptr, packet_ptr, destination_ip, NX_FRAGMENT_OKAY,
                                                   next_hop_address);
                    return;
                }
            }
