...
/*                                                                        */
/**************************************************************************/
static UINT _nxd_tcp_client_socket_driver_connect(NX_TCP_SOCKET *socket_ptr, NXD_ADDRESS *server_ip, UINT server_port,
                                                  ULONG wait_option) {
    UINT status;
    ...
    status = interface_ptr->nx_interface_tcpip_offload_handler(
        ip_ptr, interface_ptr, socket_ptr, NX_TCPIP_OFFLOAD_TCP_CLIENT_SOCKET_CONNECT, NX_NULL, NX_NULL, server_ip,
...
UINT _nxd_tcp_client_socket_connect(NX_TCP_SOCKET *socket_ptr, NXD_ADDRESS *server_ip, UINT server_port,
...
#ifndef NX_DISABLE_IPV4
    if (server_ip->nxd_ip_version == NX_IP_VERSION_V4) {
        if (_nx_ip_route_find(ip_ptr, server_ip->nxd_ip_address.v4, &outgoing_interface,
    ...
    if (server_ip->nxd_ip_version == NX_IP_VERSION_V6) {
        ...
        status = _nxd_ipv6_interface_find(ip_ptr, server_ip->nxd_ip_address.v6, &socket_ptr->nx_tcp_socket_ipv6_addr,
                                          NX_NULL);
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_TCP_CLIENT_SOCKET_CONNECT, ip_ptr, socket_ptr, ip_address_log, server_port,
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_STATE_CHANGE, ip_ptr, socket_ptr, socket_ptr->nx_tcp_socket_state,
...
#ifdef FEATURE_NX_IPV6
    if (server_ip->nxd_ip_version == NX_IP_VERSION_V6) {
        COPY_IPV6_ADDRESS(server_ip->nxd_ip_address.v6, socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_address.v6);
    ...
    if (socket_ptr->nx_tcp_socket_tx_sequence == 0) {
        ...
        socket_ptr->nx_tcp_socket_tx_sequence =
            socket_ptr->nx_tcp_socket_tx_sequence + ((ULONG)(((ULONG)0x10000))) + ((ULONG)NX_RAND());
    }
