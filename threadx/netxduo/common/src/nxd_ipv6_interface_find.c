...
UINT _nxd_ipv6_interface_find(NX_IP *ip_ptr, ULONG *dest_address, NXD_IPV6_ADDRESS **ipv6_addr, NX_INTERFACE *if_ptr) {
        ...
        for (i = 0; i < NX_MAX_MULTICAST_GROUPS; i++) {
            ...
            if (CHECK_IPV6_ADDRESSES_SAME(ip_ptr->nx_ipv6_multicast_entry[i].nx_ip_mld_join_list, dest_address)) {
                *ipv6_addr = ip_ptr->nx_ipv6_multicast_entry[i]
                                 .nx_ip_mld_join_interface_list->nxd_interface_ipv6_address_list_head;
    ...
    for (i = start_index; i < end_index; i++) {
        ...
        for (ipv6_address = ip_ptr->nx_ip_interface[i].nxd_interface_ipv6_address_list_head; ipv6_address;
