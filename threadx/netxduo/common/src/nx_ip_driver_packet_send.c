...
VOID _nx_ip_driver_packet_send(NX_IP *ip_ptr, NX_PACKET *packet_ptr, ULONG destination_ip, ULONG fragment,
    ...
    if (packet_ptr->nx_packet_address.nx_packet_interface_ptr->nx_interface_address_mapping_needed) {
        ...
        if ((destination_ip == NX_IP_LIMITED_BROADCAST) ||
            (((destination_ip & network_mask) == network) && ((destination_ip & ~network_mask) == ~network_mask))) {
        ...
        /* Determine if we have a class D multicast address.  */
            ...
            if ((arp_ptr) && (arp_ptr->nx_arp_physical_address_msw | arp_ptr->nx_arp_physical_address_lsw)) {
                ...
                if (packet_ptr->nx_packet_length >
                    packet_ptr->nx_packet_address.nx_packet_interface_ptr->nx_interface_ip_mtu_size) {
                ...
                if (arp_ptr) {
                    ...
                    (ip_ptr->nx_ip_arp_packet_send)(ip_ptr, destination_ip,
        ...
        /* Determine if we have a loopback address.  */
        if ((((destination_ip >= NX_IP_LOOPBACK_FIRST) && (destination_ip <= NX_IP_LOOPBACK_LAST))) ||
    ...
    if (driver_request.nx_ip_driver_interface) {
        ...
        NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_IO_DRIVER_PACKET_SEND, ip_ptr, packet_ptr,
