...
VOID _nx_ipv4_packet_receive(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_IP_RECEIVE, ip_ptr, ip_header_ptr->nx_ip_header_source_ip, packet_ptr,
                            packet_ptr->nx_packet_length, NX_TRACE_INTERNAL_EVENTS, 0, 0);
    ...
    if (if_ptr->nx_interface_address_mapping_needed == NX_TRUE) {
        if (((ip_header_ptr->nx_ip_header_source_ip & ~(if_ptr->nx_interface_ip_network_mask)) ==
    ...
    if (ip_header_length > NX_IP_NORMAL_LENGTH) {
        ...
        *word_ptr = (ULONG)(((ip_header_ptr->nx_ip_header_word_0) & (~NX_IP_LENGTH_MASK)) | NX_IP_VERSION);
        ...
        packet_ptr->nx_packet_length =
            packet_ptr->nx_packet_length - ((ip_header_length - NX_IP_NORMAL_LENGTH) * (ULONG)sizeof(ULONG));
    }
    ...
    if ((ip_header_ptr->nx_ip_header_destination_ip == if_ptr->nx_interface_ip_address) ||
        ...
        if (ip_header_ptr->nx_ip_header_word_1 & NX_IP_FRAGMENT_MASK) {
            ...
            if (ip_ptr->nx_ip_fragment_assembly && (packet_ptr->nx_packet_pool_owner->nx_packet_pool_available >=
