...
UINT _nx_icmpv6_process_packet_too_big(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
    ...
    if ((mtu > 0) && (mtu <= if_ptr->nx_interface_ip_mtu_size)) {
        ...
        status =
            _nx_icmpv6_dest_table_add(ip_ptr, original_destination_ip, &dest_entry_ptr, &default_next_hop_address[0],
                                      mtu, 0, packet_ptr->nx_packet_address.nx_packet_ipv6_address_ptr);
    }
