...
/*                                                                        */
/**************************************************************************/
VOID _nx_icmpv6_send_error_message(NX_IP *ip_ptr, NX_PACKET *offending_packet, ULONG word1, ULONG error_pointer) {
    ...
    if (((INT)((bytes_to_copy + sizeof(NX_IPV6_HEADER) + sizeof(NX_ICMPV6_ERROR) + NX_PHYSICAL_HEADER) - payload)) >
        0) {
    ...
    if (IPv6_Address_Type(dest_ip) & IPV6_ADDRESS_MULTICAST) {
        ...
        if (_nxd_ipv6_interface_find(ip_ptr, dest_ip, &pkt_ptr->nx_packet_address.nx_packet_ipv6_address_ptr,
                                     NX_NULL)) {
    ...
    if (pkt_ptr->nx_packet_address.nx_packet_ipv6_address_ptr->nxd_ipv6_address_attached->nx_interface_capability_flag &
        ...
        */
    {
        ...
        checksum =
            _nx_ip_checksum_compute(pkt_ptr, NX_PROTOCOL_ICMPV6, (UINT)pkt_ptr->nx_packet_length,
                                    pkt_ptr->nx_packet_address.nx_packet_ipv6_address_ptr->nxd_ipv6_address, src_ip);
