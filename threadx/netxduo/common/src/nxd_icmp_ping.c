...
UINT _nxd_icmp_ping(NX_IP *ip_ptr, NXD_ADDRESS *ip_address, CHAR *data_ptr, ULONG data_size, NX_PACKET **response_ptr,
    ...
    if (ip_address->nxd_ip_version == NX_IP_VERSION_V4) {
        status = _nx_icmp_ping(ip_ptr, ip_address->nxd_ip_address.v4, data_ptr, data_size, response_ptr, wait_option);
    }
#endif /* !NX_DISABLE_IPV4  */

#ifdef FEATURE_NX_IPV6
    if (ip_address->nxd_ip_version == NX_IP_VERSION_V6) {
        status = _nx_icmp_ping6(ip_ptr, ip_address, data_ptr, data_size, response_ptr, wait_option);
    }
