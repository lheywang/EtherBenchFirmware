...
VOID _nx_tcp_packet_process(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
    ...
    if (compute_checksum)
        ...
        */
    {
        checksum = _nx_ip_checksum_compute(packet_ptr, NX_PROTOCOL_TCP, (UINT)packet_ptr->nx_packet_length, source_ip,
                                           dest_ip);
    ...
    if (((INT)option_words < 0) || ((UINT)(packet_ptr->nx_packet_append_ptr - packet_ptr->nx_packet_prepend_ptr) <
    ...
    if (option_words) {
        ...
        status = _nx_tcp_mss_option_get((packet_ptr->nx_packet_prepend_ptr + sizeof(NX_TCP_HEADER)),
...
#ifdef NX_ENABLE_TCP_WINDOW_SCALING
        status = _nx_tcp_window_scaling_option_get((packet_ptr->nx_packet_prepend_ptr + sizeof(NX_TCP_HEADER)),
    ...
    if (socket_ptr) {
        ...
        do {
            ...
            if ((socket_ptr->nx_tcp_socket_port == port) && (socket_ptr->nx_tcp_socket_connect_port == source_port)) {
...
#ifdef FEATURE_NX_IPV6
                    if (packet_ptr->nx_packet_ip_version == NX_IP_VERSION_V6) {
                        if (CHECK_IPV6_ADDRESSES_SAME(socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_address.v6,
                                                      source_ip)) {
                            find_a_match = 1;
                        }
                    }
    ...
    /* Handle new connection requests without ACK bit in NX_TCP_SYN_RECEIVED
       ...
       Section3.9, Page65. */
    if ((!(tcp_header_ptr->nx_tcp_header_word_3 & NX_TCP_ACK_BIT)) && (ip_ptr->nx_ip_tcp_active_listen_requests)) {
        ...
        do {
            ...
            if (listen_ptr->nx_tcp_listen_port == port) {
                ...
                if ((listen_ptr->nx_tcp_listen_socket_ptr) &&
                    ...
                    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_SYN_RECEIVE, ip_ptr, socket_ptr, packet_ptr,
                    ...
                    if (socket_ptr->nx_tcp_socket_state == NX_TCP_SYN_RECEIVED) {
                        ...
                        /* If trace is enabled, insert this event into the trace buffer.  */
                        NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_STATE_CHANGE, ip_ptr, socket_ptr,
                                                socket_ptr->nx_tcp_socket_state, socket_ptr->nx_tcp_socket_state,
                    ...
                    if (!(tcp_header_ptr->nx_tcp_header_word_3 & NX_TCP_RST_BIT)) {
                        ...
                        /* If trace is enabled, insert this event into the trace buffer.  */
                        NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_SYN_RECEIVE, ip_ptr, NX_NULL, packet_ptr,
                                                tcp_header_ptr->nx_tcp_sequence_number, NX_TRACE_INTERNAL_EVENTS, 0, 0);
                    }
