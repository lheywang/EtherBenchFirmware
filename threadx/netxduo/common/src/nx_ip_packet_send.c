...
VOID _nx_ip_packet_send(NX_IP *ip_ptr, NX_PACKET *packet_ptr, ULONG destination_ip, ULONG type_of_service,
    ...
    if (packet_ptr->nx_packet_ipsec_sa_ptr && packet_ptr->nx_packet_ipsec_state != NX_IPSEC_ESP_PACKET &&
        packet_ptr->nx_packet_ipsec_state != NX_IPSEC_AH_PACKET &&
        ((NX_IPSEC_SA *)(packet_ptr->nx_packet_ipsec_sa_ptr))->nx_ipsec_sa_mode == NX_IPSEC_TRANSPORT_MODE) {
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_IP_SEND, ip_ptr, destination_ip, packet_ptr, packet_ptr->nx_packet_length,
    ...
    if (packet_ptr->nx_packet_ipsec_sa_ptr && packet_ptr->nx_packet_ipsec_state == NX_IPSEC_AH_PACKET) {
    ...
    if (next_hop_address == 0) {
        ...
        if (ip_ptr->nx_ip_forward_packet_process) {
            ...
            _nx_ip_route_find(ip_ptr, destination_ip, &packet_ptr->nx_packet_address.nx_packet_interface_ptr,
                              &next_hop_address);
        }
