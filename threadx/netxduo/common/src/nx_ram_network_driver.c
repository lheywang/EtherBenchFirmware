...
#ifdef NX_ENABLE_VLAN
        /* Add Ethernet header.  */
        if (nx_link_ethernet_header_add(ip_ptr, driver_req_ptr->nx_ip_driver_interface->nx_interface_index, packet_ptr,
                                        driver_req_ptr->nx_ip_driver_physical_address_msw,
                                        driver_req_ptr->nx_ip_driver_physical_address_lsw, (UINT)ether_type)) {
        ...
        if (((destination_address_msw == ((ULONG)0x0000FFFF)) &&
            ...
            if (nx_packet_copy(packet_ptr, &packet_copy, next_ip->nx_ip_default_packet_pool, NX_NO_WAIT)) {
                ...
                nx_link_packet_transmitted(
                    ...
                    nx_ram_driver[interface_instance_id].nx_ram_driver_interface_ptr->nx_interface_index, packet_ptr,
                    NX_NULL);
#else
...
void _nx_ram_network_driver_receive(NX_IP *ip_ptr, NX_PACKET *packet_ptr, UINT interface_instance_id) {
    ...
    nx_link_ethernet_packet_received(
        ip_ptr, nx_ram_driver[interface_instance_id].nx_ram_driver_interface_ptr->nx_interface_index, packet_ptr,
        NX_NULL);
#else
    UINT packet_type;
