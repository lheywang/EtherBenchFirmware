...
UINT _nx_arp_static_entry_delete(NX_IP *ip_ptr, ULONG ip_address, ULONG physical_msw, ULONG physical_lsw) {
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_ARP_STATIC_ENTRY_DELETE, ip_ptr, ip_address, physical_msw, physical_lsw,
    ...
    while (arp_entry) {
        ...
        if ((arp_entry->nx_arp_ip_address == ip_address) && (arp_entry->nx_arp_physical_address_msw == physical_msw) &&
