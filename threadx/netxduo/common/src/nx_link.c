...
/*                                                                        */
/**************************************************************************/
UINT nx_link_multicast_join(NX_IP *ip_ptr, UINT interface_index, ULONG physical_address_msw,
...
/*                                                                        */
/**************************************************************************/
UINT nx_link_multicast_leave(NX_IP *ip_ptr, UINT interface_index, ULONG physical_address_msw,
                             ULONG physical_address_lsw) {
    NX_IP_DRIVER driver_request;
...
UINT nx_link_ethernet_packet_send(NX_IP *ip_ptr, UINT interface_index, NX_PACKET *packet_ptr,
    ...
    /* Add Ethernet header.  */
    status = nx_link_ethernet_header_add(ip_ptr, interface_index, packet_ptr, physical_address_msw,
...
UINT nx_link_packet_receive_callback_add(NX_IP *ip_ptr, UINT interface_index, NX_LINK_RECEIVE_QUEUE *queue_ptr,
    ...
    /* Check for invalid input pointers.  */
    if ((ip_ptr == NX_NULL) || (ip_ptr->nx_ip_id != NX_IP_ID) || (queue_ptr == NX_NULL) || (callback_ptr == NX_NULL)) {
        return (NX_PTR_ERROR);
    }
...
/*                                                                        */
/**************************************************************************/
UINT nx_link_ethernet_header_add(NX_IP *ip_ptr, UINT interface_index, NX_PACKET *packet_ptr, ULONG physical_address_msw,
    ...
    if (interface_ptr->nx_interface_vlan_valid) {
        ...
        *(ethernet_frame_ptr) |= NX_LINK_ETHERNET_TPID;
        ...
        if (packet_ptr->nx_packet_vlan_priority != NX_VLAN_PRIORITY_INVALID) {
            vlan_tag =
                (USHORT)((interface_ptr->nx_interface_vlan_tag & (~NX_LINK_VLAN_PCP_MASK)) |
...
VOID nx_link_ethernet_packet_received(NX_IP *ip_ptr, UINT interface_index, NX_PACKET *packet_ptr,
    ...
    nx_link_ethernet_header_parse(packet_ptr, &physical_address_msw, &physical_address_lsw, NULL, NULL, &packet_type,
    ...
    if ((packet_type == NX_LINK_ETHERNET_IP) || (packet_type == NX_LINK_ETHERNET_IPV6)) {
        ...
        while (queue_ptr) {
            ...
            if ((queue_ptr->packet_type == packet_type) || (queue_ptr->packet_type == NX_LINK_PACKET_TYPE_ALL)) {
                ...
                if (queue_ptr->callback(ip_ptr, interface_index, packet_ptr, physical_address_msw, physical_address_lsw,
...
UINT nx_link_vlan_interface_create(NX_IP *ip_ptr, CHAR *interface_name, ULONG ip_address, ULONG network_mask,
    ...
    memcpy(interface_ptr, &(ip_ptr->nx_ip_interface[parent_interface_index]), /* Use case of memcpy
                                                                                 is verified. */
           sizeof(NX_INTERFACE));
...
void nx_link_vlan_interface_status_change(NX_IP *ip_ptr, UINT interface_index) {
    ...
    for (i = 0; i < NX_MAX_PHYSICAL_INTERFACES; i++) {
        ...
        if ((nx_interface->nx_interface_valid) && (nx_interface->nx_interface_parent_ptr != NX_NULL) &&
