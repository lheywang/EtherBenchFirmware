...
VOID _nx_ip_thread_entry(ULONG ip_ptr_value) {
    ...
    for (i = 0; i < NX_MAX_PHYSICAL_INTERFACES; i++) {
        ...
        if ((ip_ptr->nx_ip_interface[i].nx_interface_valid) &&
            ...
            NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_IO_DRIVER_INITIALIZE, ip_ptr, 0, 0, 0, NX_TRACE_INTERNAL_EVENTS,
                                    0, 0);
            ...
            /* If trace is enabled, insert this event into the trace buffer.  */
            NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_IO_DRIVER_LINK_ENABLE, ip_ptr, 0, 0, 0, NX_TRACE_INTERNAL_EVENTS,
                                    0, 0);
                ...
                for (i = 0; i < NX_MAX_PHYSICAL_INTERFACES; i++) {
                    ...
                    if (interface_ipv6_address &&
                        ...
                        /* Note that the 2nd last parameter sendUnicast is set to Zero. In
                           ...
                           _nx_icmpv6_send_ns. */
                        _nx_icmpv6_send_ns(ip_ptr, interface_ipv6_address->nxd_ipv6_address, 0, interface_ipv6_address,
                                           0, &ip_ptr->nx_ipv6_nd_cache[0]);
            ...
            for (i = 0; i < NX_MAX_PHYSICAL_INTERFACES; i++) {
                ...
                if (ip_ptr->nx_ip_interface[i].nx_interface_valid) {
                    ...
                    driver_request.nx_ip_driver_physical_address_lsw =
                        NX_IP_MULTICAST_LOWER | (NX_ALL_HOSTS_ADDRESS & NX_IP_MULTICAST_MASK);
                    driver_request.nx_ip_driver_interface = &(ip_ptr->nx_ip_interface[i]);

                    /* If trace is enabled, insert this event into the trace buffer.  */
                    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_IO_DRIVER_MULTICAST_JOIN, ip_ptr, 0, 0, 0,
