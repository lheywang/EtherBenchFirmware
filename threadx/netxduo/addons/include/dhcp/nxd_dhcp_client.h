...
/* Enables DHCP Client send Maximum DHCP Message Size Option. RFC2132, Section9.10,
Page29. #define NX_DHCP_CLIENT_SEND_MAX_DHCP_MESSAGE_OPTION
*/

/* Defined, the host name is checked, the host name must follow the rules for ARPANET host
names. RFC 1035, Section 2.3.1, Page 8.  The default is disabled. #define
...
/* Define the DHCP stack priority. This priority must be high enough to insure the
   ...
   needs to initialize the network driver (which is required to be able to transmit
   packets). */
...
/* Define the max number of user request parameter.
   Subnet mask, gateway and dns server options are added in _nx_dhcp_request_parameters
   arrary by default.  */
...
/* Define the minimum and maximum variation in the interval between ARP probe
 * transmissions. */
...
/*  Define the wait time before restarting the configuration process when DHCP detects
   that the address is already in use.

    The client SHOULD wait a minimum of ten seconds before restarting the configuration
   process to avoid excessive network traffic in case of looping. RFC2131, Section 3.1,
   Page 17.  */
#ifndef NX_DHCP_RESTART_WAIT
#define NX_DHCP_RESTART_WAIT (10 * NX_IP_PERIODIC_RATE)
#endif

/* Define the BootP Message Area Offsets.  The DHCP message format is identical to that of
...
#define NX_BOOTP_OFFSET_OP                                                               \
    0 /* 1 BootP Operation 1=req, 2=reply                         */
...
#define NX_BOOTP_OFFSET_GATEWAY_IP                                                       \
    24 /* 4 gateway IP, usually 0.0.0.0, only for BootP and TFTP   */
...
#define NX_DHCP_OPTION_CLIENT_ID_SIZE                                                    \
    7 /* 1 byte for address type (01 = Ethernet), 6 bytes for address */
...
#define NX_DHCP_STATE_ADDRESS_PROBING                                                    \
    9 /* Address probing, address conflict detection              */

/* Define error codes from DHCP API.  */
#define NX_DHCP_ERROR 0x90 /* General DHCP error code */
#define NX_DHCP_NO_RESPONSE                                                              \
    0x91 /* No response from server for option request               */
#define NX_DHCP_BAD_IP_ADDRESS                                                           \
    0x92 /* Bad IP address or invalid interface input                */
#define NX_DHCP_ALREADY_STARTED                                                          \
    0x93 /* DHCP was already started                                 */
#define NX_DHCP_NOT_BOUND                                                                \
    0x94 /* DHCP is not in a bound state                             */
#define NX_DHCP_DEST_TO_SMALL                                                            \
    0x95 /* DHCP response is too big for destination                 */
#define NX_DHCP_NOT_STARTED                                                              \
    0x96 /* DHCP was not started when stop was issued                */
#define NX_DHCP_PARSE_ERROR                                                              \
    0x97 /* Error extracting DHCP option data                        */
#define NX_DHCP_BAD_XID                                                                  \
    0x98 /* DHCP packet received with mismatched XID                 */
#define NX_DHCP_BAD_MAC_ADDRESS                                                          \
    0x99 /* DHCP packet received with mismatched MAC address         */
#define NX_DHCP_INVALID_MESSAGE                                                          \
    0x9B /* Invalid message received or requested to send            */
#define NX_DHCP_INVALID_PAYLOAD                                                          \
    0x9C /* Client receives DHCP message exceeding packet payload    */
#define NX_DHCP_INVALID_IP_REQUEST                                                       \
    0x9D /* Null IP address input for requesting IP address          */
#define NX_DHCP_UNKNOWN_OPTION                                                           \
    0x9F /* This option is unknow.                                   */
#define NX_DHCP_INTERFACE_ALREADY_ENABLED                                                \
    0xA3 /* Interface is already enabled                             */
#define NX_DHCP_INTERFACE_NOT_ENABLED                                                    \
    0xA4 /* If interface not enabled for DHCP interaction            */
#define NX_DHCP_NO_INTERFACES_ENABLED                                                    \
    0xA5 /* No interfaces enabled for DHCP interaction               */
#define NX_DHCP_NO_INTERFACES_STARTED                                                    \
    0xA6 /* If DHCP CLient fails to start any interfacers            */
#define NX_DHCP_NO_RECORDS_AVAILABLE                                                     \
    0xA7 /* No Client record available to start DHCP on an interface */
#define NX_DHCP_INVALID_NAME                                                             \
    0xA8 /* Client host name has invalid characters                  */

/* Define DHCP Client thread events.  */
#define NX_DHCP_CLIENT_RECEIVE_EVENT                                                     \
    0x00000001 /* DHCP Server data received                            */
#define NX_DHCP_CLIENT_TIMER_EVENT                                                       \
    0x00000002 /* DHCP timer expires                                   */
#define NX_DHCP_CLIENT_CONFLICT_EVENT                                                    \
    0x00000004 /* IP conflict event detected                           */
#define NX_DHCP_CLIENT_ALL_EVENTS                                                        \
    0xFFFFFFFF /* All DHCP events                                      */

#ifdef NX_DHCP_CLIENT_RESTORE_STATE

/* Define a Client record for restore DHCP Client state from non volatile memory/across
 * reboots. */
typedef struct NX_DHCP_CLIENT_RECORD_STRUCT {
    ...
    ULONG nx_dhcp_renewal_remain_time; /* Time remaining to renew (before rebinding
                                          necessary)     */
    ULONG
    nx_dhcp_rebind_remain_time; /* Time remaining to rebind (before lease expires) */
...
typedef struct NX_DHCP_INTERFACE_RECORD_STRUCT {
    UCHAR
    nx_dhcp_record_valid; /* The flag indicate this record is valid. NX_TRUE: valid */
    UCHAR nx_dhcp_state;  /* The current state of the DHCP Client                     */
...
#ifdef NX_DHCP_CLIENT_SEND_ARP_PROBE
    UINT
        nx_dhcp_probe_count; /* Number of ARP probes to send to prove IP address unique */
#endif
    ...
    ULONG nx_dhcp_force_renewal_rec; /* The number of Forced Renewal received by the
                                        Client      */
    ULONG nx_dhcp_informs_sent;      /* The number of Inform (option requests) sent by the
                                        Client*/
...
typedef struct NX_DHCP_STRUCT {
...
#ifdef NX_DHCP_CLIENT_SEND_ARP_PROBE
    UINT
        nx_dhcp_interface_conflict_flag; /* The flag indicate IP addresses conflict on
                                            which interfaces, one bit represent one
                                            interface. For examples: 0x00000001: interface
                                            index 0; 0x00000002: interface index 1;
    ...
    VOID (*nx_dhcp_state_change_callback)(
        struct NX_DHCP_STRUCT *dhcp_ptr, UCHAR new_state);
    ...
    /* Define the callback function for DHCP interface state change notification.
       ...
       Note: Suggest using state change notification, and use
       nx_dhcp_interface_state_change_callback if DHCP is running on multiple interfaces.
     */
    VOID (*nx_dhcp_interface_state_change_callback)(
        struct NX_DHCP_STRUCT *dhcp_ptr, UINT iface_index, UCHAR new_state);

    /* Define the callback function for adding specific DHCP user option.  */
    UINT (*nx_dhcp_user_option_add)(
        struct NX_DHCP_STRUCT *dhcp_ptr,
        UINT iface_index,
        UINT message_type,
...
UINT nx_dhcp_request_client_ip(
    NX_DHCP *dhcp_ptr, ULONG client_ip_address, UINT skip_discover_message);
...
UINT nx_dhcp_user_option_add_callback_set(
    NX_DHCP *dhcp_ptr,
...
UINT nx_dhcp_client_restore_record(
    NX_DHCP *dhcp_ptr, NX_DHCP_CLIENT_RECORD *record_ptr, ULONG time_elapsed);
UINT nx_dhcp_client_update_time_remaining(NX_DHCP *dhcp_ptr, ULONG time_elapsed);
UINT nx_dhcp_client_interface_get_record(
    NX_DHCP *dhcp_ptr, UINT iface_index, NX_DHCP_CLIENT_RECORD *record_ptr);
UINT nx_dhcp_client_interface_restore_record(
    NX_DHCP *dhcp_ptr,
    UINT iface_index,
    NX_DHCP_CLIENT_RECORD *record_ptr,
    ULONG time_elapsed);
UINT nx_dhcp_client_interface_update_time_remaining(
