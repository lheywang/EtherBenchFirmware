...
#define NX_FTP_ACTIVITY_TIMEOUT                                                          \
    240 /* Seconds allowed with no activity                    */
#endif
...
#define NX_FTP_SERVER_RETRY_SECONDS                                                      \
    2 /* 2 second initial timeout                            */
#endif

#ifndef NX_FTP_SERVER_TRANSMIT_QUEUE_DEPTH
#define NX_FTP_SERVER_TRANSMIT_QUEUE_DEPTH                                               \
    20 /* Maximum of 20 queued transmit packets               */
#endif

#ifndef NX_FTP_SERVER_RETRY_MAX
#define NX_FTP_SERVER_RETRY_MAX                                                          \
    10 /* Maximum of 10 retries per packet                    */
#endif

#ifndef NX_FTP_SERVER_RETRY_SHIFT
#define NX_FTP_SERVER_RETRY_SHIFT                                                        \
    1 /* Every retry is twice as long                        */
#endif

/* The minimum FTP Server packet size must accomodate the largest expected packet sent
   e.g. the LIST command: The filename can be up to 256 bytes, plus file info (which has a
   fixed size of 52 bytes). Note the 256 value is the assumed max size of a filename,
   defined by FX_MAX_LONG_NAME_LEN.
                                                                                            .
   File download (data) packets can be split up over multiple packets, but commands should
   be in one packet. */
...
#define NX_FTP_OPEN_FOR_READ                                                             \
    0x01 /* FTP file open for reading                           */
#define NX_FTP_OPEN_FOR_WRITE                                                            \
    0x02 /* FTP file open for writing                           */

/* Define transfer modes. Note: just support stream mode and block mode yet.  */

#define NX_FTP_TRANSFER_MODE_STREAM                                                      \
    0 /* FTP stream transfer mode                            */
#define NX_FTP_TRANSFER_MODE_BLOCK                                                       \
    1 /* FTP block transfer mode                             */
#define NX_FTP_TRANSFER_MODE_COMPRESSED                                                  \
    2 /* FTP compressed transfer mode                        */

/* Define Server thread events.  */

#define NX_FTP_SERVER_CONNECT                                                            \
    0x01 /* FTP connection is present                           */
#define NX_FTP_SERVER_DATA_DISCONNECT                                                    \
    0x02 /* FTP data disconnection is present                   */
#define NX_FTP_SERVER_COMMAND                                                            \
    0x04                        /* FTP client command is present                       */
#define NX_FTP_SERVER_DATA 0x08 /* FTP client data is present */
#define NX_FTP_SERVER_ACTIVITY_TIMEOUT                                                   \
    0x10 /* FTP activity timeout check                          */
#define NX_FTP_SERVER_CONTROL_DISCONNECT                                                 \
    0x20                       /* FTP client disconnect of control socket             */
...
#define NX_FTP_NOT_CONNECTED                                                             \
    0xD3 /* FTP not connected error                             */
#define NX_FTP_NOT_DISCONNECTED                                                          \
    0xD4                        /* FTP not disconnected error                          */
...
#define NX_FTP_END_OF_LISTING                                                            \
    0xD8 /* FTP end of directory listing status                 */
#define NX_FTP_EXPECTED_1XX_CODE                                                         \
    0xD9 /* Expected a 1xx response from server                 */
#define NX_FTP_EXPECTED_2XX_CODE                                                         \
    0xDA /* Expected a 2xx response from server                 */
#define NX_FTP_EXPECTED_22X_CODE                                                         \
    0xDB /* Expected a 22x response from server                 */
#define NX_FTP_EXPECTED_23X_CODE                                                         \
    0xDC /* Expected a 23x response from server                 */
#define NX_FTP_EXPECTED_3XX_CODE                                                         \
    0xDD /* Expected a 3xx response from server                 */
#define NX_FTP_EXPECTED_33X_CODE                                                         \
    0xDE /* Expected a 33x response from server                 */
#define NX_FTP_INVALID_NUMBER                                                            \
    0xDF /* Extraced an invalid number from server response     */
#define NX_FTP_INVALID_ADDRESS                                                           \
    0x1D0 /* Invalid IP address parsed from FTP command          */
#define NX_FTP_INVALID_COMMAND                                                           \
    0x1D1 /* Invalid FTP command (bad syntax, unknown command)   */
#define NX_FTP_INVALID_LOGIN                                                             \
    0x1D2 /* Unable to perform successful user login             */
#define NX_FTP_INSUFFICIENT_PACKET_PAYLOAD                                               \
    0x1D3 /* Packet payload less than the max length filename    */
#define NX_FTP_SERVER_INVALID_SIZE                                                       \
    0x1D4 /* Invalid FTP file size                               */
#define NX_FTP_SERVER_END_OF_BLOCK                                                       \
    0x1D5 /* FTP end of block                                    */
...
#define NX_FTP_STATE_WRITE_OPEN                                                          \
    4 /* FTP file open for writing                           */

/* Define the FTP Server TCP port numbers.  */

#define NX_FTP_SERVER_CONTROL_PORT                                                       \
    21 /* Control Port for FTP server                         */
#define NX_FTP_SERVER_DATA_PORT                                                          \
    20 /* Data Port for FTP server                            */

/* Define the size for buffer to store an IPv6 address represented in ASCII. */
#define NX_FTP_IPV6_ADDRESS_BUFSIZE 60

/* Define the FTP basic commands.  The ASCII command will be parsed and converted to the
   numerical representation shown below.  */
...
typedef struct NX_FTP_CLIENT_REQUEST_STRUCT {
    ...
    CHAR nx_ftp_client_request_passive_transfer_enabled; /* Client enabled for passive
                                                            transfer */
...
typedef struct NX_FTP_SERVER_STRUCT {
    ...
    struct NX_FTP_SERVER_STRUCT *nx_ftp_next_server_ptr, *nx_ftp_previous_server_ptr;
} NX_FTP_SERVER;
...
UINT _nx_ftp_server_block_header_retrieve(
    NX_FTP_CLIENT_REQUEST *client_request_ptr, NX_PACKET *packet_ptr);
