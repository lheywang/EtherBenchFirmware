...
static UINT _nxd_mqtt_client_create_internal(
...
static UINT _nxd_mqtt_packet_send(
    NXD_MQTT_CLIENT *client_ptr, NX_PACKET *packet_ptr, UINT wait_option);
static UINT _nxd_mqtt_packet_receive(
    NXD_MQTT_CLIENT *client_ptr, NX_PACKET **packet_ptr, UINT wait_option);
static UINT _nxd_mqtt_copy_transmit_packet(
    NXD_MQTT_CLIENT *client_ptr,
    NX_PACKET *packet_ptr,
    NX_PACKET **new_packet_ptr,
    USHORT packet_id,
    UCHAR set_duplicate_flag,
    UINT wait_option);
static VOID _nxd_mqtt_release_transmit_packet(
    NXD_MQTT_CLIENT *client_ptr, NX_PACKET *packet_ptr, NX_PACKET *previous_packet_ptr);
static VOID _nxd_mqtt_release_receive_packet(
    NXD_MQTT_CLIENT *client_ptr, NX_PACKET *packet_ptr, NX_PACKET *previous_packet_ptr);
static UINT
_nxd_mqtt_client_retransmit_message(NXD_MQTT_CLIENT *client_ptr, ULONG wait_option);
static UINT
_nxd_mqtt_client_connect_packet_send(NXD_MQTT_CLIENT *client_ptr, ULONG wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_set_fixed_header(
    NXD_MQTT_CLIENT *client_ptr,
    NX_PACKET *packet_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_read_remaining_length(
    NX_PACKET *packet_ptr, UINT *remaining_length, ULONG *offset_ptr) {
    UINT value = 0;
    UCHAR bytes[4] = {0};
    UINT multiplier = 1;
    UINT byte_count = 0;
    ...
    if (nx_packet_data_extract_offset(
            packet_ptr, 1, &bytes, sizeof(bytes), &bytes_copied)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_sub_unsub(
    NXD_MQTT_CLIENT *client_ptr,
    UINT op,
    CHAR *topic_name,
    UINT topic_name_length,
    USHORT *packet_id_ptr,
    UINT QoS) {
    ...
    ret = _nxd_mqtt_client_set_fixed_header(
        client_ptr, packet_ptr, (UCHAR)op, length, NX_WAIT_FOREVER);
    ...
    /* Append topic name */
    ret = _nxd_mqtt_client_append_message(
        client_ptr, packet_ptr, topic_name, topic_name_length, NX_WAIT_FOREVER);
    ...
    if (_nxd_mqtt_copy_transmit_packet(
            client_ptr,
            packet_ptr,
            &transmit_packet_ptr,
    ...
    if (client_ptr->message_transmit_queue_head == NX_NULL) {
        ...
        client_ptr->message_transmit_queue_tail->nx_packet_queue_next =
            transmit_packet_ptr;
    }
    client_ptr->message_transmit_queue_tail = transmit_packet_ptr;

    client_ptr->nxd_mqtt_client_packet_identifier =
        (client_ptr->nxd_mqtt_client_packet_identifier + 1) & 0xFFFF;
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_packet_allocate(
    NXD_MQTT_CLIENT *client_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {
    UINT status = NXD_MQTT_SUCCESS;

#ifdef NXD_MQTT_OVER_WEBSOCKET
    if (client_ptr->nxd_mqtt_client_use_websocket) {

        /* Use WebSocket packet allocate since it is able to count for WebSocket-related
         * header space */
        status = nx_websocket_client_packet_allocate(
            &(client_ptr->nxd_mqtt_client_websocket), packet_ptr, wait_option);
    } else
        ...
        status = nx_secure_tls_packet_allocate(
            &client_ptr->nxd_mqtt_tls_session,
            client_ptr->nxd_mqtt_client_packet_pool_ptr,
            packet_ptr,
            wait_option);
    }
    /* Allocate a packet  */
        ...
        if (client_ptr->nxd_mqtt_client_socket.nx_tcp_socket_connect_ip.nxd_ip_version ==
            NX_IP_VERSION_V4) {
...
#ifdef NXD_MQTT_OVER_WEBSOCKET
    if (client_ptr->nxd_mqtt_client_use_websocket) {
        status = nx_websocket_client_send(
            &(client_ptr->nxd_mqtt_client_websocket),
            packet_ptr,
...
#ifdef NX_SECURE_ENABLE
        if (client_ptr->nxd_mqtt_client_use_tls) {
        status = nx_secure_tls_session_send(
            &(client_ptr->nxd_mqtt_tls_session), packet_ptr, wait_option);
    } else
#endif /* NX_SECURE_ENABLE */
    {
        status = nx_tcp_socket_send(
            &client_ptr->nxd_mqtt_client_socket, packet_ptr, wait_option);
    }
...
#ifdef NXD_MQTT_OVER_WEBSOCKET
    if (client_ptr->nxd_mqtt_client_use_websocket) {
        status = nx_websocket_client_receive(
            &(client_ptr->nxd_mqtt_client_websocket), packet_ptr, &op_code, wait_option);
...
#ifdef NX_SECURE_ENABLE
        if (client_ptr->nxd_mqtt_client_use_tls) {
        status = nx_secure_tls_session_receive(
            &(client_ptr->nxd_mqtt_tls_session), packet_ptr, wait_option);
    } else
#endif /* NX_SECURE_ENABLE */
    {
...
static VOID _nxd_mqtt_tcp_establish_notify(NX_TCP_SOCKET *socket_ptr) {
    ...
    if (&(client_ptr->nxd_mqtt_client_socket) == socket_ptr) {
        ...
        nx_cloud_module_event_set(
            &(client_ptr->nxd_mqtt_client_cloud_module), MQTT_TCP_ESTABLISH_EVENT);
...
#ifndef NXD_MQTT_CLOUD_ENABLE
        tx_event_flags_set(
            &client_ptr->nxd_mqtt_events, MQTT_PACKET_RECEIVE_EVENT, TX_OR);
#else
        nx_cloud_module_event_set(
            &(client_ptr->nxd_mqtt_client_cloud_module), MQTT_PACKET_RECEIVE_EVENT);
#endif /* NXD_MQTT_CLOUD_ENABLE */
    }
}
...
static UINT _nxd_mqtt_copy_transmit_packet(
    ...
    if (client_ptr->message_transmit_queue_depth >=
    ...
    *((USHORT *)(*new_packet_ptr)->nx_packet_data_start) = packet_id;
    ...
    if (set_duplicate_flag) {
        ...
        *((*new_packet_ptr)->nx_packet_prepend_ptr) =
            (*((*new_packet_ptr)->nx_packet_prepend_ptr)) | MQTT_PUBLISH_DUP_FLAG;
    }
...
/*                                                                        */
/**************************************************************************/
static UINT _nxd_mqtt_process_connack(
    NXD_MQTT_CLIENT *client_ptr, NX_PACKET *packet_ptr, ULONG wait_option) {

    UINT ret = NXD_MQTT_COMMUNICATION_FAILURE;
    MQTT_PACKET_CONNACK *connack_packet_ptr =
        (MQTT_PACKET_CONNACK *)(packet_ptr->nx_packet_prepend_ptr);

    /* Check the length.  */
    if ((packet_ptr->nx_packet_length != sizeof(MQTT_PACKET_CONNACK)) ||
        (connack_packet_ptr->mqtt_connack_packet_header >> 4 !=
        ...
        /* Follow MQTT-3.2.2-1 rule.  */
            ...
            (client_ptr->nxd_mqtt_clean_session) &&
            (connack_packet_ptr->mqtt_connack_packet_ack_flags &
             MQTT_CONNACK_CONNECT_FLAGS_SP)) {

            /* Client requested clean session, and server responded with Session Present.
             * This is a violation. */
            ret = NXD_MQTT_SERVER_MESSAGE_FAILURE;
        } else if (
            connack_packet_ptr->mqtt_connack_packet_return_code >
            MQTT_CONNACK_CONNECT_RETURN_CODE_NOT_AUTHORIZED) {
            ret = NXD_MQTT_SERVER_MESSAGE_FAILURE;
        } else if (connack_packet_ptr->mqtt_connack_packet_return_code > 0) {

            /* Pass the server return code to the application. */
            ret = (UINT)(NXD_MQTT_ERROR_CONNECT_RETURN_CODE +
                         connack_packet_ptr->mqtt_connack_packet_return_code);
        } else {
            ...
            client_ptr->nxd_mqtt_client_packet_identifier =
                NXD_MQTT_INITIAL_PACKET_ID_VALUE;
    ...
    if (client_ptr->nxd_mqtt_connect_notify) {
        client_ptr->nxd_mqtt_connect_notify(
            client_ptr, ret, client_ptr->nxd_mqtt_connect_context);
    }

    if (ret == NXD_MQTT_SUCCESS) {

        /* If client doesn't start with Clean Session, and there are un-acked PUBLISH
           messages, we shall re-publish these messages. */
        if ((client_ptr->nxd_mqtt_clean_session != NX_TRUE) &&
            (client_ptr->message_transmit_queue_head)) {

            tx_mutex_get(client_ptr->nxd_mqtt_client_mutex_ptr, NX_WAIT_FOREVER);

            /* There are messages from the previous session that has not been
             * acknowledged. */
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_process_publish_packet(
    NX_PACKET *packet_ptr,
    ULONG *topic_offset_ptr,
    USHORT *topic_length_ptr,
    ULONG *message_offset_ptr,
    ...
    QoS = (UCHAR)((*(packet_ptr->nx_packet_prepend_ptr) & MQTT_PUBLISH_QOS_LEVEL_FIELD) >>
                  1);
    ...
    if (nx_packet_data_extract_offset(
            packet_ptr, offset, &bytes, sizeof(bytes), &bytes_copied) ||
...
/**************************************************************************/
static UINT
_nxd_mqtt_process_publish(NXD_MQTT_CLIENT *client_ptr, NX_PACKET *packet_ptr) {
    ...
    if (QoS == 0) {
        ...
        if (transmit_packet_ptr) {
            /* This published data is already in our system.  No need to deliver this
    ...
    if (enqueue_message) {
        ...
        if (client_ptr->nxd_mqtt_client_receive_notify) {
            (*(client_ptr->nxd_mqtt_client_receive_notify))(
                client_ptr, client_ptr->message_receive_queue_depth);
        }
    }
    ...
    pubresp_ptr->mqtt_publish_response_packet_packet_identifier_msb =
        (UCHAR)(packet_id >> 8);
    pubresp_ptr->mqtt_publish_response_packet_packet_identifier_lsb =
        (UCHAR)(packet_id & 0xFF);

    if (QoS == 1) {

        pubresp_ptr->mqtt_publish_response_packet_header = MQTT_CONTROL_PACKET_TYPE_PUBACK
                                                           << 4;
    } else {
        pubresp_ptr->mqtt_publish_response_packet_header = MQTT_CONTROL_PACKET_TYPE_PUBREC
                                                           << 4;
    }

    packet_ptr->nx_packet_append_ptr =
        packet_ptr->nx_packet_prepend_ptr + sizeof(MQTT_PACKET_PUBLISH_RESPONSE);
...
/**************************************************************************/
static UINT
_nxd_mqtt_process_publish_response(NXD_MQTT_CLIENT *client_ptr, NX_PACKET *packet_ptr) {
    ...
    packet_id =
        (USHORT)((response_ptr->mqtt_publish_response_packet_packet_identifier_msb << 8) |
    ...
    while (transmit_packet_ptr) {
        ...
        if (transmit_packet_id == packet_id) {
            ...
            if (((response_ptr->mqtt_publish_response_packet_header) >> 4) ==
                MQTT_CONTROL_PACKET_TYPE_PUBACK) {

                /* PUBACK is the response to a PUBLISH packet with QoS Level 1*/
                /* Therefore we verify that packet contains PUBLISH packet with QoS level
                 * 1*/
                if ((fixed_header & 0xF6) == ((MQTT_CONTROL_PACKET_TYPE_PUBLISH << 4) |
                                              MQTT_PUBLISH_QOS_LEVEL_1)) {

                    /* Check ack notify function.  */
                    if (client_ptr->nxd_mqtt_ack_receive_notify) {

                        /* Call notify function. Note: user routine should not release the
                         * packet.  */
                    ...
                    _nxd_mqtt_release_transmit_packet(
                        client_ptr, transmit_packet_ptr, previous_packet_ptr);

                    /* Return with value 1, so the caller will release packet_ptr */
                    return (1);
                }
            } else if (
                ((response_ptr->mqtt_publish_response_packet_header) >> 4) ==
                ...
                if ((fixed_header & 0xF6) == (MQTT_CONTROL_PACKET_TYPE_PUBREC << 4)) {
                    ...
                    ret = _nxd_mqtt_client_packet_allocate(
                        client_ptr, &response_packet, NX_WAIT_FOREVER);
                    if (ret) {
                        return (1);
                    }

                    if (4u > ((ULONG)(response_packet->nx_packet_data_end) -
                              (ULONG)(response_packet->nx_packet_append_ptr))) {
                    ...
                    response_ptr = (MQTT_PACKET_PUBLISH_RESPONSE *)
                                       response_packet->nx_packet_prepend_ptr;

                    response_ptr->mqtt_publish_response_packet_header =
                        MQTT_CONTROL_PACKET_TYPE_PUBCOMP << 4;
                    response_ptr->mqtt_publish_response_packet_remaining_length = 2;

                    /* Fill in packet ID */
                    response_packet->nx_packet_prepend_ptr[3] =
                        packet_ptr->nx_packet_prepend_ptr[3];
                    response_packet->nx_packet_prepend_ptr[4] =
                        packet_ptr->nx_packet_prepend_ptr[4];
                    response_packet->nx_packet_append_ptr =
                        response_packet->nx_packet_prepend_ptr + 4;
                    ...
                    /* Send packet to server.  */
                    ret = _nxd_mqtt_packet_send(
                        client_ptr, response_packet, NX_WAIT_FOREVER);

                    tx_mutex_get(client_ptr->nxd_mqtt_client_mutex_ptr, TX_WAIT_FOREVER);

                    /* Update the timeout value. */
                    client_ptr->nxd_mqtt_timeout =
                        tx_time_get() + client_ptr->nxd_mqtt_keepalive;
...
/**************************************************************************/
static UINT
_nxd_mqtt_process_sub_unsub_ack(NXD_MQTT_CLIENT *client_ptr, NX_PACKET *packet_ptr) {
    ...
    while (transmit_packet_ptr) {
        ...
        if (transmit_packet_id == packet_id) {
            ...
            if (((response_header >> 4) == MQTT_CONTROL_PACKET_TYPE_SUBACK) &&
                ...
                ((response_header >> 4) == MQTT_CONTROL_PACKET_TYPE_UNSUBACK) &&
...
static VOID _nxd_mqtt_process_disconnect(NXD_MQTT_CLIENT *client_ptr) {
    ...
    /* If a connect callback notification is defined and is still in connecting stage,
     * call it now. */
    if ((disconnect_callback == NX_FALSE) && (client_ptr->nxd_mqtt_connect_notify)) {
        client_ptr->nxd_mqtt_connect_notify(
            client_ptr, NXD_MQTT_CONNECT_FAILURE, client_ptr->nxd_mqtt_connect_context);
    }
    ...
    if (status == TX_SUCCESS) {
        ...
        while (client_ptr->message_receive_queue_head) {
            _nxd_mqtt_release_receive_packet(
                client_ptr, client_ptr->message_receive_queue_head, NX_NULL);
        }
...
#ifndef NXD_MQTT_CLOUD_ENABLE
        tx_event_flags_set(
            &client_ptr->nxd_mqtt_events, ~MQTT_PACKET_RECEIVE_EVENT, TX_AND);
#else
    ...
    for (;;) {
...
#ifndef NXD_MQTT_CLOUD_ENABLE
                tx_event_flags_set(
                    &client_ptr->nxd_mqtt_events, MQTT_NETWORK_DISCONNECT_EVENT, TX_OR);
#else
                nx_cloud_module_event_set(
                    &(client_ptr->nxd_mqtt_client_cloud_module),
                    MQTT_NETWORK_DISCONNECT_EVENT);
#endif /* NXD_MQTT_CLOUD_ENABLE */
            }
        ...
        if (client_ptr->nxd_mqtt_client_processing_packet) {
            ...
            if (client_ptr->nxd_mqtt_client_processing_packet->nx_packet_last) {
                client_ptr->nxd_mqtt_client_processing_packet->nx_packet_last
                    ->nx_packet_next = packet_ptr;
            } else {
                client_ptr->nxd_mqtt_client_processing_packet->nx_packet_next =
                    packet_ptr;
            }
            ...
            client_ptr->nxd_mqtt_client_processing_packet->nx_packet_length +=
        ...
        if (client_ptr->nxd_mqtt_packet_receive_notify) {
            ...
            if (client_ptr->nxd_mqtt_packet_receive_notify(
                    client_ptr,
                    packet_ptr,
                    client_ptr->nxd_mqtt_packet_receive_context) == NX_TRUE) {
                continue;
            }
        }
        ...
        while (packet_ptr) {
            ...
            status =
                _nxd_mqtt_read_remaining_length(packet_ptr, &remaining_length, &offset);
            ...
            /* Get packet type. */
            if (nx_packet_data_extract_offset(
                    packet_ptr, 0, &packet_type, 1, &bytes_copied)) {
            ...
            if (packet_length > offset) {
                ...
                while ((ULONG)(packet_ptr->nx_packet_append_ptr -
                               packet_ptr->nx_packet_prepend_ptr) <= offset) {
                    offset -= (ULONG)(packet_ptr->nx_packet_append_ptr -
...
static VOID _nxd_mqtt_tcp_establish_process(NXD_MQTT_CLIENT *client_ptr) {
    ...
    if (client_ptr->nxd_mqtt_client_use_tls) {
        status = nx_secure_tls_session_start(
            &(client_ptr->nxd_mqtt_tls_session),
            &(client_ptr->nxd_mqtt_client_socket),
            NX_NO_WAIT);
        ...
        if (status != NX_CONTINUE) {
            ...
            if (client_ptr->nxd_mqtt_connect_notify) {
                client_ptr->nxd_mqtt_connect_notify(
                    client_ptr, status, client_ptr->nxd_mqtt_connect_context);
            }
    ...
    if (client_ptr->nxd_mqtt_client_use_websocket) {
        status = nx_websocket_client_connect(
            &(client_ptr->nxd_mqtt_client_websocket),
            &(client_ptr->nxd_mqtt_client_socket),
            client_ptr->nxd_mqtt_client_websocket_host,
            client_ptr->nxd_mqtt_client_websocket_host_length,
            client_ptr->nxd_mqtt_client_websocket_uri_path,
            client_ptr->nxd_mqtt_client_websocket_uri_path_length,
            (UCHAR *)NXD_MQTT_OVER_WEBSOCKET_PROTOCOL,
            sizeof(NXD_MQTT_OVER_WEBSOCKET_PROTOCOL) - 1,
            client_ptr->nxd_mqtt_client_websocket_bearer,
            client_ptr->nxd_mqtt_client_websocket_bearer_length,
            NX_NO_WAIT);
...
static VOID _nxd_mqtt_tls_establish_process(NXD_MQTT_CLIENT *client_ptr) {
    ...
    status =
        _nx_secure_tls_handshake_process(&(client_ptr->nxd_mqtt_tls_session), NX_NO_WAIT);
    if (status == NX_SUCCESS) {
        ...
        /* If using websocket, start websocket connection.  */
        if (client_ptr->nxd_mqtt_client_use_websocket) {
            status = nx_websocket_client_secure_connect(
                &(client_ptr->nxd_mqtt_client_websocket),
                &(client_ptr->nxd_mqtt_tls_session),
                client_ptr->nxd_mqtt_client_websocket_host,
                client_ptr->nxd_mqtt_client_websocket_host_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_append_message(
    NXD_MQTT_CLIENT *client_ptr,
    NX_PACKET *packet_ptr,
    CHAR *message,
    UINT length,
    ...
    ret = nx_packet_data_append(
        packet_ptr, len, 2, client_ptr->nxd_mqtt_client_packet_pool_ptr, wait_option);
...
VOID _nxd_mqtt_client_connection_end(NXD_MQTT_CLIENT *client_ptr, ULONG wait_option) {
    ...
    if (client_ptr->nxd_mqtt_client_use_websocket) {
        nx_websocket_client_disconnect(
            &(client_ptr->nxd_mqtt_client_websocket), wait_option);
    }
#endif /* NXD_MQTT_OVER_WEBSOCKET */
...
static UINT
_nxd_mqtt_send_simple_message(NXD_MQTT_CLIENT *client_ptr, UCHAR header_value);
...
static VOID _nxd_mqtt_periodic_timer_entry(ULONG client) {
    ...
    /* If an outstanding ping response has not been received, and the client exceeds the
       time waiting for ping response, the client shall disconnect from the server. */
    if (client_ptr->nxd_mqtt_ping_not_responded) {
        /* If current time is greater than the ping timeout */
        if ((tx_time_get() - client_ptr->nxd_mqtt_ping_sent_time) >=
            client_ptr->nxd_mqtt_ping_timeout) {
            /* Ping timed out.  Need to terminate the connection. */
#ifndef NXD_MQTT_CLOUD_ENABLE
            tx_event_flags_set(
                &client_ptr->nxd_mqtt_events, MQTT_PING_TIMEOUT_EVENT, TX_OR);
#else
            nx_cloud_module_event_set(
                &(client_ptr->nxd_mqtt_client_cloud_module), MQTT_PING_TIMEOUT_EVENT);
#endif /* NXD_MQTT_CLOUD_ENABLE */
    ...
    /* About to timeout? */
    if ((client_ptr->nxd_mqtt_timeout - tx_time_get()) <=
        client_ptr->nxd_mqtt_timer_value) {
        /* Set the flag so the MQTT thread can send the ping. */
#ifndef NXD_MQTT_CLOUD_ENABLE
        tx_event_flags_set(&client_ptr->nxd_mqtt_events, MQTT_TIMEOUT_EVENT, TX_OR);
#else
        nx_cloud_module_event_set(
            &(client_ptr->nxd_mqtt_client_cloud_module), MQTT_TIMEOUT_EVENT);
#endif /* NXD_MQTT_CLOUD_ENABLE */
    }
...
/*                                                                        */
/**************************************************************************/
static VOID _nxd_mqtt_client_event_process(
    VOID *mqtt_client, ULONG common_events, ULONG module_own_events) {
    ...
    if (module_own_events & MQTT_PING_TIMEOUT_EVENT) {
        /* The server/broker didn't respond to our ping request message. Disconnect from
         * the server. */
        _nxd_mqtt_process_disconnect(client_ptr);
    }
    ...
    if (module_own_events & MQTT_DELETE_EVENT) {
        ...
        while (client_ptr->message_transmit_queue_head) {
            _nxd_mqtt_release_transmit_packet(
                client_ptr, client_ptr->message_transmit_queue_head, NX_NULL);
        }
        ...
        /* Deleting the socket, (the socket ID is cleared); this signals it is ok to
         * delete this thread. */
...
static VOID _mqtt_client_disconnect_callback(NX_TCP_SOCKET *socket_ptr) {
    NXD_MQTT_CLIENT *client_ptr =
        (NXD_MQTT_CLIENT *)(socket_ptr->nx_tcp_socket_reserved_ptr);
...
#else
    nx_cloud_module_event_set(
        &(client_ptr->nxd_mqtt_client_cloud_module), MQTT_NETWORK_DISCONNECT_EVENT);
#endif /* NXD_MQTT_CLOUD_ENABLE */
...
UINT _nxd_mqtt_client_create(
    ...
    /* Create MQTT client.  */
    status = _nxd_mqtt_client_create_internal(
        client_ptr,
        client_name,
        client_id,
        client_id_length,
    ...
    status = nx_cloud_create(
        &(client_ptr->nxd_mqtt_client_cloud),
        "Cloud Helper",
        stack_ptr,
        stack_size,
        mqtt_thread_priority);
    ...
    client_ptr->nxd_mqtt_client_mutex_ptr =
        &(client_ptr->nxd_mqtt_client_cloud.nx_cloud_mutex);

    /* Register MQTT on cloud helper.  */
    status = nx_cloud_module_register(
        client_ptr->nxd_mqtt_client_cloud_ptr,
        &(client_ptr->nxd_mqtt_client_cloud_module),
        client_name,
        NX_CLOUD_MODULE_MQTT_EVENT,
        _nxd_mqtt_client_event_process,
        client_ptr);
...
static UINT _nxd_mqtt_client_create_internal(
    ...
    /* Create MQTT mutex.  */
    status =
        tx_mutex_create(&client_ptr->nxd_mqtt_protection, client_name, TX_NO_INHERIT);
    ...
    status = tx_thread_create(
        ...
        mqtt_thread_priority,
        mqtt_thread_priority,
        NXD_MQTT_CLIENT_THREAD_TIME_SLICE,
        TX_DONT_START);
    ...
    nx_tcp_socket_create(
        ...
        &(client_ptr->nxd_mqtt_client_socket),
        client_ptr->nxd_mqtt_client_name,
        NX_IP_NORMAL,
        NX_DONT_FRAGMENT,
        0x80,
        NXD_MQTT_CLIENT_SOCKET_WINDOW_SIZE,
        NX_NULL,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_login_set(
    NXD_MQTT_CLIENT *client_ptr,
    CHAR *username,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_will_message_set(
    NXD_MQTT_CLIENT *client_ptr,
    ...
    client_ptr->nxd_mqtt_client_will_qos_retain =
        (UCHAR)(client_ptr->nxd_mqtt_client_will_qos_retain | will_QoS);
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_will_message_set(
    NXD_MQTT_CLIENT *client_ptr,
    const UCHAR *will_topic,
    UINT will_topic_length,
    const UCHAR *will_message,
    ...
    return (_nxd_mqtt_client_will_message_set(
        client_ptr,
        will_topic,
        will_topic_length,
        will_message,
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_login_set(
    NXD_MQTT_CLIENT *client_ptr,
    CHAR *username,
    UINT username_length,
    CHAR *password,
    ...
    return (_nxd_mqtt_client_login_set(
        client_ptr, username, username_length, password, password_length));
}
...
_nxd_mqtt_client_retransmit_message(NXD_MQTT_CLIENT *client_ptr, ULONG wait_option) {
    ...
    while (transmit_packet_ptr) {
        ...
        if ((fixed_header & 0xF0) == (MQTT_CONTROL_PACKET_TYPE_PUBLISH << 4)) {
            ...
            /* Obtain the mutex. */
            mutex_status =
                tx_mutex_get(client_ptr->nxd_mqtt_client_mutex_ptr, wait_option);
...
UINT _nxd_mqtt_client_connect(
    ...
    if (wait_option == 0) {
        nx_tcp_socket_establish_notify(
            &client_ptr->nxd_mqtt_client_socket, _nxd_mqtt_tcp_establish_notify);

        /* Set the receive callback. */
        nx_tcp_socket_receive_notify(
            &client_ptr->nxd_mqtt_client_socket, _nxd_mqtt_receive_callback);

#ifdef NXD_MQTT_OVER_WEBSOCKET
        if (client_ptr->nxd_mqtt_client_use_websocket) {

            /* Set the websocket connection callback.  */
            nx_websocket_client_connection_status_callback_set(
                &client_ptr->nxd_mqtt_client_websocket,
                client_ptr,
        ...
        if (client_ptr->nxd_mqtt_client_use_websocket) {
            ...
            nx_websocket_client_connection_status_callback_set(
                &client_ptr->nxd_mqtt_client_websocket, NX_NULL, NX_NULL);
        }
#endif /* NXD_MQTT_OVER_WEBSOCKET */
    }
    ...
    nx_tcp_client_socket_bind(
        &(client_ptr->nxd_mqtt_client_socket), NX_ANY_PORT, wait_option);
    ...
    status = nxd_tcp_client_socket_connect(
        &(client_ptr->nxd_mqtt_client_socket), server_ip, server_port, wait_option);
    ...
    /* Increase priority to the same of internal thread to avoid out of order packet
     * process. */
#ifndef NXD_MQTT_CLOUD_ENABLE
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_connect_packet_send(
    NXD_MQTT_CLIENT *client_ptr, ULONG wait_option) {
    NX_PACKET *packet_ptr;
    UINT status;
    UINT length = 0;
    ...
    /* Construct connect flags by taking the connect flag user supplies, or'ing the
       username and password bits, if they are supplied. */
    ...
    if (client_ptr->nxd_mqtt_client_will_topic) {
        ...
        connection_flags =
            (UCHAR)(connection_flags |
                    ((client_ptr->nxd_mqtt_client_will_qos_retain & 0x3) << 3));
    }
    ...
    ret = _nxd_mqtt_client_set_fixed_header(
        client_ptr, packet_ptr, temp_data[0], length, wait_option);
    ...
    ret = _nxd_mqtt_client_append_message(
        client_ptr,
        packet_ptr,
        client_ptr->nxd_mqtt_client_id,
        client_ptr->nxd_mqtt_client_id_length,
        wait_option);
    ...
    if (!ret && (connection_flags & MQTT_CONNECT_FLAGS_WILL_FLAG)) {
        ret = _nxd_mqtt_client_append_message(
            client_ptr,
            packet_ptr,
            (CHAR *)client_ptr->nxd_mqtt_client_will_topic,
        ...
        if (!ret) {
            ret = _nxd_mqtt_client_append_message(
                client_ptr,
                packet_ptr,
                (CHAR *)client_ptr->nxd_mqtt_client_will_message,
    ...
    if (!ret && (connection_flags & MQTT_CONNECT_FLAGS_USERNAME)) {
        ret = _nxd_mqtt_client_append_message(
            client_ptr,
            packet_ptr,
            client_ptr->nxd_mqtt_client_username,
            client_ptr->nxd_mqtt_client_username_length,
            wait_option);
    }
    ...
    if (!ret && (connection_flags & MQTT_CONNECT_FLAGS_PASSWORD)) {
        ret = _nxd_mqtt_client_append_message(
            client_ptr,
            packet_ptr,
            client_ptr->nxd_mqtt_client_password,
            client_ptr->nxd_mqtt_client_password_length,
            wait_option);
    }
...
/**************************************************************************/
#ifdef NX_SECURE_ENABLE
UINT _nxd_mqtt_client_secure_connect(
    NXD_MQTT_CLIENT *client_ptr,
    NXD_ADDRESS *server_ip,
    UINT server_port,
    UINT (*tls_setup)(
    ...
    ret = _nxd_mqtt_client_connect(
        client_ptr, server_ip, server_port, keepalive, clean_session, wait_option);
...
#else
    nx_cloud_module_event_set(
        &(client_ptr->nxd_mqtt_client_cloud_module), MQTT_DELETE_EVENT);
#endif /* NXD_MQTT_CLOUD_ENABLE */
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_publish_packet_send(
    NXD_MQTT_CLIENT *client_ptr,
    NX_PACKET *packet_ptr,
    USHORT packet_id,
    UINT QoS,
...
UINT _nxd_mqtt_client_publish(
    ...
    /* Write out the control header and remaining length field. */
    ret = _nxd_mqtt_client_set_fixed_header(
        client_ptr, packet_ptr, flags, length, wait_option);
    ...
    /* Write out topic */
    ret = _nxd_mqtt_client_append_message(
        client_ptr, packet_ptr, topic_name, topic_name_length, wait_option);
    ...
    /* Send publish packet. */
    ret = _nxd_mqtt_client_publish_packet_send(
        client_ptr, packet_ptr, packet_id, QoS, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_subscribe(
    NXD_MQTT_CLIENT *client_ptr, CHAR *topic_name, UINT topic_name_length, UINT QoS) {

    if (QoS == 2) {
        return (NXD_MQTT_QOS2_NOT_SUPPORTED);
    }

    return (_nxd_mqtt_client_sub_unsub(
        client_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_unsubscribe(
    NXD_MQTT_CLIENT *client_ptr, CHAR *topic_name, UINT topic_name_length) {
    return (_nxd_mqtt_client_sub_unsub(
        client_ptr,
        (MQTT_CONTROL_PACKET_TYPE_UNSUBSCRIBE << 4) | 0x02,
        topic_name,
        topic_name_length,
        NX_NULL,
        0));
...
_nxd_mqtt_send_simple_message(NXD_MQTT_CLIENT *client_ptr, UCHAR header_value) {
    ...
    if (2u > ((ULONG)(packet_ptr->nx_packet_data_end) -
    ...
    *(byte + 1) = 0;
    ...
    if (header_value == MQTT_CONTROL_PACKET_TYPE_PINGREQ) {
        /* Do not update the ping sent time if the outstanding ping has not been responded
         * yet */
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_receive_notify_set(
    NXD_MQTT_CLIENT *client_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_message_get(
    NXD_MQTT_CLIENT *client_ptr,
    UCHAR *topic_buffer,
    ...
    while (client_ptr->message_receive_queue_depth) {
        ...
        status = _nxd_mqtt_process_publish_packet(
            packet_ptr, &topic_offset, &topic_length, &message_offset, &message_length);
        if (status == NXD_MQTT_SUCCESS) {
        ...
        if (status == NXD_MQTT_SUCCESS) {
            ...
            *actual_message_length = 0;
            nx_packet_data_extract_offset(
                ...
                topic_buffer,
                topic_length,
                (ULONG *)actual_topic_length);
            nx_packet_data_extract_offset(
                packet_ptr,
                message_offset,
                message_buffer,
                message_length,
                (ULONG *)actual_message_length);
            nx_packet_release(packet_ptr);
...
UINT _nxde_mqtt_client_create(
    ...
    if ((client_ptr == NX_NULL) || (ip_ptr == NX_NULL) ||
        (ip_ptr->nx_ip_id != NX_IP_ID) || (stack_ptr == NX_NULL) || (stack_size == 0) ||
        (pool_ptr == NX_NULL)) {
        return (NX_PTR_ERROR);
    }

    return (_nxd_mqtt_client_create(
        client_ptr,
        client_name,
        client_id,
        client_id_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_connect(
    NXD_MQTT_CLIENT *client_ptr,
    NXD_ADDRESS *server_ip,
    UINT server_port,
    UINT keepalive,
    ...
    status = _nxd_mqtt_client_connect(
        client_ptr, server_ip, server_port, keepalive, clean_session, wait_option);
...
UINT _nxde_mqtt_client_secure_connect(
    NXD_MQTT_CLIENT *client_ptr,
    NXD_ADDRESS *server_ip,
    UINT server_port,
    UINT (*tls_setup)(
    ...
    status = _nxd_mqtt_client_secure_connect(
        client_ptr,
        server_ip,
        server_port,
        tls_setup,
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_publish(
    NXD_MQTT_CLIENT *client_ptr,
    CHAR *topic_name,
    UINT topic_name_length,
    CHAR *message,
    ...
    return (_nxd_mqtt_client_publish(
        client_ptr,
        topic_name,
        topic_name_length,
        message,
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_subscribe(
    NXD_MQTT_CLIENT *client_ptr, CHAR *topic_name, UINT topic_name_length, UINT QoS) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_unsubscribe(
    NXD_MQTT_CLIENT *client_ptr, CHAR *topic_name, UINT topic_name_length) {
    /* Validate client_ptr */
    if (client_ptr == NX_NULL) {
        return (NX_PTR_ERROR);
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_message_get(
    NXD_MQTT_CLIENT *client_ptr,
    UCHAR *topic_buffer,
    UINT topic_buffer_size,
    UINT *actual_topic_length,
    ...
    /* Validate message.  Message_length can be NULL if caller does not care message
     * length. */
    if ((message_buffer == NX_NULL) || (topic_buffer == NX_NULL)) {
        return (NXD_MQTT_INVALID_PARAMETER);
    }

    return (_nxd_mqtt_client_message_get(
        client_ptr,
        topic_buffer,
        topic_buffer_size,
        actual_topic_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_receive_notify_set(
    NXD_MQTT_CLIENT *client_ptr,
    VOID (*receive_notify)(NXD_MQTT_CLIENT *client_ptr, UINT message_count)) {
    /* Validate client_ptr */
    if (client_ptr == NX_NULL) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_disconnect_notify_set(
    NXD_MQTT_CLIENT *client_ptr, VOID (*disconnect_notify)(NXD_MQTT_CLIENT *)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_disconnect_notify_set(
    NXD_MQTT_CLIENT *client_ptr, VOID (*disconnect_notify)(NXD_MQTT_CLIENT *)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_cloud_create(
    NXD_MQTT_CLIENT *client_ptr,
    CHAR *client_name,
    CHAR *client_id,
    UINT client_id_length,
    ...
    /* Check for invalid input pointers.  */
    if ((client_ptr == NX_NULL) || (ip_ptr == NX_NULL) ||
        (ip_ptr->nx_ip_id != NX_IP_ID) || (pool_ptr == NX_NULL) ||
        (cloud_ptr == NX_NULL) || (cloud_ptr->nx_cloud_id != NX_CLOUD_ID)) {
        return (NX_PTR_ERROR);
    }
...
VOID _nxd_mqtt_client_websocket_connection_status_callback(
    NX_WEBSOCKET_CLIENT *websocket_client_ptr, VOID *context, UINT status) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_mqtt_client_websocket_set(
    NXD_MQTT_CLIENT *client_ptr,
    UCHAR *host,
    UINT host_length,
    UCHAR *uri_path,
    UINT uri_path_length,
    UCHAR *bearer,
    UINT bearer_length) {
    UINT status;
    ...
    /* Create WebSocket.  */
    status = nx_websocket_client_create(
        &client_ptr->nxd_mqtt_client_websocket,
        (UCHAR *)"",
        client_ptr->nxd_mqtt_client_ip_ptr,
        client_ptr->nxd_mqtt_client_packet_pool_ptr);
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_mqtt_client_websocket_set(
    NXD_MQTT_CLIENT *client_ptr,
    UCHAR *host,
    UINT host_length,
    UCHAR *uri_path,
    ...
    return (_nxd_mqtt_client_websocket_set(
        client_ptr, host, host_length, uri_path, uri_path_length, bearer, bearer_length));
}
#endif /* NXD_MQTT_OVER_WEBSOCKET */
