...
UINT _nxde_tftp_client_create(
    ...
    status = _nxd_tftp_client_create(
        tftp_client_ptr, tftp_client_name, ip_ptr, pool_ptr, ip_type);
...
UINT _nxd_tftp_client_create(
    ...
    status = nx_udp_socket_create(
        ip_ptr,
        &(tftp_client_ptr->nx_tftp_client_socket),
        tftp_client_name,
    ...
    status = nx_udp_socket_bind(
        &(tftp_client_ptr->nx_tftp_client_socket), NX_TFTP_SOURCE_PORT, NX_WAIT_FOREVER);
    ...
    if (tftp_client_ptr->nx_tftp_client_state == NX_TFTP_STATE_WRITE_OPEN) {
        ...
        if (ip_type == NX_IP_VERSION_V4) {
            status = nx_packet_allocate(
                tftp_client_ptr->nx_tftp_client_packet_pool_ptr,
                &packet_ptr,
                NX_IPv4_UDP_PACKET,
                NX_NO_WAIT);
        } else {
        ...
        if (4u > ((ULONG)(packet_ptr->nx_packet_data_end) -
        ...
        *buffer_ptr++ =
            (UCHAR)((UCHAR)(tftp_client_ptr->nx_tftp_client_block_number >> 8));
        *buffer_ptr =
            (UCHAR)((UCHAR)(tftp_client_ptr->nx_tftp_client_block_number & 0xFF));
...
UINT _nxe_tftp_client_file_open(
    ...
    status = _nx_tftp_client_file_open(
        tftp_client_ptr, file_name, server_ip_address, open_type, wait_option);
...
UINT _nx_tftp_client_file_open(
    ...
    status = _nx_tftp_client_file_open_internal(
        tftp_client_ptr,
        file_name,
        &server_address,
        open_type,
...
UINT _nxde_tftp_client_file_open(
    ...
    /* Call actual client file open function.  */
    status = _nxd_tftp_client_file_open(
        tftp_client_ptr, file_name, server_ip_address, open_type, wait_option, ip_type);
...

{
...
#ifndef NX_DISABLE_IPV4
        /* If not IPv6, assume the client IP request is using IPv4 if not explicitly
         * specified. */
        tftp_client_ptr->nx_tftp_client_server_ip.nxd_ip_version = ip_type;
        tftp_client_ptr->nx_tftp_client_server_ip.nxd_ip_address.v4 =
    ...
    /* Allocate a packet for initial open request message. Determine whether we are
       sending IPv4 or IPv6 packets.   */

    if (ip_type == NX_IP_VERSION_V4) {

        status = nx_packet_allocate(
            (tftp_client_ptr->nx_tftp_client_ip_ptr)->nx_ip_default_packet_pool,
            &packet_ptr,
            NX_IPv4_UDP_PACKET,
            wait_option);
    } else {
    ...
    *buffer_ptr++ = NX_NULL;
    ...
    packet_ptr->nx_packet_length =
        (ULONG)(buffer_ptr - packet_ptr->nx_packet_prepend_ptr);
    ...
    if (open_type == NX_TFTP_OPEN_FOR_WRITE) {
        ...
        /* Open for write request is present.  We now need to wait for the ACK of block
           number 0 from the server.  */
        status = nx_udp_socket_receive(
            &(tftp_client_ptr->nx_tftp_client_socket), &packet_ptr, wait_option);
        ...
        if ((ip_type == NX_IP_VERSION_V4) &&
            ...
            if (CHECK_IPV6_ADDRESSES_SAME(
                    ...
                    &tftp_client_ptr->nx_tftp_client_server_ip.nxd_ip_address.v6[0])) {
        ...
        if ((buffer_ptr[0] != 0) || (buffer_ptr[1] != NX_TFTP_CODE_ACK) ||
            ...
            if (code == NX_TFTP_CODE_ERROR) {
                ...
                tftp_client_ptr->nx_tftp_client_error_string
                    [sizeof(tftp_client_ptr->nx_tftp_client_error_string) - 1] = NX_NULL;
...
UINT _nxde_tftp_client_file_read(
    ...
    status =
        _nxd_tftp_client_file_read(tftp_client_ptr, packet_ptr, wait_option, ip_type);
...
UINT _nxd_tftp_client_file_read(
    ...
    if (ip_type == NX_IP_VERSION_V4) {
        ...
        if (ip_address.nxd_ip_address.v4 ==
            tftp_client_ptr->nx_tftp_client_server_ip.nxd_ip_address.v4) {
            matching = NX_TRUE;
        }
    } else
    ...
    if (code == NX_TFTP_CODE_DATA) {
        ...
        if (4u > ((ULONG)(ack_packet->nx_packet_data_end) -
                  (ULONG)(ack_packet->nx_packet_append_ptr))) {
            nx_packet_release(*packet_ptr);
        ...
        *buffer_ptr = (UCHAR)(block_number & 0xFF);
        ...
        if (resend_ACK_packet == NX_TRUE) {
            ...
            *packet_ptr = NX_NULL;
            ...
            /* Do not handle as an error. Indicate the TFTP Client is still downloading a
             * file. */
            return NX_TFTP_INVALID_BLOCK_NUMBER;
        }
        ...
        /* Check for end of file condition. Anything less than 512 bytes signals an end of
         * file.  */
        ...
        for (i = 0; (i < (sizeof(tftp_client_ptr->nx_tftp_client_error_string) - 1)) &&
                    (*buffer_ptr);
             i++) {
...
UINT _nxde_tftp_client_file_write(
    ...
    status =
        _nxd_tftp_client_file_write(tftp_client_ptr, packet_ptr, wait_option, ip_type);
...
UINT _nxd_tftp_client_file_write(
    ...
    *buffer_ptr++ = (UCHAR)(tftp_client_ptr->nx_tftp_client_block_number & 0xFF);
    ...
    if ((buffer_ptr[0] != 0) || (buffer_ptr[1] != NX_TFTP_CODE_ACK) ||
        ...
        (buffer_ptr[3] !=
         ((UCHAR)(tftp_client_ptr->nx_tftp_client_block_number & 0xFF))) ||
        (packet_ptr->nx_packet_length != 4)) {
        ...
        if (code == NX_TFTP_CODE_ERROR) {
            ...
            /* Loop to save error message.  */
            for (i = 0;
                 (i < (sizeof(tftp_client_ptr->nx_tftp_client_error_string) - 1)) &&
                 (*buffer_ptr);
                 i++) {
...
UINT _nxd_tftp_client_packet_allocate(
    ...
    if (ip_type == NX_IP_VERSION_V4) {
        ...
        status =
            nx_packet_allocate(pool_ptr, packet_ptr, NX_IPv4_UDP_PACKET, wait_option);
    } else {

        status =
            nx_packet_allocate(pool_ptr, packet_ptr, NX_IPv6_UDP_PACKET, wait_option);
    }
