...
static UINT
_nx_dhcp_server_packet_process(NX_DHCP_SERVER *dhcp_ptr, NX_PACKET *packet_ptr);
static UINT _nx_dhcp_respond_to_client_message(
    NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr);
static UINT _nx_dhcp_server_extract_information(
    NX_DHCP_SERVER *dhcp_ptr,
    NX_DHCP_CLIENT **dhcp_client_ptr,
    NX_PACKET *packet_ptr,
    UINT iface_index);
static UINT _nx_dhcp_process_option_data(
    NX_DHCP_CLIENT *dhcp_ptr, CHAR *buffer, UCHAR value, UINT get_option_data, UINT size);
static UINT _nx_dhcp_add_option(
    UCHAR *bootp_message, UINT option, UINT size, ULONG value, UINT *index);
static UINT _nx_dhcp_add_requested_option(
    NX_DHCP_SERVER *dhcp_ptr, UINT iface_index, UCHAR *buffer, UINT option, UINT *index);
static UINT
_nx_dhcp_set_server_options(NX_DHCP_SERVER *dhcp_ptr, CHAR *buffer, UINT buffer_length);
static UINT _nx_dhcp_load_server_options(
    NX_DHCP_SERVER *dhcp_ptr,
    NX_DHCP_CLIENT *dhcp_client_ptr,
    UCHAR *buffer,
    UINT option_type,
...
static UINT
_nx_dhcp_clear_client_session(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr);
static UINT _nx_dhcp_validate_client_message(
    NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr);
static UINT _nx_dhcp_find_client_record_by_chaddr(
    NX_DHCP_SERVER *dhcp_ptr,
    UINT iface_index,
    ...
    NX_DHCP_CLIENT **dhcp_client_ptr,
    UINT add_on);
static UINT _nx_dhcp_find_client_record_by_ip_address(
    NX_DHCP_SERVER *dhcp_ptr,
    NX_DHCP_CLIENT **dhcp_client_ptr,
    UINT iface_index,
    ULONG assigned_ip_address);
static UINT _nx_dhcp_server_assign_ip_address(
    NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr);
static UINT _nx_dhcp_find_interface_table_ip_address(
    NX_DHCP_SERVER *dhcp_ptr,
    UINT iface_index,
    ULONG ip_address,
    NX_DHCP_INTERFACE_IP_ADDRESS **return_interface_address);
static UINT _nx_dhcp_update_assignable_ip_address(
    NX_DHCP_SERVER *dhcp_ptr,
    NX_DHCP_CLIENT *dhcp_client_ptr,
    ULONG ip_address,
    UINT assign_status);
static UINT _nx_dhcp_find_ip_address_owner(
    NX_DHCP_INTERFACE_IP_ADDRESS *iface_owner,
    NX_DHCP_CLIENT *client_record_ptr,
    UINT *assigned_to_client);
static UINT _nx_dhcp_record_ip_address_owner(
    NX_DHCP_INTERFACE_IP_ADDRESS *iface_owner,
    NX_DHCP_CLIENT *client_record_ptr,
    UINT lease_time);
...
UINT _nxe_dhcp_server_create(
    ...
    if ((dhcp_ptr == NX_NULL) || (packet_pool == NX_NULL) || (stack_ptr == NX_NULL) ||
        (ip_ptr == NX_NULL)) {
        return (NX_PTR_ERROR);
    }
    ...
    status = _nx_dhcp_server_create(
        dhcp_ptr, ip_ptr, stack_ptr, stack_size, name_ptr, packet_pool);
...
UINT _nx_dhcp_server_create(
    ...
    /* Check the Server packet pool is at least large enough for a full sized DHCP message
       as per RFC 2131, plus header data. */
    ...
    for (i = 0; i < NX_MAX_PHYSICAL_INTERFACES; i++) {
        ...
        for (j = 0; j < NX_DHCP_IP_ADDRESS_MAX_LIST_SIZE; j++) {
            ...
            memset(
                &dhcp_ptr->nx_dhcp_interface_table[i].nx_dhcp_ip_address_list[j],
                0,
                sizeof(NX_DHCP_INTERFACE_IP_ADDRESS));
        }
    ...
    status = _nx_dhcp_set_server_options(
        dhcp_ptr,
        (CHAR *)NX_DHCP_SERVER_OPTION_LIST,
        sizeof(NX_DHCP_SERVER_OPTION_LIST) - 1);
    ...
    status = nx_udp_socket_create(
        ip_ptr,
        &(dhcp_ptr->nx_dhcp_socket),
        "NetX DHCP Server Socket",
        NX_DHCP_TYPE_OF_SERVICE,
        NX_DHCP_FRAGMENT_OPTION,
        NX_DHCP_TIME_TO_LIVE,
        NX_DHCP_QUEUE_DEPTH);
    ...
    status = nx_udp_socket_receive_notify(
        &(dhcp_ptr->nx_dhcp_socket), _nx_dhcp_server_socket_receive_notify);
    ...
    status = tx_thread_create(
        &(dhcp_ptr->nx_dhcp_server_thread),
        "NetX DHCP Server Thread",
        _nx_dhcp_server_thread_entry,
        (ULONG)(ALIGN_TYPE)dhcp_ptr,
        stack_ptr,
        stack_size,
    ...
    status =
        tx_mutex_create(&dhcp_ptr->nx_dhcp_mutex, "DHCP Server Mutex", TX_NO_INHERIT);
    ...
    /* Create the timer for Client DHCP session. This will keep track of when leases
       expire and when a client session has timed out. */
    status = tx_timer_create(
        &(dhcp_ptr->nx_dhcp_slow_periodic_timer),
        "DHCP Server IP Lease Timer",
    ...
    status =
        tx_event_flags_create(&dhcp_ptr->nx_dhcp_server_events, "DHCP Server Events");
...
static VOID _nx_dhcp_fast_periodic_timer_entry(ULONG info) {
    ...
    tx_event_flags_set(
        &(dhcp_ptr->nx_dhcp_server_events), NX_DHCP_SERVER_FAST_PERIODIC_EVENT, TX_OR);
...
static VOID _nx_dhcp_slow_periodic_timer_entry(ULONG info) {
    ...
    tx_event_flags_set(
        &(dhcp_ptr->nx_dhcp_server_events), NX_DHCP_SERVER_SLOW_PERIODIC_EVENT, TX_OR);
...
VOID _nx_dhcp_server_socket_receive_notify(NX_UDP_SOCKET *socket_ptr) {
    ...
    tx_event_flags_set(
        &(dhcp_server_ptr->nx_dhcp_server_events), NX_DHCP_SERVER_RECEIVE_EVENT, TX_OR);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_create_server_ip_address_list(
    NX_DHCP_SERVER *dhcp_ptr,
    UINT iface_index,
    ...
    status = _nx_dhcp_create_server_ip_address_list(
        dhcp_ptr, iface_index, start_ip_address, end_ip_address, addresses_added);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_create_server_ip_address_list(
    NX_DHCP_SERVER *dhcp_ptr,
    UINT iface_index,
    ULONG start_ip_address,
    ULONG end_ip_address,
    ...
    *addresses_added = 0;
     ...
     */
    dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_incoming_interface =
        &(ip_ptr->nx_ip_interface[iface_index]);
    ...
    dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_server_ip_address =
        ip_ptr->nx_ip_interface[iface_index].nx_interface_ip_address;

    /* Set the dns server, subnet mask, and router (default gateway) for this interface.
     */
    dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_dns_ip_address =
        ip_ptr->nx_ip_interface[iface_index].nx_interface_ip_address;
    dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_subnet_mask =
        ip_ptr->nx_ip_interface[iface_index].nx_interface_ip_network_mask;
    dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_router_ip_address =
    ...
    if ((dhcp_interface_table_ptr->nx_dhcp_subnet_mask & start_ip_address) !=
        (dhcp_interface_table_ptr->nx_dhcp_subnet_mask &
         dhcp_interface_table_ptr->nx_dhcp_server_ip_address)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_set_interface_network_parameters(
    NX_DHCP_SERVER *dhcp_ptr,
    UINT iface_index,
    ULONG subnet_mask,
    ...
    if ((subnet_mask == 0) || (default_gateway_address == 0) ||
        (dns_server_address == 0)) {
        return (NX_DHCP_INVALID_NETWORK_PARAMETERS);
    }

    /* Call the actual service. */
    status = _nx_dhcp_set_interface_network_parameters(
        dhcp_ptr, iface_index, subnet_mask, default_gateway_address, dns_server_address);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_set_interface_network_parameters(
    NX_DHCP_SERVER *dhcp_ptr,
    UINT iface_index,
    ULONG subnet_mask,
    ULONG default_gateway_address,
...
UINT _nx_dhcp_server_start(NX_DHCP_SERVER *dhcp_ptr) {
    ...
    status = nx_udp_socket_bind(
        &(dhcp_ptr->nx_dhcp_socket), NX_DHCP_SERVER_UDP_PORT, TX_WAIT_FOREVER);
...
static VOID _nx_dhcp_server_thread_entry(ULONG info) {
    ...
    while (1) {
        ...
        tx_event_flags_get(
            &dhcp_ptr->nx_dhcp_server_events,
            NX_DHCP_SERVER_ALL_EVENTS,
            TX_OR_CLEAR,
            &dhcp_events,
            TX_WAIT_FOREVER);
        ...
        if (dhcp_events & NX_DHCP_SERVER_RECEIVE_EVENT) {
            ...
            while (1) {
                ...
                status = _nx_udp_socket_receive(
                    &dhcp_ptr->nx_dhcp_socket, &packet_ptr, NX_NO_WAIT);
        ...
        if (dhcp_events & NX_DHCP_SERVER_FAST_PERIODIC_EVENT) {
            ...
            for (i = 0; i < NX_DHCP_CLIENT_RECORD_TABLE_SIZE; i++) {
                ...
                /* Skip empty records. */
                if ((dhcp_client_ptr->nx_dhcp_client_mac_lsw == 0) &&
                    (dhcp_client_ptr->nx_dhcp_client_mac_msw == 0))
                    continue;
                ...
                /* Check how much time is left before decreasing the time remaining on the
                 * session timeout. */
                if (dhcp_client_ptr->nx_dhcp_session_timeout >=
                    NX_DHCP_FAST_PERIODIC_TIME_INTERVAL) {
                    /* Ok to decrement the full amount. */
                    dhcp_client_ptr->nx_dhcp_session_timeout -=
                        NX_DHCP_FAST_PERIODIC_TIME_INTERVAL;
                } else {
                ...
                if (dhcp_client_ptr->nx_dhcp_session_timeout == 0) {
                    ...
                    /* Yes, the client has bailed on the session, intentionally or
                       otherwise. If it wants to get an IP address it must start again.
                       Reset status to 'INIT'. */
                    dhcp_client_ptr->nx_dhcp_client_state = NX_DHCP_STATE_INIT;

#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF(
                        "DHCPserv: client session has timed out. Clear session data\n");
#endif
                    ...
                    if (dhcp_client_ptr->nx_dhcp_assigned_ip_address) {
                        ...
                        /* Clear the session data, including any data read from last
                         * client DHCP message. */
        ...
        if (dhcp_events & NX_DHCP_SERVER_SLOW_PERIODIC_EVENT) {
            ...
            for (iface_index = 0; iface_index < NX_MAX_PHYSICAL_INTERFACES;
                 iface_index++) {
                ...
                for (i = 0; i < iface_table_ptr->nx_dhcp_address_list_size; i++) {
                    ...
                    /* Skip all the entries who are not assigned or assigned indefinately
                     * (static). */
                    ...
                    /* Check how much time is left before decreasing the time remaining on
                     * the lease timeout. */
                    if (iface_address_ptr->lease_time >=
                        NX_DHCP_SLOW_PERIODIC_TIME_INTERVAL) {

                        /* Ok to decrement the full amount. */
                        iface_address_ptr->lease_time -=
                            NX_DHCP_SLOW_PERIODIC_TIME_INTERVAL;
                    } else {
                    ...
                    if (iface_address_ptr->lease_time == 0) {
                        ...
                        /* Look up the client in the server database by its assigned
                         * address. */
                        _nx_dhcp_find_client_record_by_ip_address(
                            dhcp_ptr,
                            &dhcp_client_ptr,
                            iface_index,
                        ...
                        if (dhcp_client_ptr != NX_NULL) {
                            ...
                            /* Remove the assigned IP address and reset the Client to the
                             * INIT state. */
                            dhcp_client_ptr->nx_dhcp_assigned_ip_address =
                                NX_DHCP_NO_ADDRESS;
                            dhcp_client_ptr->nx_dhcp_client_state = NX_DHCP_STATE_INIT;
...
#ifdef EL_PRINTF_ENABLE
                            EL_PRINTF("DHCPserv: client IP lease expired. Clear client "
                                      "data, release IP address\n");
#endif

                            /* Clear all session data if there is any for this client. */
                            _nx_dhcp_clear_client_session(dhcp_ptr, dhcp_client_ptr);

                            /* No, we should have this client in the server client records
                            table but we don't. That's all we can do right now. */
                        }
                    }
                }
            }
...
static UINT _nx_dhcp_respond_to_client_message(
    ...
    _nx_dhcp_server_store_data(
        buffer + NX_DHCP_OFFSET_XID, 4, dhcp_client_ptr->nx_dhcp_xid);
    ...
    _nx_dhcp_server_store_data(
        buffer + NX_DHCP_OFFSET_FLAGS, 1, dhcp_client_ptr->nx_dhcp_broadcast_flag_set);
    _nx_dhcp_server_store_data(
        buffer + NX_DHCP_OFFSET_CLIENT_HW, 2, dhcp_client_ptr->nx_dhcp_client_mac_msw);
    _nx_dhcp_server_store_data(
        buffer + NX_DHCP_OFFSET_CLIENT_HW + 2,
        4,
        dhcp_client_ptr->nx_dhcp_client_mac_lsw);
    ...
    status = _nx_dhcp_load_server_options(
        dhcp_ptr, dhcp_client_ptr, buffer, NX_DHCP_OPTIONS_FOR_ALL_REPLIES, &index);

    /* Unless the server is sending a NACK, there are more options to add for the server
     * reply. */
    if (dhcp_client_ptr->nx_dhcp_response_type_to_client != NX_DHCP_TYPE_DHCPNACK) {

        /* Add the list of options that go out with server ACK replies (standard network
         * parameters). */
        _nx_dhcp_load_server_options(
            dhcp_ptr, dhcp_client_ptr, buffer, NX_DHCP_OPTIONS_FOR_GENERIC_ACK, &index);
        ...
        case NX_DHCP_TYPE_DHCPDISCOVER:
            ...
            if (dhcp_client_ptr->nx_dhcp_response_type_to_client ==
                NX_DHCP_TYPE_DHCPOFFER) {

                _nx_dhcp_server_store_data(
                    buffer + NX_DHCP_OFFSET_YOUR_IP,
                    4,
                    dhcp_client_ptr->nx_dhcp_your_ip_address);
                ...
                _nx_dhcp_load_server_options(
                    ...
                    dhcp_client_ptr,
                    buffer,
                    NX_DHCP_OPTIONS_FOR_REPLY_TO_OFFER,
                    &index);
            }
        ...
        case NX_DHCP_TYPE_DHCPREQUEST:
            ...
            if (dhcp_client_ptr->nx_dhcp_response_type_to_client ==
                ...
                _nx_dhcp_load_server_options(
                    ...
                    dhcp_client_ptr,
                    buffer,
                    NX_DHCP_OPTIONS_FOR_REPLY_TO_REQUEST,
                    &index);
            }
        ...
        /* Note: DHCP Servers should not reply to NX_DHCP_REPLY_TO_RELEASE or
         * NX_DHCP_REPLY_TO_DECLINE messages. */
        ...
        case NX_DHCP_TYPE_DHCPINFORM:
            ...
            /* At this time the server sends just standard network parameters to INFORM
               requests. so no additional options to add. */
        ...
        /* Determine which of the remaining client requested options the server has
         * information for. */
        _nx_dhcp_load_server_options(
            dhcp_ptr,
            dhcp_client_ptr,
            buffer,
            NX_DHCP_OPTIONS_REQUESTED_BY_CLIENT,
            &index);
    }
    ...
    *(buffer + index) = NX_DHCP_SERVER_OPTION_END;
    ...
    if (index > NX_DHCP_OFFSET_END) {
        ...
        packet_ptr->nx_packet_append_ptr =
            packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_END;
    }

    /* DHCP Server set the correct destination address and port to send resposne. RFC2131,
       Section4.1, Page23.
       1. If the 'giaddr' field in a DHCP message from a client is non-zero, the server
       sends any return messages to the 'DHCP server' port on the BOOTP relay agent whose
       address appears in 'giaddr'.
       2. If the 'giaddr'field is zero and the 'ciaddr' field is nonzero, then the server
       unicasts DHCPOFFER and DHCPACK messages to the address in 'ciaddr'.
       3. If 'giaddr' is zero and 'ciaddr' is zero, and the broadcast bit is set, then the
       server broadcasts DHCPOFFER and DHCPACK messages to 0xffffffff.
       4. If the broadcast bit is not set and 'giaddr' is zero and 'ciaddr' is zero, then
       the server unicasts DHCPOFFER and DHCPACK messages to the client's hardware address
       and 'yiaddr' address.
       5. In all cases, when 'giaddr' is zero, the server broadcasts any DHCPNAK messages
       to 0xffffffff.  */
    ...
    if (dhcp_client_ptr->nx_dhcp_relay_ip_address) {
        ...
        if (dhcp_client_ptr->nx_dhcp_response_type_to_client != NX_DHCP_TYPE_DHCPNACK) {
            ...
            /* Check the broadcast bit.  */
                ...
                status = nx_arp_dynamic_entry_set(
                    dhcp_ptr->nx_dhcp_ip_ptr,
                    destination_ip_address,
                    dhcp_client_ptr->nx_dhcp_client_mac_msw,
                    dhcp_client_ptr->nx_dhcp_client_mac_lsw);
    ...
    EL_PRINTF(
        "DHCPserv: DHCP reply buffer size (bytes) %d\n", packet_ptr->nx_packet_length);
    EL_PRINTF(
        "DHCPserv: DHCP reply packet payload size (bytes) %d\n\n",
    ...
    /* Send the packet.  */
    status = nx_udp_socket_interface_send(
        &(dhcp_ptr->nx_dhcp_socket),
        packet_ptr,
        destination_ip_address,
        destination_port,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_clear_client_record(
    NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_clear_client_record(
    NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr) {
    ...
    iface_table_ptr =
        &dhcp_ptr->nx_dhcp_interface_table[dhcp_client_ptr->nx_dhcp_client_iface_index];
    ...
    if (iface_table_ptr->nx_dhcp_ip_address_list[i].nx_assignable_ip_address) {
        ...
        while (i < iface_table_ptr->nx_dhcp_address_list_size) {
            ...
            if (iface_table_ptr->nx_dhcp_ip_address_list[i].nx_assignable_ip_address ==
                ...
                _nx_dhcp_clear_ip_address_owner(
                    &(iface_table_ptr->nx_dhcp_ip_address_list[i]));
...
static UINT _nx_dhcp_load_server_options(
    ...
    if (option_type == NX_DHCP_OPTIONS_FOR_ALL_REPLIES) {
        ...
        _nx_dhcp_add_option(
            buffer,
            NX_DHCP_SERVER_OPTION_DHCP_TYPE,
            NX_DHCP_SERVER_OPTION_DHCP_TYPE_SIZE,
            dhcp_client_ptr->nx_dhcp_response_type_to_client,
            index);

        /* Add the server identifier to all messages to the client. */
        _nx_dhcp_add_option(
            buffer,
            NX_DHCP_SERVER_OPTION_DHCP_SERVER_ID,
            NX_DHCP_SERVER_OPTION_DHCP_SERVER_SIZE,
            dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_server_ip_address,
            index);
        ...
        _nx_dhcp_add_option(
            buffer,
            NX_DHCP_SERVER_OPTION_DHCP_LEASE,
            NX_DHCP_SERVER_OPTION_DHCP_LEASE_SIZE,
            lease_time,
            index);
        /* With the computed renew time. */
        _nx_dhcp_add_option(
            buffer,
            NX_DHCP_SERVER_OPTION_RENEWAL,
            NX_DHCP_SERVER_OPTION_RENEWAL_SIZE,
            renew_time,
            index);
        /* And rebind time. */
        _nx_dhcp_add_option(
            buffer,
            NX_DHCP_SERVER_OPTION_REBIND,
            NX_DHCP_SERVER_OPTION_REBIND_SIZE,
            rebind_time,
            index);
        ...
        /* The NetX DHCP Server reply to Inform messages includes the standard server
           option list which is automatically included. */
        ...
        for (i = NX_DHCP_REQUIRED_SERVER_OPTION_SIZE;
             i < dhcp_ptr->nx_dhcp_server_option_count;
             i++) {

            /* Append this DHCP option to the client message. */
            _nx_dhcp_add_requested_option(
                dhcp_ptr,
                dhcp_client_ptr->nx_dhcp_client_iface_index,
                buffer,
                dhcp_ptr->nx_dhcp_server_options[i],
                index);
        }
    } else if (option_type == NX_DHCP_OPTIONS_REQUESTED_BY_CLIENT) {

        /* The NetX DHSP Server does not (yet) support this feature in the current
         * release. */
...
/**************************************************************************/
static UINT
_nx_dhcp_set_server_options(NX_DHCP_SERVER *dhcp_ptr, CHAR *buffer, UINT buffer_size) {
...
#ifdef EL_PRINTF_ENABLE
            EL_PRINTF(
                "DHCPserv: Unable to set server DHCP option data list. Status 0x%x\n",
                status);
#endif
...
static UINT _nx_dhcp_parse_next_option(CHAR **option_list, UINT *option, UINT length) {
    ...
    /* Initialize parsed option to undefined option (sorry, zero is taken by the PAD
       option which I personally think was a bad programming decision). */
    *option = 999;
    j = 0;
    buffer_index = 0;
    ...
    /* Now parse the next characters until the end of the buffer or next separator
     * character. */
...
_nx_dhcp_server_packet_process(NX_DHCP_SERVER *dhcp_ptr, NX_PACKET *packet_ptr) {
    ...
    /* Check for minimum sized DHCP packet (e.g. just the DHCP header and 0 bytes of
     * option data). */
    if (packet_ptr->nx_packet_length < NX_DHCP_HEADER_SIZE) {
#ifdef EL_PRINTF_ENABLE
        EL_PRINTF(
            "DHCPserv: Client DHCP packet is malformed or empty. Invalid DHCP packet.\n");
#endif
    ...
    /* Update the prepend pointer to make sure that the IP header and UDP header also are
     * copied into new packet. */
    ...
    new_packet_ptr->nx_packet_append_ptr =
        new_packet_ptr->nx_packet_prepend_ptr + new_packet_ptr->nx_packet_length;

    /* Now we can release the original packet. */
    nx_packet_release(packet_ptr);

    /* Extract the DHCP specific information from the packet. This will create new record
       or update existing client record in the server database. */
    status = _nx_dhcp_server_extract_information(
        dhcp_ptr, &dhcp_client_ptr, new_packet_ptr, iface_index);
...
#ifdef PACKET_DUMP
    {
        ...
        /* Get a pointer to the IP header and adjust the packet length for IP and UDP
         * header size. */
        work_ptr = (ULONG *)(packet_ptr->nx_packet_prepend_ptr - sizeof(NX_UDP_HEADER) -
    ...
    if ((dhcp_client_ptr->nx_dhcp_response_type_to_client != NX_DHCP_TYPE_DHCPNACK) &&
        ...
        /* Assign/confirm an IP address to the client. If the client is renewing or
           rebinding this will verify they sent the correct IP address and reset the lease
           time. */
...
#ifdef EL_PRINTF_ENABLE
            EL_PRINTF(
                "DHCPserv: Error sending response to DHCP client. Status: 0x%x\n",
                status);
#endif
...
/* Output Server session data if enabled.  */
    ...
    if ((TRACE_NTH_CLIENT_PACKET > 0) && (add_client % TRACE_NTH_CLIENT_PACKET == 0)) {
        ...
        for (i = 0; i < dhcp_ptr->nx_dhcp_interface_table[0].nx_dhcp_address_list_size;
             i++) {
            NX_DHCP_INTERFACE_IP_ADDRESS *interface_address_ptr;

            interface_address_ptr =
                &(dhcp_ptr->nx_dhcp_interface_table[0].nx_dhcp_ip_address_list[i]);
            EL_PRINTF(
        ...
        EL_PRINTF("\nInterface 1\n");
        for (i = 0; i < dhcp_ptr->nx_dhcp_interface_table[1].nx_dhcp_address_list_size;
             i++) {
            NX_DHCP_INTERFACE_IP_ADDRESS *interface_address_ptr;

            interface_address_ptr =
                &(dhcp_ptr->nx_dhcp_interface_table[1].nx_dhcp_ip_address_list[i]);
            EL_PRINTF(
                "0x%x 0x%x\t0x%x\t%d\t  0x%x\n",
                interface_address_ptr->owner_mac_msw,
                interface_address_ptr->owner_mac_lsw,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_find_client_record_by_ip_address(
    NX_DHCP_SERVER *dhcp_ptr,
    NX_DHCP_CLIENT **dhcp_client_ptr,
    UINT iface_index,
    ULONG assigned_ip_address) {
        ...
        if (client_record_ptr->nx_dhcp_assigned_ip_address == assigned_ip_address) {
            ...
            if (client_record_ptr->nx_dhcp_client_iface_index == iface_index) {
                ...
                *dhcp_client_ptr = client_record_ptr;
                ...
                /* It appears the client has changed its subnet location but not hardware
                   type e.g. same mac address/hardware type but not interface. Clear the
                   old record. */
...
static UINT _nx_dhcp_find_client_record_by_chaddr(
    ...
    *dhcp_client_ptr = NX_NULL;
    ...
    while (i < NX_DHCP_CLIENT_RECORD_TABLE_SIZE) {
        ...
        if ((client_record_ptr->nx_dhcp_client_mac_msw == 0) &&
            (client_record_ptr->nx_dhcp_client_mac_lsw == 0)) {
        ...
        if ((client_record_ptr->nx_dhcp_client_mac_msw == client_mac_msw) &&
            ...
            if (client_record_ptr->nx_dhcp_client_iface_index == iface_index) {
                ...
                *dhcp_client_ptr = client_record_ptr;
                ...
                /* It appears the client has changed its location (subnet) but not
                   hardware type e.g. same mac address/hware type but not interface.
                   Remove the client record entirely and free up any assigned IP address
...
#ifdef EL_PRINTF_ENABLE
        EL_PRINTF(
            "DHCPserv: Client HW address not found. Do not add to server database\n");
#endif
...
static UINT _nx_dhcp_find_interface_table_ip_address(
    ...
    *return_interface_address = NX_NULL;
    ...
    for (i = 0; i < dhcp_interface_table_ptr->nx_dhcp_address_list_size; i++) {
        ...
        if (dhcp_interface_table_ptr->nx_dhcp_ip_address_list[i]
                .nx_assignable_ip_address == ip_address) {
            /* Yes, set a pointer to the location and return. */
            *return_interface_address =
                &dhcp_interface_table_ptr->nx_dhcp_ip_address_list[i];
...
static UINT _nx_dhcp_update_assignable_ip_address(
    ...
    _nx_dhcp_find_interface_table_ip_address(
        dhcp_ptr, iface_index, ip_address, &interface_address_ptr);
    ...
    if (assign_status == NX_DHCP_ADDRESS_STATUS_MARK_AVAILABLE) {
        ...
        _nx_dhcp_find_ip_address_owner(
            interface_address_ptr, dhcp_client_ptr, &assigned_to_client);
    ...
    /* Was the IP address assigned externally from the DHCP process?  */
        ...
        if (interface_address_ptr->lease_time) {
            /* If this already has a lease time (because we assigned it most likely) don't
             * change it. */
            lease_time = interface_address_ptr->lease_time;
        } else {
            /* Otherwise, make this lease time infinity e.g. the time out will not expire
             * on it.*/
            lease_time = NX_WAIT_FOREVER;
        }

        /*  Set the current client as the owner.  */
        _nx_dhcp_record_ip_address_owner(
            interface_address_ptr, dhcp_client_ptr, lease_time);
    }
       ...
       sent the server a DECLINE message) or accepting another DHCP server's offer? */
        ...
        if (dhcp_client_ptr->nx_dhcp_message_type == NX_DHCP_TYPE_DHCPDECLINE) {
            ...
            /* Yes, remove the Client as owner of this IP lease.
               Record owner information with null 'owner' ID since we don't know who the
               owner is.  */
            _nx_dhcp_record_ip_address_owner(
                interface_address_ptr, NX_NULL, NX_WAIT_FOREVER);
        } else {

            /* Record owner information.  */
            _nx_dhcp_record_ip_address_owner(
                interface_address_ptr, dhcp_client_ptr, NX_WAIT_FOREVER);
        }
    } else {
...
static UINT _nx_dhcp_validate_client_message(
...
#ifdef EL_PRINTF_ENABLE
        EL_PRINTF("DHCPserv: DHCP packet missing transaction ID. Reject the packet "
                  "silently. \n");
#endif
    ...
    /* Client messages should never fill in the "Your (Client) IP" or "Next Server IP"
       fields. */
    else if (
        (dhcp_client_ptr->nx_dhcp_your_ip_address != NX_DHCP_NO_ADDRESS) ||
        (dhcp_client_ptr->nx_dhcp_clientrec_server_ip != NX_DHCP_NO_ADDRESS)) {

#ifdef EL_PRINTF_ENABLE
        EL_PRINTF(
            "DHCPserv: NACK! 'Your IP'/'Server IP' fields should not be filled in.\n");
#endif
    ...
    /* Check for missing required fields. */
        ...
        (dhcp_client_ptr->nx_dhcp_client_hwtype == 0) ||
        ...
        */
        ...
        case NX_DHCP_TYPE_DHCPDISCOVER: {
            ...
            /* Determine if server needs to assign an IP address based on assigned IP
               address. e.g. If this appears to be a retransmission of a previous IP
               address DISOVERY request, only assign another IP address if the client
               appears not to have been assigned one already.*/
...
#ifdef EL_PRINTF_ENABLE
                EL_PRINTF(
                    "DHCPserv: NACK! Client IP address field must not be filled in.\n");
#endif
        ...
        case NX_DHCP_TYPE_DHCPREQUEST: {
            ...
            /* If the client does not specify a server id but is requesting an IP
             * address...  */
...
#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF("DHCPserv: NACK! REQUEST message may not specify Client IP "
                              "address.\n");
#endif

                    /* Let the client know it has an invalid request. */
                    dhcp_client_ptr->nx_dhcp_response_type_to_client =
                        NX_DHCP_TYPE_DHCPNACK;
                    break;
                }

                /* Server SHOULD send a DHCPNAK message to the client if the 'requested IP
                   address' is incorrect, or is on the wrong network. RFC 2131,
                   Section 4.3.2, Page31.  */
                if ((dhcp_client_ptr->nx_dhcp_assigned_ip_address != 0) &&
                    (dhcp_client_ptr->nx_dhcp_assigned_ip_address !=
                     dhcp_client_ptr->nx_dhcp_requested_ip_address)) {

                    ULONG ip_address_assigned =
                        dhcp_client_ptr->nx_dhcp_assigned_ip_address;

#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF("DHCPserv: NACK! REQUEST message contains incorrect "
                    ...
                    /* Clear requested IP lease time because the client is not granted an
                     * IP address lease. */
             ...
             */
                ...
                dhcp_client_ptr->nx_dhcp_server_id &&
                (dhcp_client_ptr->nx_dhcp_requested_ip_address != NX_DHCP_NO_ADDRESS)) {
                ...
                /* Check if the Client has chosen this server's offer (e.g.server ID
                ...
                if (dhcp_client_ptr->nx_dhcp_server_id != server_ip_address) {
                    ...
                    if (dhcp_client_ptr->nx_dhcp_assigned_ip_address ==
                        ...
                        /* Yes, so do not change the client record IP address and IP
                           address status in the server interface table as assigned.  */
                        ip_address_assigned =
                            dhcp_client_ptr->nx_dhcp_assigned_ip_address;
                    } else {
                        ...
                        /* So update the client record's assigned address to the one it is
                           accepting from another server. */
                        dhcp_client_ptr->nx_dhcp_assigned_ip_address =
                            dhcp_client_ptr->nx_dhcp_requested_ip_address;
                    }
...
#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF("DHCPserv: SILENCE! REQUEST message indicates Client "
                              "accepting another server IP.\n");
#endif
                    ...
                    /* No need to respond. */
                    dhcp_client_ptr->nx_dhcp_response_type_to_client =
                        NX_DHCP_TYPE_DHCPSILENT;

                    return (NX_SUCCESS);
                }

                /* Make sure the Client did in fact receive an OFFER from this server
                   (e.g. has been tentatively assigned an IP address from us). */
                if (dhcp_client_ptr->nx_dhcp_assigned_ip_address == NX_DHCP_NO_ADDRESS) {

#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF("DHCPserv: NACK! REQUEST message does not indicate valid "
                              "assigned IP address.\n");
#endif
                    ...
                    /* Clear requested IP lease times because the client was not assigned
                     * an IP address. */
                    dhcp_client_ptr->nx_dhcp_requested_lease_time = 0;
                ...
                /* Does the Client's requested IP address match the "Your IP" field in the
                   server OFFER previously sent to this client (e.g. Client's assigned IP
                   address)? */
                else if (
                    dhcp_client_ptr->nx_dhcp_assigned_ip_address !=
                    dhcp_client_ptr->nx_dhcp_requested_ip_address) {
...
#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF("DHCPserv: NACK! REQUEST message contains requested IP "
                              "address. \n");
#endif
                ...
                /* The Client accepts this Server's offer. We will grant the client the IP
                 * lease now. */
                dhcp_client_ptr->nx_dhcp_response_type_to_client = NX_DHCP_TYPE_DHCPACK;
            ...
            /* Is this a RENEW or REBIND request? If so the server ID and requested IP
               options must NOT be filled in as per RFC 2131 4.3.2. */
            else if (
                (dhcp_client_ptr->nx_dhcp_server_id == NX_DHCP_NO_ADDRESS) &&
                (dhcp_client_ptr->nx_dhcp_requested_ip_address == NX_DHCP_NO_ADDRESS)) {
...
#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF("DHCPserv: NACK! REQUEST message no 'Client IP' address "
                              "specified. \n");
#endif
                ...
                /* The "Client IP" address subnet should match the Client packet source IP
                 * address subnet. */
                if ((dhcp_interface_table_ptr->nx_dhcp_subnet_mask &
                     dhcp_client_ptr->nx_dhcp_clientip_address) !=
                    (dhcp_interface_table_ptr->nx_dhcp_subnet_mask &
                     dhcp_interface_table_ptr->nx_dhcp_server_ip_address)) {
#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF("DHCPserv: NACK! REQUEST message; the 'Client IP' address "
                              "subnet does not match the DHCP server subnet. \n");
#endif
...
#ifdef EL_PRINTF_ENABLE
                    EL_PRINTF("DHCPserv: NACK! REQUEST message; server has no record of "
                              "assigned IP address.\n");
#endif

                    /* No; possibly assigned by another DHCP server or outside of DHCP
                       protocol. Reject the renewal request. */
                    dhcp_client_ptr->nx_dhcp_response_type_to_client =
                        NX_DHCP_TYPE_DHCPNACK;
                ...
                if (dhcp_client_ptr->nx_dhcp_destination_ip_address !=
                    NX_DHCP_BC_ADDRESS) {
        ...
        case NX_DHCP_TYPE_DHCPRELEASE: {
            ...
            /* The Client is releasing it's IP address back to the server. A client is not
               ...
               Clear the client record and mark the IP address as available. If the
               request proves invalid, this server will ignore it and let the Client's
               assigned IP lease expire.  */

            /* Regardless what happens the server does not send a reply to a RELEASE
             * message. */
            dhcp_client_ptr->nx_dhcp_response_type_to_client = NX_DHCP_TYPE_DHCPSILENT;
            ...
            if (dhcp_client_ptr->nx_dhcp_destination_ip_address == NX_DHCP_BC_ADDRESS) {
                ...
                ULONG server_ip = (&dhcp_ptr->nx_dhcp_interface_table[index])
        ...
        case NX_DHCP_TYPE_DHCPDECLINE: {
            ...
            /* The Client is informing us an IP address we assigned to it is apparently
               already in use by another host.  It needs to restart in the INIT/BOOT state
               for this server to assign it another IP address. */
            dhcp_client_ptr->nx_dhcp_client_state = NX_DHCP_STATE_INIT;
        ...
        case NX_DHCP_TYPE_DHCPINFORM: {
            ...
            /* A DHCP client sends an INFORM message when it has been assigned an IP
               address with another server or outside DHCP but still needs local
               configuration data. The DHCP server must update its database with their
               address (specified in the "CI-ADDR" field) as no longer available. When the
               server forms a reply ACK, it must leave the 'Your IP' and lease time fields
               blank as per RFC 2131 3.4. */
    ...
    /* Check if the Client has requested a valid IP address. This is only permitted
       ...
       or SELECT states.
        ...
        As per RFC 2131 4.3.1, the DHCP server should assign an IP address in order of
       preference as follows: 1) the client currently is bound to 2) client ip address in
       client records (e.g. previously assigned to) 3) the currently requested IP 4) the
    ...
    if (dhcp_client_ptr->nx_dhcp_source_ip_address != NX_DHCP_NO_ADDRESS) {
        ...
        dhcp_client_ptr->nx_dhcp_assigned_ip_address =
            dhcp_client_ptr->nx_dhcp_source_ip_address;
    }
...
static UINT _nx_dhcp_server_assign_ip_address(
    ...
    if (dhcp_client_ptr->nx_dhcp_assigned_ip_address) {
        ...
        if (interface_address_ptr) {
            ...
            if (interface_address_ptr->assigned == NX_FALSE) {
...
#ifdef EL_PRINTF_ENABLE
                EL_PRINTF("DHCPserv: NACK! Cannot assign requested IP address (already "
                          "assigned)\n");
#endif
                ...
                /* Clear the client's assigned IP address since it is assigned to someone
                 * else. */
                dhcp_client_ptr->nx_dhcp_assigned_ip_address = NX_DHCP_NO_ADDRESS;
            ...
            if (dhcp_client_ptr->nx_dhcp_message_type != NX_DHCP_TYPE_DHCPDISCOVER) {
...
#ifdef EL_PRINTF_ENABLE
                EL_PRINTF("DHCPserv: NACK! Cannot assign requested IP address (not owned "
                          "by server)\n");
#endif
                ...
                /* Clear the client record's 'assigned' IP address since none was
                 * assigned. */
                dhcp_client_ptr->nx_dhcp_assigned_ip_address = NX_DHCP_NO_ADDRESS;
    ...
    /* If the client doesn't have an address assigned yet, find one in the server table
       not yet assigned. */
    if (interface_address_ptr == 0x0) {
        ...
        /* Set a local varible to the IP address list for this client. */
        dhcp_interface_table_ptr =
            &(dhcp_ptr
                  ->nx_dhcp_interface_table[dhcp_client_ptr->nx_dhcp_client_iface_index]);
        ...
        if (assigned_ip == NX_FALSE) {
...
#ifdef EL_PRINTF_ENABLE
            EL_PRINTF("DHCPserv: NACK! Cannot assign requested IP address (not found in "
                      "server IP list)\n");
#endif
            ...
            /* Make sure the client record's 'assigned' IP address ir removed since it was
             * not assigned. */
            dhcp_client_ptr->nx_dhcp_assigned_ip_address = NX_DHCP_NO_ADDRESS;
    ...
    /* Make sure the client record's 'assigned' IP address ir removed since it was not
     * assigned. */
    dhcp_client_ptr->nx_dhcp_assigned_ip_address = NX_DHCP_NO_ADDRESS;
...
static UINT _nx_dhcp_server_extract_information(
    ...
    *dhcp_client_ptr = NX_NULL;
    ...
    client_mac_lsw = (((ULONG)work_ptr[2]) << 24) | (((ULONG)work_ptr[3]) << 16) |
                     (((ULONG)work_ptr[4]) << 8) | ((ULONG)work_ptr[5]);
    ...
    /* Look up the client record by IP address and interface index in Client Records
     * table. */
    status = _nx_dhcp_find_client_record_by_chaddr(
        dhcp_ptr,
        iface_index,
        client_mac_msw,
        client_mac_lsw,
    ...
    temp_client_rec_ptr->nx_dhcp_source_ip_address =
        ip_header_ptr->nx_ip_header_source_ip;
    temp_client_rec_ptr->nx_dhcp_destination_ip_address =
        ip_header_ptr->nx_ip_header_destination_ip;

    /* Get the message type. */
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_OP, 1, &value);
    if (status) {
        return (status);
    }
...
#ifdef EL_PRINTF_ENABLE
        EL_PRINTF("DHCPserv: SILENCE! Client DHCP packet not coded as a BOOT Request. "
                  "Invalid request.\n");
#endif
    ...
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_HTYPE, 1, &value);
    if (status) {
        return (status);
    }

    /* This is the Client interface type which is used in the default client identifier
     * tag. */
    temp_client_rec_ptr->nx_dhcp_client_hwtype = value;

    /* Get the hardware length. */
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_HLEN, 1, &value);
    if (status) {
        return (status);
    }

    /* This is the size of the hardware address. Used in the default client identifier
     * tag. */
    temp_client_rec_ptr->nx_dhcp_client_hwlen = value;

    /* Extract the message ID. */
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_XID, 4, &xid);
    if (status) {
        return (status);
    }
    ...
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_FLAGS, 1, &value);
    if (status) {
        return (status);
    }

    /* Get the Client's preference for unicast vs broadcast. */
    temp_client_rec_ptr->nx_dhcp_broadcast_flag_set =
        (value & NX_DHCP_FLAGS_BROADCAST) ? 0x80 : 0x0;

    /* Extract the Requested Client IP address, if there is one. */
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_CLIENT_IP, 4, &value);
    if (status) {
        return (status);
    }
    ...
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_YOUR_IP, 4, &value);
    if (status) {
        return (status);
    }

    /* This is the "Your IP address" which only the server should fill in. Should be zero
       in client messages. */
    temp_client_rec_ptr->nx_dhcp_your_ip_address = value;

    /* Extract the "Next Server IP address".     */
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_SERVER_IP, 4, &value);
    if (status) {
        return (status);
    }
    ...
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_RELAY_IP, 4, &value);
    if (status) {
        return (status);
    }
    ...
    status = _nx_dhcp_server_get_data(
        packet_ptr->nx_packet_prepend_ptr + NX_DHCP_OFFSET_VENDOR, 4, &value);
    if (status) {
        return (status);
    }
    ...
    if (value == NX_DHCP_MAGIC_COOKIE) {
        ...
        while (*work_ptr != 0xFF) {
            ...
            /* Guard against a missing "END" marker by checking if we are at the end of
             * the DHCP packet data. */
            if (work_ptr + 2 > packet_ptr->nx_packet_append_ptr) {

                /* Yes, Client must have sent a DHCP message with improperly terminated
                   option data (or Client is using a super large DHCP packet). */
            ...
            if (value != NX_DHCP_SERVER_OPTION_CLIENT_ID) {
                ...
                if (temp_client_rec_ptr->nx_dhcp_client_option_count <
                    NX_DHCP_CLIENT_OPTIONS_MAX) {

                    /* Process as any other option. */
                    _nx_dhcp_process_option_data(
                        temp_client_rec_ptr,
                        (CHAR *)work_ptr,
                        (UCHAR)value,
                        NX_TRUE,
            ...
            /* Move up the buffer pointer past the current option data size to the next
             * option. */
            work_ptr += size;
        }
    }
...
static UINT _nx_dhcp_process_option_data(
    ...
    case 1:
        /* Subnet mask */
        dhcp_client_ptr
            ->nx_dhcp_user_options[dhcp_client_ptr->nx_dhcp_client_option_count] = option;
        if (get_option_data) {
    ...
    case 12:
        ...
        if (get_option_data) {
            ...
            size = (size <= NX_DHCP_CLIENT_HOSTNAME_MAX) ? size
                                                         : NX_DHCP_CLIENT_HOSTNAME_MAX;

            /* Clear the memory buffer before writing the host name to it. */
            memset(
                &dhcp_client_ptr->nx_dhcp_client_name[0], 0, NX_DHCP_CLIENT_HOSTNAME_MAX);
            memcpy(
                &dhcp_client_ptr->nx_dhcp_client_name[0],
                buffer,
                size); /* Use case of memcpy is verified. */
        }
        dhcp_client_ptr->nx_dhcp_client_option_count++;
        break;
    ...
    case 54:
        /* Server ID by which server identifies itself to client(usually server IP
           address). The Client includes this option in a REQUEST message to identify
           which server it wants to lease an IP address from. */
        dhcp_client_ptr
            ->nx_dhcp_user_options[dhcp_client_ptr->nx_dhcp_client_option_count] = option;
        if (get_option_data) {
    ...
    case 55:
        ...
        if (dhcp_client_ptr->nx_dhcp_client_option_count + size >
            NX_DHCP_CLIENT_OPTIONS_MAX) {
            size =
                NX_DHCP_CLIENT_OPTIONS_MAX - dhcp_client_ptr->nx_dhcp_client_option_count;
        }

        /* Update the client record with that option. */
        memcpy(
            &(dhcp_client_ptr
                  ->nx_dhcp_user_options[dhcp_client_ptr->nx_dhcp_client_option_count]),
            buffer,
            size); /* Use case of memcpy is verified. */
...
static UINT _nx_dhcp_add_requested_option(
    ...
    /* The NetX DHCP server provides a limited list of DHCP options for ACK responses.
       These do NOT include the server's 'required' options. */
    switch (option) {

    case 1: /* Subnet mask  */
        status = _nx_dhcp_add_option(
            buffer,
            option,
            NX_DHCP_SERVER_OPTION_SUBNET_MASK_SIZE,
            dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_subnet_mask,
            index);
        break;
    ...
    case 3: /* Router IP address */
        ...
        status = _nx_dhcp_add_option(
            buffer,
            option,
            NX_DHCP_SERVER_OPTION_ROUTER_SIZE,
            dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_router_ip_address,
            index);
        break;
    ...
    case 6: /* DNS IP address */
        status = _nx_dhcp_add_option(
            buffer,
            option,
            NX_DHCP_SERVER_OPTION_ADDRESS_SIZE,
            dhcp_ptr->nx_dhcp_interface_table[iface_index].nx_dhcp_dns_ip_address,
            index);
        break;
...
/**************************************************************************/
static UINT _nx_dhcp_add_option(
    UCHAR *dhcp_message, UINT option, UINT size, ULONG value, UINT *index) {
