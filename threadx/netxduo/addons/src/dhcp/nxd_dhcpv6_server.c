...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_server_create(
    ...
    VOID *stack_ptr,
    ULONG stack_size,
    VOID (*dhcpv6_address_declined_handler)(
    ...
    status = _nx_dhcpv6_server_create(
        dhcpv6_server_ptr,
        ip_ptr,
        name_ptr,
        packet_pool_ptr,
        stack_ptr,
        stack_size,
        dhcpv6_address_declined_handler,
...
UINT _nx_dhcpv6_server_create(
    ...
    status = tx_event_flags_create(
        &dhcpv6_server_ptr->nx_dhcpv6_server_timer_events,
        "DHCPv6 Server Timer Events Queue");
    ...
    status = tx_mutex_create(
        &(dhcpv6_server_ptr->nx_dhcpv6_server_mutex),
        "DHCPV6 Server Mutex",
        TX_NO_INHERIT);
    ...
    status = tx_thread_create(
        &(dhcpv6_server_ptr->nx_dhcpv6_server_thread),
        "NetX DHCPV6 Server",
        _nx_dhcpv6_server_thread_entry,
        (ULONG)(ALIGN_TYPE)dhcpv6_server_ptr,
        stack_ptr,
        stack_size,
        NX_DHCPV6_SERVER_THREAD_PRIORITY,
        NX_DHCPV6_SERVER_THREAD_PRIORITY,
        1,
        TX_DONT_START);

    NX_THREAD_EXTENSION_PTR_SET(
        &(dhcpv6_server_ptr->nx_dhcpv6_server_thread), dhcpv6_server_ptr)
    ...
    status = tx_timer_create(
        &(dhcpv6_server_ptr->nx_dhcpv6_lease_timer),
        "NetX DHCPV6 Server Lease timer",
        _nx_dhcpv6_server_lease_timeout_entry,
        (ULONG)(ALIGN_TYPE)dhcpv6_server_ptr,
    ...
    NX_TIMER_EXTENSION_PTR_SET(
        &(dhcpv6_server_ptr->nx_dhcpv6_lease_timer), dhcpv6_server_ptr)
    ...
    status = tx_timer_create(
        &(dhcpv6_server_ptr->nx_dhcpv6_session_timer),
        "NetX DHCPV6 Session Duration timer",
        _nx_dhcpv6_server_session_timeout_entry,
        (ULONG)(ALIGN_TYPE)dhcpv6_server_ptr,
    ...
    NX_TIMER_EXTENSION_PTR_SET(
        &(dhcpv6_server_ptr->nx_dhcpv6_session_timer), dhcpv6_server_ptr)
    ...
    dhcpv6_server_ptr->nx_dhcpv6_server_socket.nx_udp_socket_reserved_ptr =
        (void *)dhcpv6_server_ptr;

    /* Set the DHCPv6 Client ID.  */
    dhcpv6_server_ptr->nx_dhcpv6_id = NX_DHCPV6_SERVER_ID;

    /* Set the server preference for clients using multicast only for all request message
     * types.  */
    dhcpv6_server_ptr->nx_dhcpv6_server_multicast_only = NX_TRUE;

    /* Assign the various handlers (server error messages, state change and
     * expired/deprecated addresses). */
    dhcpv6_server_ptr->nx_dhcpv6_IP_address_declined_handler =
        dhcpv6_address_declined_handler;
    dhcpv6_server_ptr->nx_dhcpv6_server_option_request_handler =
        dhcpv6_option_request_handler;
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_server_option_request_handler_set(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    ...
    status = _nx_dhcpv6_server_option_request_handler_set(
        dhcpv6_server_ptr, dhcpv6_option_request_handler_extended);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_server_option_request_handler_set(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    VOID (*dhcpv6_option_request_handler_extended)(
        struct NX_DHCPV6_SERVER_STRUCT *dhcpv6_server_ptr,
        UINT option_request,
    ...
    dhcpv6_server_ptr->nx_dhcpv6_server_option_request_handler_extended =
        dhcpv6_option_request_handler_extended;
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_add_client_record(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    UINT table_index,
    ULONG message_xid,
    ...
    /* Check for invalid LINK LAYER duid data. */
        ...
        (duid_type == NX_DHCPV6_SERVER_DUID_TYPE_LINK_TIME) ||
        (duid_type == NX_DHCPV6_SERVER_DUID_TYPE_LINK_ONLY)) {
    ...
    status = _nx_dhcpv6_add_client_record(
        dhcpv6_server_ptr,
        table_index,
        message_xid,
        client_address,
        client_state,
        IP_lease_time_accrued,
        valid_lifetime,
        duid_type,
        duid_hardware,
...
UINT _nx_dhcpv6_add_client_record(
    ...
    if (table_index != 0xFFFFFFFF) {
        ...
        if (dhcpv6_server_ptr->nx_dhcpv6_clients[table_index].nx_dhcpv6_message_xid !=
            0) {
    ...
    memcpy(
        &(dhcpv6_client_ptr->nx_dhcpv6_server_duid),
        &(dhcpv6_server_ptr->nx_dhcpv6_server_duid),
        sizeof(NX_DHCPV6_SVR_DUID)); /* Use case of memcpy is verified. */
    ...
    if (duid_type == NX_DHCPV6_SERVER_DUID_TYPE_VENDOR_ASSIGNED) {
        ...
        dhcpv6_client_ptr->nx_dhcpv6_client_duid.nx_duid_enterprise_number =
            duid_vendor_number;
        memcpy(
            &(dhcpv6_client_ptr->nx_dhcpv6_client_duid.nx_duid_private_identifier[0]),
            duid_vendor_private,
            duid_private_length); /* Use case of memcpy is verified. */
        dhcpv6_client_ptr->nx_dhcpv6_client_duid.nx_option_length =
            (USHORT)(duid_private_length + 6);
    } else {

        /* Either link layer or link layer plus time. */
        dhcpv6_client_ptr->nx_dhcpv6_client_duid.nx_link_layer_address_msw =
            (USHORT)physical_address_msw;
        dhcpv6_client_ptr->nx_dhcpv6_client_duid.nx_link_layer_address_lsw =
            physical_address_lsw;
    ...
    COPY_NXD_ADDRESS(
        client_address, &(dhcpv6_client_ptr->nx_dhcpv6_ia.nx_global_address));
    ...
    for (i = 0; i < NX_DHCPV6_MAX_LEASES; i++) {
        ...
        if (CHECK_IPV6_ADDRESSES_SAME(
                ...
                &(dhcpv6_server_ptr->nx_dhcpv6_lease_list[i]
                      .nx_dhcpv6_lease_IP_address.nxd_ip_address.v6[0]))) {

            /* They match! Cross link the tables. */
            dhcpv6_server_ptr->nx_dhcpv6_lease_list[i].nx_dhcpv6_lease_assigned_to =
                dhcpv6_client_ptr;
            found = NX_TRUE;
            break;
        }
    ...
    if (!found) {
        ...
        /* This client apparently does not have a valid IP lease. Ok to keep in the
           tables, but they will need to request an IP address! */
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_retrieve_client_record(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    UINT table_index,
    ULONG *message_xid,
    NXD_ADDRESS *client_address,
    ...
    /* Check for invalid pointer input. */
    if ((dhcpv6_server_ptr == NX_NULL) || (message_xid == NX_NULL) ||
        (client_address == NX_NULL) || (client_state == NX_NULL) ||
        (IP_lease_time_accrued == NX_NULL) || (valid_lifetime == NX_NULL) ||
        (duid_type == NX_NULL)) {
    ...
    status = _nx_dhcpv6_retrieve_client_record(
        dhcpv6_server_ptr,
        table_index,
        message_xid,
        client_address,
...
UINT _nx_dhcpv6_retrieve_client_record(
    ...
    *duid_type = client_ptr->nx_dhcpv6_client_duid.nx_duid_type;
    ...
    if ((*duid_type == NX_DHCPV6_SERVER_DUID_TYPE_LINK_ONLY) ||
        (*duid_type == NX_DHCPV6_SERVER_DUID_TYPE_LINK_TIME)) {

        /* Check for invalid pointer input for DUID of the link layer type. */
        if ((duid_hardware == NX_NULL) || (physical_address_msw == NX_NULL) ||
            (physical_address_lsw == NX_NULL)) {
            return NX_PTR_ERROR;
        }

        *duid_hardware = client_ptr->nx_dhcpv6_client_duid.nx_hardware_type;
        *physical_address_msw =
            client_ptr->nx_dhcpv6_client_duid.nx_link_layer_address_msw;
        *physical_address_lsw =
            client_ptr->nx_dhcpv6_client_duid.nx_link_layer_address_lsw;
        ...
        if ((duid_vendor_number == NX_NULL) || (duid_vendor_private == NX_NULL) ||
            (duid_private_length == NX_NULL)) {
            return NX_PTR_ERROR;
        }
        ...
        *duid_vendor_number = client_ptr->nx_dhcpv6_client_duid.nx_duid_enterprise_number;
        ...
        /* NOTE: The duid_vendor_private buffer is provided by user, and the buffer size
         * must be not less than NX_DHCPV6_SERVER_DUID_VENDOR_ASSIGNED_LENGTH.  */
        memcpy(
            duid_vendor_private,
            &(client_ptr->nx_dhcpv6_client_duid.nx_duid_private_identifier[0]),
            (UINT)length); /* Use case of memcpy is verified. The buffer is provided by
                              user.  */
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_add_ip_address_lease(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    UINT table_index,
    ...
    /* Call the actual service and return completion status. */
    status = _nx_dhcpv6_add_ip_address_lease(
        dhcpv6_server_ptr,
        table_index,
        lease_IP_address,
        T1,
...
UINT _nx_dhcpv6_add_ip_address_lease(
    ...
    prefix_length = dhcpv6_server_ptr->nx_dhcpv6_ip_ptr->nx_ipv6_address[ga_address_index]
                        .nxd_ipv6_address_prefix_length;

    /* Verify the input addresses match the server interface address prefix (in IPv4
       terms, the network masked addresses should be equal). */

    if (!CHECK_IP_ADDRESSES_BY_PREFIX(
            &(dhcpv6_server_ptr->nx_dhcpv6_ip_ptr->nx_ipv6_address[ga_address_index]
                  .nxd_ipv6_address[0]),
            &lease_IP_address->nxd_ip_address.v6[0],
            prefix_length)) {
    ...
    if (table_index == 0xFFFFFFFF) {
        ...
        for (i = 0; i < NX_DHCPV6_MAX_LEASES; i++) {
            ...
            if (CHECK_UNSPECIFIED_ADDRESS(
                    &(lease_ptr->nx_dhcpv6_lease_IP_address.nxd_ip_address.v6[0]))) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_retrieve_ip_address_lease(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    UINT table_index,
    NXD_ADDRESS *lease_IP_address,
    ULONG *T1,
    ...
    /* Check for invalid pointer input. */
    if ((dhcpv6_server_ptr == NX_NULL) || (lease_IP_address == NX_NULL) ||
        (T1 == NX_NULL) || (T2 == NX_NULL) || (valid_lifetime == NX_NULL) ||
        (preferred_lifetime == NX_NULL)) {

        return NX_PTR_ERROR;
    }

    status = _nx_dhcpv6_retrieve_ip_address_lease(
        dhcpv6_server_ptr,
        table_index,
        lease_IP_address,
        T1,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_create_ip_address_range(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    ...
    if ((dhcpv6_server_ptr == NX_NULL) || (start_ipv6_address == NX_NULL) ||
        (end_ipv6_address == NX_NULL) || (addresses_added == NX_NULL)) {
        return (NX_PTR_ERROR);
    }
    ...
    if (start_ipv6_address->nxd_ip_address.v6[3] >
        end_ipv6_address->nxd_ip_address.v6[3]) {

        return (NX_DHCPV6_INVALID_IP_ADDRESS);
    }

    status = _nx_dhcpv6_create_ip_address_range(
        dhcpv6_server_ptr, start_ipv6_address, end_ipv6_address, addresses_added);
...
UINT _nx_dhcpv6_create_ip_address_range(
    ...
    *addresses_added = 0;
    ...
    ga_address_index = dhcpv6_server_ptr->nx_dhcpv6_server_ga_address_index;
    prefix_length =
        ip_ptr->nx_ipv6_address[ga_address_index].nxd_ipv6_address_prefix_length;

    /* Verify the input addresses match the server interface address prefix (in IPv4
       terms, the network masked addresses should be equal). */
    if (!CHECK_IP_ADDRESSES_BY_PREFIX(
            &(ip_ptr->nx_ipv6_address[ga_address_index].nxd_ipv6_address[0]),
            &start_ipv6_address->nxd_ip_address.v6[0],
            prefix_length)) {
    ...
    if (!CHECK_IP_ADDRESSES_BY_PREFIX(
            &(ip_ptr->nx_ipv6_address[ga_address_index].nxd_ipv6_address[0]),
            &end_ipv6_address->nxd_ip_address.v6[0],
            prefix_length)) {
    ...
    /* Clear out existing entries and start adding IP addresses at the beginning of the
     * list. */
    i = 0;
    ...
    while (i < NX_DHCPV6_MAX_LEASES) {
        ...
        memset(
            &dhcpv6_server_ptr->nx_dhcpv6_lease_list[i],
            0,
            sizeof(NX_DHCPV6_ADDRESS_LEASE));
        ...
        if (CHECK_IPV6_ADDRESSES_SAME(
                &next_ipv6_address.nxd_ip_address.v6[0],
                &end_ipv6_address->nxd_ip_address.v6[0])) {
            break;
        }
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_reserve_ip_address_range(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NXD_ADDRESS *start_ipv6_address,
    NXD_ADDRESS *end_ipv6_address,
    ...
    /* Check for invalid input.  */
    if ((dhcpv6_server_ptr == NX_NULL) || (start_ipv6_address == NX_NULL) ||
        (end_ipv6_address == NX_NULL) || (addresses_reserved == NX_NULL)) {
        return (NX_PTR_ERROR);
    }
    ...
    status = _nx_dhcpv6_reserve_ip_address_range(
        dhcpv6_server_ptr, start_ipv6_address, end_ipv6_address, addresses_reserved);
...
UINT _nx_dhcpv6_reserve_ip_address_range(
    ...
    *addresses_reserved = 0;
    ...
    while (i < NX_DHCPV6_MAX_LEASES) {
        ...
        if (CHECK_IPV6_ADDRESSES_SAME(
                &ip_lease_ptr->nx_dhcpv6_lease_IP_address.nxd_ip_address.v6[0],
                &start_ipv6_address->nxd_ip_address.v6[0])) {
            break;
        }
    }
    ...
    ip_lease_ptr->nx_dhcpv6_lease_assigned_to =
        (NX_DHCPV6_CLIENT *)(ALIGN_TYPE)0xFFFFFFFF;
    ...
    while ((!CHECK_IPV6_ADDRESSES_SAME(
               &next_ipv6_address.nxd_ip_address.v6[0],
               &end_ipv6_address->nxd_ip_address.v6[0])) &&
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_create_dns_address(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr, NXD_ADDRESS *dns_ipv6_address) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_create_dns_address(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr, NXD_ADDRESS *dns_ipv6_address) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_set_server_duid(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    UINT duid_type,
    UINT hardware_type,
    ...
    /* Check for invalid non pointer input. */
    if ((duid_type == 0) || (hardware_type == 0) || (mac_address_msw == 0) ||
        (mac_address_lsw == 0)) {
        return NX_INVALID_PARAMETERS;
    }

    /* Call the actual service. */
    status = _nx_dhcpv6_set_server_duid(
        dhcpv6_server_ptr,
        duid_type,
        hardware_type,
        mac_address_msw,
...
UINT _nx_dhcpv6_set_server_duid(
    ...
    if (duid_ptr->nx_duid_type != NX_DHCPV6_SERVER_DUID_TYPE_VENDOR_ASSIGNED) {
        ...
        if (sizeof(NX_DHCPV6_SERVER_DUID_VENDOR_ASSIGNED_ID) >
            NX_DHCPV6_SERVER_DUID_VENDOR_ASSIGNED_LENGTH) {
            return NX_DHCPV6_INVALID_VENDOR_DATA;
        }

        memcpy(
            &duid_ptr->nx_duid_private_identifier[0],
            NX_DHCPV6_SERVER_DUID_VENDOR_ASSIGNED_ID,
            sizeof(NX_DHCPV6_SERVER_DUID_VENDOR_ASSIGNED_ID)); /* Use case of memcpy is
                                                                  verified. */
    }
    ...
    if (duid_ptr->nx_duid_type == NX_DHCPV6_SERVER_DUID_TYPE_LINK_ONLY) {
        ...
        /* Set the length to include the duid type, hardware type, time, and msw and lsw
         * fields. */
        ...
        duid_ptr->nx_option_length = sizeof(USHORT) + sizeof(ULONG) +
                                     sizeof(NX_DHCPV6_SERVER_DUID_VENDOR_ASSIGNED_ID);
    } else {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_prepare_iana_status(
    NX_DHCPV6_SERVER_IANA_STATUS *dhcpv6_status_ptr, UINT flag) {

    /* Make sure the basic header fields are filled in. No need to check the status code
       has been set; it is part of the decision to send a response. */
    ...
    case NX_DHCPV6_STATUS_SUCCESS:
        ...
        memcpy(
            &dhcpv6_status_ptr->nx_status_message[0],
            NX_DHCPV6_STATUS_MESSAGE_SUCCESS,
            sizeof(
                NX_DHCPV6_STATUS_MESSAGE_SUCCESS)); /* Use case of memcpy is verified. */

        /* Update the option length. */
        dhcpv6_status_ptr->nx_option_length =
            (USHORT)(dhcpv6_status_ptr->nx_option_length +
                     sizeof(NX_DHCPV6_STATUS_MESSAGE_SUCCESS));
    ...
    case NX_DHCPV6_STATUS_UNSPECIFIED:
        ...
        memcpy(
            &dhcpv6_status_ptr->nx_status_message[0],
            NX_DHCPV6_STATUS_MESSAGE_UNSPECIFIED,
            sizeof(NX_DHCPV6_STATUS_MESSAGE_UNSPECIFIED)); /* Use case of memcpy is
                                                              verified. */

        /* Update the option length. */
        dhcpv6_status_ptr->nx_option_length =
            (USHORT)(dhcpv6_status_ptr->nx_option_length +
                     sizeof(NX_DHCPV6_STATUS_MESSAGE_UNSPECIFIED));
    ...
    case NX_DHCPV6_STATUS_NO_ADDRS_AVAILABLE:
        ...
        memcpy(
            ...
            sizeof(NX_DHCPV6_STATUS_MESSAGE_NO_ADDRS_AVAILABLE)); /* Use case of memcpy is
                                                                     verified. */

        /* Update the option length. */
        dhcpv6_status_ptr->nx_option_length =
            (USHORT)(dhcpv6_status_ptr->nx_option_length +
                     sizeof(NX_DHCPV6_STATUS_MESSAGE_NO_ADDRS_AVAILABLE));
    ...
    case NX_DHCPV6_STATUS_NO_BINDING:
        ...
        memcpy(
            ...
            sizeof(NX_DHCPV6_STATUS_MESSAGE_NO_BINDING)); /* Use case of memcpy is
                                                             verified. */

        /* Update the option length. */
        dhcpv6_status_ptr->nx_option_length =
            (USHORT)(dhcpv6_status_ptr->nx_option_length +
                     sizeof(NX_DHCPV6_STATUS_MESSAGE_NO_BINDING));
    ...
    case NX_DHCPV6_STATUS_NOT_ON_LINK:
        ...
        memcpy(
            ...
            sizeof(NX_DHCPV6_STATUS_MESSAGE_NOT_ON_LINK)); /* Use case of memcpy is
                                                              verified. */

        /* Update the option length. */
        dhcpv6_status_ptr->nx_option_length =
            (USHORT)(dhcpv6_status_ptr->nx_option_length +
                     sizeof(NX_DHCPV6_STATUS_MESSAGE_NOT_ON_LINK));
    ...
    case NX_DHCPV6_STATUS_USE_MULTICAST:
        ...
        memcpy(
            ...
            sizeof(NX_DHCPV6_STATUS_MESSAGE_USE_MULTICAST)); /* Use case of memcpy is
                                                                verified. */

        /* Update the option length. */
        dhcpv6_status_ptr->nx_option_length =
            (USHORT)(dhcpv6_status_ptr->nx_option_length +
                     sizeof(NX_DHCPV6_STATUS_MESSAGE_USE_MULTICAST));
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_add_preference(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    UCHAR *buffer_ptr,
    UINT *index) {
    ...
    available_payload =
        dhcpv6_server_ptr->nx_dhcpv6_packet_pool_ptr->nx_packet_pool_payload_size -
        NX_IPv6_UDP_PACKET - *index;

    /* Check if the DUID will fit in the packet buffer. */
    if (available_payload <
        (UINT)(dhcpv6_client_ptr->nx_dhcpv6_preference.nx_option_length + 4)) {
    ...
    *(buffer_ptr + *index) =
        (UCHAR)(dhcpv6_client_ptr->nx_dhcpv6_preference.nx_pref_value);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_add_option_requests(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    UCHAR *buffer_ptr,
    UINT *index) {
    ...
    for (i = 0; i < NX_DHCPV6_MAX_OPTION_REQUEST_OPTIONS; i++) {
        ...
        if (dhcpv6_option_ptr->nx_op_request[i] == NX_DHCPV6_RFC_DNS_SERVER_OPTION) {
            ...
            *index += (ULONG)sizeof(ULONG);
            ...
            available_payload = (UINT)(available_payload -
                                       (UINT)(dhcpv6_option_ptr->nx_option_length + 4));
        }
        /* Does the DHCPv6 server have an extended option handler? */
            ...
            /* Yes, so call it.  This handler shall fill the buffer with the requested
               ...
               Note: The handler must update the index pointing to the end of data in the
               buffer after it inserts the data. This is done for the DNS server option
               above.  */
            (dhcpv6_server_ptr->nx_dhcpv6_server_option_request_handler_extended)(
                dhcpv6_server_ptr,
                dhcpv6_option_ptr->nx_op_request[i],
                buffer_ptr,
                index,
                available_payload);

            /* Check if exceed the payload.  */
            if ((NX_IPv6_UDP_PACKET + *index) <=
                dhcpv6_server_ptr->nx_dhcpv6_packet_pool_ptr
                    ->nx_packet_pool_payload_size) {

                /* Update the available payload.  */
                available_payload = dhcpv6_server_ptr->nx_dhcpv6_packet_pool_ptr
                                        ->nx_packet_pool_payload_size -
                                    NX_IPv6_UDP_PACKET - *index;
            } else {
                return (NX_DHCPV6_INSUFFICIENT_PACKET_PAYLOAD);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcpv6_server_interface_set(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr, UINT iface_index, UINT ga_address_index) {
    ...
    /* Call the actual service. */
    status =
        _nx_dhcpv6_server_interface_set(dhcpv6_server_ptr, iface_index, ga_address_index);
...
VOID _nx_dhcpv6_server_lease_timeout_entry(ULONG dhcpv6_server_ptr_value) {
    ...
    NX_TIMER_EXTENSION_PTR_GET(
        dhcpv6_server_ptr, NX_DHCPV6_SERVER, dhcpv6_server_ptr_value)

    /* Signal the DHCPv6 Server it is time to do check the lease time remaining on
       clients it has leased IP addresses to.  */
    tx_event_flags_set(
        &(dhcpv6_server_ptr->nx_dhcpv6_server_timer_events),
        NX_DHCPV6_IP_LEASE_CHECK_PERIODIC_EVENT,
        TX_OR);
...
VOID _nx_dhcpv6_server_session_timeout_entry(ULONG dhcpv6_server_ptr_value) {
    ...
    tx_event_flags_set(
        &(dhcpv6_server_ptr->nx_dhcpv6_server_timer_events),
        NX_DHCPV6_CHECK_SESSION_PERIODIC_EVENT,
        TX_OR);
...
UINT _nx_dhcpv6_server_start(NX_DHCPV6_SERVER *dhcpv6_server_ptr) {
    ...
    status = nx_udp_socket_create(
        dhcpv6_server_ptr->nx_dhcpv6_ip_ptr,
        &(dhcpv6_server_ptr->nx_dhcpv6_server_socket),
        (CHAR *)"NetX DHCPV6 Server",
    ...
    status = nx_udp_socket_receive_notify(
        &(dhcpv6_server_ptr->nx_dhcpv6_server_socket),
        _nx_dhcpv6_server_socket_receive_notify);
    ...
    /* Check if the host application has created or retrieved a previously created server
     * DUID. */
...
VOID _nx_dhcpv6_server_socket_receive_notify(NX_UDP_SOCKET *socket_ptr) {
    ...
    tx_event_flags_set(
        &(dhcpv6_server_ptr->nx_dhcpv6_server_timer_events),
        NX_DHCPV6_SERVER_RECEIVE_EVENT,
        TX_OR);
    ...
    while (1) {
        ...
        if (dhcpv6_events & NX_DHCPV6_CHECK_SESSION_PERIODIC_EVENT) {
            ...
            for (i = 0; i < NX_DHCPV6_MAX_CLIENTS; i++) {
                ...
                if (dhcpv6_client_ptr->nx_dhcpv6_client_session_time) {
                    ...
                    dhcpv6_client_ptr->nx_dhcpv6_client_session_time +=
                        NX_DHCPV6_SESSION_TIMER_INTERVAL;

                    if (dhcpv6_client_ptr->nx_dhcpv6_client_session_time >=
                        NX_DHCPV6_SESSION_TIMEOUT) {

                        /* The session has timed out without a response from the Client.
                           Invalidate the client entry. */
                        ...
                        /* Clear the client record. */
                        _nx_dhcpv6_clear_client_record(
                            dhcpv6_server_ptr, dhcpv6_client_ptr);
                    }
                }
        ...
        if (dhcpv6_events & NX_DHCPV6_IP_LEASE_CHECK_PERIODIC_EVENT) {
            ...
            for (i = 0; i < NX_DHCPV6_MAX_CLIENTS; i++) {
                ...
                dhcpv6_client_ptr->nx_dhcpv6_IP_lease_time_accrued +=
                    NX_DHCPV6_IP_LEASE_TIMER_INTERVAL;

                /* Has this address become obsolete? */
                if (dhcpv6_client_ptr->nx_dhcpv6_IP_lease_time_accrued >=
                    dhcpv6_client_ptr->nx_dhcpv6_ia.nx_valid_lifetime) {
                    ...
                    /* Address is deprecated! Client *should* not use it but no action
                     * taken by the server yet. */
                }
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_server_extract_packet_information(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_CLIENT **dhcpv6_client_ptr,
    ...
    *dhcpv6_client_ptr = NX_NULL;
    ...
    /* Create a scratch record for this client so we can qualify it before adding it to
     * the server table. */
    ...
    COPY_IPV6_ADDRESS(
        &source_address.nxd_ip_address.v6[0],
        &temp_client_rec.nx_dhcp_source_ip_address.nxd_ip_address.v6[0]);
    ...
    COPY_IPV6_ADDRESS(
        &destination_address.nxd_ip_address.v6[0],
        &temp_client_rec.nx_dhcp_destination_ip_address.nxd_ip_address.v6[0]);
    ...
    while (index + 4 <= packet_ptr->nx_packet_length) {
        ...
        status = _nx_dhcpv6_server_utility_get_block_option_length(
        ...
        case NX_DHCPV6_OP_DUID_CLIENT:
            ...
            status = _nx_dhcpv6_process_duid(
                &client_duid, option_code, option_length, buffer_ptr);
            ...
            memcpy(
                &temp_client_rec.nx_dhcpv6_client_duid,
                &client_duid,
                sizeof(NX_DHCPV6_SVR_DUID)); /* Use case of memcpy is verified. */
        ...
        case NX_DHCPV6_OP_DUID_SERVER:
            ...
            status = _nx_dhcpv6_process_duid(
                &server_duid, option_code, option_length, buffer_ptr);
            ...
            /*  Copy to the temporary client record. */
            memcpy(
                &temp_client_rec.nx_dhcpv6_server_duid,
                &server_duid,
                sizeof(NX_DHCPV6_SVR_DUID)); /* Use case of memcpy is verified. */
        ...
        case NX_DHCPV6_OP_IA_NA:
            ...
            status = _nx_dhcpv6_server_process_iana(
                &temp_client_rec, option_code, option_length, buffer_ptr);
        ...
        case NX_DHCPV6_OP_OPTION_REQUEST:
            ...
            /* Option request option contains request usually for network configuration
               parameters such as the DNS server. */
            status = _nx_dhcpv6_process_option_request(
                &temp_client_rec, option_code, option_length, buffer_ptr);
        ...
        case NX_DHCPV6_OP_ELAPSED_TIME:
            ...
            /* The elapsed time is time elapsed since the client initiated the IP lease
             * request. */
            status = _nx_dhcpv6_process_elapsed_time(
                &temp_client_rec, option_code, option_length, buffer_ptr);
    ...
    /* The following message types must include a client and server DUID. */
        ...
        (temp_client_rec.nx_dhcpv6_message_type == NX_DHCPV6_MESSAGE_TYPE_REQUEST) ||
    ...
    /* An inform request only requires a server DUID. */
        ...
        temp_client_rec.nx_dhcpv6_message_type == NX_DHCPV6_MESSAGE_TYPE_INFORM_REQUEST) {
    ...
    /* Validate the client message, and determine how (if) the server should respond to
     * it. */
    ...
    if (status != NX_SUCCESS) {
        ...
        /* Return all other errors as completion status. Do not send a message to the
         * client. */
        return (status);
    }

    if (temp_client_rec.nx_dhcpv6_response_back_from_server ==
        NX_DHCPV6_MESSAGE_TYPE_DHCPSILENT) {
    ...
    if ((received_message_type == NX_DHCPV6_MESSAGE_TYPE_SOLICIT) ||
        ...
        /* Check first if the validation process indicates we do not reply to this
         * message. */
        if (temp_client_rec.nx_dhcpv6_response_back_from_server !=
            NX_DHCPV6_MESSAGE_TYPE_DHCPSILENT) {
    ...
    status = _nx_dhcpv6_find_client_record_by_duid(
        dhcpv6_server_ptr,
        &client_duid,
        &record_index,
        add_on,
        temp_client_rec.nx_dhcpv6_message_xid,
        &matching);
    ...
    *dhcpv6_client_ptr = &dhcpv6_server_ptr->nx_dhcpv6_clients[record_index];
    ...
    if (matching) {
        ...
        /*  This is a different message from the Client. Before updating the record check
           the client global address to be the same as what the server assigned or is
           offering to assign. */
        if (!CHECK_IPV6_ADDRESSES_SAME(
                &temp_client_rec.nx_dhcpv6_ia.nx_global_address.nxd_ip_address.v6[0],
                &(*dhcpv6_client_ptr)
                     ->nx_dhcpv6_ia.nx_global_address.nxd_ip_address.v6[0])) {

            /* This is a different address. Disregard this message. */
            (*dhcpv6_client_ptr)->nx_dhcpv6_response_back_from_server =
                NX_DHCPV6_MESSAGE_TYPE_DHCPSILENT;
    ...
    status = _nx_dhcpv6_update_client_record(
        dhcpv6_server_ptr, &temp_client_rec, *dhcpv6_client_ptr);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_validate_client_message(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr, NX_DHCPV6_CLIENT *dhcpv6_client_ptr) {
    ...
    /* Initialize the response to the client. If the client request is invalid, ignore the
       request (no response from server) or indicate the problem in the IANA status
       option. */
    ...
    if (dhcpv6_client_ptr->nx_dhcpv6_message_type == NX_DHCPV6_MESSAGE_TYPE_SOLICIT) {
        ...
        dhcpv6_client_ptr->nx_dhcpv6_iana_status.nx_status_code =
            NX_DHCPV6_STATUS_SUCCESS;
    } else {
    ...
    dest_address_type = IPv6_Address_Type(
        &dhcpv6_client_ptr->nx_dhcp_destination_ip_address.nxd_ip_address.v6[0]);
    ...
    case NX_DHCPV6_MESSAGE_TYPE_SOLICIT: {
        ...
        if (dhcpv6_client_ptr->nx_dhcpv6_server_duid.nx_link_layer_address_msw != 0) {
            ...
            dhcpv6_client_ptr->nx_dhcpv6_iana_status.nx_status_code =
                NX_DHCPV6_STATUS_UNSPECIFIED;

            return (NX_SUCCESS);
        }

        /* The Solicit message should never be unicast. Check if the destination is
         * unicast. */
    ...
    case NX_DHCPV6_MESSAGE_TYPE_DECLINE: {
        ...
        if (dhcpv6_client_ptr->nx_dhcpv6_message_type != NX_DHCPV6_MESSAGE_TYPE_REBIND) {
            ...
            /* The DHCPv6 server does not support unicast. Check for unicast client
             * destination. */
            if (IPV6_ADDRESS_UNICAST & dest_address_type) {

                /* It is unicast. Set the use multicast error status. */
                dhcpv6_client_ptr->nx_dhcpv6_iana_status.nx_status_code =
                    NX_DHCPV6_STATUS_USE_MULTICAST;

                return NX_SUCCESS;
            }
        }

        /* This client message is not valid if an matching client record does not exist
         * with the server. */
        status = _nx_dhcpv6_find_client_record_by_duid(
            dhcpv6_server_ptr,
            &dhcpv6_client_ptr->nx_dhcpv6_client_duid,
            &record_index,
            NX_FALSE,
        ...
        if (!matching_clients) {
            ...
            /* For Release and Decline messages let the client know using the IANA status
               code that the server has no record of the assigned or requested address for
               this client.  */
            if ((dhcpv6_client_ptr->nx_dhcpv6_message_type ==
                 NX_DHCPV6_MESSAGE_TYPE_RELEASE) ||
                (dhcpv6_client_ptr->nx_dhcpv6_message_type ==
        ...
        if (dhcpv6_client_ptr->nx_dhcpv6_message_type == NX_DHCPV6_MESSAGE_TYPE_REBIND) {
            ...
            status = _nx_dhcpv6_check_duids_same(
                &dhcpv6_client_ptr->nx_dhcpv6_server_duid,
                &dhcpv6_server_ptr->nx_dhcpv6_server_duid,
                &matching_server_duids);
    ...
    case NX_DHCPV6_MESSAGE_TYPE_CONFIRM: {
        ...
        if (!CHECK_IP_ADDRESSES_BY_PREFIX(
            ...
            dhcpv6_client_ptr->nx_dhcpv6_iana_status.nx_status_code =
                NX_DHCPV6_STATUS_NOT_ON_LINK;

            return (NX_SUCCESS);
        }

        /* More checks will be done when the client rebind address is searched in the
           server address table. For now we are ok. */
    ...
    case NX_DHCPV6_MESSAGE_TYPE_INFORM_REQUEST: {
        ...
        /* Verify Client's server duid matches the server DUID. Note that if the server
           DUID type is a link layer time type (LLT), this comparison should ignore the
           time field because this is an 'asynchronous' request from the client, so just
           match on the LL data. */

        /* Temporarily set the server DUID type to LL if it was LLT. */
        if ((dhcpv6_client_ptr->nx_dhcpv6_server_duid.nx_duid_type ==
             NX_DHCPV6_SERVER_DUID_TYPE_LINK_TIME) ||
            (dhcpv6_client_ptr->nx_dhcpv6_server_duid.nx_duid_type ==
             NX_DHCPV6_SERVER_DUID_TYPE_LINK_ONLY)) {

            /* Now verify the physical address matches what the server has for this
             * client. */
            ...
            if (dhcpv6_client_ptr->nx_dhcpv6_server_duid.nx_link_layer_address_msw !=
                dhcpv6_server_ptr->nx_dhcpv6_ip_ptr->nx_ip_interface[if_index]
                ...
                dhcpv6_client_ptr->nx_dhcpv6_server_duid.nx_link_layer_address_lsw !=
                dhcpv6_server_ptr->nx_dhcpv6_ip_ptr->nx_ip_interface[if_index]
            ...
            /* Verify the client inform request was intended for this server. If the
               Client did not include a server DUID, which is acceptable in DHCPv6
               protocol, it will 'pass' this check. */
            status = _nx_dhcpv6_check_duids_same(
                &dhcpv6_client_ptr->nx_dhcpv6_server_duid,
                &dhcpv6_client_ptr->nx_dhcpv6_server_duid,
                &matching_server_duids);
    ...
    default: {
        ...
        /* No further processing to do. Leave the client state unchanged, whatever it
         * was/is. */
    }
    }
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_server_utility_get_block_option_length(
...
UINT _nx_dhcpv6_listen_for_messages(NX_DHCPV6_SERVER *dhcpv6_server_ptr) {
    ...
    COPY_IPV6_ADDRESS(
        &ipv6_header_ptr->nx_ip_header_source_ip[0],
        &source_address.nxd_ip_address.v6[0]);
    source_address.nxd_ip_version = NX_IP_VERSION_V6;

    /* Get the destination address. */
    COPY_IPV6_ADDRESS(
        &ipv6_header_ptr->nx_ip_header_destination_ip[0],
        &destination_address.nxd_ip_address.v6[0]);
    destination_address.nxd_ip_version = NX_IP_VERSION_V6;

    /* We will copy the received packet (datagram) over to a packet from the DHCP Server
       pool and release the packet from the receive packet pool as soon as possible. */
    ...
    /* Extract the DHCPv6 specific information from the packet. This will create new
       Client record or update existing client record in the server table. */
    status = _nx_dhcpv6_server_extract_packet_information(
        dhcpv6_server_ptr,
        &dhcpv6_client_ptr,
        new_packet_ptr,
        iface_index,
    ...
    if (dhcpv6_client_ptr->nx_dhcpv6_iana_status.nx_status_code ==
        NX_DHCPV6_STATUS_SUCCESS) {

        /* Yes it will. Check if the client is requesting an IP address, or renewing or
           rebinding a previously assigned one. */
        if ((dhcpv6_client_ptr->nx_dhcpv6_message_type ==
            ...
            status = _nx_dhcpv6_assign_ip_address(
                dhcpv6_server_ptr, dhcpv6_client_ptr, &dhcpv6_interface_list_ptr);
            ...
            if (status != NX_SUCCESS) {
                ...
                /* Return all other errors as completion status. Do not send a message to
                   the client. The server will otherwise wait for the client to try again.
                   Do not clear client record. */
                return (status);
            }
            ...
            if (dhcpv6_interface_list_ptr) {
                ...
                /* Now ensure we are sending valid lease time parameters with the IP
                 * address. */
                if (dhcpv6_client_ptr->nx_dhcpv6_message_type ==
                    NX_DHCPV6_MESSAGE_TYPE_REQUEST) {

                    /* Ignore invalid preferred or valid lifetimes from the client by
                     * setting to zero. */
                    ...
                    if (dhcpv6_client_ptr->nx_dhcpv6_ia.nx_preferred_lifetime &&
                        (dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T1 >
                         dhcpv6_client_ptr->nx_dhcpv6_ia.nx_preferred_lifetime)) {
                        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T1 = 0;
                        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 = 0;
                    } else if (
                        dhcpv6_client_ptr->nx_dhcpv6_ia.nx_valid_lifetime &&
                        (dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 >
                         dhcpv6_client_ptr->nx_dhcpv6_ia.nx_valid_lifetime)) {
                        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T1 = 0;
                        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 = 0;
                    } else if (
                ...
                /* For a solicit message, ignore the preferred and valid lifetimes and
                   check for valid T1 and T2 times. */
                else if (
                    dhcpv6_client_ptr->nx_dhcpv6_message_type ==
                    NX_DHCPV6_MESSAGE_TYPE_SOLICIT) {

                    if (dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T1 >
                        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2) {
                        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T1 = 0;
                        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 = 0;
                    }
                ...
                /* If lifetime values are zero, or exceed the server default time, set to
                 * the default lease times. */
                ...
                if ((dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 == 0) ||
                    (dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 >
                     NX_DHCPV6_DEFAULT_T2_TIME)) {

                    dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 = NX_DHCPV6_DEFAULT_T2_TIME;
                }

                /* Update the lease length in the server lease table. */
                dhcpv6_interface_list_ptr->nx_dhcpv6_lease_preferred_lifetime =
                    dhcpv6_client_ptr->nx_dhcpv6_ia.nx_preferred_lifetime;
                dhcpv6_interface_list_ptr->nx_dhcpv6_lease_valid_lifetime =
                    dhcpv6_client_ptr->nx_dhcpv6_ia.nx_valid_lifetime;
                dhcpv6_interface_list_ptr->nx_dhcpv6_lease_T1_lifetime =
                    dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T1;
                dhcpv6_interface_list_ptr->nx_dhcpv6_lease_T2_lifetime =
                    dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2;

                /* Ok to copy this address into the client record. */
                COPY_NXD_ADDRESS(
                    &dhcpv6_interface_list_ptr->nx_dhcpv6_lease_IP_address,
                    &dhcpv6_client_ptr->nx_dhcpv6_ia.nx_global_address);
            }
        }
    }
    ...
    /* Check if we need to remove the client record e.g. if the Client sent a Decline
     * message. */
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_send_response_to_client(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr, NX_DHCPV6_CLIENT *dhcpv6_client_ptr) {
    ...
    /* Clear the payload and packet interface field before preparing the packet for
     * transmission. */
    memset(
        buffer,
        0,
    ...
    dhcpv6_client_ptr->nx_dhcpv6_message_xid =
        dhcpv6_client_ptr->nx_dhcpv6_message_xid & 0x0ffffff;

    /* Clear memory to make the message header. */
    memset(&message_word, 0, sizeof(ULONG));

    /* Add the server message type and matching client transaction ID as the DHCPv6 header
     * fields. */
    message_word =
        (ULONG)((dhcpv6_client_ptr->nx_dhcpv6_response_back_from_server << 24) |
    ...
    /* Handle the special case of an Inform Request which does not include a Client
     * identifier. */
    if (dhcpv6_client_ptr->nx_dhcpv6_message_type ==
        NX_DHCPV6_MESSAGE_TYPE_INFORM_REQUEST) {
        ...
        if (dhcpv6_client_ptr->nx_dhcpv6_client_duid.nx_op_code != 0) {
            ...
            status = _nx_dhcpv6_add_duid(
                dhcpv6_server_ptr,
                &dhcpv6_client_ptr->nx_dhcpv6_client_duid,
                buffer,
                &buffer_index,
                NX_DHCPV6_CLIENT_DUID_TYPE);
        }
    } else {
    ...
    if (dhcpv6_client_ptr->nx_dhcpv6_message_type == NX_DHCPV6_MESSAGE_TYPE_CONFIRM) {
        ...
        status = _nx_dhcpv6_prepare_iana_status(
            &dhcpv6_client_ptr->nx_dhcpv6_iana_status, NX_TRUE);
        ...
        status = _nx_dhcpv6_add_iana_status(
            dhcpv6_server_ptr,
            &dhcpv6_client_ptr->nx_dhcpv6_iana_status,
            buffer,
            &buffer_index);

        /* Update the IANA status message. */
        status = _nx_dhcpv6_prepare_iana_status(
            &dhcpv6_client_ptr->nx_dhcpv6_iana_status, NX_FALSE);
    ...
    if ((dhcpv6_client_ptr->nx_dhcpv6_message_type !=
        ...
        status = _nx_dhcpv6_server_add_iana(
            dhcpv6_server_ptr, dhcpv6_client_ptr, buffer, &buffer_index);
    ...
    if (dhcpv6_client_ptr->nx_dhcpv6_response_back_from_server ==
        NX_DHCPV6_MESSAGE_TYPE_ADVERTISE)

    {

        status = _nx_dhcpv6_add_preference(
            dhcpv6_server_ptr, dhcpv6_client_ptr, buffer, &buffer_index);
    }
    ...
    if ((dhcpv6_client_ptr->nx_dhcpv6_message_type == NX_DHCPV6_MESSAGE_TYPE_SOLICIT) ||
        ...
        if (dhcpv6_client_ptr->nx_dhcpv6_iana_status.nx_status_code ==
            ...
            status = _nx_dhcpv6_add_option_requests(
                dhcpv6_server_ptr, dhcpv6_client_ptr, buffer, &buffer_index);
    ...
    status = nxd_udp_socket_interface_send(
        &(dhcpv6_server_ptr->nx_dhcpv6_server_socket),
        packet_ptr,
        &dhcpv6_client_ptr->nx_dhcp_source_ip_address,
        NX_DHCPV6_CLIENT_UDP_PORT,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_assign_ip_address(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    ...
    *interface_address_ptr = NX_NULL;
    ...
    if (!CHECK_UNSPECIFIED_ADDRESS(
        ...
        status = _nx_dhcpv6_find_ip_address(
            dhcpv6_server_ptr, dhcpv6_client_ptr, &temp_interface_address_ptr);
        ...
        if (temp_interface_address_ptr) {
            ...
            if (temp_interface_address_ptr->nx_dhcpv6_lease_assigned_to) {
                ...
                status = _nx_dhcpv6_check_duids_same(
                    &dhcpv6_client_ptr->nx_dhcpv6_client_duid,
                    &(temp_interface_address_ptr->nx_dhcpv6_lease_assigned_to
            ...
            if (matching) {
                ...
                *interface_address_ptr = temp_interface_address_ptr;
                ...
                /* Is it Ok to assign another IP address only if this is a Solicit
                 * request? */
                if (dhcpv6_client_ptr->nx_dhcpv6_message_type !=
                    NX_DHCPV6_MESSAGE_TYPE_SOLICIT) {
            ...
            if (dhcpv6_client_ptr->nx_dhcpv6_message_type !=
                ...
                /* Because the server can not validate the IP address to assign, set the
                 * lifetimes to zero RFC 3315 Sect 18.2.3, 18.2.4. */
                dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T1 = 0;
                dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 = 0;
                dhcpv6_client_ptr->nx_dhcpv6_ia.nx_valid_lifetime = 0;
                dhcpv6_client_ptr->nx_dhcpv6_ia.nx_preferred_lifetime = 0;
    ...
    if (i == NX_DHCPV6_MAX_LEASES) {
        ...
        /* Because the server can not assign the IP address, set the lifetimes to zero RFC
         * 3315 Sect 18.2.3, 18.2.4. */
        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T1 = 0;
        dhcpv6_client_ptr->nx_dhcpv6_iana.nx_T2 = 0;
        dhcpv6_client_ptr->nx_dhcpv6_ia.nx_valid_lifetime = 0;
        dhcpv6_client_ptr->nx_dhcpv6_ia.nx_preferred_lifetime = 0;
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_find_client_record_by_duid(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_SVR_DUID *duid_ptr,
    UINT *record_index,
    UINT add_on,
    ...
    *matching = NX_FALSE;
    ...
    /* Initialize an available slot in the table as outside the boundary (e.g. no slots
     * available). */
    ...
    while (i < NX_DHCPV6_MAX_CLIENTS) {
        ...
        if (!client_record_ptr->nx_dhcpv6_id) {
            ...
            /* Flag the first empty record in case we need to add the current client to
             * the table. */
        ...
        status = _nx_dhcpv6_check_duids_same(
            &client_record_ptr->nx_dhcpv6_client_duid, duid_ptr, matching);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_find_ip_address(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    NX_DHCPV6_ADDRESS_LEASE **interface_address_ptr) {
    ...
    *interface_address_ptr = NX_NULL;
    ...
    /* Search through the interface specific address table. For single homed hosts, there
     * is only one address table. */
    for (i = 0; i < NX_DHCPV6_MAX_LEASES; i++) {

        /* Get a pointer to the address, using the client's interface index to access the
         * correct IP address table. */
        server_table_address =
            dhcpv6_server_ptr->nx_dhcpv6_lease_list[i].nx_dhcpv6_lease_IP_address;

        /* Check for a match. */
        if (CHECK_IPV6_ADDRESSES_SAME(
                &server_table_address.nxd_ip_address.v6[0],
                &dhcpv6_client_ptr->nx_dhcpv6_ia.nx_global_address.nxd_ip_address
                     .v6[0])) {

            /* Set the location of the matching server entry, to indicate we found a
             * match. */
            *interface_address_ptr = &dhcpv6_server_ptr->nx_dhcpv6_lease_list[i];
            break;
        }
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_clear_client_record(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr, NX_DHCPV6_CLIENT *dhcpv6_client_ptr) {
    ...
    if (!CHECK_UNSPECIFIED_ADDRESS(
        ...
        /* Yes, We need to free up the Client's assigned IP address in the server
         * database. */
        while (i < NX_DHCPV6_MAX_LEASES) {
            ...
            if (CHECK_IPV6_ADDRESSES_SAME(
                ...
                if (dhcpv6_client_ptr->nx_dhcpv6_message_type ==
                    ...
                    /* Apparently another client has this address so do not clear the
                     * assignment status. */
                    dhcpv6_server_ptr->nx_dhcpv6_lease_list[i]
                        .nx_dhcpv6_lease_assigned_to =
                        (NX_DHCPV6_CLIENT *)(ALIGN_TYPE)0xFFFFFFFF;
                } else {

                    /* Restore availability by clearing the owner. */
                    dhcpv6_server_ptr->nx_dhcpv6_lease_list[i]
                        .nx_dhcpv6_lease_assigned_to = NX_NULL;
                }

                dhcpv6_server_ptr->nx_dhcpv6_lease_list[i].nx_dhcpv6_lease_T1_lifetime =
                    0;
                dhcpv6_server_ptr->nx_dhcpv6_lease_list[i].nx_dhcpv6_lease_T2_lifetime =
                    0;
                dhcpv6_server_ptr->nx_dhcpv6_lease_list[i]
                    .nx_dhcpv6_lease_valid_lifetime = 0;
                dhcpv6_server_ptr->nx_dhcpv6_lease_list[i]
                    .nx_dhcpv6_lease_preferred_lifetime = 0;
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_update_client_record(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    ...
    if (from_extracted_client_data_ptr->nx_dhcpv6_message_type ==
        NX_DHCPV6_MESSAGE_TYPE_SOLICIT) {

        /* Ok to copy the whole record over. */
        memcpy(
            to_client_data_on_record_ptr,
            from_extracted_client_data_ptr,
            sizeof(NX_DHCPV6_CLIENT)); /* Use case of memcpy is verified. */

        /* Update various fields now that the client is in a DHCPv6 session with the
         * server. */
        to_client_data_on_record_ptr->nx_dhcpv6_client_session_time = 1;
        memcpy(
            &to_client_data_on_record_ptr->nx_dhcpv6_client_duid,
            &from_extracted_client_data_ptr->nx_dhcpv6_client_duid,
            sizeof(NX_DHCPV6_SVR_DUID)); /* Use case of memcpy is verified. */
        memcpy(
            &to_client_data_on_record_ptr->nx_dhcpv6_server_duid,
            &from_extracted_client_data_ptr->nx_dhcpv6_server_duid,
            sizeof(NX_DHCPV6_SVR_DUID)); /* Use case of memcpy is verified. */
    } else if (
        from_extracted_client_data_ptr->nx_dhcpv6_message_type ==
        NX_DHCPV6_MESSAGE_TYPE_INFORM_REQUEST) {

        /* Update selected fields for an INFORM REQUEST. */
        to_client_data_on_record_ptr->nx_dhcpv6_id =
            from_extracted_client_data_ptr->nx_dhcpv6_id;
        to_client_data_on_record_ptr->nx_dhcpv6_message_xid =
            from_extracted_client_data_ptr->nx_dhcpv6_message_xid;
        to_client_data_on_record_ptr->nx_dhcpv6_message_type =
            from_extracted_client_data_ptr->nx_dhcpv6_message_type;
        to_client_data_on_record_ptr->nx_dhcpv6_response_back_from_server =
            from_extracted_client_data_ptr->nx_dhcpv6_response_back_from_server;

        COPY_IPV6_ADDRESS(
            &from_extracted_client_data_ptr->nx_dhcp_source_ip_address.nxd_ip_address
                 .v6[0],
            &to_client_data_on_record_ptr->nx_dhcp_source_ip_address.nxd_ip_address
                 .v6[0]);
        to_client_data_on_record_ptr->nx_dhcp_source_ip_address.nxd_ip_version =
            NX_IP_VERSION_V6;

        COPY_IPV6_ADDRESS(
            &from_extracted_client_data_ptr->nx_dhcp_destination_ip_address.nxd_ip_address
                 .v6[0],
            &to_client_data_on_record_ptr->nx_dhcp_destination_ip_address.nxd_ip_address
                 .v6[0]);
        to_client_data_on_record_ptr->nx_dhcp_destination_ip_address.nxd_ip_version =
            NX_IP_VERSION_V6;

        memcpy(
            &to_client_data_on_record_ptr->nx_dhcpv6_option_request,
            &from_extracted_client_data_ptr->nx_dhcpv6_option_request,
            sizeof(NX_DHCPV6_SERVER_OPTIONREQUEST)); /* Use case of memcpy is verified. */
        memcpy(
            &to_client_data_on_record_ptr->nx_dhcpv6_client_duid,
            &from_extracted_client_data_ptr->nx_dhcpv6_client_duid,
            sizeof(NX_DHCPV6_SVR_DUID)); /* Use case of memcpy is verified. */
    ...
    /* If this is not a solicit message, update the selected fields into the specified
       record. */
    else if (
        (from_extracted_client_data_ptr->nx_dhcpv6_message_type ==
         NX_DHCPV6_MESSAGE_TYPE_REQUEST) ||
        ...
        memcpy(
            ...
            sizeof(NX_DHCPV6_SVR_DUID)); /* Use case of memcpy is verified. */
        memcpy(
            &to_client_data_on_record_ptr->nx_dhcpv6_elapsed_time,
            &from_extracted_client_data_ptr->nx_dhcpv6_elapsed_time,
            sizeof(NX_DHCPV6_SERVER_ELAPSED_TIME)); /* Use case of memcpy is verified. */
        memcpy(
            &to_client_data_on_record_ptr->nx_dhcpv6_iana,
            &from_extracted_client_data_ptr->nx_dhcpv6_iana,
            sizeof(NX_DHCPV6_SERVER_IA_NA)); /* Use case of memcpy is verified. */
        memcpy(
            &to_client_data_on_record_ptr->nx_dhcpv6_ia,
            &from_extracted_client_data_ptr->nx_dhcpv6_ia,
            sizeof(NX_DHCPV6_SERVER_IA_ADDRESS)); /* Use case of memcpy is verified. */

        /* Update the IANA status option code, length and status message. The DHCPv6
           server will figure out the actual lenght with the status message text added in
           later. */
        to_client_data_on_record_ptr->nx_dhcpv6_iana_status.nx_status_code =
            from_extracted_client_data_ptr->nx_dhcpv6_iana_status.nx_status_code;
        to_client_data_on_record_ptr->nx_dhcpv6_iana_status.nx_option_length =
            sizeof(USHORT);
        ...
        /* Update fields based on the state of the client. At this point the client is
           bound to a valid IP address to the session is 'over'.  Set timeout fields to
           stop the session timer and begin the lease life timer. */
        ...
        /* No need to set the client state to unbound. We will remove the client record
           and restore the IP address status back to available after sending the server
           reply. */
    }
    /* else
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_server_process_iana(
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    ULONG option_code,
    UINT option_length,
    UCHAR *option_data) {
    ...
    while (index + 4 <= option_length) {
        ...
        if (iana_option_code == NX_DHCPV6_OP_IA_ADDRESS) {
            ...
            status = _nx_dhcpv6_server_process_ia(
                dhcpv6_client_ptr,
                iana_option_code,
                iana_option_length,
                (option_data + index),
            ...
            /* Indicate we already have an IA option if the client request contains
             * multiple IAs. */
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_process_option_request(
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    ULONG option_code,
    UINT option_length,
    UCHAR *option_data) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_process_elapsed_time(
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    ULONG option_code,
    UINT option_length,
    UCHAR *option_data) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_server_process_ia(
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    ULONG option_code,
    UINT option_length,
    UCHAR *option_data,
    ...
    /* Skip 4 bytes preferred lifetime data and 4 bytes valid lifetime data from the
       client buffer to IA. This is because in RFC 8415, page 105, it requires that the
       server MUST ignore any received preferred-lifetime and valid-lifetime values in a
...
UINT _nx_dhcpv6_process_duid(
    ...
    if ((duid_ptr->nx_duid_type == NX_DHCPV6_SERVER_DUID_TYPE_LINK_TIME) ||
        ...
        if (option_length - 6 <= NX_DHCPV6_SERVER_DUID_VENDOR_ASSIGNED_LENGTH) {
            ...
            /* Set the identifier.  */
            memcpy(
                duid_ptr->nx_duid_private_identifier,
                option_data + index,
                option_length - 6); /* Use case of memcpy is verified. */
            index = option_length;
        } else {
            return (NX_DHCPV6_INVALID_DUID);
        }
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_check_duids_same(
    NX_DHCPV6_SVR_DUID *dhcpv6_duid1_ptr,
    NX_DHCPV6_SVR_DUID *dhcpv6_duid2_ptr,
    UINT *matching) {
    ...
    *matching = NX_FALSE;
    ...
    if (dhcpv6_duid1_ptr->nx_link_layer_address_msw !=
        dhcpv6_duid2_ptr->nx_link_layer_address_msw) {
        return NX_SUCCESS;
    } else if (
        dhcpv6_duid1_ptr->nx_link_layer_address_lsw !=
        dhcpv6_duid2_ptr->nx_link_layer_address_lsw) {
        return NX_SUCCESS;
        ...
        dhcpv6_duid1_ptr->nx_duid_type == NX_DHCPV6_SERVER_DUID_TYPE_VENDOR_ASSIGNED) {

        /* First compare the enterprise ID. */
        if (dhcpv6_duid1_ptr->nx_duid_enterprise_number !=
            dhcpv6_duid2_ptr->nx_duid_enterprise_number) {
            return NX_SUCCESS;
        }

        /* Then compare the private ID. */
        if (memcmp(
                dhcpv6_duid1_ptr->nx_duid_private_identifier,
                dhcpv6_duid2_ptr->nx_duid_private_identifier,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_add_duid(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_SVR_DUID *dhcpv6_duid_ptr,
    UCHAR *buffer_ptr,
    UINT *index,
    UINT duid_type) {
    ...
    *index += (ULONG)sizeof(UINT);
    ...
    memcpy(
        buffer_ptr + *index,
        &(dhcpv6_duid_ptr->nx_link_layer_address_msw),
        sizeof(USHORT)); /* Use case of memcpy is verified. */
    *index += (ULONG)sizeof(USHORT);

    memcpy(
        buffer_ptr + *index,
        &(dhcpv6_duid_ptr->nx_link_layer_address_lsw),
        sizeof(ULONG)); /* Use case of memcpy is verified. */
    *index += (ULONG)sizeof(ULONG);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_server_add_iana(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_CLIENT *dhcpv6_client_ptr,
    UCHAR *buffer_ptr,
    UINT *index) {
    ...
    *index += (ULONG)sizeof(ULONG);
    ...
    /* Determine if we include an IA address option. This requires a successful IANA
       status and applies only if the client is requesting or confirming an address
       including renew/rebinding one. */
    add_ia = NX_FALSE;
    ...
    /* Or if we are confirming the IP address a client is releasing or declining,
       regardless if our records show this IP address is bound to this client.
       Sect 18.2.6-7 RFC 3315. */
    if ((dhcpv6_client_ptr->nx_dhcpv6_message_type == NX_DHCPV6_MESSAGE_TYPE_RELEASE) ||
        (dhcpv6_client_ptr->nx_dhcpv6_message_type == NX_DHCPV6_MESSAGE_TYPE_DECLINE)) {
        add_ia = NX_TRUE;
    }
    ...
    if (add_ia) {
        ...
        if (dhcpv6_client_ptr->nx_dhcpv6_ia.nx_op_code == 0) {
            ...
            /* Set the length to include option code, lenght (one word), IPv6 address (4
             * words), and lifetime data (2 words). */
            dhcpv6_client_ptr->nx_dhcpv6_ia.nx_option_length =
                (2 * sizeof(ULONG)) + (4 * sizeof(ULONG));
            ...
            /* Find the Client's tentatively assigned address in the Server IP address
             * list. */
            _nx_dhcpv6_find_ip_address(
                dhcpv6_server_ptr, dhcpv6_client_ptr, &client_address_lease);
            ...
            COPY_IPV6_ADDRESS(
                &client_address_lease->nx_dhcpv6_lease_IP_address.nxd_ip_address.v6[0],
                &dhcpv6_client_ptr->nx_dhcpv6_ia.nx_global_address.nxd_ip_address.v6[0]);
        }

        status = _nx_dhcpv6_add_ia(
            dhcpv6_server_ptr, &dhcpv6_client_ptr->nx_dhcpv6_ia, buffer_ptr, &temp);
        ...
        /* Add the size of the IA(s) to the client IANA. Note that we include the option
           code and length field of any nested options. */
        iana_ptr->nx_option_length =
            (USHORT)(iana_ptr->nx_option_length +
                     dhcpv6_client_ptr->nx_dhcpv6_ia.nx_option_length + 4);
    ...
    status = _nx_dhcpv6_add_iana_status(
        dhcpv6_server_ptr, &dhcpv6_client_ptr->nx_dhcpv6_iana_status, buffer_ptr, &temp);
    ...
    *index = temp;
    ...
    iana_ptr->nx_option_length =
        (USHORT)(iana_ptr->nx_option_length +
                 dhcpv6_client_ptr->nx_dhcpv6_iana_status.nx_option_length + 4);

    /* Rewrite the IANA opcode and update the changed IANA data length with the additional
     * IA address option. */
    message_word = (ULONG)((iana_ptr->nx_op_code) << 16);
    ...
    /* Erase the IANA status message. We will add another message on the next (if any)
     * IANA option for the client. */
    status = _nx_dhcpv6_prepare_iana_status(
        &dhcpv6_client_ptr->nx_dhcpv6_iana_status, NX_FALSE);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_add_ia(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_SERVER_IA_ADDRESS *dhcpv6_ia_ptr,
    UCHAR *buffer_ptr,
    UINT *index) {
    ...
    *index += (ULONG)sizeof(ULONG);
    ...
    memcpy(
        buffer_ptr + *index,
        &(dhcpv6_ia_ptr->nx_global_address.nxd_ip_address.v6[0]),
        sizeof(ULONG)); /* Use case of memcpy is verified. */
    *index += (ULONG)sizeof(ULONG);

    memcpy(
        buffer_ptr + *index,
        &(dhcpv6_ia_ptr->nx_global_address.nxd_ip_address.v6[1]),
        sizeof(ULONG)); /* Use case of memcpy is verified. */
    *index += (ULONG)sizeof(ULONG);

    memcpy(
        buffer_ptr + *index,
        &(dhcpv6_ia_ptr->nx_global_address.nxd_ip_address.v6[2]),
        sizeof(ULONG)); /* Use case of memcpy is verified. */
    *index += (ULONG)sizeof(ULONG);

    memcpy(
        buffer_ptr + *index,
        &(dhcpv6_ia_ptr->nx_global_address.nxd_ip_address.v6[3]),
        sizeof(ULONG)); /* Use case of memcpy is verified. */
    *index += (ULONG)sizeof(ULONG);
    ...
    memcpy(
        buffer_ptr + *index,
        &(dhcpv6_ia_ptr->nx_preferred_lifetime),
        sizeof(ULONG)); /* Use case of memcpy is verified. */
    *index += (ULONG)sizeof(ULONG);

    memcpy(
        buffer_ptr + *index,
        &(dhcpv6_ia_ptr->nx_valid_lifetime),
        sizeof(ULONG)); /* Use case of memcpy is verified. */
    *index += (ULONG)sizeof(ULONG);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcpv6_add_iana_status(
    NX_DHCPV6_SERVER *dhcpv6_server_ptr,
    NX_DHCPV6_SERVER_IANA_STATUS *iana_status_ptr,
    UCHAR *buffer_ptr,
    UINT *index) {
    ...
    *index += (ULONG)sizeof(USHORT);
    ...
    memcpy(
        buffer_ptr + *index,
        &iana_status_ptr->nx_status_message[0],
