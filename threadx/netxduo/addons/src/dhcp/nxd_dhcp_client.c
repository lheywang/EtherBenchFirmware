...
static UINT _nx_dhcp_extract_information(
    NX_DHCP *dhcp_ptr,
    NX_DHCP_INTERFACE_RECORD *interface_record,
    UCHAR *dhcp_message,
    UINT length);
static UINT
_nx_dhcp_get_option_value(UCHAR *bootp_message, UINT option, ULONG *value, UINT length);
static UINT _nx_dhcp_add_option_value(
    UCHAR *bootp_message, UINT option, UINT size, ULONG value, UINT *index);
static UINT _nx_dhcp_add_option_string(
    UCHAR *bootp_message, UINT option, UINT size, UCHAR *value, UINT *index);
static UINT _nx_dhcp_add_option_parameter_request(
    NX_DHCP *dhcp_ptr, UCHAR *bootp_message, UINT *index);
...
static UINT _nx_dhcp_send_request_internal(
    NX_DHCP *dhcp_ptr,
    NX_DHCP_INTERFACE_RECORD *interface_record,
    UINT dhcp_message_type);
static UINT _nx_dhcp_client_send_with_zero_source_address(
    NX_DHCP *dhcp_ptr, UINT iface_index, NX_PACKET *packet_ptr);
static ULONG _nx_dhcp_add_randomize(ULONG timeout);
static VOID _nx_dhcp_udp_receive_notify(NX_UDP_SOCKET *socket_ptr);
static VOID _nx_dhcp_packet_process(
    NX_DHCP *dhcp_ptr, NX_DHCP_INTERFACE_RECORD *dhcp_interface, NX_PACKET *packet_ptr);
static VOID _nx_dhcp_timeout_entry(ULONG dhcp);
static VOID _nx_dhcp_timeout_process(NX_DHCP *dhcp_ptr);
static UINT _nx_dhcp_interface_record_find(
    NX_DHCP *dhcp_ptr, UINT iface_index, NX_DHCP_INTERFACE_RECORD **interface_record);

#ifdef NX_DHCP_CLIENT_SEND_ARP_PROBE
static VOID _nx_dhcp_ip_conflict(
    NX_IP *ip_ptr,
...
UCHAR _nx_dhcp_request_parameters[] = {
    NX_DHCP_OPTION_SUBNET_MASK, NX_DHCP_OPTION_GATEWAYS, NX_DHCP_OPTION_DNS_SVR};
...
UINT _nx_dhcp_create(NX_DHCP *dhcp_ptr, NX_IP *ip_ptr, CHAR *name_ptr) {
    ...
    while (*temp_ptr) {
        ...
        if (*temp_ptr == '.') {
            ...
            /* Labels must be 63 characters or less, and check the continuous dot '..' for
             * host name.  */
            ...
            /* Hyphen or Digits.  */
            if ((*temp_ptr != '-') && ((*temp_ptr < '0') || (*temp_ptr > '9')))
                return (NX_DHCP_INVALID_NAME);
        }
    ...
    /* Check the packet payload size, DHCP Client must be prepared to receive a message of
       up to 576 octets. RFC 2131, Section 2, Page10.  */
    ...
    nx_udp_socket_receive_notify(
        &(dhcp_ptr->nx_dhcp_socket), _nx_dhcp_udp_receive_notify);

    dhcp_ptr->nx_dhcp_socket.nx_udp_socket_reserved_ptr = (VOID *)dhcp_ptr;

    /* Create the ThreadX activity timeout timer.  This will be used to periodically check
       to see if a client connection has gone silent and needs to be terminated.  */
    status = tx_timer_create(
        &(dhcp_ptr->nx_dhcp_timer),
        "DHCP Client Timer",
        _nx_dhcp_timeout_entry,
        (ULONG)(ALIGN_TYPE)dhcp_ptr,
        (NX_DHCP_TIME_INTERVAL),
        (NX_DHCP_TIME_INTERVAL),
        TX_NO_ACTIVATE);
    ...
    status =
        tx_mutex_create(&(dhcp_ptr->nx_dhcp_mutex), "NetX DHCP Client", TX_NO_INHERIT);
    ...
    status = tx_thread_create(
        &(dhcp_ptr->nx_dhcp_thread),
        "NetX DHCP Client",
        _nx_dhcp_thread_entry,
        (ULONG)(ALIGN_TYPE)dhcp_ptr,
        dhcp_ptr->nx_dhcp_thread_stack,
        NX_DHCP_THREAD_STACK_SIZE,
        NX_DHCP_THREAD_PRIORITY,
        NX_DHCP_THREAD_PRIORITY,
        1,
        TX_DONT_START);
    ...
    status =
        tx_event_flags_create(&(dhcp_ptr->nx_dhcp_events), (CHAR *)"DHCP Client Events");
...
UINT _nxe_dhcp_interface_clear_broadcast_flag(
    NX_DHCP *dhcp_ptr, UINT iface_index, UINT clear_flag) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_interface_clear_broadcast_flag(
    NX_DHCP *dhcp_ptr, UINT iface_index, UINT clear_flag) {
...
UINT _nxe_dhcp_packet_pool_set(NX_DHCP *dhcp_ptr, NX_PACKET_POOL *packet_pool_ptr) {
    ...
    /* Client not configured for the user creating the packet pool. Return an error
     * status. */
    ...
    /* Check the packet payload size, DHCP Client must be prepared to receive a message of
       up to 576 octets. RFC2131, Section2, Page10.  */
...
UINT _nx_dhcp_packet_pool_set(NX_DHCP *dhcp_ptr, NX_PACKET_POOL *packet_pool_ptr) {
    ...
    /* Set the maximum DHCP message size.  */
    dhcp_ptr->nx_dhcp_max_dhcp_message_size =
        packet_pool_ptr->nx_packet_pool_payload_size - NX_PHYSICAL_HEADER;
#endif /* NX_DHCP_CLIENT_SEND_MAX_DHCP_MESSAGE_OPTION  */
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if (dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid == NX_TRUE) {
            ...
            _nx_dhcp_interface_reinitialize(
                dhcp_ptr, dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_interface_index);
        }
    }
...
UINT _nx_dhcp_interface_reinitialize(NX_DHCP *dhcp_ptr, UINT iface_index) {
    ...
    if (interface_record->nx_dhcp_ip_address) {
        ...
        nx_ip_interface_address_get(
            dhcp_ptr->nx_dhcp_ip_ptr, iface_index, &ip_address, &network_mask);
    ...
    if (interface_record->nx_dhcp_gateway_address) {
        ...
        if ((status == NX_SUCCESS) &&
            (gateway_address == interface_record->nx_dhcp_gateway_address)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_request_client_ip(
    NX_DHCP *dhcp_ptr, ULONG client_request_address, UINT skip_discover_message) {
    ...
    status = _nx_dhcp_request_client_ip(
        dhcp_ptr, client_request_address, skip_discover_message);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_request_client_ip(
    NX_DHCP *dhcp_ptr, ULONG client_request_address, UINT skip_discover_message) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if (dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid) {
            ...
            /* Set the request IP.  */
            status = _nx_dhcp_interface_request_client_ip(
                dhcp_ptr,
                dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_interface_index,
                client_request_address,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_interface_request_client_ip(
    NX_DHCP *dhcp_ptr,
    UINT iface_index,
    ULONG client_request_address,
    ...
    status = _nx_dhcp_interface_request_client_ip(
        dhcp_ptr, iface_index, client_request_address, skip_discover_message);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_interface_request_client_ip(
    NX_DHCP *dhcp_ptr,
    UINT iface_index,
    ULONG client_request_address,
    UINT skip_discover_message) {
...
UINT _nx_dhcp_delete(NX_DHCP *dhcp_ptr) {
    ...
    if (_nx_dhcp_created_ptr == dhcp_ptr) {
        ...
        for (dhcp_previous = _nx_dhcp_created_ptr; dhcp_previous->nx_dhcp_created_next;
...
UINT _nx_dhcp_force_renew(NX_DHCP *dhcp_ptr) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        /* Check if this record is valid.  */
        if ((dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid) &&
            (dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_state >=
             NX_DHCP_STATE_BOUND)) {
...
UINT _nx_dhcp_interface_force_renew(NX_DHCP *dhcp_ptr, UINT iface_index) {
    ...
    if (interface_record->nx_dhcp_state >= NX_DHCP_STATE_BOUND) {
        ...
        _nx_dhcp_send_request_internal(
            dhcp_ptr, interface_record, NX_DHCP_TYPE_DHCPREQUEST);
        ...
        interface_record->nx_dhcp_renewal_remain_time =
            interface_record->nx_dhcp_rebind_time -
            interface_record->nx_dhcp_renewal_time;
        interface_record->nx_dhcp_timeout = _nx_dhcp_update_renewal_timeout(
            interface_record->nx_dhcp_renewal_remain_time);

        /* Record the retransmission interval.  */
        interface_record->nx_dhcp_rtr_interval = interface_record->nx_dhcp_timeout;

        /* Determine if the application has specified a routine for DHCP state change
         * notification.  */
        if (dhcp_ptr->nx_dhcp_state_change_callback) {

            /* Yes, call the application's state change notify function with the new
             * state.  */
            (dhcp_ptr->nx_dhcp_state_change_callback)(
                dhcp_ptr, interface_record->nx_dhcp_state);
        }

        /* Determine if the application has specified a routine for DHCP interface state
         * change notification.  */
        if (dhcp_ptr->nx_dhcp_interface_state_change_callback) {

            /* Yes, call the application's state change notify function with the new
             * state.  */
            (dhcp_ptr->nx_dhcp_interface_state_change_callback)(
                dhcp_ptr,
...
UINT _nx_dhcp_interface_decline(NX_DHCP *dhcp_ptr, UINT iface_index) {
    ...
    /* Wait for some time before restarting the configuration process to avoid excessive
     * network traffic in case of looping. RFC2131, Section3.1, Page17.  */
...
UINT _nx_dhcp_interface_release(NX_DHCP *dhcp_ptr, UINT iface_index) {
    ...
    /* Reinitialize DHCP. Note: the state is changed to NX_DHCP_STATE_NOT_STARTED in this
     * function. */
    _nx_dhcp_interface_reinitialize(dhcp_ptr, iface_index);
...
UINT _nx_dhcp_start(NX_DHCP *dhcp_ptr) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if (dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid == NX_TRUE) {
            ...
            status = _nx_dhcp_interface_start(
                dhcp_ptr, dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_interface_index);
...
UINT _nx_dhcp_interface_start(NX_DHCP *dhcp_ptr, UINT iface_index) {
    ...
    if (i == NX_DHCP_CLIENT_MAX_RECORDS) {
        ...
        status = nx_udp_socket_bind(
            &(dhcp_ptr->nx_dhcp_socket), NX_DHCP_CLIENT_UDP_PORT, NX_WAIT_FOREVER);
    ...
    /* The client begins in INIT state and forms a DHCPDISCOVER message.
       ...
       the use of DHCP at startup. RFC2131, Section4.4.1, Page36.  */

    /* Use the minimum value, Wait one second to begain in INIT state and forms a DHCP
     * Discovery message.  */
...
UINT _nx_dhcp_interface_enable(NX_DHCP *dhcp_ptr, UINT iface_index) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if (dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid == NX_TRUE) {
            ...
            if (dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_interface_index ==
                iface_index) {
    ...
    client_physical_msw = dhcp_ptr->nx_dhcp_ip_ptr->nx_ip_interface[iface_index]
                              .nx_interface_physical_address_msw;
    client_physical_lsw = dhcp_ptr->nx_dhcp_ip_ptr->nx_ip_interface[iface_index]
                              .nx_interface_physical_address_lsw;

    /* Generate a 'unique' client transaction ID from the MAC address for each message to
     * the server. */
    interface_record->nx_dhcp_xid =
        (ULONG)(client_physical_msw ^ client_physical_lsw ^ (ULONG)NX_RAND());
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_state_change_notify(
    NX_DHCP *dhcp_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_state_change_notify(
    NX_DHCP *dhcp_ptr,
    VOID (*dhcp_state_change_notify)(NX_DHCP *dhcp_ptr, UCHAR new_state)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_interface_state_change_notify(
    NX_DHCP *dhcp_ptr,
    VOID (*dhcp_state_interface_change_notify)(
        NX_DHCP *dhcp_ptr, UINT iface_index, UCHAR new_state)) {
    ...
    /* Call actual DHCP notify service.  */
    status = _nx_dhcp_interface_state_change_notify(
        dhcp_ptr, dhcp_state_interface_change_notify);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_interface_state_change_notify(
    NX_DHCP *dhcp_ptr,
...
UINT _nxe_dhcp_user_option_retrieve(
    ...
    status = _nx_dhcp_user_option_retrieve(
        dhcp_ptr, option_request, destination_ptr, destination_size);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_user_option_retrieve(
    NX_DHCP *dhcp_ptr,
    UINT option_request,
    UCHAR *destination_ptr,
    UINT *destination_size) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if ((dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid) &&
            ...
            status = _nx_dhcp_interface_user_option_retrieve(
                dhcp_ptr,
                dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_interface_index,
                option_request,
                destination_ptr,
...
UINT _nx_dhcp_user_option_request(NX_DHCP *dhcp_ptr, UINT option_code) {
    ...
    /* Add the option.  */
    dhcp_ptr->nx_dhcp_user_request_parameter
        [dhcp_ptr->nx_dhcp_user_request_parameter_size++] = (UCHAR)option_code;
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_interface_user_option_retrieve(
    NX_DHCP *dhcp_ptr,
    UINT iface_index,
    UINT option_request,
    UCHAR *destination_ptr,
    ...
    /* Call actual DHCP interface user option request service.  */
    status = _nx_dhcp_interface_user_option_retrieve(
        dhcp_ptr, iface_index, option_request, destination_ptr, destination_size);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_interface_user_option_retrieve(
    NX_DHCP *dhcp_ptr,
    UINT iface_index,
    UINT option_request,
    UCHAR *destination_ptr,
    ...
    buffer_ptr = _nx_dhcp_search_buffer(
        interface_record->nx_dhcp_options_buffer,
        option_request,
        interface_record->nx_dhcp_options_size);
...
ULONG _nx_dhcp_user_option_convert(UCHAR *source_ptr) {
    ...
    /* Pickup the ULONG.  */
    temp = (((ULONG) * (source_ptr)) << 24) | (((ULONG) * (source_ptr + 1)) << 16) |
           (((ULONG) * (source_ptr + 2)) << 8) | ((ULONG) * (source_ptr + 3));
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_user_option_add_callback_set(
    NX_DHCP *dhcp_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_user_option_add_callback_set(
    NX_DHCP *dhcp_ptr,
    UINT (*dhcp_user_option_add)(
        NX_DHCP *dhcp_ptr,
        UINT iface_index,
...
static VOID _nx_dhcp_thread_entry(ULONG dhcp_instance) {
    ...
    do {
        ...
        tx_event_flags_get(
            &(dhcp_ptr->nx_dhcp_events),
            NX_DHCP_CLIENT_ALL_EVENTS,
            TX_OR_CLEAR,
            &events,
            TX_WAIT_FOREVER);
        ...
        if (events & NX_DHCP_CLIENT_RECEIVE_EVENT) {
            ...
            while (1) {
                ...
                status = nx_udp_packet_info_extract(
                    packet_ptr,
                    &source_ip_address,
                    &protocol,
                    &source_port,
                ...
                /* Find the interface record.  */
                status = _nx_dhcp_interface_record_find(
                    dhcp_ptr, iface_index, &interface_record);
        ...
        if (events & NX_DHCP_CLIENT_CONFLICT_EVENT) {
            ...
            for (iface_index = 0; iface_index < NX_MAX_PHYSICAL_INTERFACES;
                ...
                if (dhcp_ptr->nx_dhcp_interface_conflict_flag &
                    ((UINT)(1 << iface_index))) {
                    ...
                    /* Clear the flag.  */
                    dhcp_ptr->nx_dhcp_interface_conflict_flag &=
                        (UINT)(~(1 << iface_index));
                }
            }
        }
...
VOID _nx_dhcp_packet_process(
    ...
    /* Copy the received packet (datagram) over to a packet from the DHCP Client pool and
       release the packet back to receive packet pool as soon as possible. */
    ...
    dhcp_client_mac_msw =
        ip_ptr->nx_ip_interface[iface_index].nx_interface_physical_address_msw;
    dhcp_client_mac_lsw =
        ip_ptr->nx_ip_interface[iface_index].nx_interface_physical_address_lsw;
    ...
    packet_client_mac_lsw = (((ULONG)work_ptr[2]) << 24) | (((ULONG)work_ptr[3]) << 16) |
                            (((ULONG)work_ptr[4]) << 8) | ((ULONG)work_ptr[5]);

    /* Determine if the  MAC address matches ours.  */
    if ((packet_client_mac_msw != dhcp_client_mac_msw) ||
        (packet_client_mac_lsw != dhcp_client_mac_lsw)) {
    ...
    if (_nx_dhcp_get_data(
            new_packet_ptr->nx_packet_prepend_ptr + NX_BOOTP_OFFSET_XID, 4) !=
        interface_record->nx_dhcp_xid) {
    ...
    /* Save the original state for the state change callback; after this point we will
     * likely change it. */
    ...
    case NX_DHCP_STATE_SELECTING: {
        ...
        status = _nx_dhcp_get_option_value(
            buffer,
            NX_DHCP_OPTION_DHCP_TYPE,
            &dhcp_type,
            new_packet_ptr->nx_packet_length);
        ...
        if ((status == NX_SUCCESS) && (dhcp_type == NX_DHCP_TYPE_DHCPOFFER)) {
            ...
            /* Update the DHCP Client interface parameters (IP address, server IP, lease,
             * renewal and rebind times */
            if (_nx_dhcp_extract_information(
                    dhcp_ptr, interface_record, buffer, new_packet_ptr->nx_packet_length))
                break;
            ...
            /* This will modify the timeout by up to +/- 1 second as recommended by RFC
             * 2131, Section 4.1, Page 24. */
            interface_record->nx_dhcp_timeout =
                _nx_dhcp_add_randomize(interface_record->nx_dhcp_timeout);
    ...
    case NX_DHCP_STATE_REQUESTING: {
        ...
        if (status == NX_SUCCESS) {
            ...
            if (dhcp_type == NX_DHCP_TYPE_DHCPACK) {
                ...
                /* Update the parameters (IP address, server IP, lease, renewal and rebind
                 * times */
                if (_nx_dhcp_extract_information(
                        dhcp_ptr,
                        interface_record,
                ...
                /* If the host is configured to send an ARP probe to verify Client address
                   is not in use, do so now. */
                ...
                ip_ptr->nx_ip_interface[iface_index]
                    .nx_interface_ip_conflict_notify_handler = _nx_dhcp_ip_conflict;
                ...
                if (interface_record->nx_dhcp_gateway_address) {
                    ...
                    nx_ip_gateway_address_set(
                        ...
                        interface_record->nx_dhcp_gateway_address);
                }
...
#ifdef NX_DHCP_ENABLE_BOOTP
                /* BOOTP does not use timeouts.  For the life of this DHCP Client
                 * application, keep the same IP address. */
                ...
                /* Reinitialize DHCP.  */
                _nx_dhcp_interface_reinitialize(
                    dhcp_ptr, interface_record->nx_dhcp_interface_index);
                ...
                /* The client begins in INIT state and forms a DHCPDISCOVER message.
                   ...
                   desynchronize the use of DHCP at startup. RFC2131, Section4.4.1,
                   Page36.  */

                /* Use the minimum value, Wait one second to begain in INIT state and
                 * forms a DHCP Discovery message.  */
                interface_record->nx_dhcp_timeout = NX_IP_PERIODIC_RATE;
                interface_record->nx_dhcp_rtr_interval = 0;
            }
        }
    ...
    case NX_DHCP_STATE_BOUND: {
        ...
        /* Silently discard all received packets in the BOUND state, RFC2131, Section 4.4
         * Figure 5  */
    ...
    case NX_DHCP_STATE_RENEWING: {
        ...
        if (status == NX_SUCCESS) {
            ...
            if (dhcp_type == NX_DHCP_TYPE_DHCPACK) {
                ...
                /* Set the IP address and gateway address from the value extracted from
                 * the Server's DHCP response. */
                nx_ip_interface_address_set(
                    ip_ptr,
                    iface_index,
                    interface_record->nx_dhcp_ip_address,
                ...
                /* Lease has been renewed, set the countdown timer back to the renewal
                   time and go back to the Bound state*/
                interface_record->nx_dhcp_timeout =
                    interface_record->nx_dhcp_renewal_time;
        ...
        if (interface_record->nx_dhcp_timeout != 0) {
            ...
            if (interface_record->nx_dhcp_timeout > NX_DHCP_TIME_INTERVAL) {
                ...
                case NX_DHCP_STATE_INIT: {
                    ...
                    /* Initial state when there is no address.  Send a DHCPDISCOVER
                       message to find a DHCP server and switch to the SELECTING state.
                       Initial timeout is NX_DHCP_MIN_RETRANS_TIMEOUT seconds. */
#ifndef NX_DHCP_ENABLE_BOOTP
                    /* Only if the DHCP Client is requesting an IP address and is
                     * configured to skip the Discovery message. */
                    ...
                    _nx_dhcp_send_request_internal(
                        dhcp_ptr, interface_record, NX_DHCP_TYPE_BOOT_REQUEST);
                    ...
                    if (interface_record->nx_dhcp_rtr_interval == 0) {
                        ...
                        interface_record->nx_dhcp_rtr_interval =
                            NX_DHCP_MIN_RETRANS_TIMEOUT;
                    } else {

                        /* Record the retransmission interval for next retransmission.  */
                        interface_record->nx_dhcp_rtr_interval = _nx_dhcp_update_timeout(
                            interface_record->nx_dhcp_rtr_interval);
                    }
                    ...
                    /* This will modify the timeout by up to +/- 1 second as recommended
                     * by RFC 2131, Section 4.1, Page 24. */
                    interface_record->nx_dhcp_timeout =
                        _nx_dhcp_add_randomize(interface_record->nx_dhcp_timeout);
                ...
                case NX_DHCP_STATE_SELECTING: {
                    ...
                    interface_record->nx_dhcp_rtr_interval =
                        _nx_dhcp_update_timeout(interface_record->nx_dhcp_rtr_interval);
                ...
                case NX_DHCP_STATE_ADDRESS_PROBING: {
                    ...
                    if (interface_record->nx_dhcp_probe_count) {
                        ...
                        /* No address conflict.  */
                        ip_ptr->nx_ip_interface[interface_record->nx_dhcp_interface_index]
                            .nx_interface_ip_conflict_notify_handler = NX_NULL;
                ...
                case NX_DHCP_STATE_RENEWING: {
                    ...
                    if (interface_record->nx_dhcp_renewal_remain_time >=
                        interface_record->nx_dhcp_rtr_interval) {
                        interface_record->nx_dhcp_renewal_remain_time -=
                            interface_record->nx_dhcp_rtr_interval;
                    } else {
                    ...
                    if (interface_record->nx_dhcp_timeout == 0) {
                        ...
                        interface_record->nx_dhcp_rebind_remain_time =
                            interface_record->nx_dhcp_lease_time -
                            interface_record->nx_dhcp_rebind_time;

                        /* Calculate the timeout for the response.  */
                        interface_record->nx_dhcp_timeout =
                            _nx_dhcp_update_renewal_timeout(
                                interface_record->nx_dhcp_rebind_remain_time);
                ...
                case NX_DHCP_STATE_REBINDING: {
                    ...
                    if (interface_record->nx_dhcp_rebind_remain_time >=
                        interface_record->nx_dhcp_rtr_interval) {
                        interface_record->nx_dhcp_rebind_remain_time -=
                            interface_record->nx_dhcp_rtr_interval;
                    } else {
                    ...
                    if (interface_record->nx_dhcp_timeout == 0) {
                        ...
                        /* Start the DHCP protocol again by setting the state back to
                         * INIT. */
                        interface_record->nx_dhcp_state = NX_DHCP_STATE_INIT;

                        /* The client begins in INIT state and forms a DHCPDISCOVER
                           message. The client should wait a random time between one and
                           ten seconds to desynchronize the use of DHCP at startup.
                           RFC2131, Section4.4.1, Page36.  */

                        /* Use the minimum value, Wait one second to begain in INIT state
                         * and forms a DHCP Discovery message.  */
                        interface_record->nx_dhcp_timeout = NX_IP_PERIODIC_RATE;
                        interface_record->nx_dhcp_rtr_interval = 0;
                    } else {
                ...
                if (original_state != interface_record->nx_dhcp_state) {
                    ...
                    /* Determine if the application has specified a routine for DHCP state
                     * change notification.  */
                    if (dhcp_ptr->nx_dhcp_state_change_callback) {

                        /* Yes, call the application's state change notify function with
                         * the new state.  */
                        (dhcp_ptr->nx_dhcp_state_change_callback)(
                            dhcp_ptr, interface_record->nx_dhcp_state);
                    }

                    /* Determine if the application has specified a routine for DHCP
...
UINT _nxe_dhcp_send_request(NX_DHCP *dhcp_ptr, UINT dhcp_message_type) {
    ...
    /* Check for invalid input. */
    if (!dhcp_ptr || (dhcp_message_type == 0) ||
        (dhcp_message_type > NX_DHCP_TYPE_DHCPFORCERENEW)) {
        return (NX_PTR_ERROR);
    }
...
UINT _nx_dhcp_send_request(NX_DHCP *dhcp_ptr, UINT dhcp_message_type) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if (dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid) {
            ...
            status = _nx_dhcp_interface_send_request(
                dhcp_ptr,
                dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_interface_index,
                dhcp_message_type);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_interface_send_request(
    NX_DHCP *dhcp_ptr, UINT iface_index, UINT dhcp_message_type) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_interface_send_request(
    NX_DHCP *dhcp_ptr, UINT iface_index, UINT dhcp_message_type) {
    ...
    /* If the message is a RELEASE or DECLINE request, the host should call
       nx_dhcp_release or nx_dhcp_decline respectively. */
    if ((dhcp_message_type == NX_DHCP_TYPE_DHCPRELEASE) ||
        (dhcp_message_type == NX_DHCP_TYPE_DHCPDECLINE)) {
    ...
    if (dhcp_message_type != NX_DHCP_TYPE_DHCPINFORM) {
        ...
        /* Determine if DHCP is started.  */
        if (dhcp_ptr->nx_dhcp_interface_record[iface_index].nx_dhcp_state ==
            NX_DHCP_STATE_NOT_STARTED) {
    ...
    status =
        _nx_dhcp_send_request_internal(dhcp_ptr, interface_record, dhcp_message_type);
...
static UINT _nx_dhcp_send_request_internal(
    ...
    status = nx_packet_allocate(
        dhcp_ptr->nx_dhcp_packet_pool_ptr, &packet_ptr, NX_IPv4_UDP_PACKET, NX_NO_WAIT);
    ...
    /* Set the interface index and MAC address.  */
    dhcp_client_mac_msw = dhcp_ptr->nx_dhcp_ip_ptr->nx_ip_interface[iface_index]
                              .nx_interface_physical_address_msw;
    dhcp_client_mac_lsw = dhcp_ptr->nx_dhcp_ip_ptr->nx_ip_interface[iface_index]
                              .nx_interface_physical_address_lsw;
    ...
    if ((dhcp_message_type == NX_DHCP_TYPE_DHCPDECLINE) ||
        (dhcp_message_type == NX_DHCP_TYPE_DHCPRELEASE)) {
        _nx_dhcp_store_data(buffer + NX_BOOTP_OFFSET_SECS, 2, 0);
    } else {
        _nx_dhcp_store_data(
            buffer + NX_BOOTP_OFFSET_SECS, 2, interface_record->nx_dhcp_seconds);
    }
    ...
    /* Set the 'broadcast' flag according to user requirement for DHCP Discover, DHCP
       Request and DHCP Inform.  */
    ...
    /* ...unless this is an INFORM REQUEST message. */
        ...
        _nx_dhcp_store_data(
            buffer + NX_BOOTP_OFFSET_CLIENT_IP, 4, interface_record->nx_dhcp_ip_address);
    }
    ...
    _nx_dhcp_add_option_value(
        buffer,
        NX_DHCP_OPTION_DHCP_TYPE,
        NX_DHCP_OPTION_DHCP_TYPE_SIZE,
        dhcp_message_type,
        &index);
#endif

    /* Determine if any additional options need to be added relative to the DHCP message
       type. RFC 2131, Table 5: Fields and options used by DHCP Clients.  */
    ...
    case NX_DHCP_TYPE_BOOT_REQUEST:
        ...
        _nx_dhcp_store_data(
            buffer + NX_BOOTP_OFFSET_CLIENT_IP, 4, interface_record->nx_dhcp_ip_address);
        _nx_dhcp_store_data(
            buffer + NX_BOOTP_OFFSET_SERVER_IP, 4, interface_record->nx_dhcp_server_ip);
    ...
    case NX_DHCP_TYPE_DHCPDISCOVER:
        ...
        _nx_dhcp_add_option_value(
            ...
            NX_DHCP_OPTION_DHCP_LEASE_SIZE,
            NX_DHCP_INFINITE_LEASE,
            &index);
        ...
        if (dhcp_ptr->nx_dhcp_name) {
            ...
            if (_nx_utility_string_length_check(
                    dhcp_ptr->nx_dhcp_name, &name_length, 255)) {
                nx_packet_release(packet_ptr);
                return (NX_DHCP_INVALID_NAME);
            }
            ...
            _nx_dhcp_add_option_string(
                buffer,
                NX_DHCP_OPTION_HOST_NAME,
                name_length,
                (UCHAR *)dhcp_ptr->nx_dhcp_name,
                &index);
        }
        ...
        /* Add an option to specify the maximum length DHCP message that DHCP Client is
           willing to accept. RFC2132, Section9.10, Page28.  */
        _nx_dhcp_add_option_value(
            buffer,
            NX_DHCP_OPTION_MAX_DHCP_MESSAGE,
            2,
            dhcp_ptr->nx_dhcp_max_dhcp_message_size,
            &index);
#endif
    ...
    case NX_DHCP_TYPE_DHCPREQUEST:
        ...
        /* Determine if we have a valid IP address. Must not include if Renewing or
         * Rebinding RCV 2131 4.3.2.  */
        if ((interface_record->nx_dhcp_ip_address != NX_BOOTP_NO_ADDRESS) &&
            (interface_record->nx_dhcp_ip_address != NX_BOOTP_BC_ADDRESS) &&
            (interface_record->nx_dhcp_state != NX_DHCP_STATE_RENEWING) &&
            (interface_record->nx_dhcp_state != NX_DHCP_STATE_REBINDING)) {
        ...
        if ((interface_record->nx_dhcp_state != NX_DHCP_STATE_RENEWING) &&
            ...
            _nx_dhcp_add_option_value(
                ...
                NX_DHCP_OPTION_DHCP_SERVER_SIZE,
                interface_record->nx_dhcp_server_ip,
                &index);
        } else if (
            (interface_record->nx_dhcp_state == NX_DHCP_STATE_RENEWING) ||
            (interface_record->nx_dhcp_state == NX_DHCP_STATE_REBINDING))

        {
            ...
            _nx_dhcp_store_data(
                buffer + NX_BOOTP_OFFSET_CLIENT_IP,
                4,
                interface_record->nx_dhcp_ip_address);
        }
    ...
    packet_ptr->nx_packet_append_ptr =
        packet_ptr->nx_packet_prepend_ptr + NX_BOOTP_OFFSET_END;
    ...
    *(buffer + index) = NX_DHCP_OPTION_END;
    ...
    /* Set the target address according to RFC2131, Section4.3.6, Page33, Table4 and
       Section4.4.4, Page40. DHCP Request for renewing and DHCP Release message must be
       unicast.  */
    if (((dhcp_message_type == NX_DHCP_TYPE_DHCPREQUEST) &&
         (interface_record->nx_dhcp_state == NX_DHCP_STATE_RENEWING)) ||
        (dhcp_message_type == NX_DHCP_TYPE_DHCPRELEASE)) {
    ...
    if ((dhcp_message_type == NX_DHCP_TYPE_DHCPDISCOVER) ||
        ((dhcp_message_type == NX_DHCP_TYPE_DHCPREQUEST) &&
         (interface_record->nx_dhcp_state < NX_DHCP_STATE_BOUND))) {

        /* Call function to send the special packet with zero source address.*/
        status = _nx_dhcp_client_send_with_zero_source_address(
            dhcp_ptr, iface_index, packet_ptr);
    } else {
...
static UINT _nx_dhcp_client_send_with_zero_source_address(
...
#endif /* defined(NX_DISABLE_UDP_TX_CHECKSUM) || defined(NX_DISABLE_IP_TX_CHECKSUM) ||   \
          defined(NX_ENABLE_INTERFACE_CAPABILITY) */
    ULONG checksum;
    ULONG val;
    ...
    udp_header_ptr->nx_udp_header_word_0 =
        (((ULONG)socket_ptr->nx_udp_socket_port) << NX_SHIFT_BY_16) |
        (ULONG)NX_DHCP_SERVER_UDP_PORT;
    ...
    ip_header_ptr->nx_ip_header_word_0 =
        (NX_IP_VERSION | socket_ptr->nx_udp_socket_type_of_service |
    ...
    ip_header_ptr->nx_ip_header_word_2 =
        ((socket_ptr->nx_udp_socket_time_to_live << NX_IP_TIME_TO_LIVE_SHIFT) |
         NX_IP_UDP);
    ...
    if (ip_ptr->nx_ip_packet_filter) {
        ...
        if ((ip_ptr->nx_ip_packet_filter((VOID *)(ip_header_ptr), NX_IP_PACKET_OUT)) !=
            NX_SUCCESS) {
    ...
    if (ip_ptr->nx_ip_packet_filter_extended) {
        ...
        if (ip_ptr->nx_ip_packet_filter_extended(ip_ptr, packet_ptr, NX_IP_PACKET_OUT) !=
            NX_SUCCESS) {
    ...
    if (packet_ptr->nx_packet_length >
        packet_ptr->nx_packet_ip_interface->nx_interface_ip_mtu_size) {

#ifndef NX_DISABLE_FRAGMENTATION
        /* Check the DF bit flag.  */
        if ((ip_ptr->nx_ip_fragment_processing) &&
            (socket_ptr->nx_udp_socket_fragment_enable != NX_DONT_FRAGMENT)) {
...
static UINT _nx_dhcp_extract_information(
    ...
    if (_nx_dhcp_get_option_value(
            dhcp_message, NX_DHCP_OPTION_SUBNET_MASK, &value, length) == NX_SUCCESS) {
        ...
        if (value != NX_BOOTP_NO_ADDRESS) {
            ...
            /* No valid network mask info supplied; use the current network mask if any.
               Don't care about current IP address for now. */
            nx_ip_interface_address_get(
                dhcp_ptr->nx_dhcp_ip_ptr,
                interface_record->nx_dhcp_interface_index,
                &ip_address,
    ...
    if ((value != NX_BOOTP_NO_ADDRESS) &&
        ...
        if (_nx_dhcp_get_option_value(
                dhcp_message, NX_DHCP_OPTION_GATEWAYS, &value, length) == NX_SUCCESS) {
    ...
    if (_nx_dhcp_get_option_value(
            dhcp_message, NX_DHCP_OPTION_DHCP_SERVER, &value, length) == NX_SUCCESS) {
    ...
    /* Get the lease time.  */
    if (_nx_dhcp_get_option_value(
            dhcp_message, NX_DHCP_OPTION_DHCP_LEASE, &value, length) == NX_SUCCESS) {
        ...
        if (value == 0xFFFFFFFF) {
            ...
            interface_record->nx_dhcp_renewal_time =
                interface_record->nx_dhcp_lease_time / 2;
            interface_record->nx_dhcp_rebind_time =
                interface_record->nx_dhcp_lease_time -
                (interface_record->nx_dhcp_lease_time / 8);
        }
    }

    /* Overwrite the renew and rebind times with the specified values if the options are
     * present.  */
    if ((_nx_dhcp_get_option_value(
             dhcp_message, NX_DHCP_OPTION_RENEWAL, &value, length) == NX_SUCCESS) &&
    ...
    if (_nx_dhcp_get_option_value(dhcp_message, NX_DHCP_OPTION_REBIND, &value, length) ==
        NX_SUCCESS) {
    ...
    /* Determine if this is an ACK from a server response, which can only happen from a
     * handful of states.  */
    ...
    memcpy(
        interface_record->nx_dhcp_options_buffer, /* Use case of memcpy is verified. */
        &dhcp_message[NX_BOOTP_OFFSET_OPTIONS],
        interface_record->nx_dhcp_options_size);
...
_nx_dhcp_get_option_value(UCHAR *bootp_message, UINT option, ULONG *value, UINT length) {
    ...
    /* There is no need to check whether the option is PAD or END here since no caller
       will pass these 2 options and for denfensive purpose, the function below could
       check them and guarantee appropriate behaviour */
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_add_option_value(
    UCHAR *bootp_message, UINT option, UINT size, ULONG value, UINT *index) {
...
static UINT _nx_dhcp_add_option_parameter_request(
    ...
    *(bootp_message + (*index)) = NX_DHCP_OPTION_DHCP_PARAMETERS;
    ...
    /* Store the option size.  */
    *(bootp_message + (*index)) = (UCHAR)(NX_DHCP_REQUEST_PARAMETER_SIZE +
                                          dhcp_ptr->nx_dhcp_user_request_parameter_size);
    (*index)++;

    /* Store the option value.  */
    _nx_dhcp_move_string(
        bootp_message + (*index),
        _nx_dhcp_request_parameters,
        NX_DHCP_REQUEST_PARAMETER_SIZE);
...
static ULONG _nx_dhcp_add_randomize(ULONG timeout) {
    ...
    /* Uniform random number chosen from the range -1 to +1 second as recommended by
     * RFC2131, Section4.1, Page24. */

    /* Calculate random time adjustment in timer ticks from the range 0 to
    ...
    if (adjustment < NX_IP_PERIODIC_RATE) {
        ...
        if (timeout > (NX_IP_PERIODIC_RATE - adjustment))
            ...
            timeout = 1; /* Set 1 here since the minmum tick for timeout shall be larger
                            than 0 */
    } else {
...
UINT _nx_dhcp_server_address_get(NX_DHCP *dhcp_ptr, ULONG *server_address) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if ((dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid) &&
            ...
            status = _nx_dhcp_interface_server_address_get(
                dhcp_ptr,
                dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_interface_index,
                server_address);
...
UINT _nxe_dhcp_interface_server_address_get(
    NX_DHCP *dhcp_ptr, UINT iface_index, ULONG *server_address) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_interface_server_address_get(
    NX_DHCP *dhcp_ptr, UINT iface_index, ULONG *server_address) {
...
VOID _nx_dhcp_ip_conflict(
    ...
    for (dhcp_ptr = _nx_dhcp_created_ptr; dhcp_ptr;
         dhcp_ptr = dhcp_ptr->nx_dhcp_created_next) {
        if (dhcp_ptr->nx_dhcp_ip_ptr == ip_ptr) {
            ...
            tx_event_flags_set(
                &(_nx_dhcp_created_ptr->nx_dhcp_events),
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_client_get_record(
    NX_DHCP *dhcp_ptr, NX_DHCP_CLIENT_RECORD *client_record_ptr) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_client_get_record(
    NX_DHCP *dhcp_ptr, NX_DHCP_CLIENT_RECORD *client_record_ptr) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if ((dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid) &&
            ...
            status = _nx_dhcp_client_interface_get_record(
                dhcp_ptr,
                dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_interface_index,
                client_record_ptr);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_client_interface_get_record(
    NX_DHCP *dhcp_ptr, UINT iface_index, NX_DHCP_CLIENT_RECORD *client_record_ptr) {
    ...
    status =
        _nx_dhcp_client_interface_get_record(dhcp_ptr, iface_index, client_record_ptr);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_client_interface_get_record(
    NX_DHCP *dhcp_ptr, UINT iface_index, NX_DHCP_CLIENT_RECORD *client_record_ptr) {
    ...
    if (interface_record->nx_dhcp_state < NX_DHCP_STATE_BOUND) {
        ...
        /* The DHCP Client is not bound to an IP address. Cannot create a record for
           restoring Client state if the Client not bound to an IP address. */
    ...
    client_record_ptr->nx_dhcp_ip_address =
        interface_record->nx_dhcp_ip_address; /* Server assigned IP Address */
    client_record_ptr->nx_dhcp_network_mask =
        interface_record->nx_dhcp_network_mask; /* Server assigned network mask */
    client_record_ptr->nx_dhcp_gateway_address =
        interface_record->nx_dhcp_gateway_address;
    ...
    client_record_ptr->nx_dhcp_renewal_remain_time =
        interface_record->nx_dhcp_renewal_remain_time;
    client_record_ptr->nx_dhcp_rebind_remain_time =
        interface_record->nx_dhcp_rebind_remain_time;
    client_record_ptr->nx_dhcp_interface_index =
        interface_record->nx_dhcp_interface_index;
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_client_restore_record(
    NX_DHCP *dhcp_ptr, NX_DHCP_CLIENT_RECORD *client_record_ptr, ULONG time_elapsed) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_client_restore_record(
    NX_DHCP *dhcp_ptr, NX_DHCP_CLIENT_RECORD *client_record_ptr, ULONG time_elapsed) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_client_interface_restore_record(
    NX_DHCP *dhcp_ptr,
    UINT iface_index,
    NX_DHCP_CLIENT_RECORD *client_record_ptr,
    ULONG time_elapsed) {
    ...
    status = _nx_dhcp_client_interface_restore_record(
        dhcp_ptr, iface_index, client_record_ptr, time_elapsed);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_client_interface_restore_record(
    NX_DHCP *dhcp_ptr,
    UINT iface_index,
    NX_DHCP_CLIENT_RECORD *client_record_ptr,
    ULONG time_elapsed) {
    ...
    if (interface_record->nx_dhcp_gateway_address) {
        ...
        status = nx_ip_gateway_address_set(
            dhcp_ptr->nx_dhcp_ip_ptr, interface_record->nx_dhcp_gateway_address);
    ...
    /* Now apply the time elapsed to update the DHCP Client time remaining on its lease
     * and current DHCP state. */
    status = _nx_dhcp_client_interface_update_time_remaining(
        dhcp_ptr, interface_record->nx_dhcp_interface_index, time_elapsed);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_dhcp_client_interface_update_time_remaining(
    NX_DHCP *dhcp_ptr, UINT iface_index, ULONG time_elapsed) {
    ...
    status = _nx_dhcp_client_interface_update_time_remaining(
        dhcp_ptr, iface_index, time_elapsed);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_dhcp_client_interface_update_time_remaining(
    NX_DHCP *dhcp_ptr, UINT iface_index, ULONG time_elapsed) {
    ...
    if (interface_record->nx_dhcp_state == NX_DHCP_STATE_BOUND) {
        ...
        time_accrued = interface_record->nx_dhcp_rebind_time -
                       interface_record->nx_dhcp_renewal_remain_time;

        /* Note if dhcp_ptr -> nx_dhcp_rebind_time < dhcp_ptr ->
           nx_dhcp_renewal_remain_time, the CLient either received invalid DHCP parameters
           or there is an internal error. The renewal time should never exceed the rebind
           time.  */
        ...
        time_accrued = interface_record->nx_dhcp_lease_time -
                       interface_record->nx_dhcp_rebind_remain_time;

        /* Note if dhcp_ptr -> nx_dhcp_lease_time < dhcp_ptr ->
           nx_dhcp_rebind_remain_time, the CLient either received invalid DHCP parameters
           or there is an internal error. The rebind time should never exceed the lease
           time.  */
    } else {
        /* Expired! */
    ...
    /* Determine if the Client should renew. */
        ...
        if (interface_record->nx_dhcp_state == NX_DHCP_STATE_RENEWING) {
            ...
            /* Compute how many seconds into the renew period the Client is (total time on
               lease over and above the time to start renewing. */
            if (accrued_time_adjusted == interface_record->nx_dhcp_renewal_time) {
                ...
                interface_record->nx_dhcp_renewal_remain_time =
                    interface_record->nx_dhcp_rebind_time - accrued_time_adjusted;
            }
    ...
    /* Determine if it is time for the Client to rebind (e.g. check the lease has not
       completely expired). */
        ...
        if (interface_record->nx_dhcp_state == NX_DHCP_STATE_REBINDING) {
            ...
            /* Compute how many seconds into the rebind period the Client is (total time
               on lease over and above the time to start rebinding. */
            if (accrued_time_adjusted == interface_record->nx_dhcp_rebind_time) {
                ...
                interface_record->nx_dhcp_rebind_remain_time =
                    interface_record->nx_dhcp_lease_time - accrued_time_adjusted;
            }
        ...
        /* Clear the existing DHCP Client network parameters for restarting in the INIT
         * state. */
        _nx_dhcp_interface_reinitialize(dhcp_ptr, iface_index);
        ...
        /* The client begins in INIT state and forms a DHCPDISCOVER message.
           The client should wait a random time between one and ten seconds to
           desynchronize the use of DHCP at startup. RFC2131, Section4.4.1, Page36.  */
...
UINT _nx_dhcp_resume(NX_DHCP *dhcp_ptr) {
    ...
    for (i = 0; i < NX_DHCP_CLIENT_MAX_RECORDS; i++) {
        ...
        if (dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_record_valid) {
            ...
            dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_xid =
                client_physical_msw ^ client_physical_lsw ^ (ULONG)NX_RAND();

            /* If the DHCP Client is renewing or rebinding on being resumed, send a DHCP
             * request. */
            if ((dhcp_ptr->nx_dhcp_interface_record[i].nx_dhcp_state ==
