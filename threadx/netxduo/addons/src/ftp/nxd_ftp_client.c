...
UINT _nxe_ftp_client_connect(
    ...
    status = _nx_ftp_client_connect(
        ftp_client_ptr, server_ip, username, password, wait_option);
...
UINT _nx_ftp_client_connect(
    ...
    return (_nx_ftp_client_connect_internal(
        ftp_client_ptr, &server_ipduo, username, password, wait_option));
#else
    NX_PARAMETER_NOT_USED(ftp_client_ptr);
    NX_PARAMETER_NOT_USED(server_ip);
    NX_PARAMETER_NOT_USED(username);
...
UINT _nxde_ftp_client_connect(
    ...
    /* Call actual client connect function.  */
    status = _nxd_ftp_client_connect(
        ftp_client_ptr, server_ipduo, username, password, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_connect_internal(
    NX_FTP_CLIENT *ftp_client_ptr,
    ...
    /* Now wait for the "220 " response from the FTP server to indicate the connection has
       been established.  */
    status = nx_tcp_socket_receive(
        &(ftp_client_ptr->nx_ftp_client_control_socket), &packet_ptr, wait_option);
    ...
    if (status) {
        ...
        nx_tcp_socket_disconnect(
            &(ftp_client_ptr->nx_ftp_client_control_socket), wait_option);
    ...
    if (((!username) && ((UINT)(packet_ptr->nx_packet_data_end -
                                packet_ptr->nx_packet_prepend_ptr) < 11)) ||
        ((username) && ((UINT)(packet_ptr->nx_packet_data_end -
                               packet_ptr->nx_packet_prepend_ptr) < 7))) {
    ...
    if (username == NX_NULL) {
        ...
        for (i = 0; username[i]; i++) {
            ...
            /* Check if out of boundary.  */
            if ((i + 7) >= (UINT)(packet_ptr->nx_packet_data_end -
    ...
    /* Check if out of boundary.  */
    if (((!password) && ((UINT)(packet_ptr->nx_packet_data_end -
                                packet_ptr->nx_packet_prepend_ptr) < 11)) ||
        ((password) && ((UINT)(packet_ptr->nx_packet_data_end -
                               packet_ptr->nx_packet_prepend_ptr) < 7))) {
    ...
    ftp_client_ptr->nx_ftp_client_data_port =
        ftp_client_ptr->nx_ftp_client_control_socket.nx_tcp_socket_port;
...
UINT _nxe_ftp_client_create(
    ...
    if ((ip_ptr == NX_NULL) || (ip_ptr->nx_ip_id != NX_IP_ID) ||
        (ftp_client_ptr == NX_NULL) ||
        (ftp_client_ptr->nx_ftp_client_id == NXD_FTP_CLIENT_ID) || (pool_ptr == NX_NULL))
        return (NX_PTR_ERROR);

    /* Call actual client create function.  */
    status = _nx_ftp_client_create(
        ftp_client_ptr, ftp_client_name, ip_ptr, window_size, pool_ptr);
...
UINT _nx_ftp_client_create(
    ...
    status = nx_tcp_socket_create(
        ...
        window_size,
        NX_NULL,
        _nx_ftp_client_data_disconnect);
...
UINT _nxe_ftp_client_directory_create(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *directory_name, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_directory_create(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *directory_name, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_directory_default_set(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *directory_path, ULONG wait_option) {
    ...
    status =
        _nx_ftp_client_directory_default_set(ftp_client_ptr, directory_path, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_directory_default_set(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *directory_path, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_directory_delete(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *directory_name, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_directory_delete(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *directory_name, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_directory_listing_get(
    NX_FTP_CLIENT *ftp_client_ptr,
    CHAR *directory_path,
    NX_PACKET **packet_ptr,
    ULONG wait_option) {
    ...
    status = _nx_ftp_client_directory_listing_get(
        ftp_client_ptr, directory_path, packet_ptr, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_directory_listing_get(
    NX_FTP_CLIENT *ftp_client_ptr,
    CHAR *directory_path,
    NX_PACKET **packet_ptr,
    ULONG wait_option) {
    ...
    *packet_ptr = NX_NULL;
    ...
    /* Check if out of boundary.  */
    if ((UINT)(new_packet_ptr->nx_packet_data_end -
               new_packet_ptr->nx_packet_prepend_ptr) < 7) {
    ...
    for (i = 0; directory_path[i]; i++) {
        ...
        /* Check if out of boundary.  */
        if ((i + 7) >= (UINT)(new_packet_ptr->nx_packet_data_end -
    ...
    status = nx_tcp_socket_send(
        &(ftp_client_ptr->nx_ftp_client_control_socket), new_packet_ptr, wait_option);
    ...
    if (ftp_client_ptr->nx_ftp_client_passive_transfer_enabled == NX_FALSE) {
        ...
        status = nx_tcp_server_socket_accept(
            &(ftp_client_ptr->nx_ftp_client_data_socket), wait_option);
    ...
    status = nx_tcp_socket_receive(
        &(ftp_client_ptr->nx_ftp_client_data_socket), packet_ptr, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_directory_listing_continue(
    NX_FTP_CLIENT *ftp_client_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {
    ...
    status = _nx_ftp_client_directory_listing_continue(
        ftp_client_ptr, packet_ptr, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_directory_listing_continue(
    NX_FTP_CLIENT *ftp_client_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {
    ...
    *packet_ptr = NX_NULL;
    ...
    if (status) {
        ...
        status = nx_tcp_socket_receive(
            &(ftp_client_ptr->nx_ftp_client_control_socket), &response_ptr, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_file_delete(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *file_name, ULONG wait_option) {

    UINT status;

    /* Check for invalid input pointers.  */
    if ((ftp_client_ptr == NX_NULL) ||
        (ftp_client_ptr->nx_ftp_client_id != NXD_FTP_CLIENT_ID) || (file_name == NX_NULL))
        return (NX_PTR_ERROR);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_file_delete(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *file_name, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_passive_mode_set(
    NX_FTP_CLIENT *ftp_client_ptr, UINT passive_mode_enabled) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_passive_mode_set(
    NX_FTP_CLIENT *ftp_client_ptr, UINT passive_mode_enabled) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_transfer_mode_set(
    NX_FTP_CLIENT *ftp_client_ptr, UINT transfer_mode) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_file_open(
    NX_FTP_CLIENT *ftp_client_ptr, CHAR *file_name, UINT open_type, ULONG wait_option) {
...
UINT _nx_ftp_client_file_open(
    ...
    /* Flush the control socket queue for any old messages before starting a new file open
     * request. */
    status = nx_tcp_socket_receive(
        &(ftp_client_ptr->nx_ftp_client_control_socket), &packet_ptr, 1);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_file_read(
    NX_FTP_CLIENT *ftp_client_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_file_rename(
    NX_FTP_CLIENT *ftp_client_ptr,
    CHAR *filename,
    CHAR *new_filename,
    ULONG wait_option) {
    ...
    status =
        _nx_ftp_client_file_rename(ftp_client_ptr, filename, new_filename, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_file_rename(
    NX_FTP_CLIENT *ftp_client_ptr,
    CHAR *filename,
    CHAR *new_filename,
    ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_ftp_client_file_write(
    NX_FTP_CLIENT *ftp_client_ptr, NX_PACKET *packet_ptr, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_file_write(
    NX_FTP_CLIENT *ftp_client_ptr, NX_PACKET *packet_ptr, ULONG wait_option) {
    ...
    if (ftp_client_ptr->nx_ftp_client_transfer_mode == NX_FTP_TRANSFER_MODE_BLOCK) {
        ...
        /* Check the file size.  */
        if (ftp_client_ptr->nx_ftp_client_block_remaining_size <
...
UINT _nx_ftp_utility_convert_IPv6_to_ascii(
    NX_TCP_SOCKET *socket_ptr, CHAR *buffer, UINT buffer_length, UINT *size) {
    ...
    _nx_ftp_utility_convert_number_ascii(
        socket_ptr->nx_tcp_socket_ipv6_addr->nxd_ipv6_address[0], number_buf);
    ...
    _nx_ftp_utility_convert_number_ascii(
        socket_ptr->nx_tcp_socket_ipv6_addr->nxd_ipv6_address[1], number_buf);
    ...
    _nx_ftp_utility_convert_number_ascii(
        socket_ptr->nx_tcp_socket_ipv6_addr->nxd_ipv6_address[2], number_buf);
    ...
    _nx_ftp_utility_convert_number_ascii(
        socket_ptr->nx_tcp_socket_ipv6_addr->nxd_ipv6_address[3], number_buf);
...
UINT _nx_ftp_utility_convert_portnumber_ascii(
    UINT number, CHAR *numstring, UINT *numstring_length) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_packet_allocate(
    NX_FTP_CLIENT *ftp_client_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {
    ...
    if (ftp_client_ptr->nx_ftp_client_control_socket.nx_tcp_socket_connect_ip
    ...
    status = nx_packet_allocate(
        ftp_client_ptr->nx_ftp_client_packet_pool_ptr,
        packet_ptr,
        packet_type,
        wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_active_transfer_setup(
    NX_FTP_CLIENT *ftp_client_ptr, ULONG wait_option) {
    ...
    status = nx_tcp_free_port_find(
        ftp_client_ptr->nx_ftp_client_ip_ptr,
        ftp_client_ptr->nx_ftp_client_data_port + 1,
        &data_port);
    ...
    if (ftp_client_ptr->nx_ftp_client_control_socket.nx_tcp_socket_connect_ip
        ...
        /* Check if out of boundary.
           Max EPRT command sring: 8 ("EPRT |2|") + 39 (IPv6 address string) + 1 ('|') + 5
           (Max port string) + 1 ('|') + 2 ("\r\n") = 56.  */
        if ((UINT)(packet_ptr->nx_packet_data_end - packet_ptr->nx_packet_prepend_ptr) <
            ...
            nx_tcp_server_socket_unlisten(
                ftp_client_ptr->nx_ftp_client_ip_ptr, data_port);
        ...
        status = _nx_ftp_utility_convert_IPv6_to_ascii(
            &(ftp_client_ptr->nx_ftp_client_control_socket),
            &ipduo_buffer[0],
            NX_FTP_IPV6_ADDRESS_BUFSIZE,
            &ipduo_size);
        ...
        memcpy(
            buffer_ptr + 8,
            ipduo_buffer,
            ipduo_size); /* Use case of memcpy is verified. */
        ...
        status = _nx_ftp_utility_convert_portnumber_ascii(
            data_port, &ipduo_buffer[0], &ipduo_size);
        ...
        ip_address = ftp_client_ptr->nx_ftp_client_control_socket
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_passive_transfer_setup(
    NX_FTP_CLIENT *ftp_client_ptr, ULONG wait_option) {
    ...
    if (ftp_client_ptr->nx_ftp_client_control_socket.nx_tcp_socket_connect_ip
        ...
        while (i < packet_ptr->nx_packet_length) {
            ...
            if ((buffer_ptr[i - 3] == '(') && (buffer_ptr[i - 2] == '|') &&
                (buffer_ptr[i - 1] == '|') && (buffer_ptr[i] == '|')) {
                break;
            }
        ...
        COPY_NXD_ADDRESS(
            &(ftp_client_ptr->nx_ftp_client_control_socket.nx_tcp_socket_connect_ip),
        ...
        /* Now parse the address and port for the Client to connect to on the data socket.
           IP address and port are comma delimited and port number is the last two
           numbers, PASV port = (p1 * 256) + p2.  */

        /* Skip non numeric text and whitespace.  */
        i = 3;
        while (((buffer_ptr[i] > '9') || (buffer_ptr[i] < '0')) &&
        ...
        while (i < packet_ptr->nx_packet_length) {
            ...
            if ((buffer_ptr[i] == 0x0D) || (buffer_ptr[i] == 0x0A) ||
                (buffer_ptr[i] == 0)) {
        ...
        /* Determine if an error occurred.  */
        if ((status != NX_SUCCESS) || (buffer_ptr[i] != 13) || (commas != 5) ||
            (ip_address == 0) || (data_port == 0)) {
        ...
        ftp_client_ptr->nx_ftp_client_data_socket.nx_tcp_socket_connect_ip.nxd_ip_address
            .v4 = ip_address;
        ftp_client_ptr->nx_ftp_client_data_socket.nx_tcp_socket_connect_port = data_port;
    ...
    status = nx_tcp_client_socket_bind(
        (&ftp_client_ptr->nx_ftp_client_data_socket), NX_ANY_PORT, wait_option);
...
/*                                                                        */
/**************************************************************************/
VOID _nx_ftp_client_data_socket_cleanup(
    NX_FTP_CLIENT *ftp_client_ptr, ULONG wait_option) {
...
UINT _nx_ftp_client_block_mode_send(NX_FTP_CLIENT *ftp_client_ptr, ULONG wait_option) {
    ...
    /* Check for 2xx message, signaling the data port was connected properly and ready for
     * transfer.  */
    if (buffer_ptr[0] != '2') {
...
UINT _nx_ftp_client_block_header_send(NX_FTP_CLIENT *ftp_client_ptr, ULONG block_size) {
    ...
    /* Write packet payload to the file.  */
    status = nx_tcp_socket_send(
        &(ftp_client_ptr->nx_ftp_client_data_socket), packet_ptr, NX_NO_WAIT);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_client_block_header_retrieve(
    NX_FTP_CLIENT *ftp_client_ptr, NX_PACKET *packet_ptr) {
    ...
    if (ftp_client_ptr->nx_ftp_client_block_total_size == 0) {
        ...
        ftp_client_ptr->nx_ftp_client_block_remaining_size =
            ftp_client_ptr->nx_ftp_client_block_total_size;
    ...
    if (ftp_client_ptr->nx_ftp_client_block_remaining_size <
        ...
        delta = packet_ptr->nx_packet_length -
                ftp_client_ptr->nx_ftp_client_block_remaining_size;
