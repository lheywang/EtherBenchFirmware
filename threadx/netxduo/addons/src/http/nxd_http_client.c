...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_client_create(
    NX_HTTP_CLIENT *client_ptr,
    CHAR *client_name,
    NX_IP *ip_ptr,
    NX_PACKET_POOL *pool_ptr,
    ULONG window_size,
    UINT http_client_size) {
    ...
    if ((ip_ptr == NX_NULL) || (ip_ptr->nx_ip_id != NX_IP_ID) ||
        (client_ptr == NX_NULL) ||
        (client_ptr->nx_http_client_id == NXD_HTTP_CLIENT_ID) || (pool_ptr == NX_NULL) ||
        (http_client_size != sizeof(NX_HTTP_CLIENT)))
        return (NX_PTR_ERROR);
...
UINT _nx_http_client_create(
    ...
    status = nx_tcp_socket_create(
        ip_ptr,
        &(client_ptr->nx_http_client_socket),
        client_name,
...
UINT _nx_http_client_delete(NX_HTTP_CLIENT *client_ptr) {
    ...
    if ((client_ptr->nx_http_client_state == NX_HTTP_CLIENT_STATE_PUT) ||
        ...
        nx_tcp_socket_disconnect(
            &(client_ptr->nx_http_client_socket), NX_HTTP_CLIENT_TIMEOUT);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_client_get_start(
    NX_HTTP_CLIENT *client_ptr,
    ULONG ip_address,
    CHAR *resource,
    CHAR *input_ptr,
    ...
    if ((client_ptr == NX_NULL) ||
        (client_ptr->nx_http_client_id != NXD_HTTP_CLIENT_ID) || (resource == NX_NULL))
        return (NX_PTR_ERROR);
    ...
    status = _nx_http_client_get_start(
        client_ptr,
        ip_address,
        resource,
        input_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_client_get_start_extended(
    NX_HTTP_CLIENT *client_ptr,
    ULONG ip_address,
    CHAR *resource,
    ...
    /* Call actual GET start routine.  */
    status = _nx_http_client_get_start_extended(
        client_ptr,
        ip_address,
        resource,
        resource_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_client_get_start(
    NX_HTTP_CLIENT *client_ptr,
    ULONG ip_address,
    CHAR *resource,
    CHAR *input_ptr,
    ...
    status = _nxd_http_client_get_start(
        client_ptr,
        &server_ip_addr,
        resource,
        input_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_client_get_start_extended(
    NX_HTTP_CLIENT *client_ptr,
    ULONG ip_address,
    CHAR *resource,
    UINT resource_length,
    ...
    /* Call the NetX HTTP 'duo' get start service. */
    status = _nxd_http_client_get_start_extended(
        client_ptr,
        &server_ip_addr,
        resource,
        resource_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_http_client_get_start(
    NX_HTTP_CLIENT *client_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_http_client_get_start_extended(
    NX_HTTP_CLIENT *client_ptr,
    NXD_ADDRESS *ip_address,
    CHAR *resource,
    UINT resource_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_http_client_get_start(
    NX_HTTP_CLIENT *client_ptr,
    ...
    if ((username) && (password)) {
        if (_nx_utility_string_length_check(
                username, &username_length, NX_HTTP_MAX_NAME) ||
            _nx_utility_string_length_check(
                password, &password_length, NX_HTTP_MAX_PASSWORD)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_http_client_get_start_extended(
    NX_HTTP_CLIENT *client_ptr,
    NXD_ADDRESS *server_ip,
    CHAR *resource,
    UINT resource_length,
    ...
    if ((username) && (password)) {
        if (_nx_utility_string_length_check(
            ...
            _nx_utility_string_length_check(
                password, &temp_password_length, password_length)) {
    ...
    /* Check resource length.  */
    if (_nx_utility_string_length_check(
            resource, &temp_resource_length, resource_length)) {
        return (NX_HTTP_ERROR);
    }
    ...
    status = nx_tcp_client_socket_bind(
        &(client_ptr->nx_http_client_socket), NX_ANY_PORT, wait_option);
    ...
    status = nxd_tcp_client_socket_connect(
        &(client_ptr->nx_http_client_socket),
        server_ip,
        client_ptr->nx_http_client_connect_port,
        wait_option);
    ...
    if (server_ip->nxd_ip_version == NX_IP_VERSION_V4) {
        status = nx_packet_allocate(
            client_ptr->nx_http_client_packet_pool_ptr,
            &packet_ptr,
            NX_IPv4_TCP_PACKET,
            wait_option);
    } else {
    ...
    if (resource[0] != '/') {
        ...
        if (((resource[0] == 'h') || (resource[0] == 'H')) &&
            ((resource[1] == 't') || (resource[1] == 'T')) &&
            ((resource[2] == 't') || (resource[2] == 'T')) &&
            ((resource[3] == 'p') || (resource[3] == 'P')) && (resource[4] == ':')) {
    ...
    /* Else URI string refers to root directory file, and has a leading '/' character
     * already. */
    ...
    nx_packet_data_append(
        packet_ptr, crlf, 2, client_ptr->nx_http_client_packet_pool_ptr, NX_WAIT_FOREVER);
    ...
    if ((username) && (password)) {
        ...
        _nx_utility_base64_encode(
            (UCHAR *)string1,
            username_length + password_length + 1,
            (UCHAR *)string2,
            sizeof(string2),
            &string2_length);
        nx_packet_data_append(
    ...
    status =
        nx_tcp_socket_send(&(client_ptr->nx_http_client_socket), packet_ptr, wait_option);
    ...
    status = nx_tcp_socket_receive(
        &(client_ptr->nx_http_client_socket), &response_packet_ptr, wait_option);
    ...
    if (status == NX_SUCCESS) {
        ...
        if (buffer_ptr[9] == '2') {
            ...
            if ((response_packet_ptr->nx_packet_data_end -
                 response_packet_ptr->nx_packet_data_start) <
                NX_HTTP_CLIENT_MIN_PACKET_SIZE) {
        ...
        /* Determine if it is an authentication error.  */
            ...
            (buffer_ptr[9] == '4') && (buffer_ptr[10] == '0') &&
            (buffer_ptr[11] == '1')) {
    ...
    if (response_packet_ptr->nx_packet_length > (offset + 3)) {
        ...
        response_packet_ptr->nx_packet_prepend_ptr =
            response_packet_ptr->nx_packet_prepend_ptr + offset;
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_client_get_packet(
    NX_HTTP_CLIENT *client_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {

    UINT status;

    /* Check for invalid input pointers.  */
    if ((client_ptr == NX_NULL) ||
        (client_ptr->nx_http_client_id != NXD_HTTP_CLIENT_ID) || (packet_ptr == NX_NULL))
        return (NX_PTR_ERROR);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_client_get_packet(
    NX_HTTP_CLIENT *client_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {
    ...
    *packet_ptr = NX_NULL;
    ...
    if (client_ptr->nx_http_client_total_transfer_bytes ==
        client_ptr->nx_http_client_actual_bytes_transferred) {
    ...
    if (client_ptr->nx_http_client_first_packet) {
        ...
        status = nx_tcp_socket_receive(
            &(client_ptr->nx_http_client_socket), &data_packet_ptr, wait_option);
    ...
    if (data_packet_ptr->nx_packet_length >
        (client_ptr->nx_http_client_total_transfer_bytes -
         client_ptr->nx_http_client_actual_bytes_transferred)) {
...
UINT _nxe_http_client_put_start(
    NX_HTTP_CLIENT *client_ptr,
    ULONG ip_address,
    CHAR *resource,
    CHAR *username,
    ...
    status = _nx_http_client_put_start(
        client_ptr, ip_address, resource, username, password, total_bytes, wait_option);
...
UINT _nxe_http_client_put_start_extended(
    NX_HTTP_CLIENT *client_ptr,
    ULONG ip_address,
    CHAR *resource,
    UINT resource_length,
    ...
    /* Call actual PUT start routine.  */
    status = _nx_http_client_put_start_extended(
        client_ptr,
        ip_address,
        resource,
        resource_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_client_put_start(
    NX_HTTP_CLIENT *client_ptr,
    ULONG ip_address,
    CHAR *resource,
    CHAR *username,
    ...
    status = _nxd_http_client_put_start(
        client_ptr,
        &server_ip_addr,
        resource,
        username,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_client_put_start_extended(
    NX_HTTP_CLIENT *client_ptr,
    ULONG ip_address,
    CHAR *resource,
    UINT resource_length,
    ...
    status = _nxd_http_client_put_start_extended(
        client_ptr,
        &server_ip_addr,
        resource,
        resource_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_http_client_put_start(
    NX_HTTP_CLIENT *client_ptr,
    NXD_ADDRESS *server_ip,
    CHAR *resource,
    CHAR *username,
    ...
    /* Call actual PUT start routine.  */
    status = _nxd_http_client_put_start(
        client_ptr, server_ip, resource, username, password, total_bytes, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_http_client_put_start_extended(
    NX_HTTP_CLIENT *client_ptr,
    NXD_ADDRESS *server_ip,
    CHAR *resource,
    UINT resource_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_http_client_put_start(
    NX_HTTP_CLIENT *client_ptr,
    NXD_ADDRESS *server_ip,
    CHAR *resource,
    CHAR *username,
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_http_client_put_start_extended(
    NX_HTTP_CLIENT *client_ptr,
    NXD_ADDRESS *server_ip,
    CHAR *resource,
    UINT resource_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_client_put_packet(
    NX_HTTP_CLIENT *client_ptr, NX_PACKET *packet_ptr, ULONG wait_option) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_client_put_packet(
    NX_HTTP_CLIENT *client_ptr, NX_PACKET *packet_ptr, ULONG wait_option) {
    ...
    status = nx_tcp_socket_receive(
        &(client_ptr->nx_http_client_socket), &response_packet_ptr, NX_NO_WAIT);

    /* Check for an early response from the Server.  */
    if (status == NX_SUCCESS) {

        /* This is an error condition since the Server should not respond until the PUT is
         * complete.  */
    ...
    client_ptr->nx_http_client_actual_bytes_transferred =
        client_ptr->nx_http_client_actual_bytes_transferred + length;
