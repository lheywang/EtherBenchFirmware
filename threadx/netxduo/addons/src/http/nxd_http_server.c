...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_content_get(
    ...
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) ||
        (packet_ptr == NX_NULL) || (destination_ptr == NX_NULL) ||
        (actual_size == NX_NULL))
        return (NX_PTR_ERROR);
    ...
    status = _nx_http_server_content_get(
        server_ptr,
        packet_ptr,
        byte_offset,
        destination_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_content_get(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET *packet_ptr,
    ULONG byte_offset,
    CHAR *destination_ptr,
    ...
    *actual_size = 0;
    ...
    get_offset =
        ((ULONG)_nx_http_server_calculate_content_offset(packet_ptr)) + byte_offset;
    ...
    while (get_offset >= packet_ptr->nx_packet_length) {
        ...
        if (new_packet_ptr->nx_packet_next) {
            ...
            /* No, there isn't a last packet pointer.  Point to new packet from the head
             * packet.  */
        ...
        packet_ptr->nx_packet_length =
            packet_ptr->nx_packet_length + new_packet_ptr->nx_packet_length;
#endif /* NX_DISABLE_PACKET_CHAIN */
    }
    ...
    do {
        ...
        if (get_offset < (offset + (ULONG)(new_packet_ptr->nx_packet_append_ptr -
            ...
            buffer_ptr =
                ((CHAR *)new_packet_ptr->nx_packet_prepend_ptr) + (get_offset - offset);
            break;
        }

        /* Otherwise update the offset.  */
        offset = offset + (ULONG)(new_packet_ptr->nx_packet_append_ptr -
                                  new_packet_ptr->nx_packet_prepend_ptr);
    ...
    if (destination_size >
        (UINT)(new_packet_ptr->nx_packet_append_ptr - (UCHAR *)buffer_ptr)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_packet_content_find(
    NX_HTTP_SERVER *server_ptr, NX_PACKET **packet_ptr, ULONG *content_length) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_packet_content_find(
    NX_HTTP_SERVER *server_ptr, NX_PACKET **packet_ptr, ULONG *content_length) {
    ...
    while ((*packet_ptr)->nx_packet_length <= get_offset) {
        /* Data is not present in this packet. So we have to read the next packet from the
         * socket. */
...
UINT _nxe_http_server_packet_get(NX_HTTP_SERVER *server_ptr, NX_PACKET **packet_ptr) {
    ...
    /* Check for invalid packet pointer.  */
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) || (packet_ptr == NX_NULL))
        return (NX_PTR_ERROR);
...
UINT _nx_http_server_content_length_get(NX_PACKET *packet_ptr) {
    ...
    while (((buffer_ptr + 17) < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_content_get_extended(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET *packet_ptr,
    ULONG byte_offset,
    CHAR *destination_ptr,
    ...
    /* Call actual server content get function.  */
    status = _nx_http_server_content_get_extended(
        server_ptr,
        packet_ptr,
        byte_offset,
        destination_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_content_get_extended(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET *packet_ptr,
    ULONG byte_offset,
    CHAR *destination_ptr,
    ...
    *actual_size = 0;
    ...
    /* Determine if there is content in this HTTP request.  */
    status =
        (ULONG)_nx_http_server_content_length_get_extended(packet_ptr, &content_length);
...
UINT _nxe_http_server_content_length_get_extended(
    NX_PACKET *packet_ptr, ULONG *content_length) {
...
UINT _nxe_http_server_create(
    ...
    /* Determine if the packet payload is equal to or greater than the maximum HTTP header
     * supported.  */
    if (((packet_ptr->nx_packet_data_end - packet_ptr->nx_packet_data_start) -
         NX_PHYSICAL_TRAILER) < NX_HTTP_SERVER_MIN_PACKET_SIZE)
        return (NX_HTTP_POOL_ERROR);

    /* Call actual server create function.  */
    status = _nx_http_server_create(
        http_server_ptr,
        http_server_name,
        ip_ptr,
        media_ptr,
...
UINT _nx_http_server_create(
    ...
    status = nx_tcp_socket_create(
        ip_ptr,
        &(http_server_ptr->nx_http_server_socket),
        http_server_name,
        NX_HTTP_TYPE_OF_SERVICE,
        NX_HTTP_FRAGMENT_OPTION,
        NX_HTTP_TIME_TO_LIVE,
        NX_HTTP_SERVER_WINDOW_SIZE,
        NX_NULL,
        NX_NULL);
    ...
    status = tx_thread_create(
        &(http_server_ptr->nx_http_server_thread),
        "HTTP Server Thread",
        _nx_http_server_thread_entry,
        (ULONG)http_server_ptr,
        stack_ptr,
        stack_size,
        NX_HTTP_SERVER_PRIORITY,
        NX_HTTP_SERVER_PRIORITY,
        NX_HTTP_SERVER_THREAD_TIME_SLICE,
        TX_DONT_START);
    ...
    http_server_ptr->nx_http_server_socket.nx_tcp_socket_reserved_ptr =
        (void *)http_server_ptr;
...
UINT _nx_http_server_delete(NX_HTTP_SERVER *http_server_ptr) {
    ...
    nx_tcp_server_socket_unlisten(
        http_server_ptr->nx_http_server_ip_ptr, NX_HTTP_SERVER_PORT);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_param_get(
    NX_PACKET *packet_ptr, UINT param_number, CHAR *param_ptr, UINT max_param_size) {
    ...
    status =
        _nx_http_server_param_get(packet_ptr, param_number, param_ptr, max_param_size);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_param_get(
    NX_PACKET *packet_ptr, UINT param_number, CHAR *param_ptr, UINT max_param_size) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_query_get(
    NX_PACKET *packet_ptr, UINT query_number, CHAR *query_ptr, UINT max_query_size) {
    ...
    status =
        _nx_http_server_query_get(packet_ptr, query_number, query_ptr, max_query_size);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_query_get(
    NX_PACKET *packet_ptr, UINT query_number, CHAR *query_ptr, UINT max_query_size) {
...
UINT _nx_http_server_start(NX_HTTP_SERVER *http_server_ptr) {
    ...
    status = nx_tcp_server_socket_listen(
        http_server_ptr->nx_http_server_ip_ptr,
        NX_HTTP_SERVER_PORT,
        &(http_server_ptr->nx_http_server_socket),
...
UINT _nx_http_server_disconnect(NX_HTTP_SERVER *http_server_ptr, UINT wait_option) {
    ...
    for (i = 0; i < NX_HTTP_SERVER_NONCE_MAX; i++) {
        if (http_server_ptr->nx_http_server_nonces[i].nonce_state ==
            NX_HTTP_SERVER_NONCE_ACCEPTED) {
            http_server_ptr->nx_http_server_nonces[i].nonce_state =
                NX_HTTP_SERVER_NONCE_INVALID;
            break;
        }
    }
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_callback_data_send(
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_callback_data_send(
    NX_HTTP_SERVER *server_ptr, VOID *data_ptr, ULONG data_length) {
    ...
    status = nx_packet_allocate(
        server_ptr->nx_http_server_packet_pool_ptr,
        &new_packet_ptr,
        NX_TCP_PACKET,
        NX_WAIT_FOREVER);
...
UINT _nxe_http_server_callback_response_send(
    NX_HTTP_SERVER *server_ptr, CHAR *header, CHAR *information, CHAR *additional_info) {
    UINT status;

    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) || (header == NX_NULL))
        return (NX_PTR_ERROR);
    ...
    /* Call actual data send function.  */
    status = _nx_http_server_callback_response_send(
        server_ptr, header, information, additional_info);
...
UINT _nx_http_server_callback_response_send(
    ...
    if (_nx_utility_string_length_check(header, &header_length, NX_MAX_STRING_LENGTH) ||
        (information && _nx_utility_string_length_check(
                            information, &information_length, NX_MAX_STRING_LENGTH)) ||
        (additional_info &&
         _nx_utility_string_length_check(
             additional_info, &additional_info_length, NX_MAX_STRING_LENGTH))) {
        return (NX_HTTP_ERROR);
    }

    /* Call the internal HTTP response send function.  */
    status = _nx_http_server_response_send(
        server_ptr,
        header,
        header_length,
        information,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_callback_response_send_extended(
    NX_HTTP_SERVER *server_ptr,
    CHAR *header,
    UINT header_length,
    CHAR *information,
    ...
    /* Call actual data send function.  */
    status = _nx_http_server_callback_response_send_extended(
        server_ptr,
        header,
        header_length,
        information,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_callback_response_send_extended(
    NX_HTTP_SERVER *server_ptr,
    CHAR *header,
    UINT header_length,
    CHAR *information,
    ...
    if (_nx_utility_string_length_check(
            header, &temp_header_length, NX_MAX_STRING_LENGTH) ||
        (information &&
         _nx_utility_string_length_check(
             information, &temp_information_length, NX_MAX_STRING_LENGTH)) ||
        (additional_info &&
         _nx_utility_string_length_check(
             additional_info, &temp_additional_info_length, NX_MAX_STRING_LENGTH))) {
        return (NX_HTTP_ERROR);
    }
...
VOID _nx_http_server_thread_entry(ULONG http_server) {
    ...
    while (loop_endlessly) {
        ...
        tx_thread_preemption_change(
            &(server_ptr->nx_http_server_thread), 0, &original_threshold);
        ...
        /* Restore preemption.  */
        tx_thread_preemption_change(
            &(server_ptr->nx_http_server_thread),
            original_threshold,
            &original_threshold);

        /* Wait for connection on HTTP Server port.  */
        status = nx_tcp_server_socket_accept(
            &(server_ptr->nx_http_server_socket), NX_HTTP_SERVER_TIMEOUT_ACCEPT);
        ...
        if ((buffer_ptr[0] == 'G') && (buffer_ptr[1] == 'E') && (buffer_ptr[2] == 'T') &&
            (buffer_ptr[3] == ' ')) {
            ...
            _nx_http_server_get_process(
                server_ptr, NX_HTTP_SERVER_GET_REQUEST, packet_ptr);
        }

        /* Check for a PUT request.  */
        else if (
            (buffer_ptr[0] == 'P') && (buffer_ptr[1] == 'U') && (buffer_ptr[2] == 'T') &&
            (buffer_ptr[3] == ' ')) {
        ...
        /* Check for a DELETE request.  */
            ...
            (buffer_ptr[0] == 'D') && (buffer_ptr[1] == 'E') && (buffer_ptr[2] == 'L') &&
            (buffer_ptr[3] == 'E') && (buffer_ptr[4] == 'T') && (buffer_ptr[5] == 'E') &&
            (buffer_ptr[6] == ' ')) {
        ...
        /* Check for a POST request.  */
            ...
            (buffer_ptr[0] == 'P') && (buffer_ptr[1] == 'O') && (buffer_ptr[2] == 'S') &&
            (buffer_ptr[3] == 'T') && (buffer_ptr[4] == ' ')) {

            /* We have a HTTP POST request to send data to this HTTP Server for
               processing.  Note that the POST request is nearly identical to the GET
               request, except the parameter/query data is found in the content rather
               than as part of the URL (resource). */
            ...
            server_ptr->nx_http_server_multipart.nx_http_server_multipart_last_packet =
                NX_NULL;
#endif /* NX_HTTP_MULTIPART_ENABLE */

            /* Process the POST request.  */
            _nx_http_server_get_process(
                server_ptr, NX_HTTP_SERVER_POST_REQUEST, packet_ptr);

#ifdef NX_HTTP_MULTIPART_ENABLE
            /* Restore the packet to release. */
            if (server_ptr->nx_http_server_multipart.nx_http_server_multipart_last_packet)
                packet_ptr = server_ptr->nx_http_server_multipart
                                 .nx_http_server_multipart_last_packet;
#endif /* NX_HTTP_MULTIPART_ENABLE */
        }

        /* Check for a HEAD request.  */
        else if (
            (buffer_ptr[0] == 'H') && (buffer_ptr[1] == 'E') && (buffer_ptr[2] == 'A') &&
            (buffer_ptr[3] == 'D') && (buffer_ptr[4] == ' ')) {

            /* We have a HTTP HEAD request to get a resource header from this HTTP Server.
               Note that the HEAD request is nearly identical to the GET request, except
               the requested content is not returned to the client.  */
            ...
            _nx_http_server_get_process(
                server_ptr, NX_HTTP_SERVER_HEAD_REQUEST, packet_ptr);
        }
        ...
        /* Unhandled request.  */
            ...
            _nx_http_server_response_send(
                server_ptr,
                NX_HTTP_STATUS_NOT_IMPLEMENTED,
                sizeof(NX_HTTP_STATUS_NOT_IMPLEMENTED) - 1,
                "NetX HTTP Request Not Implemented",
                sizeof("NetX HTTP Request Not Implemented") - 1,
                NX_NULL,
                0);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_get_client_request(
    NX_HTTP_SERVER *server_ptr, NX_PACKET **packet_ptr) {
    ...
    *packet_ptr = NX_NULL;
    ...
    if ((buffer_ptr[0] != 'G') && (buffer_ptr[0] != 'g') && (buffer_ptr[0] != 'P') &&
        (buffer_ptr[0] != 'p') && (buffer_ptr[0] != 'D') && (buffer_ptr[0] != 'd') &&
        (buffer_ptr[0] != 'H') && (buffer_ptr[0] != 'h') && (buffer_ptr[0] != 'T') &&
        (buffer_ptr[0] != 't')) {
    ...
    if (((head_packet_ptr->nx_packet_data_end - head_packet_ptr->nx_packet_data_start) -
        ...
        /* Copy the packet into a packet allocated from the Server's packet pool (which
           must have packets with a payload size >= NX_HTTP_SERVER_MIN_PACKET_SIZE.  */
        status = nx_packet_copy(
            head_packet_ptr,
            &new_packet_ptr,
            server_ptr->nx_http_server_packet_pool_ptr,
        ...
        /* Copy is successful, release the original pointer and move the new pointer to
         * the head pointer.  */
        nx_packet_release(head_packet_ptr);
    ...
    do {
...
#endif
        {
            ...
            /* Successfully received another packet.  Its contents now need to be placed
             * in the head packet.  */
...
/*                                                                        */
/**************************************************************************/
VOID _nx_http_server_get_process(
    NX_HTTP_SERVER *server_ptr, UINT request_type, NX_PACKET *packet_ptr) {
    ...
    status = _nx_http_server_retrieve_resource(
        packet_ptr,
        server_ptr->nx_http_server_request_resource,
        NX_HTTP_MAX_RESOURCE + 1);
    ...
    if (status != NX_SUCCESS) {
        ...
        _nx_http_server_response_send(
            server_ptr,
            NX_HTTP_STATUS_BAD_REQUEST,
            sizeof(NX_HTTP_STATUS_BAD_REQUEST) - 1,
            "NetX HTTP URL Bad",
            sizeof("NetX HTTP URL Bad") - 1,
            NX_NULL,
            0);
    ...
    if (request_type == NX_HTTP_SERVER_POST_REQUEST) {
        ...
        if (status != NX_SUCCESS) {
            ...
            _nx_http_server_response_send(
                ...
                sizeof("NetX HTTP Invalid Content Length") - 1,
                NX_NULL,
                0);
        ...
        if (_nx_http_server_calculate_content_offset(packet_ptr) == 0) {
            ...
            _nx_http_server_response_send(
                ...
                sizeof("NetX HTTP Invalid Content Offset") - 1,
                NX_NULL,
                0);
        ...
        memset(
            &server_ptr->nx_http_server_multipart, 0, sizeof(NX_HTTP_SERVER_MULTIPART));
#endif /* NX_HTTP_MULTIPART_ENABLE */
    }

    /* Determine if the application has specified an authentication function for this
     * server.  */
    if (server_ptr->nx_http_server_authentication_check ||
        ...
        if (server_ptr->nx_http_server_authentication_check_extended) {
            status = (server_ptr->nx_http_server_authentication_check_extended)(
                server_ptr,
                request_type,
                server_ptr->nx_http_server_request_resource,
                &name_ptr,
            ...
            status = (server_ptr->nx_http_server_authentication_check)(
                server_ptr,
                request_type,
                server_ptr->nx_http_server_request_resource,
                &name_ptr,
        ...
        if (status == NX_HTTP_BASIC_AUTHENTICATE ||
            ...
            if (_nx_utility_string_length_check(
                    name_ptr, &temp_name_length, NX_HTTP_MAX_NAME) ||
                _nx_utility_string_length_check(
                    password_ptr, &temp_password_length, NX_HTTP_MAX_PASSWORD) ||
                _nx_utility_string_length_check(
                    realm_ptr, &temp_realm_length, NX_MAX_STRING_LENGTH)) {
        ...
        if (status == NX_HTTP_BASIC_AUTHENTICATE) {
            ...
            status = _nx_http_server_basic_authenticate(
                server_ptr,
                packet_ptr,
                name_ptr,
                password_ptr,
            ...
            status = _nx_http_server_digest_authenticate(
                server_ptr,
                packet_ptr,
                name_ptr,
                temp_name_length,
        ...
        if ((status != NX_HTTP_DONT_AUTHENTICATE) && (status != NX_SUCCESS)) {
            ...
            /*  Yes it did. Inform the HTTP server application about the failed
             * authentication. */
            if (server_ptr->nx_http_server_invalid_username_password_callback &&
                auth_request_present) {
                ...
                status = nxd_tcp_socket_peer_info_get(
                    &(server_ptr->nx_http_server_socket),
                    &client_nxd_address,
                    &client_port);
                ...
                if (status == NX_SUCCESS) {
                    ...
                    (server_ptr->nx_http_server_invalid_username_password_callback)(
                        server_ptr->nx_http_server_request_resource,
                        &client_nxd_address,
                        request_type);
                }
    ...
    if ((server_ptr->nx_http_server_cache_info_get) &&
        ...
        if (_nx_http_server_field_value_get(
                packet_ptr,
                (UCHAR *)"if-modified-since",
                17,
                (UCHAR *)temp_string,
                sizeof(temp_string)) == NX_SUCCESS) {
            UINT max_age;
            ...
            if (server_ptr->nx_http_server_cache_info_get(
                    server_ptr->nx_http_server_request_resource, &max_age, &date) ==
                NX_TRUE) {
                ...
                if (memcmp(temp_string, date_string, temp + 1) == 0) {
                    ...
                    _nx_http_server_response_send(
                        server_ptr,
                        NX_HTTP_STATUS_NOT_MODIFIED,
                        sizeof(NX_HTTP_STATUS_NOT_MODIFIED) - 1,
                        NX_NULL,
                        0,
                        NX_NULL,
                        0);
    ...
    /* At this point, either there isn't any required authentication for this resource or
       the authentication is complete.  */

    /* If the HTTP server receives an empty POST request (no message body, content length
       = 0, it is the responsibility of the request notify callback to send a response (if
       any) to the HTTP Client. The HTTP server will process the request no further. */
    ...
    if (server_ptr->nx_http_server_request_notify) {
        ...
        status = (server_ptr->nx_http_server_request_notify)(
            server_ptr,
            request_type,
            server_ptr->nx_http_server_request_resource,
            packet_ptr);
        ...
        if (server_ptr->nx_http_server_multipart.nx_http_server_multipart_next_packet)
            nx_packet_release(server_ptr->nx_http_server_multipart
                                  .nx_http_server_multipart_next_packet);
#endif /* NX_HTTP_MULTIPART_ENABLE */
        ...
        if (status != NX_SUCCESS) {
            ...
            /* Determine if the user callback routine successfully completed the request
             * processing.  */
            if (status == NX_HTTP_CALLBACK_COMPLETED) {

                /* User callback routine already sent success response back to HTTP
                 * Client.  */
                return;
            }
            ...
            _nx_http_server_response_send(
                ...
                sizeof(NX_HTTP_STATUS_INTERNAL_ERROR) - 1,
                "NetX HTTP Request Aborted",
                sizeof("NetX HTTP Request Aborted") - 1,
                NX_NULL,
                0);
    ...
    if ((request_type == NX_HTTP_SERVER_POST_REQUEST) && (length == 0)) {
        ...
        /* No. Regardless if a reply was sent via the request notify callback, we are done
         * with this packet. */
        return;
    }
    ...
    status = fx_file_open(
        server_ptr->nx_http_server_media_ptr,
        &(server_ptr->nx_http_server_file),
        server_ptr->nx_http_server_request_resource,
        FX_OPEN_FOR_READ);
    ...
    if (status != NX_SUCCESS) {
        ...
        _nx_http_server_response_send(
            server_ptr,
            NX_HTTP_STATUS_NOT_FOUND,
            sizeof(NX_HTTP_STATUS_NOT_FOUND) - 1,
    ...
    temp = (ULONG)_nx_http_server_type_get_extended(
        server_ptr,
        server_ptr->nx_http_server_request_resource,
        resource_length,
        temp_string,
    ...
    status = _nx_http_server_generate_response_header(
        server_ptr,
        &new_packet_ptr,
        NX_HTTP_STATUS_OK,
        sizeof(NX_HTTP_STATUS_OK) - 1,
        length,
        temp_string,
        temp,
        NX_NULL,
    ...
    /* Now, loop to send the data contents.  If the request type is "HEAD", skip this loop
    ...
    while ((length) && (request_type != NX_HTTP_SERVER_HEAD_REQUEST)) {
        ...
        temp = ((ULONG)(new_packet_ptr->nx_packet_data_end -
        ...
        new_packet_ptr->nx_packet_append_ptr =
            new_packet_ptr->nx_packet_append_ptr + temp;
        ...
        server_ptr->nx_http_server_total_bytes_sent =
            server_ptr->nx_http_server_total_bytes_sent + temp;
...
VOID _nx_http_server_put_process(NX_HTTP_SERVER *server_ptr, NX_PACKET *packet_ptr) {
    ...
    /* If the request is empty (content-length = 0) the request notify callback is where a
       response is generated to return to the Client.  NetX HTTP Server will not process
       an empty request further. */
    ...
    if (status != NX_SUCCESS) {
        ...
        _nx_http_server_response_send(
            ...
            sizeof("NetX HTTP File Create Failed") - 1,
            NX_NULL,
            0);
    ...
    if (status != NX_SUCCESS) {
        ...
        _nx_http_server_response_send(
            ...
            sizeof("NetX HTTP File Open Failed") - 1,
            NX_NULL,
            0);
    ...
    if ((UINT)(packet_ptr->nx_packet_append_ptr - packet_ptr->nx_packet_prepend_ptr) >
        ...
        status = fx_file_write(
            &(server_ptr->nx_http_server_file),
            (packet_ptr->nx_packet_prepend_ptr + offset),
            ((ULONG)(packet_ptr->nx_packet_append_ptr -
        ...
        if (status != NX_SUCCESS) {
            ...
            _nx_http_server_response_send(
                ...
                sizeof("NetX HTTP File Write Failed") - 1,
                NX_NULL,
                0);
    ...
    while ((length) && (next_packet_ptr)) {
        ...
        /* Update the length.  */
        consumed_length = (ULONG)(next_packet_ptr->nx_packet_append_ptr -
                                  next_packet_ptr->nx_packet_prepend_ptr);
        if ((length - consumed_length) > length) {
            /* Underflow error has occurred. */
    ...
    /* If necessary, receive more packets from the TCP socket to complete the write
     * request.  */
    while (length) {
        ...
        if (status != NX_SUCCESS) {
            ...
            _nx_http_server_response_send(
                ...
                sizeof("NetX HTTP Receive Timeout") - 1,
                NX_NULL,
                0);
...
VOID _nx_http_server_delete_process(NX_HTTP_SERVER *server_ptr, NX_PACKET *packet_ptr) {
    ...
    if (server_ptr->nx_http_server_request_notify) {
        ...
        /* Determine if the user supplied routine is requesting the delete should be
         * aborted.  */
        if (status != NX_SUCCESS) {
    ...
    if (status != NX_SUCCESS) {
        ...
        _nx_http_server_response_send(
            ...
            sizeof("NetX HTTP File Delete Failed") - 1,
            NX_NULL,
            0);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_invalid_userpassword_notify_set(
    NX_HTTP_SERVER *http_server_ptr,
    UINT (*invalid_username_password_callback)(
        CHAR *resource, NXD_ADDRESS *client_nxd_address, UINT request_type)) {
    ...
    status = _nx_http_server_invalid_userpassword_notify_set(
        http_server_ptr, invalid_username_password_callback);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_invalid_userpassword_notify_set(
    NX_HTTP_SERVER *http_server_ptr,
    UINT (*invalid_username_password_callback)(
        CHAR *resource, NXD_ADDRESS *client_nxd_address, UINT request_type)) {

    http_server_ptr->nx_http_server_invalid_username_password_callback =
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_response_send(
    NX_HTTP_SERVER *server_ptr,
    CHAR *status_code,
    ...
    /* Send the response back to the client.  */
    status = nx_tcp_socket_send(
        &(server_ptr->nx_http_server_socket), packet_ptr, NX_HTTP_SERVER_TIMEOUT_SEND);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_basic_authenticate(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET *packet_ptr,
    CHAR *name_ptr,
    CHAR *password_ptr,
    ...
    *auth_request_present = NX_FALSE;
    ...
    length =
        _nx_http_server_retrieve_basic_authorization(packet_ptr, authorization_request);
    if (length) {
        ...
        *auth_request_present = NX_TRUE;
        ...
        _nx_utility_base64_decode(
            (UCHAR *)authorization_request,
            length,
            (UCHAR *)authorization_decoded,
            sizeof(authorization_decoded),
    ...
    if (status == NX_HTTP_BASIC_AUTHENTICATE) {
        ...
        /* We need authorization so build the HTTP 401 Unauthorized message to send to the
         * server.  */

        /* Allocate a packet for sending the response back.  */
        status1 = nx_packet_allocate(
            server_ptr->nx_http_server_packet_pool_ptr,
            &packet_ptr,
            NX_TCP_PACKET,
            NX_WAIT_FOREVER);
        ...
        nx_packet_data_append(
            ...
            sizeof(NX_HTTP_STATUS_UNAUTHORIZED) - 1,
            server_ptr->nx_http_server_packet_pool_ptr,
            NX_WAIT_FOREVER);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_retrieve_basic_authorization(
    NX_PACKET *packet_ptr, CHAR *authorization_request_ptr) {
    ...
    while (((buffer_ptr + 15) < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0)) {
    ...
    while (((buffer_ptr + 6) < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0)) {
    ...
    while ((buffer_ptr < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0) && (*buffer_ptr != ' ') &&
           (*buffer_ptr != (CHAR)13) && (length < NX_HTTP_MAX_STRING)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_retrieve_resource(
    NX_PACKET *packet_ptr, CHAR *destination, UINT max_size) {
    ...
    if ((buffer_ptr + 7 < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
        ...
        if (((*buffer_ptr == 'h') || (*buffer_ptr == 'H')) &&
            ...
            /* Find the first slash character.  The first slash marks the beginning of the
             * URL.  */
            while ((buffer_ptr < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
                   (*buffer_ptr != (CHAR)0) && (*buffer_ptr != '/')) {
    ...
    /* Copy the rest of the resource to the destination.  Space, semicolon, and question
       mark characters signal the end of the resource.  */
    i = 0;
    while ((buffer_ptr < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
    ...
    if ((destination[0] == (CHAR)0) ||
        (buffer_ptr >= (CHAR *)packet_ptr->nx_packet_append_ptr) ||
        ((*buffer_ptr != ' ') && (*buffer_ptr != '?') && (*buffer_ptr != ';'))) {
...
UINT _nx_http_server_calculate_content_offset(NX_PACKET *packet_ptr) {
    ...
    while (((buffer_ptr + 3) < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0)) {

        /* Check for the <cr,lf,cr,lf> token.  This signals a blank line, which also
           specifies the start of the content.  */
        if ((*buffer_ptr == (CHAR)13) && (*(buffer_ptr + 1) == (CHAR)10) &&
            (*(buffer_ptr + 2) == (CHAR)13) && (*(buffer_ptr + 3) == (CHAR)10)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_type_get(
    NX_HTTP_SERVER *server_ptr, CHAR *name, CHAR *http_type_string) {

    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) || (name == NX_NULL) ||
        (http_type_string == NX_NULL))
        return (0);
...
UINT _nx_http_server_type_get(
    ...
    return _nx_http_server_type_get_extended(
        server_ptr, name, name_length, http_type_string, NX_MAX_STRING_LENGTH + 1);
}
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_type_get_extended(
    NX_HTTP_SERVER *server_ptr,
    CHAR *name,
    UINT name_length,
    ...
    return _nx_http_server_type_get_extended(
        server_ptr, name, name_length, http_type_string, http_type_string_max_size);
}
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_type_get_extended(
    NX_HTTP_SERVER *server_ptr,
    CHAR *name,
    UINT name_length,
    CHAR *http_type_string,
    ...
    if (*ch != '.') {
        ...
        memcpy(
            http_type_string,
            NX_HTTP_SERVER_DEFAULT_MIME,
            sizeof(NX_HTTP_SERVER_DEFAULT_MIME)); /* Use case of memcpy is verified. */
    ...
    if (server_ptr->nx_http_server_mime_maps_additional &&
        ...
        for (i = 0; i < server_ptr->nx_http_server_mime_maps_additional_num; i++) {
            ...
            if (_nx_utility_string_length_check(
                    server_ptr->nx_http_server_mime_maps_additional[i]
                        .nx_http_server_mime_map_extension,
                    &map_ext_length,
                    ext_length) ||
                _nx_utility_string_length_check(
                    server_ptr->nx_http_server_mime_maps_additional[i]
                        .nx_http_server_mime_map_type,
                    &map_type_length,
                    http_type_string_max_size - 1)) {
                continue;
            }

            if (_nx_http_server_memicmp(
                    (UCHAR *)ch,
                    ext_length,
                    (UCHAR *)server_ptr->nx_http_server_mime_maps_additional[i]
                        .nx_http_server_mime_map_extension,
                ...
                memcpy(
                    ...
                    server_ptr->nx_http_server_mime_maps_additional[i]
                        .nx_http_server_mime_map_type,
                    map_type_length + 1); /* Use case of memcpy is verified. */
    ...
    for (i = 0; i < sizeof(_nx_http_server_mime_maps) / sizeof(NX_HTTP_SERVER_MIME_MAP);
         i++) {

        /* Check map extension and type length.  */
        if (_nx_utility_string_length_check(
                _nx_http_server_mime_maps[i].nx_http_server_mime_map_extension,
                &map_ext_length,
                ext_length) ||
            _nx_utility_string_length_check(
                _nx_http_server_mime_maps[i].nx_http_server_mime_map_type,
                &map_type_length,
                http_type_string_max_size - 1)) {
            continue;
        }
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_nonce_allocate(
    NX_HTTP_SERVER *server_ptr, NX_HTTP_SERVER_NONCE **nonce_ptr) {
    UINT i;
    UCHAR random_value;
    ...
    if (i == NX_HTTP_SERVER_NONCE_MAX) {
        ...
        for (i = 0; i < NX_HTTP_SERVER_NONCE_MAX; i++) {
            if (nonces_list[i].nonce_state == NX_HTTP_SERVER_NONCE_VALID) {
                if (tx_time_get() >
                    nonces_list[i].nonce_timestamp + NX_HTTP_SERVER_NONCE_TIMEOUT) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_digest_authenticate(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET *packet_ptr,
    CHAR *name_ptr,
    UINT name_length,
    ...
    *auth_request_present = NX_FALSE;
    ...
    /* Is the authorization request present?  */
    if (_nx_http_server_retrieve_digest_authorization(
            server_ptr,
            packet_ptr,
        ...
        *auth_request_present = NX_TRUE;
        ...
        buffer_ptr = (CHAR *)packet_ptr->nx_packet_prepend_ptr;
        i = 0;
        while (((buffer_ptr + i) < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
               (buffer_ptr[i] != ' ') && (i < (sizeof(method) - 1))) {
        ...
        /* If the digest authenticate callback function is set, invoke the callback
         * function. */
        if (server_ptr->nx_http_server_digest_authenticate_callback) {
            callback_status = (server_ptr->nx_http_server_digest_authenticate_callback)(
                server_ptr,
                name_ptr,
                realm_ptr,
                password_ptr,
            ...
            /* If the digest authenticate callback is not set, assume it is success, for
               backward compatibility reasons. */
            callback_status = NX_SUCCESS;
        }

        /* Calculate what the MD5 should be.  */
        _nx_http_server_digest_response_calculate(
            server_ptr,
            name_ptr,
            name_length,
            realm_ptr,
        ...
        /* If digest authenticate callback function returns non-success value, the request
    ...
    if (status == NX_HTTP_DIGEST_AUTHENTICATE) {
        ...
        if (status1) {
            ...
            _nx_http_server_response_send(
                ...
                NX_HTTP_STATUS_INTERNAL_ERROR,
                sizeof(NX_HTTP_STATUS_INTERNAL_ERROR) - 1,
                "NetX HTTP Server Internal Error",
                sizeof("NetX HTTP Server Internal Error") - 1,
                NX_NULL,
                0);
        ...
        nx_packet_data_append(
            packet_ptr,
            nonce_ptr->nonce_buffer,
            NX_HTTP_SERVER_NONCE_SIZE,
            server_ptr->nx_http_server_packet_pool_ptr,
            NX_WAIT_FOREVER);
...
/*                                                                        */
/**************************************************************************/
VOID _nx_http_server_digest_response_calculate(
    NX_HTTP_SERVER *server_ptr,
    CHAR *username,
    ...
    _nx_md5_update(
        &(server_ptr->nx_http_server_md5data),
        (unsigned char *)username,
        username_length);
    _nx_md5_update(&(server_ptr->nx_http_server_md5data), (unsigned char *)":", 1);
    _nx_md5_update(
        &(server_ptr->nx_http_server_md5data), (unsigned char *)realm, realm_length);
    _nx_md5_update(&(server_ptr->nx_http_server_md5data), (unsigned char *)":", 1);
    _nx_md5_update(
        &(server_ptr->nx_http_server_md5data),
    ...
    _nx_md5_digest_calculate(
        &(server_ptr->nx_http_server_md5data), (unsigned char *)md5_binary);
    ...
    _nx_md5_initialize(&(server_ptr->nx_http_server_md5data));
    _nx_md5_update(
        &(server_ptr->nx_http_server_md5data), (unsigned char *)method, method_length);
    _nx_md5_update(&(server_ptr->nx_http_server_md5data), (unsigned char *)":", 1);
    _nx_md5_update(
        &(server_ptr->nx_http_server_md5data), (unsigned char *)uri, uri_length);
    _nx_md5_digest_calculate(
        &(server_ptr->nx_http_server_md5data), (unsigned char *)md5_binary);
    ...
    _nx_md5_update(&(server_ptr->nx_http_server_md5data), (unsigned char *)":", 1);
    _nx_md5_update(
        &(server_ptr->nx_http_server_md5data), (unsigned char *)cnonce, cnonce_length);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_retrieve_digest_authorization(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET *packet_ptr,
    CHAR *response,
    CHAR *uri,
    CHAR *nc,
    CHAR *cnonce,
    NX_HTTP_SERVER_NONCE **nonce_ptr) {
    ...
    while (((buffer_ptr + 7) < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0)) {
    ...
    while ((buffer_ptr < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           ((*buffer_ptr == ' ') || (*buffer_ptr == (CHAR)0x22))) {
    ...
    for (i = 0; i < NX_HTTP_SERVER_NONCE_MAX; i++) {
        if ((server_ptr->nx_http_server_nonces[i].nonce_state !=
             NX_HTTP_SERVER_NONCE_INVALID) &&
            (memcmp(
                 server_ptr->nx_http_server_nonces[i].nonce_buffer,
                 nonce_buffer,
                 NX_HTTP_SERVER_NONCE_SIZE) == 0)) /* Use case of memcmp is verified. */
        {
    ...
    while ((buffer_ptr < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0) && (*buffer_ptr != ' ') &&
           (*buffer_ptr != (CHAR)13) && (length < NX_HTTP_MAX_RESOURCE)) {
    ...
    while (((buffer_ptr + 4) < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0)) {
    ...
    while (((buffer_ptr + 9) < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0)) {
    ...
    while ((buffer_ptr < (CHAR *)packet_ptr->nx_packet_append_ptr) &&
           (*buffer_ptr != (CHAR)0) && (*buffer_ptr != ' ') &&
           (*buffer_ptr != (CHAR)13) && (*buffer_ptr != (CHAR)0x3A) &&
           (length < NX_HTTP_MAX_ASCII_MD5)) {
    ...
    /* Check for an error.  */
    if ((length != 32) || (buffer_ptr >= (CHAR *)packet_ptr->nx_packet_append_ptr) ||
        (*buffer_ptr != (CHAR)0x22)) {
...
/*                                                                        */
/**************************************************************************/
VOID _nx_http_server_hex_ascii_convert(
    CHAR *source, UINT source_length, CHAR *destination) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_get_entity_header(
    NX_HTTP_SERVER *server_ptr,
    ...
    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) || (packet_pptr == NX_NULL))
        return NX_PTR_ERROR;

    /* Check for appropriate caller.  */
    NX_THREADS_ONLY_CALLER_CHECKING

    return _nx_http_server_get_entity_header(
        server_ptr, packet_pptr, entity_header_buffer, buffer_size);
#else
    NX_PARAMETER_NOT_USED(server_ptr);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_get_entity_header(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET **packet_pptr,
    UCHAR *entity_header_buffer,
    ULONG buffer_size) {
    ...
    if (multipart_ptr->nx_http_server_multipart_boundary[0] == 0) {
        ...
        /* Get content type. */
        status = _nx_http_server_field_value_get(
            *packet_pptr,
            (UCHAR *)"content-type",
            12,
            multipart_ptr->nx_http_server_multipart_boundary,
            NX_HTTP_MAX_HEADER_FIELD + 1);
        if (status)
            return status;
        ...
        if (_nx_utility_string_length_check(
                (CHAR *)multipart_ptr->nx_http_server_multipart_boundary,
                (UINT *)&field_length,
        ...
        if (multipart_ptr->nx_http_server_multipart_boundary[index] == '"') {
            ...
            index++;
            for (quotation_index = field_length; quotation_index > index;
                 quotation_index--) {
            ...
            memmove(
                &multipart_ptr->nx_http_server_multipart_boundary[4],
                &multipart_ptr->nx_http_server_multipart_boundary[index],
            ...
            index -= 4;
            do {
                multipart_ptr->nx_http_server_multipart_boundary[i] =
                    multipart_ptr->nx_http_server_multipart_boundary[index + i];
                i++;
    ...
    if (offset >= available_packet->nx_packet_length) {
        ...
        if (nx_tcp_socket_receive(
                &(server_ptr->nx_http_server_socket),
                &available_packet,
                NX_HTTP_SERVER_TIMEOUT_RECEIVE) != NX_SUCCESS) {
        ...
        *packet_pptr = available_packet;
        ...
        while ((UINT)(available_packet->nx_packet_append_ptr -
                      available_packet->nx_packet_prepend_ptr) <= offset) {
            offset -= (UINT)(available_packet->nx_packet_append_ptr -
                             available_packet->nx_packet_prepend_ptr);
    ...
    while ((entity_header_buffer == NX_NULL) || (index < buffer_size)) {
        ...
        if ((ch + 1) >= available_packet->nx_packet_append_ptr) {
...
#endif /* NX_DISABLE_PACKET_CHAIN */
            {
                ...
                /* Store the packet. */
                *packet_pptr = available_packet;
                server_ptr->nx_http_server_multipart
                    .nx_http_server_multipart_last_packet = available_packet;
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_get_entity_content(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET **packet_pptr,
    ...
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) ||
        (packet_pptr == NX_NULL) || (available_offset == NX_NULL) ||
        (available_length == NX_NULL))
        return (NX_PTR_ERROR);
    ...
    return _nx_http_server_get_entity_content(
        server_ptr, packet_pptr, available_offset, available_length);
#else
    NX_PARAMETER_NOT_USED(server_ptr);
    NX_PARAMETER_NOT_USED(packet_pptr);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_get_entity_content(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET **packet_pptr,
    ULONG *available_offset,
    ULONG *available_length) {
    ...
    /* Whether the boundary is found? */
    if (server_ptr->nx_http_server_multipart.nx_http_server_multipart_boundary_find ==
        NX_TRUE)
    ...
    *available_offset =
        server_ptr->nx_http_server_multipart.nx_http_server_multipart_available_offset;
    *available_length =
        server_ptr->nx_http_server_multipart.nx_http_server_multipart_available_length;
...
UINT _nx_http_server_boundary_find(NX_HTTP_SERVER *server_ptr, NX_PACKET **packet_pptr) {
    ...
    if (multipart_ptr->nx_http_server_multipart_boundary_find == NX_TRUE) {
        ...
        if (multipart_ptr->nx_http_server_multipart_next_packet != NX_NULL) {
            ...
            *packet_pptr = multipart_ptr->nx_http_server_multipart_next_packet;
            server_ptr->nx_http_server_multipart.nx_http_server_multipart_last_packet =
                multipart_ptr->nx_http_server_multipart_next_packet;
        }

        multipart_ptr->nx_http_server_multipart_available_offset =
            multipart_ptr->nx_http_server_multipart_next_available_offset;
    ...
    while (_nx_http_server_match_string(
               available_packet->nx_packet_prepend_ptr + offset,
               available_packet->nx_packet_append_ptr,
               multipart_ptr->nx_http_server_multipart_boundary,
               boundary_length,
               &match_count,
               &match_end_ptr) != NX_SUCCESS) {

        /* Match string in the next packet. */
#ifndef NX_DISABLE_PACKET_CHAIN
        if (available_packet->nx_packet_next) {
            multipart_ptr->nx_http_server_multipart_next_available_offset +=
                (UINT)((UINT)(available_packet->nx_packet_append_ptr -
                              available_packet->nx_packet_prepend_ptr) -
                       offset);
            available_packet = available_packet->nx_packet_next;
            offset = 0;
    ...
    if (match_end_ptr) {
        ...
        /* Yes. Found a match. */
        multipart_ptr->nx_http_server_multipart_next_available_offset +=
            (UINT)((UINT)(match_end_ptr - available_packet->nx_packet_prepend_ptr) -
                   offset);
        multipart_ptr->nx_http_server_multipart_available_length =
            multipart_ptr->nx_http_server_multipart_next_available_offset -
            multipart_ptr->nx_http_server_multipart_available_offset - match_count;
        ...
        if (_nx_http_server_match_string(
                available_packet->nx_packet_prepend_ptr,
                available_packet->nx_packet_append_ptr,
                multipart_ptr->nx_http_server_multipart_boundary,
                boundary_length,
            ...
            /* Yes. Find a match. */
            if ((ULONG)(match_end_ptr - available_packet->nx_packet_prepend_ptr) <
                match_count) {

                /* The boundary is between two packets. */
                multipart_ptr->nx_http_server_multipart_next_available_offset =
                    (UINT)(match_end_ptr - available_packet->nx_packet_prepend_ptr);
                multipart_ptr->nx_http_server_multipart_available_length -=
                    boundary_length -
                    multipart_ptr->nx_http_server_multipart_next_available_offset;
                multipart_ptr->nx_http_server_multipart_boundary_find = NX_TRUE;
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_match_string(
    UCHAR *src_start,
    UCHAR *src_end,
    UCHAR *target,
    ULONG target_length,
    ...
    *match_count = target_length;
    ...
    /* Perform part match. */
    for (*match_count = target_length - 1; *match_count > 0;
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_field_value_get(
    NX_PACKET *packet_ptr,
    UCHAR *field_name,
    ULONG name_length,
    UCHAR *field_value,
    ...
    while (ch + name_length < packet_ptr->nx_packet_append_ptr) {
        if (_nx_http_server_memicmp(ch, name_length, field_name, name_length) ==
            NX_SUCCESS) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_memicmp(
    UCHAR *src, ULONG src_length, UCHAR *dest, ULONG dest_length) {
    UCHAR ch;
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_generate_response_header(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET **packet_pptr,
    CHAR *status_code,
    UINT status_code_length,
    ...
    if (memcmp(
            status_code,
            NX_HTTP_STATUS_NOT_MODIFIED,
            sizeof(NX_HTTP_STATUS_NOT_MODIFIED)) == 0)
    ...
    /* Insert space. */
    status += nx_packet_data_append(
        packet_ptr, " ", 1, server_ptr->nx_http_server_packet_pool_ptr, NX_WAIT_FOREVER);
    ...
    /* Place the <cr,lf> into the buffer.  */
    status += nx_packet_data_append(
        packet_ptr, crlf, 2, server_ptr->nx_http_server_packet_pool_ptr, NX_WAIT_FOREVER);
    ...
    if (content_length || (status_code_not_modified == NX_TRUE)) {
        ...
        if (content_length) {
            ...
            temp = _nx_utility_uint_to_string(
                content_length, 10, temp_string, sizeof(temp_string));
    ...
    if (server_ptr->nx_http_server_gmt_get) {
        ...
        if ((server_ptr->nx_http_server_cache_info_get) &&
            ...
            if (server_ptr->nx_http_server_cache_info_get(
                ...
                temp = _nx_utility_uint_to_string(
                    max_age, 10, temp_string, sizeof(temp_string));
                status += nx_packet_data_append(
                    packet_ptr,
                    temp_string,
                    temp,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_callback_generate_response_header(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET **packet_pptr,
    CHAR *status_code,
    UINT content_length,
    ...
    return _nx_http_server_callback_generate_response_header(
        server_ptr,
        packet_pptr,
        status_code,
        content_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_callback_generate_response_header_extended(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET **packet_pptr,
    CHAR *status_code,
    UINT status_code_length,
    ...
    return _nx_http_server_callback_generate_response_header_extended(
        server_ptr,
        packet_pptr,
        status_code,
        status_code_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_callback_generate_response_header(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET **packet_pptr,
    CHAR *status_code,
    UINT content_length,
    ...
    if (_nx_utility_string_length_check(
            status_code, &status_code_length, NX_MAX_STRING_LENGTH) ||
        (content_type && _nx_utility_string_length_check(
                             content_type, &content_type_length, NX_MAX_STRING_LENGTH)) ||
        (additional_header &&
         _nx_utility_string_length_check(
             additional_header, &additional_header_length, NX_MAX_STRING_LENGTH))) {
        return (NX_HTTP_ERROR);
    }

    /* Call the internal HTTP response send function.  */
    return _nx_http_server_generate_response_header(
        server_ptr,
        packet_pptr,
        status_code,
        status_code_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_callback_generate_response_header_extended(
    NX_HTTP_SERVER *server_ptr,
    NX_PACKET **packet_pptr,
    CHAR *status_code,
    UINT status_code_length,
    ...
    /* Check status code, content type and additional header length.  */
    if (_nx_utility_string_length_check(
            status_code, &temp_status_code_length, status_code_length) ||
        (content_type &&
         _nx_utility_string_length_check(
             content_type, &temp_content_type_length, content_type_length)) ||
        (additional_header &&
         _nx_utility_string_length_check(
             additional_header, &temp_add_header_length, additional_header_length))) {
        return (NX_HTTP_ERROR);
    }
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_callback_packet_send(
    NX_HTTP_SERVER *server_ptr, NX_PACKET *packet_ptr) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_gmt_callback_set(
    NX_HTTP_SERVER *server_ptr, VOID (*gmt_get)(NX_HTTP_SERVER_DATE *)) {

    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) || (gmt_get == NX_NULL))
        return (NX_PTR_ERROR);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_cache_info_callback_set(
    NX_HTTP_SERVER *server_ptr,
    UINT (*cache_info_get)(CHAR *, UINT *, NX_HTTP_SERVER_DATE *)) {

    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) ||
        (cache_info_get == NX_NULL))
        return (NX_PTR_ERROR);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_mime_maps_additional_set(
    NX_HTTP_SERVER *server_ptr, NX_HTTP_SERVER_MIME_MAP *mime_maps, UINT mime_maps_num) {

    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) ||
        (server_ptr->nx_http_server_id != NXD_HTTP_SERVER_ID) || (mime_maps == NX_NULL))
        return (NX_PTR_ERROR);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_digest_authenticate_notify_set(
    NX_HTTP_SERVER *http_server_ptr,
    ...
    return (_nx_http_server_digest_authenticate_notify_set(
        http_server_ptr, digest_authenticate_callback));
}
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_digest_authenticate_notify_set(
    NX_HTTP_SERVER *http_server_ptr,
    UINT (*digest_authenticate_callback)(
        NX_HTTP_SERVER *server_ptr,
        CHAR *name_ptr,
...
#ifdef NX_HTTP_DIGEST_ENABLE
    http_server_ptr->nx_http_server_digest_authenticate_callback =
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_server_authentication_check_set(
    NX_HTTP_SERVER *http_server_ptr,
    ...
    status = _nx_http_server_authentication_check_set(
        http_server_ptr, authentication_check_extended);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_server_authentication_check_set(
    NX_HTTP_SERVER *http_server_ptr,
    UINT (*authentication_check_extended)(
        NX_HTTP_SERVER *server_ptr,
        UINT request_type,
    ...
    http_server_ptr->nx_http_server_authentication_check_extended =
