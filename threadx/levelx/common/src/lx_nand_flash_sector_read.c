    ...
    if (block != LX_NAND_BLOCK_UNMAPPED) {
        ...
        if (block_status & LX_NAND_BLOCK_STATUS_NON_SEQUENTIAL) {
            ...
            for (page = (LONG)available_pages - 1; page >= 0; page--) {
                ...
                status = (nand_flash->lx_nand_flash_driver_pages_read)(nand_flash, block, (ULONG)page, (UCHAR *)buffer,
                                                                       spare_buffer_ptr, 1);
#else
                status = (nand_flash->lx_nand_flash_driver_pages_read)(block, (ULONG)page, (UCHAR *)buffer,
                                                                       spare_buffer_ptr, 1);
#endif
            ...
            if (logical_sector % nand_flash->lx_nand_flash_pages_per_block < available_pages) {
                ...
                status = (nand_flash->lx_nand_flash_driver_pages_read)(
                    nand_flash, block, logical_sector % nand_flash->lx_nand_flash_pages_per_block, (UCHAR *)buffer,
                    spare_buffer_ptr, 1);
#else
                status = (nand_flash->lx_nand_flash_driver_pages_read)(
                    block, logical_sector % nand_flash->lx_nand_flash_pages_per_block, (UCHAR *)buffer,
                    spare_buffer_ptr, 1);
#endif
