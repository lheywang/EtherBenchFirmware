...
UINT _lx_nand_flash_data_page_copy(LX_NAND_FLASH *nand_flash, ULONG logical_sector, ULONG source_block,
    ...
    available_pages = (src_block_status & LX_NAND_BLOCK_STATUS_FULL)
    ...
    if (src_block_status & LX_NAND_BLOCK_STATUS_NON_SEQUENTIAL) {
        ...
        for (i = 0; i < sectors; i++) {
            ...
            for (source_page = (LONG)(available_pages - 1); source_page >= 0; source_page--) {
                ...
                status = (nand_flash->lx_nand_flash_driver_pages_read)(nand_flash, source_block, (ULONG)source_page,
                                                                       LX_NULL, spare_buffer_ptr, 1);
#else
                status = (nand_flash->lx_nand_flash_driver_pages_read)(source_block, (ULONG)source_page, LX_NULL,
                                                                       spare_buffer_ptr, 1);
#endif
                ...
                if ((spare_data1 & LX_NAND_PAGE_TYPE_USER_DATA_MASK) == (logical_sector + i)) {
                    ...
                    if ((spare_data1 & (~LX_NAND_PAGE_TYPE_USER_DATA_MASK)) == (LX_NAND_PAGE_TYPE_USER_DATA)) {
                        ...
                        status = (nand_flash->lx_nand_flash_driver_pages_copy)(
                            nand_flash, source_block, (ULONG)source_page, destination_block, destination_page, 1,
                            nand_flash->lx_nand_flash_page_buffer);
#else
                        status = (nand_flash->lx_nand_flash_driver_pages_copy)(source_block, (ULONG)source_page,
                                                                               destination_block, destination_page, 1,
                                                                               nand_flash->lx_nand_flash_page_buffer);
#endif
        ...
        number_of_pages = ((ULONG)source_page + sectors) > available_pages
                              ? (available_pages > (ULONG)source_page ? available_pages - (ULONG)source_page : 0)
                              : sectors;
...
#ifdef LX_NAND_ENABLE_CONTROL_BLOCK_FOR_DRIVER_INTERFACE
            status = (nand_flash->lx_nand_flash_driver_pages_copy)(nand_flash, source_block, (ULONG)source_page,
                                                                   destination_block, destination_page, number_of_pages,
                                                                   nand_flash->lx_nand_flash_page_buffer);
#else
            status = (nand_flash->lx_nand_flash_driver_pages_copy)(source_block, (ULONG)source_page, destination_block,
