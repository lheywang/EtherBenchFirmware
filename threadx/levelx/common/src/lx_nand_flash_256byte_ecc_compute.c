...
UINT _lx_nand_flash_256byte_ecc_compute(UCHAR *page_buffer, UCHAR *ecc_buffer) {
    ...
    /* Pack bit 21.  */
    ecc_buffer[(21 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(21 + 2) / 8] | ((odd_byte_parity >> 6) & 1) << (21 + 2) % 8) & 0xFF);

    /* Pack bit 20.  */
    ecc_buffer[(20 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(20 + 2) / 8] | ((even_byte_parity >> 6) & 1) << (20 + 2) % 8) & 0xFF);

    /* Pack bit 19.  */
    ecc_buffer[(19 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(19 + 2) / 8] | ((odd_byte_parity >> 5) & 1) << (19 + 2) % 8) & 0xFF);

    /* Pack bit 18.  */
    ecc_buffer[(18 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(18 + 2) / 8] | ((even_byte_parity >> 5) & 1) << (18 + 2) % 8) & 0xFF);

    /* Pack bit 17.  */
    ecc_buffer[(17 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(17 + 2) / 8] | ((odd_byte_parity >> 4) & 1) << (17 + 2) % 8) & 0xFF);

    /* Pack bit 16.  */
    ecc_buffer[(16 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(16 + 2) / 8] | ((even_byte_parity >> 4) & 1) << (16 + 2) % 8) & 0xFF);

    /* Pack bit 15.  */
    ecc_buffer[(15 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(15 + 2) / 8] | ((odd_byte_parity >> 3) & 1) << (15 + 2) % 8) & 0xFF);

    /* Pack bit 14.  */
    ecc_buffer[(14 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(14 + 2) / 8] | ((even_byte_parity >> 3) & 1) << (14 + 2) % 8) & 0xFF);

    /* Pack bit 13.  */
    ecc_buffer[(13 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(13 + 2) / 8] | ((odd_byte_parity >> 2) & 1) << (13 + 2) % 8) & 0xFF);

    /* Pack bit 12.  */
    ecc_buffer[(12 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(12 + 2) / 8] | ((even_byte_parity >> 2) & 1) << (12 + 2) % 8) & 0xFF);

    /* Pack bit 11.  */
    ecc_buffer[(11 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(11 + 2) / 8] | ((odd_byte_parity >> 1) & 1) << (11 + 2) % 8) & 0xFF);

    /* Pack bit 10.  */
    ecc_buffer[(10 + 2) / 8] =
        ((UCHAR)(ecc_buffer[(10 + 2) / 8] | ((even_byte_parity >> 1) & 1) << (10 + 2) % 8) & 0xFF);
