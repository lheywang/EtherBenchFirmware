...
UINT _lx_nor_flash_sector_release(LX_NOR_FLASH *nor_flash, ULONG logical_sector) {
    ...
    if (mapping_address) {
         ...
         */
        mapping_entry =
            mapping_entry & ~(((ULONG)LX_NOR_PHYSICAL_SECTOR_VALID) | ((ULONG)LX_NOR_PHYSICAL_SECTOR_SUPERCEDED));
        ...
        block =
            (ULONG)(mapping_address - nor_flash->lx_nor_flash_base_address) / nor_flash->lx_nor_flash_words_per_block;
