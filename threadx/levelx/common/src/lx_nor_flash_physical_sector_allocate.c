...
UINT _lx_nor_flash_physical_sector_allocate(LX_NOR_FLASH *nor_flash, ULONG logical_sector,
    ...
    *physical_sector_address = (ULONG *)0;
    ...
    for (i = 0; i < nor_flash->lx_nor_flash_total_blocks; i++) {
        ...
        block_word_ptr =
            nor_flash->lx_nor_flash_base_address + (search_block * nor_flash->lx_nor_flash_words_per_block);
        ...
        for (j = 0; j < nor_flash->lx_nor_flash_block_bit_map_words; j++) {
            ...
            status = _lx_nor_flash_driver_read(
                nor_flash, (block_word_ptr + nor_flash->lx_nor_flash_block_free_bit_map_offset + j), &block_word, 1);
                ...
                for (k = 0; k < 32; k++) {
                    ...
                    if (block_word & 1) {
                        ...
                        if (((block_word >> 1) == 0) && (j == (nor_flash->lx_nor_flash_block_bit_map_words - 1))) {
                             ...
                             */
                            status = _lx_nor_flash_driver_write(nor_flash,
                        ...
                        *physical_sector_map_entry =
                            block_word_ptr + (nor_flash->lx_nor_flash_block_physical_sector_mapping_offset + (j * 32)) +
                            k;
