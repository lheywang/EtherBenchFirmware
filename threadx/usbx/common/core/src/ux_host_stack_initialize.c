...
UX_COMPILE_TIME_ASSERT(!UX_OVERFLOW_CHECK_MULC_ULONG(sizeof(UX_HOST_CLASS), UX_MAX_CLASS_DRIVER),
...
UINT _ux_system_host_hcd_periodic_tree_entries[32] = {0x00, 0x10, 0x08, 0x18, 0x04, 0x14, 0x0c, 0x1c, 0x02, 0x12, 0x0a,
...
UINT _ux_host_stack_initialize(UINT (*ux_system_host_change_function)(ULONG, UX_HOST_CLASS *, VOID *)) {
    ...
    if (status == UX_SUCCESS) {
        _ux_system_host->ux_system_host_enum_thread_stack =
            _ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY, UX_HOST_ENUM_THREAD_STACK_SIZE);
    ...
    if (status == UX_SUCCESS) {
        _ux_system_host->ux_system_host_hcd_thread_stack =
            _ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY, UX_HOST_HCD_THREAD_STACK_SIZE);
    ...
    if (status == UX_SUCCESS) {
        status = _ux_utility_semaphore_create(&_ux_system_host->ux_system_host_enum_semaphore,
                                              "ux_system_host_enum_semaphore", 0);
        if (status != UX_SUCCESS)
    ...
    if (status == UX_SUCCESS) {
        status = _ux_utility_semaphore_create(&_ux_system_host->ux_system_host_hcd_semaphore,
                                              "ux_system_host_hcd_semaphore", 0);
        if (status != UX_SUCCESS)
            status = UX_SEMAPHORE_ERROR;
    }
    ...
    if (status == UX_SUCCESS) {
        status = _ux_utility_thread_create(&_ux_system_host->ux_system_host_enum_thread, "ux_system_host_enum_thread",
                                           _ux_host_stack_enum_thread_entry, 0,
                                           _ux_system_host->ux_system_host_enum_thread_stack,
    ...
    if (status == UX_SUCCESS) {
        status = _ux_utility_thread_create(
            &_ux_system_host->ux_system_host_hcd_thread, "ux_host_stack_hcd_thread", _ux_host_stack_hcd_thread_entry, 0,
            _ux_system_host->ux_system_host_hcd_thread_stack, UX_HOST_HCD_THREAD_STACK_SIZE, UX_THREAD_PRIORITY_HCD,
    ...
    if (status == UX_SUCCESS) {
        status = _ux_utility_thread_create(&_ux_system_host->ux_system_host_hnp_polling_thread,
                                           "ux_host_stack_hnp_polling_thread", _ux_host_stack_hnp_polling_thread_entry,
