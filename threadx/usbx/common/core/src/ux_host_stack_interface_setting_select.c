...
UINT _ux_host_stack_interface_setting_select(UX_INTERFACE *interface_ptr) {
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_HOST_STACK_INTERFACE_SETTING_SELECT, interface_ptr, 0, 0, 0,
    ...
    status = _ux_host_semaphore_get(&configuration->ux_configuration_device->ux_device_protection_semaphore,
                                    UX_WAIT_FOREVER);
    if (status != UX_SUCCESS)
        return (status);
