...
UINT _ux_host_stack_device_remove(UX_HCD *hcd, UX_DEVICE *parent, UINT port_index) {
    ...
    while (container_index++ < _ux_system_host->ux_system_host_max_devices) {
        ...
        if (device->ux_device_handle != UX_UNUSED) {
            ...
            if (UX_DEVICE_PARENT_MATCH(device, parent) && UX_DEVICE_PORT_LOCATION_MATCH(device, port_index) &&
    ...
    if (device->ux_device_handle == UX_UNUSED || !UX_DEVICE_HCD_MATCH(device, hcd))
#endif
    {
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_HOST_STACK_DEVICE_REMOVE, hcd, parent, port_index, device,
