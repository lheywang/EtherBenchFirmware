        ...
        for (hcd_index = 0; hcd_index < _ux_system_host->ux_system_host_registered_hcd; hcd_index++) {
            ...
            if ((hcd->ux_hcd_otg_capabilities & UX_HCD_OTG_CAPABLE) &&
                ...
                for (port_index = 0; port_index < hcd->ux_hcd_nb_root_hubs; port_index++) {
                    ...
                    port_status =
                        hcd->ux_hcd_entry_function(hcd, UX_HCD_GET_PORT_STATUS, (VOID *)((ALIGN_TYPE)port_index));
                                ...
                                if (device->ux_device_handle != UX_UNUSED) {
                                    ...
                                    if (UX_DEVICE_PORT_LOCATION_MATCH(device, port_index) &&
                                        ...
                                        if (configuration->ux_configuration_otg_capabilities & UX_OTG_HNP_SUPPORT) {
                                            ...
                                            otg_status =
                                                _ux_utility_memory_allocate(UX_SAFE_ALIGN, UX_CACHE_SAFE_MEMORY, 16);

                                            /* Check for status.  */
                                            if (otg_status == UX_NULL)
                                                return;

                                            /* Retrieve the control endpoint and the transfer request associated with
                                             * it.  */
                                            control_endpoint = &device->ux_device_control_endpoint;
                                            transfer_request = &control_endpoint->ux_endpoint_transfer_request;

                                            /* Protect the control endpoint semaphore here.  It will be unprotected in
                                               the transfer request function.  */
                                            status = _ux_host_semaphore_get(&device->ux_device_protection_semaphore,
                                                                            UX_WAIT_FOREVER);

                                            /* Perform a GET_STATUS on this device to see if it wants to become the
                                             * host.  */
                                            /* Create a transfer_request for the SET_CONFIGURATION request. No data for
                                             * this request.  */
                                            ...
                                            if (status == UX_SUCCESS &&
                                                transfer_request->ux_transfer_request_actual_length == 1) {

                                                /* We have an answer from the device. Check the HNP flag.  */
                                                if (*otg_status & UX_OTG_HOST_REQUEST_FLAG) {

                                                    /* The device has requested a Host swap. Initiate the command and
                                                       perform the stopping of the host.  */
