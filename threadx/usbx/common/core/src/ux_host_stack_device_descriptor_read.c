...
UINT _ux_host_stack_device_descriptor_read(UX_DEVICE *device) {
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_HOST_STACK_DEVICE_DESCRIPTOR_READ, device, 0, 0, 0, UX_TRACE_HOST_STACK_EVENTS, 0,
                            0)
    ...
    if (device->ux_device_descriptor.bMaxPacketSize0 != 8 && device->ux_device_descriptor.bMaxPacketSize0 != 16 &&
        device->ux_device_descriptor.bMaxPacketSize0 != 32 && device->ux_device_descriptor.bMaxPacketSize0 != 64) {
        _ux_utility_memory_free(descriptor);
    ...
    if ((status == UX_SUCCESS) &&
        (transfer_request->ux_transfer_request_actual_length == UX_DEVICE_DESCRIPTOR_LENGTH)) {
