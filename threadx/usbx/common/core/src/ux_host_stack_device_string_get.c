...
UINT _ux_host_stack_device_string_get(UX_DEVICE *device, UCHAR *descriptor_buffer, ULONG length, ULONG language_id,
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_HOST_STACK_DEVICE_STRING_GET, device, descriptor_buffer, length,
                            (language_id << 16) | string_index, UX_TRACE_HOST_CLASS_EVENTS, 0, 0)
    ...
    transfer_request->ux_transfer_request_flags |= (UX_TRANSFER_FLAG_LOCK | UX_TRANSFER_FLAG_AUTO_DEVICE_UNLOCK);
    UX_RESTORE
#else
