...
UINT _ux_device_class_pima_object_data_send(UX_SLAVE_CLASS_PIMA *pima) {
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_PIMA_OBJECT_DATA_SEND, pima, object_handle, 0, 0,
    ...
    if (status == UX_SUCCESS) {
        ...
        while (total_length != 0) {
            ...
            if (object_offset == 0) {
                ...
                status = pima->ux_device_class_pima_object_data_send(
                    pima, object_handle, UX_DEVICE_CLASS_PIMA_OBJECT_TRANSFER_PHASE_ACTIVE,
                    object_data + UX_DEVICE_CLASS_PIMA_DATA_HEADER_SIZE, object_offset,
                ...
                status = pima->ux_device_class_pima_object_data_send(pima, object_handle,
                                                                     UX_DEVICE_CLASS_PIMA_OBJECT_TRANSFER_PHASE_ACTIVE,
                                                                     object_data, object_offset, transfer_length);
    ...
    if (status == UX_SUCCESS) {
        ...
        status = pima->ux_device_class_pima_object_data_send(
            pima, object_handle, UX_DEVICE_CLASS_PIMA_OBJECT_TRANSFER_PHASE_COMPLETED, UX_NULL, 0, 0);
    } else {
        ...
        status = pima->ux_device_class_pima_object_data_send(
            pima, object_handle, UX_DEVICE_CLASS_PIMA_OBJECT_TRANSFER_PHASE_COMPLETED_ERROR, UX_NULL, 0, 0);
    }
