...
#if UX_DEVICE_CLASS_PIMA_TRANSFER_BUFFER_LENGTH <                                                                      \
    UX_DEVICE_CLASS_PIMA_DATA_HEADER_SIZE + 4 + (4 * UX_DEVICE_CLASS_PIMA_MAX_STORAGE_IDS)
...
UINT _ux_device_class_pima_storage_id_send(UX_SLAVE_CLASS_PIMA *pima) {
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_PIMA_STORAGE_ID_SEND, pima, 0, 0, 0, UX_TRACE_DEVICE_CLASS_EVENTS, 0,
                            0)
    ...
    _ux_utility_short_put(storage_id + UX_DEVICE_CLASS_PIMA_DATA_HEADER_TYPE, UX_DEVICE_CLASS_PIMA_CT_DATA_BLOCK);

    /* Fill in the data code.  */
    _ux_utility_short_put(storage_id + UX_DEVICE_CLASS_PIMA_DATA_HEADER_CODE, UX_DEVICE_CLASS_PIMA_OC_GET_STORAGE_IDS);
    ...
    _ux_utility_long_put(storage_id + UX_DEVICE_CLASS_PIMA_DATA_HEADER_SIZE + sizeof(ULONG),
                         pima->ux_device_class_pima_storage_id);

    /* Compute the overall length of the device info structure.  */
    storage_id_length =
        UX_DEVICE_CLASS_PIMA_DATA_HEADER_SIZE + sizeof(ULONG) + (sizeof(ULONG) * UX_DEVICE_CLASS_PIMA_MAX_STORAGE_IDS);

    /* Fill in the size of the response header.  */
    _ux_utility_long_put(storage_id + UX_DEVICE_CLASS_PIMA_DATA_HEADER_LENGTH, storage_id_length);
