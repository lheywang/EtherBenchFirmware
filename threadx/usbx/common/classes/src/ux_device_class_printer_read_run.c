...
UINT _ux_device_class_printer_read_run(UX_DEVICE_CLASS_PRINTER *printer, UCHAR *buffer, ULONG requested_length,
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_PRINTER_READ, printer, buffer, requested_length, 0,
    ...
    if (status < UX_STATE_NEXT) {
        ...
        printer->ux_device_class_printer_read_state = UX_STATE_RESET;
        printer->ux_device_class_printer_read_status = transfer_request->ux_slave_transfer_request_completion_code;
    ...
    case UX_DEVICE_CLASS_PRINTER_READ_WAIT:
        ...
        if (status == UX_STATE_NEXT) {
            ...
            _ux_utility_memory_copy(
                printer->ux_device_class_printer_read_buffer, transfer_request->ux_slave_transfer_request_data_pointer,
                transfer_request->ux_slave_transfer_request_actual_length); /* Use case of memcpy is verified. */

            /* Next buffer address.  */
            printer->ux_device_class_printer_read_buffer += transfer_request->ux_slave_transfer_request_actual_length;
