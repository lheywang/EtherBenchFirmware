...
UINT _ux_device_class_pima_control_request(UX_SLAVE_CLASS_COMMAND *command) {
        ...
        if (pima->ux_device_class_pima_state > UX_DEVICE_CLASS_PIMA_PHASE_RESPONSE) {
            ...
            if (pima->ux_device_class_pima_state == UX_DEVICE_CLASS_PIMA_PHASE_DATA_IN)
                _ux_device_stack_transfer_abort(
                    &pima->ux_device_class_pima_bulk_in_endpoint->ux_slave_endpoint_transfer_request, UX_ABORTED);
            else
                _ux_device_stack_transfer_abort(
                    &pima->ux_device_class_pima_bulk_out_endpoint->ux_slave_endpoint_transfer_request, UX_ABORTED);
    ...
    case UX_DEVICE_CLASS_PIMA_REQUEST_STATUS_COMMAND:
        ...
        /* Fill status.  */
        _ux_utility_short_put(request_data + 2, pima->ux_device_class_pima_device_status);
