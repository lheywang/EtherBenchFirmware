...
UINT _ux_device_class_video_initialize(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    UX_UTILITY_MULC_SAFE(video_parameter->ux_device_class_video_parameter_streams_nb,
                         (ULONG)sizeof(UX_DEVICE_CLASS_VIDEO_STREAM), streams_size, status);
    ...
    /* Save streams.  */
    if (streams_size) {
        video->ux_device_class_video_streams =
            (UX_DEVICE_CLASS_VIDEO_STREAM *)((UCHAR *)video + sizeof(UX_DEVICE_CLASS_VIDEO));
    ...
    for (i = 0; i < video->ux_device_class_video_streams_nb; i++) {
        ...
        stream->ux_device_class_video_stream_payload_buffer_size =
            stream_parameter->ux_device_class_video_stream_parameter_max_payload_buffer_size;
        ...
        stream->ux_device_class_video_stream_buffer =
        ...
        stream->ux_device_class_video_stream_transfer_pos =
            (UX_DEVICE_CLASS_VIDEO_PAYLOAD *)stream->ux_device_class_video_stream_buffer;
        ...
        status = _ux_utility_thread_create(
            &stream->ux_device_class_video_stream_thread, "ux_device_class_video_stream_thread",
            stream_parameter->ux_device_class_video_stream_parameter_thread_entry, (ULONG)(ALIGN_TYPE)stream,
            (VOID *)stream->ux_device_class_video_stream_thread_stack, memory_size, UX_THREAD_PRIORITY_CLASS,
        ...
        stream->ux_device_class_video_stream_task_function =
            stream_parameter->ux_device_class_video_stream_parameter_task_function;
#endif
...
UINT _uxe_device_class_video_initialize(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    for (i = 0; i < video_parameter->ux_device_class_video_parameter_streams_nb; i++) {
        if ((video_parameter->ux_device_class_video_parameter_streams[i]
                 .ux_device_class_video_stream_parameter_max_payload_buffer_size == 0) ||
            (video_parameter->ux_device_class_video_parameter_streams[i]
                 .ux_device_class_video_stream_parameter_max_payload_buffer_nb == 0))
            return (UX_INVALID_PARAMETER);
    }
