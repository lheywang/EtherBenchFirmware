        ...
        while (device->ux_slave_device_state == UX_DEVICE_CONFIGURED) {
            ...
            if (status == UX_SUCCESS) {
                ...
                storage->ux_slave_class_storage_host_length =
                    _ux_utility_long_get(scsi_command + UX_SLAVE_CLASS_STORAGE_CBW_DATA_LENGTH);
                    ...
                    if (_ux_utility_long_get(scsi_command) == UX_SLAVE_CLASS_STORAGE_CBW_SIGNATURE_MASK) {
                        ...
                        if (cbwcb_length != 0) {
                            ...
                            case UX_SLAVE_CLASS_STORAGE_SCSI_PREVENT_ALLOW_MEDIA_REMOVAL:
                                ...
                                _ux_device_class_storage_prevent_allow_media_removal(storage, lun, endpoint_in,
                                                                                     endpoint_out, cbw_cb);
                                break;

                            case UX_SLAVE_CLASS_STORAGE_SCSI_READ_FORMAT_CAPACITY:

                                _ux_device_class_storage_read_format_capacity(storage, lun, endpoint_in, endpoint_out,
                                                                              cbw_cb);
                                break;
                            ...
                            case UX_SLAVE_CLASS_STORAGE_SCSI_SYNCHRONIZE_CACHE:
                                ...
                                _ux_device_class_storage_synchronize_cache(storage, lun, endpoint_in, endpoint_out,
                                                                           cbw_cb, *(cbw_cb));
                                break;
                            ...
                            case UX_SLAVE_CLASS_STORAGE_SCSI_GET_STATUS_NOTIFICATION:
                                ...
                                _ux_device_class_storage_get_status_notification(storage, lun, endpoint_in,
                                                                                 endpoint_out, cbw_cb);
                                break;

                            case UX_SLAVE_CLASS_STORAGE_SCSI_GET_CONFIGURATION:

                                _ux_device_class_storage_get_configuration(storage, lun, endpoint_in, endpoint_out,
                                                                           cbw_cb);
                                break;

                            case UX_SLAVE_CLASS_STORAGE_SCSI_READ_DISK_INFORMATION:

                                _ux_device_class_storage_read_disk_information(storage, lun, endpoint_in, endpoint_out,
                                                                               cbw_cb);
                                break;
                            ...
                            case UX_SLAVE_CLASS_STORAGE_SCSI_GET_PERFORMANCE:
                                ...
                                _ux_device_class_storage_get_performance(storage, lun, endpoint_in, endpoint_out,
                                                                         cbw_cb);
                                break;

                            case UX_SLAVE_CLASS_STORAGE_SCSI_READ_DVD_STRUCTURE:

                                _ux_device_class_storage_read_dvd_structure(storage, lun, endpoint_in, endpoint_out,
                                                                            cbw_cb);
                                break;
                            ...
                            case UX_SLAVE_CLASS_STORAGE_SCSI_READ_TOC:
                                ...
                                status =
                                    _ux_device_class_storage_read_toc(storage, lun, endpoint_in, endpoint_out, cbw_cb);
