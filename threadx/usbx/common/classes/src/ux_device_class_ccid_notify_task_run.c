...
UINT _ux_device_class_ccid_notify_task_run(UX_DEVICE_CLASS_CCID *ccid) {
    ...
    while (immediate_state) {
        ...
        case UX_DEVICE_CLASS_CCID_NOTIFY_START:
            ...
            if (slot->ux_device_class_ccid_slot_flags & UX_DEVICE_CLASS_CCID_FLAG_NOTIFY_HW_ERROR) {
                slot->ux_device_class_ccid_slot_flags &= (UCHAR)~UX_DEVICE_CLASS_CCID_FLAG_NOTIFY_HW_ERROR;
                buffer[UX_DEVICE_CLASS_CCID_OFFSET_MESSAGE_TYPE] = UX_DEVICE_CLASS_CCID_RDR_TO_PC_HARDWARE_ERROR;
                buffer[UX_DEVICE_CLASS_CCID_OFFSET_HW_ERROR_SLOT] = 0;
                buffer[UX_DEVICE_CLASS_CCID_OFFSET_HW_ERROR_SEQ] = slot->ux_device_class_ccid_slot_hw_error_seq;
                buffer[UX_DEVICE_CLASS_CCID_OFFSET_HW_ERROR_CODE] = slot->ux_device_class_ccid_slot_hw_error;
                length = 4;
            }

            /* Check slot change notification.  */
            else if (slot->ux_device_class_ccid_slot_flags & UX_DEVICE_CLASS_CCID_FLAG_NOTIFY_CHANGE) {
                slot->ux_device_class_ccid_slot_flags &= (UCHAR)~UX_DEVICE_CLASS_CCID_FLAG_NOTIFY_CHANGE;

                /* Message type.  */
                buffer[UX_DEVICE_CLASS_CCID_OFFSET_MESSAGE_TYPE] = UX_DEVICE_CLASS_CCID_RDR_TO_PC_NOTIFY_SLOT_CHANGE;
