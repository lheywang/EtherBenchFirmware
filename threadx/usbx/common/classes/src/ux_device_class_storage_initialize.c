...
UINT _ux_device_class_storage_initialize(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    if (status == UX_SUCCESS) {
        class_inst->ux_slave_class_thread_stack =
        ...
        if (class_inst->ux_slave_class_thread_stack != UX_NULL)
            ...
            status = _ux_device_thread_create(
                &class_inst->ux_slave_class_thread, "ux_slave_storage_thread", _ux_device_class_storage_thread,
                (ULONG)(ALIGN_TYPE)class_inst, (VOID *)class_inst->ux_slave_class_thread_stack, UX_THREAD_STACK_SIZE,
    ...
    if (status == UX_SUCCESS) {
        ...
        for (lun_index = 0; lun_index < storage->ux_slave_class_storage_number_lun; lun_index++) {
            ...
            storage->ux_slave_class_storage_lun[lun_index].ux_slave_class_storage_media_flush =
                storage_parameter->ux_slave_class_storage_parameter_lun[lun_index].ux_slave_class_storage_media_flush;
            ...
            storage->ux_slave_class_storage_lun[lun_index].ux_slave_class_storage_media_status =
                storage_parameter->ux_slave_class_storage_parameter_lun[lun_index].ux_slave_class_storage_media_status;
...
UINT _uxe_device_class_storage_initialize(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    for (i = 0; i < storage_parameter->ux_slave_class_storage_parameter_number_lun; i++) {
        if ((storage_parameter->ux_slave_class_storage_parameter_lun[i].ux_slave_class_storage_media_read == UX_NULL) ||
            (storage_parameter->ux_slave_class_storage_parameter_lun[i].ux_slave_class_storage_media_write ==
             UX_NULL) ||
            ...
            || (storage_parameter->ux_slave_class_storage_parameter_lun[i].ux_slave_class_storage_media_notification ==
                UX_NULL)
#endif
        ) {
            return (UX_INVALID_PARAMETER);
