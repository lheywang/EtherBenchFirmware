...
UINT _ux_device_class_pima_device_prop_value_set(UX_SLAVE_CLASS_PIMA *pima, ULONG device_property_code) {
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_PIMA_GET_DEVICE_PROP_VALUE_SET, pima, device_property_code, 0, 0,
    ...
    if (device_property_value_length > UX_DEVICE_CLASS_PIMA_DATA_HEADER_SIZE) {
        ...
        status = pima->ux_device_class_pima_device_prop_value_set(
            pima, device_property_code, device_property_value_pointer, device_property_value_length);

        /* Now we return a response with success.  */
        _ux_device_class_pima_response_send(pima, (status == UX_SUCCESS) ? UX_DEVICE_CLASS_PIMA_RC_OK : status, 0, 0, 0,
                                            0);
    } else {
