...
UINT _ux_device_class_ccid_control_request(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    case UX_DEVICE_CLASS_CCID_GET_CLOCK_FREQUENCIES:
        ...
        if (parameter->ux_device_class_ccid_clocks == UX_NULL || parameter->ux_device_class_ccid_n_clocks == 0)
            return (UX_ERROR);
    ...
    case UX_DEVICE_CLASS_CCID_GET_DATA_RATES:
        ...
        if (parameter->ux_device_class_ccid_data_rates == UX_NULL || parameter->ux_device_class_ccid_n_data_rates == 0)
            return (UX_ERROR);
    ...
    _ux_utility_memory_copy(transfer_request->ux_slave_transfer_request_data_pointer, transmit_buffer,
                            transmit_length); /* Use case of memcpy is verified. */
