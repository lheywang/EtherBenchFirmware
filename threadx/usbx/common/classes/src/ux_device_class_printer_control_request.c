...
UINT _ux_device_class_printer_control_request(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    case UX_DEVICE_CLASS_PRINTER_GET_DEVICE_ID:
        ...
        while (length) {
            ...
            if (descriptor_length < 2 || descriptor_length > length) {
        ...
        length = _ux_utility_short_get_big_endian(
    ...
    case UX_DEVICE_CLASS_PRINTER_SOFT_RESET:
        ...
        if (printer->ux_device_class_printer_parameter.ux_device_class_printer_soft_reset != UX_NULL) {

            /* Invoke the application callback.  */
            printer->ux_device_class_printer_parameter.ux_device_class_printer_soft_reset(printer);
        }
        break;
