...
UINT _ux_device_class_cdc_acm_read_run(UX_SLAVE_CLASS_CDC_ACM *cdc_acm, UCHAR *buffer, ULONG requested_length,
    ...
    if (status < UX_STATE_NEXT) {
        cdc_acm->ux_device_class_cdc_acm_read_state = UX_STATE_RESET;
        cdc_acm->ux_device_class_cdc_acm_read_status = transfer_request->ux_slave_transfer_request_completion_code;
        return (UX_STATE_ERROR);
    }
    ...
    case UX_DEVICE_CLASS_CDC_ACM_READ_WAIT:
        ...
        /* Run the transfer state machine.  */
        status = _ux_device_stack_transfer_run(transfer_request, cdc_acm->ux_device_class_cdc_acm_read_transfer_length,
        ...
        if (status == UX_STATE_NEXT) {
            ...
            _ux_utility_memory_copy(
                cdc_acm->ux_device_class_cdc_acm_read_buffer, transfer_request->ux_slave_transfer_request_data_pointer,
                cdc_acm->ux_device_class_cdc_acm_read_transfer_length); /* Use case of memcpy is verified. */

            /* Next buffer address.  */
            cdc_acm->ux_device_class_cdc_acm_read_buffer += transfer_request->ux_slave_transfer_request_actual_length;
