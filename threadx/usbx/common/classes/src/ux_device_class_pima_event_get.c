...
/*                                                                        */
/**************************************************************************/
UINT _ux_device_class_pima_event_get(UX_SLAVE_CLASS_PIMA *pima, UX_SLAVE_CLASS_PIMA_EVENT *pima_event) {

    UX_SLAVE_CLASS_PIMA_EVENT *current_pima_event;

    /* If trace is enabled, insert this event into the trace buffer.  */
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_PIMA_EVENT_GET, pima, pima_event, 0, 0, UX_TRACE_DEVICE_CLASS_EVENTS,
                            0, 0)

    /* Check if the head and the tail of the event array is the same.  */
    if (pima->ux_device_class_pima_event_array_head == pima->ux_device_class_pima_event_array_tail)
