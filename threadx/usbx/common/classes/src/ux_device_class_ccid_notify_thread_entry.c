...
VOID _ux_device_class_ccid_notify_thread_entry(ULONG ccid_inst) {
    ...
    while (1) {
        ...
        /* Wait event.  */
        status = _ux_utility_semaphore_get(&ccid->ux_device_class_ccid_notify_semaphore, UX_WAIT_FOREVER);
        if (status != UX_SUCCESS)
            continue;
        ...
        if (buffer[UX_DEVICE_CLASS_CCID_OFFSET_MESSAGE_TYPE] == 0) {
            ...
            for (i = 0, byte_pos = 1;; byte_pos++) {
                ...
                for (bits_pos = 0; bits_pos < 8; bits_pos += 2) {
                    ...
                    if (slot->ux_device_class_ccid_slot_flags & UX_DEVICE_CLASS_CCID_FLAG_NOTIFY_CHANGE) {
