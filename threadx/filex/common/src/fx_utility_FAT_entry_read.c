...
UINT _fx_utility_FAT_entry_read(FX_MEDIA *media_ptr, ULONG cluster, ULONG *entry_ptr) {
    ...
    if (media_ptr->fx_media_12_bit_FAT) {
        ...
        FAT_sector = (byte_offset / media_ptr->fx_media_bytes_per_sector) + (ULONG)media_ptr->fx_media_reserved_sectors;
...
#ifdef FX_ENABLE_EXFAT
    else if ((media_ptr->fx_media_FAT_type == FX_FAT32) || (media_ptr->fx_media_FAT_type == FX_exFAT))
#else
    else
#endif /* FX_ENABLE_EXFAT */
    {
