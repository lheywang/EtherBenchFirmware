...
UINT _fx_media_close(FX_MEDIA *media_ptr) {
...
#ifdef FX_ENABLE_EXFAT
    if ((media_ptr->fx_media_FAT_type == FX_exFAT) && (FX_TRUE == media_ptr->fx_media_exfat_bitmap_cache_dirty)) {
    ...
    status = _fx_utility_logical_sector_flush(media_ptr, ((ULONG64)1), (ULONG64)(media_ptr->fx_media_total_sectors),
                                              FX_FALSE);
