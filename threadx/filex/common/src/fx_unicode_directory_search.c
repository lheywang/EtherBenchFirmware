...
UINT _fx_unicode_directory_search(FX_MEDIA *media_ptr, FX_DIR_ENTRY *entry_ptr, UCHAR *short_name,
    ...
    if (search_dir_ptr) {
        ...
        directory_size =
            (((ULONG)media_ptr->fx_media_bytes_per_sector) * ((ULONG)media_ptr->fx_media_sectors_per_cluster) * i) /
    ...
    do {
        ...
        status = _fx_unicode_directory_entry_read(media_ptr, search_dir_ptr, &i, entry_ptr, &_fx_unicode_search_name[0],
