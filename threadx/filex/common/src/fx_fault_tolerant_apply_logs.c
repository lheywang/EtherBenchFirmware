...
UINT _fx_fault_tolerant_apply_logs(FX_MEDIA *media_ptr) {
    ...
    while (remaining_logs) {
        ...
        case FX_FAULT_TOLERANT_FAT_LOG_TYPE:
            ...
            status = _fx_utility_FAT_entry_write(
                media_ptr, _fx_utility_32_unsigned_read((UCHAR *)&fat_log->fx_fault_tolerant_FAT_log_cluster),
        ...
        case FX_FAULT_TOLERANT_BITMAP_LOG_TYPE:
            ...
            status = _fx_utility_exFAT_cluster_state_set(
                media_ptr, _fx_utility_32_unsigned_read((UCHAR *)&bitmap_log->fx_fault_tolerant_bitmap_log_cluster),
        ...
        case FX_FAULT_TOLERANT_DIR_LOG_TYPE:
            ...
            status = _fx_utility_logical_sector_read(media_ptr, log_sector, media_ptr->fx_media_memory_buffer,
            ...
            status = _fx_utility_logical_sector_write(media_ptr, log_sector, media_ptr->fx_media_memory_buffer,
    ...
    if (FAT_chain->fx_fault_tolerant_FAT_chain_flag & FX_FAULT_TOLERANT_FLAG_FAT_CHAIN_VALID) {
        ...
        if (FAT_chain->fx_fault_tolerant_FAT_chain_flag & FX_FAULT_TOLERANT_FLAG_BITMAP_USED) {
            ...
            tail_cluster =
                _fx_utility_32_unsigned_read((UCHAR *)&FAT_chain->fx_fault_tolerant_FAT_chain_insertion_back);
            ...
            if ((head_cluster >= FX_FAT_ENTRY_START) && (head_cluster < media_ptr->fx_media_fat_reserved)) {
                for (current_cluster = head_cluster; current_cluster < tail_cluster; current_cluster++) {
                    ...
                    /* Free bitmap. */
                    status =
                        _fx_utility_exFAT_cluster_state_set(media_ptr, current_cluster, FX_EXFAT_BITMAP_CLUSTER_FREE);
                    if (status != FX_SUCCESS) {
