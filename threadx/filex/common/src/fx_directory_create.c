...
UINT _fx_directory_create(FX_MEDIA *media_ptr, CHAR *directory_name) {
    ...
    if (media_ptr->fx_media_FAT_type == FX_exFAT) {
        if (((dir_entry.fx_dir_entry_name[0] == '.') && (dir_entry.fx_dir_entry_name[1] == 0)) ||
            ((dir_entry.fx_dir_entry_name[0] == '.') && (dir_entry.fx_dir_entry_name[1] == '.') &&
    ...
    if (media_ptr->fx_media_FAT_type == FX_exFAT) {
        ...
        status =
            _fx_utility_exFAT_bitmap_free_cluster_find(media_ptr, media_ptr->fx_media_cluster_search_start, &FAT_index);
    ...
    /* Read the first logical sector associated with the allocated
       cluster.  */
    status = _fx_utility_logical_sector_read(media_ptr, logical_sector, media_ptr->fx_media_memory_buffer, ((ULONG)1),
                                             FX_DIRECTORY_SECTOR);
    ...
    if (media_ptr->fx_media_fault_tolerant_enabled == FX_TRUE) {
        ...
        /* Write back. */
        status = _fx_utility_logical_sector_write(media_ptr, logical_sector, media_ptr->fx_media_memory_buffer,
    ...
    if (sectors > 1) {
        ...
        while (sectors) {
            ...
            FX_TRACE_IN_LINE_INSERT(FX_TRACE_INTERNAL_IO_DRIVER_WRITE, media_ptr,
                                    ((ULONG)logical_sector) + ((ULONG)sectors), 1, media_ptr->fx_media_memory_buffer,
