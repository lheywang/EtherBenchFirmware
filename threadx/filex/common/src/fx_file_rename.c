...
UINT _fx_file_rename(FX_MEDIA *media_ptr, CHAR *old_file_name, CHAR *new_file_name) {
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_FILE_RENAME, media_ptr, old_file_name, new_file_name, 0, FX_TRACE_FILE_EVENTS, 0,
                            0)
     ...
     * not.  */
    if (((old_file_name[0] == '/') || (old_file_name[0] == '\\')) && (new_file_name[0] != '/') &&
        (new_file_name[0] != '\\')) {
    ...
    while (open_count) {
        ...
        if ((search_ptr->fx_file_dir_entry.fx_dir_entry_log_sector == old_dir_entry.fx_dir_entry_log_sector) &&
            (search_ptr->fx_file_dir_entry.fx_dir_entry_byte_offset == old_dir_entry.fx_dir_entry_byte_offset)) {
