...
UINT _fx_file_extended_best_effort_allocate(FX_FILE *file_ptr, ULONG64 size, ULONG64 *actual_size_allocated) {
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_FILE_BEST_EFFORT_ALLOCATE, file_ptr, size, 0, 0, FX_TRACE_FILE_EVENTS,
     ...
     * of the file.  */
    if (((file_ptr->fx_file_current_available_size + (((ULONG64)clusters) * ((ULONG64)bytes_per_cluster))) <
         file_ptr->fx_file_current_available_size) ||
        ((file_ptr->fx_file_current_available_size + (((ULONG64)clusters) * ((ULONG64)bytes_per_cluster))) >
         0xFFFFFFFFULL)) {
    ...
    if ((file_ptr->fx_file_dir_entry.fx_dir_entry_dont_use_fat & 1) &&
        ...
        for (FAT_index = file_ptr->fx_file_last_physical_cluster + 1;
             FAT_index < clusters + file_ptr->fx_file_last_physical_cluster + 1; FAT_index++) {
        ...
        while (FAT_index < (media_ptr->fx_media_total_clusters + FX_FAT_ENTRY_START)) {
            ...
            if (i >= clusters) {
                ...
                if (media_ptr->fx_media_FAT_type == FX_exFAT) {
                    ...
                    status = _fx_utility_exFAT_bitmap_free_cluster_find(media_ptr, FAT_index + i + 1, &FAT_value);
                    if (status != FX_SUCCESS) {
    ...
    if (found) {
        ...
        if (media_ptr->fx_media_fault_tolerant_enabled) {
            ...
            _fx_fault_tolerant_set_FAT_chain(media_ptr, FX_FALSE, file_ptr->fx_file_last_physical_cluster, FAT_index,
...
#ifdef FX_ENABLE_EXFAT
        if (file_ptr->fx_file_dir_entry.fx_dir_entry_dont_use_fat & 1) {
            if ((file_ptr->fx_file_total_clusters) && (FAT_index != file_ptr->fx_file_last_physical_cluster + 1)) {

                /* Clusters are not consecutive.  */
                file_ptr->fx_file_dir_entry.fx_dir_entry_dont_use_fat &= (CHAR)0xfe; /* Set 0bit to 0.  */

                /* Rebuild FAT.  */
                FAT_value = file_ptr->fx_file_dir_entry.fx_dir_entry_cluster + file_ptr->fx_file_total_clusters -
                            1; /* Last cluster */
        ...
        for (i = 0; i < (clusters - 1); i++) {
            ...
            if (media_ptr->fx_media_FAT_type == FX_exFAT) {
                ...
                status =
                    _fx_utility_exFAT_cluster_state_set(media_ptr, FAT_index + i, FX_EXFAT_BITMAP_CLUSTER_OCCUPIED);
        ...
        if (media_ptr->fx_media_FAT_type == FX_exFAT) {
            ...
            /* Mark the cluster as used.  */
            status = _fx_utility_exFAT_cluster_state_set(media_ptr, FAT_index + clusters - 1,
        ...
        if (file_ptr->fx_file_total_clusters) {
            ...
            if (!(file_ptr->fx_file_dir_entry.fx_dir_entry_dont_use_fat & 1)) {
                ...
                status = _fx_utility_FAT_entry_write(media_ptr, file_ptr->fx_file_last_physical_cluster, FAT_index);
        ...
        *actual_size_allocated = ((ULONG64)clusters) * bytes_per_cluster;
        ...
        FX_TRACE_EVENT_UPDATE(trace_event, trace_timestamp, FX_TRACE_FILE_BEST_EFFORT_ALLOCATE, 0, 0,
                              *actual_size_allocated, 0);
    ...
    status = _fx_utility_logical_sector_flush(media_ptr, ((ULONG64)1), (ULONG64)(media_ptr->fx_media_sectors_per_FAT),
                                              FX_FALSE);
