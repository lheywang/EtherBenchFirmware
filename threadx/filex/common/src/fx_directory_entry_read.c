...
#else
UINT _fx_directory_entry_read(FX_MEDIA *media_ptr, FX_DIR_ENTRY *source_dir, ULONG *entry_ptr,
                              FX_DIR_ENTRY *destination_ptr)
#endif /* FX_ENABLE_EXFAT */
{
...
#endif
    {
        ...
        logical_sector =
            (byte_offset / media_ptr->fx_media_bytes_per_sector) + (ULONG)media_ptr->fx_media_root_sector_start;
        ...
        byte_offset = byte_offset - ((logical_sector - (ULONG)media_ptr->fx_media_root_sector_start) *
        ...
        do {
            ...
            if (byte_offset + FX_DIR_ENTRY_SIZE >= media_ptr->fx_media_bytes_per_sector) {
                ...
                if ((source_dir) || (media_ptr->fx_media_32_bit_FAT)) {
                    ...
                    if (logical_sector >=
                        (ULONG)(media_ptr->fx_media_root_sector_start + media_ptr->fx_media_root_sectors)) {
...
#ifdef FX_ENABLE_EXFAT
UINT _fx_directory_entry_read_ex(FX_MEDIA *media_ptr, FX_DIR_ENTRY *source_dir, ULONG *entry_ptr,
                                 FX_DIR_ENTRY *destination_ptr, UINT hash) {
    UINT status = FX_SUCCESS;

    if (media_ptr->fx_media_FAT_type == FX_exFAT) {
        status = _fx_directory_exFAT_entry_read(media_ptr, source_dir, entry_ptr, destination_ptr, hash, FX_FALSE, NULL,
                                                NULL);
    } else {
        status = _fx_directory_entry_read_FAT(media_ptr, source_dir, entry_ptr, destination_ptr);
    }
