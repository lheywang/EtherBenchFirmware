...
UINT _fx_file_extended_truncate_release(FX_FILE *file_ptr, ULONG64 size) {
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_FILE_TRUNCATE_RELEASE, file_ptr, size, file_ptr->fx_file_current_file_size, 0,
    ...
    if (size < file_ptr->fx_file_current_available_size) {
        ...
        file_ptr->fx_file_current_available_size =
            ((size + bytes_per_cluster - 1) / bytes_per_cluster) * bytes_per_cluster;
        ...
        FX_TRACE_EVENT_UPDATE(trace_event, trace_timestamp, FX_TRACE_FILE_TRUNCATE_RELEASE, 0, 0, 0,
    ...
    cluster_count = (ULONG)(file_ptr->fx_file_current_available_size + (((ULONG64)bytes_per_cluster) - ((ULONG64)1))) /
                    bytes_per_cluster;
    ...
    if ((cluster_count == 0) && (media_ptr->fx_media_fault_tolerant_enabled)) {
        ...
        if (!(file_ptr->fx_file_dir_entry.fx_dir_entry_dont_use_fat & 1)) {
            ...
            /* Set undo log. */
            status =
                _fx_fault_tolerant_set_FAT_chain(media_ptr, FX_FALSE, FX_FREE_CLUSTER, media_ptr->fx_media_fat_last,
            ...
            /* Set undo log. */
            status = _fx_fault_tolerant_set_FAT_chain(media_ptr, FX_TRUE, FX_FREE_CLUSTER, media_ptr->fx_media_fat_last,
            ...
            if (cluster_count == 0) {
...
#endif /* FX_ENABLE_EXFAT */
                {
                    ...
                    if (media_ptr->fx_media_fault_tolerant_enabled) {
                        ...
                        /* Set undo log. */
                        status =
                            _fx_fault_tolerant_set_FAT_chain(media_ptr, FX_FALSE, cluster, media_ptr->fx_media_fat_last,
                    ...
                    /* Set undo log. */
                    status = _fx_fault_tolerant_set_FAT_chain(media_ptr, FX_TRUE, cluster, media_ptr->fx_media_fat_last,
                ...
                if (cluster_count) {
                    ...
                    search_ptr->fx_file_current_logical_sector =
                        ...
                        (((ULONG64)search_ptr->fx_file_current_physical_cluster - FX_FAT_ENTRY_START) *
                         ((ULONG)media_ptr->fx_media_sectors_per_cluster)) +
                        (bytes_remaining / (ULONG)media_ptr->fx_media_bytes_per_sector);
                    search_ptr->fx_file_current_relative_sector =
                        (UINT)((bytes_remaining / (ULONG)media_ptr->fx_media_bytes_per_sector));
