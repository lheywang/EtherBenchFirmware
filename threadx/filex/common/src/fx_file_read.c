...
UINT _fx_file_read(FX_FILE *file_ptr, VOID *buffer_ptr, ULONG request_size, ULONG *actual_size) {
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_FILE_READ, file_ptr, buffer_ptr, request_size, 0, FX_TRACE_FILE_EVENTS,
    ...
    if ((ULONG64)request_size > (file_ptr->fx_file_current_file_size - file_ptr->fx_file_current_file_offset)) {
    ...
    while (bytes_remaining) {
        ...
        if ((file_ptr->fx_file_current_logical_offset) || (bytes_remaining < media_ptr->fx_media_bytes_per_sector)) {
            ...
            if (sectors == 1) {
                ...
                /* Actually perform the memory copy.  */
                _fx_utility_memory_copy((UCHAR *)media_ptr->fx_media_memory_buffer, destination_ptr,
            ...
            /* Adjust the pointer to the destination buffer.  */
            destination_ptr = destination_ptr + (((ULONG)media_ptr->fx_media_bytes_per_sector) * sectors);
        }
    ...
    file_ptr->fx_file_current_file_offset = file_ptr->fx_file_current_file_offset + (ULONG64)request_size;
