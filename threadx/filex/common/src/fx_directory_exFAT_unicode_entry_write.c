...
UINT _fx_directory_exFAT_unicode_entry_write(FX_MEDIA *media_ptr, FX_DIR_ENTRY *entry_ptr, UCHAR update_level,
                                             USHORT *unicode_name, UINT unicode_length) {
    ...
    cluster =
        (ULONG)((logical_sector - media_ptr->fx_media_data_sector_start) / (media_ptr->fx_media_sectors_per_cluster)) +
        FX_FAT_ENTRY_START;
    ...
    relative_sector =
        (ULONG)(logical_sector -
                (((ULONG)media_ptr->fx_media_data_sector_start) +
                 (((ULONG64)cluster - FX_FAT_ENTRY_START) * ((ULONG)media_ptr->fx_media_sectors_per_cluster))));

    /* Read the sector.  */
    status = _fx_utility_logical_sector_read(media_ptr, (ULONG64)logical_sector, media_ptr->fx_media_memory_buffer,
    ...
    if (update_level >= UPDATE_FILE) {
        ...
        date_time = ((ULONG)entry_ptr->fx_dir_entry_created_date << 16) | (ULONG)entry_ptr->fx_dir_entry_created_time;
    ...
    if (update_level >= UPDATE_FILE) {
        ...
        date_time = ((ULONG)entry_ptr->fx_dir_entry_date << 16) | (ULONG)entry_ptr->fx_dir_entry_time;
        _fx_utility_32_unsigned_write(work_ptr, date_time);
    }
    ...
    for (i = 0; i < total_entries; ++i) {
        ...
        if (byte_offset >= media_ptr->fx_media_bytes_per_sector) {
            ...
            if (media_ptr->fx_media_fault_tolerant_enabled &&
                ...
                status = _fx_fault_tolerant_add_dir_log(media_ptr, logical_sector, changed_offset, changed_ptr,
                                                        changed_size);
            } else
#endif /* FX_ENABLE_FAULT_TOLERANT */
            {
                ...
                status = _fx_utility_logical_sector_write(media_ptr, logical_sector, sector_base_ptr, ((ULONG)1),
                                                          FX_DIRECTORY_SECTOR);
            }
            ...
            status = _fx_utility_logical_sector_read(
                media_ptr, (ULONG64)logical_sector, media_ptr->fx_media_memory_buffer, ((ULONG)1), FX_DIRECTORY_SECTOR);
            ...
            if (update_level >= UPDATE_NAME) {
                ...
                if (use_unicode) {
                    ...
                    _fx_utility_16_unsigned_write(
                        work_ptr, _fx_utility_exFAT_unicode_name_hash_get((CHAR *)unicode_name, name_length));
                } else {
    ...
    if (byte_offset < total_entries * FX_DIR_ENTRY_SIZE) {
        ...
        if (media_ptr->fx_media_fault_tolerant_enabled &&
            ...
            /* Redirect this request to log file. */
            status =
                _fx_fault_tolerant_add_dir_log(media_ptr, logical_sector, changed_offset, changed_ptr, changed_size);
        } else
#endif /* FX_ENABLE_FAULT_TOLERANT */
        {
    ...
    if (media_ptr->fx_media_default_path.fx_path_directory.fx_dir_entry_name[0]) {
        ...
        if ((media_ptr->fx_media_default_path.fx_path_directory.fx_dir_entry_log_sector ==
            ...
            media_ptr->fx_media_default_path.fx_path_directory.fx_dir_entry_cluster = entry_ptr->fx_dir_entry_cluster;
        }
    }
