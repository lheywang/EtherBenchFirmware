...
UINT _fx_utility_exFAT_geometry_check(FX_MEDIA *media_ptr, UCHAR *sector_buffer) {
    ...
    /* Check FAT Size.  */
    temp = (media_ptr->fx_media_sectors_per_FAT << media_ptr->fx_media_exfat_bytes_per_sector_shift) >>
    ...
    temp = media_ptr->fx_media_reserved_sectors + media_ptr->fx_media_sectors_per_FAT;
    ...
    status = _fx_utility_exFAT_system_area_checksum_verify(
        media_ptr, sector_buffer, FX_EXFAT_FAT_MAIN_BOOT_SECTOR_OFFSET, &main_system_area_checksum);
    ...
    status = _fx_utility_exFAT_system_area_checksum_verify(
        media_ptr, sector_buffer, FX_EXFAT_FAT_BACKUP_BOOT_SECTOR_OFFSET, &backup_system_area_checksum);
    ...
    while ((counter < media_ptr->fx_media_sectors_per_cluster) && (EXFAT_NUM_OF_DIR_ENTRIES != dir_entries_found)) {
         ...
         * dir entries.  */
        while (dir_entry_ptr < (media_ptr->fx_media_memory_buffer + media_ptr->fx_media_bytes_per_sector) &&
            ...
            if (FX_EXFAT_DIR_ENTRY_TYPE_ALLOCATION_BITMAP == dir_entry_ptr[FX_EXFAT_ENTRY_TYPE]) {
                ...
                bitmap_start_sector =
                    ...
                    ((_fx_utility_32_unsigned_read(&dir_entry_ptr[FX_EXFAT_FIRST_CLUSTER]) - FX_FAT_ENTRY_START)
    ...
    if ((0 == bitmap_start_sector) || (0 == bitmap_size_in_bytes) || (0 == upcase_table_start_sector) ||
        (0 == upcase_table_checksum) || (0 == upcase_table_size_in_bytes)) {
    ...
    while (upcase_table_size_in_bytes > 0) {
        ...
        for (counter = 0; counter < temp; counter++) {
            ...
            calculated_checksum =
                ((calculated_checksum >> 1) | (calculated_checksum << 31)) + (ULONG)sector_buffer[counter];
        }
    }
