...
UINT _fx_file_close(FX_FILE *file_ptr) {
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_FILE_CLOSE, file_ptr, file_ptr->fx_file_current_file_size, 0, 0,
    ...
    if (file_ptr == file_ptr->fx_file_opened_next) {
        ...
        (file_ptr->fx_file_opened_next)->fx_file_opened_previous = file_ptr->fx_file_opened_previous;
        (file_ptr->fx_file_opened_previous)->fx_file_opened_next = file_ptr->fx_file_opened_next;
    ...
    /* Check to see if this file needs to have its directory entry written
       back to the media.  */
    if ((file_ptr->fx_file_open_mode == FX_OPEN_FOR_WRITE) && (file_ptr->fx_file_modified)) {
