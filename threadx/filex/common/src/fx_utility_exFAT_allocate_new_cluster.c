    ...
    *last_cluster = 0;
    ...
    if (!directory_ptr || (directory_ptr->fx_dir_entry_available_file_size > 0)) {
        ...
        if (directory_ptr && (directory_ptr->fx_dir_entry_dont_use_fat & 1)) {
            ...
            cluster_count =
                (ULONG)((directory_ptr->fx_dir_entry_available_file_size + bytes_per_cluster - 1) / bytes_per_cluster);
    ...
    status = _fx_utility_exFAT_bitmap_free_cluster_find(media_ptr, media_ptr->fx_media_cluster_search_start, cluster);
