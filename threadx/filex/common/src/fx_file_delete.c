...
UINT _fx_file_delete(FX_MEDIA *media_ptr, CHAR *file_name) {
    ...
    clusters_count = (ULONG)((dir_entry.fx_dir_entry_file_size + bytes_per_cluster - 1) / bytes_per_cluster - 1);
#endif /* FX_ENABLE_EXFAT */
    ...
    if (media_ptr->fx_media_fault_tolerant_enabled) {
        ...
        if (dir_entry.fx_dir_entry_dont_use_fat & 1) {
            status = _fx_fault_tolerant_set_FAT_chain(media_ptr, FX_TRUE, 0, media_ptr->fx_media_fat_last, cluster,
                                                      cluster + clusters_count);
        } else {
#endif /* FX_ENABLE_EXFAT */
            status = _fx_fault_tolerant_set_FAT_chain(media_ptr, FX_FALSE, 0, media_ptr->fx_media_fat_last, cluster,
    ...
    media_ptr->fx_media_available_clusters = media_ptr->fx_media_available_clusters + cluster_count;
