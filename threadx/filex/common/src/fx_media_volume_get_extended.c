...
UINT _fx_media_volume_get_extended(FX_MEDIA *media_ptr, CHAR *volume_name, UINT volume_name_buffer_length,
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_MEDIA_VOLUME_GET, media_ptr, volume_name, volume_source, 0, FX_TRACE_MEDIA_EVENTS,
                            0, 0)
    ...
    if (volume_source == FX_DIRECTORY_SECTOR) {
        ...
        do {
            ...
            if (((media_ptr->fx_media_FAT_type == FX_exFAT) &&
                 ((dir_entry.fx_dir_entry_attributes & FX_VOLUME) ||
                  (dir_entry.fx_dir_entry_type == FX_EXFAT_DIR_ENTRY_TYPE_VOLUME_LABEL))) ||
                ((media_ptr->fx_media_FAT_type != FX_exFAT) && (dir_entry.fx_dir_entry_attributes & FX_VOLUME) &&
        ...
        if (i < media_ptr->fx_media_root_directory_entries) {
            ...
            status =
                _fx_utility_logical_sector_read(media_ptr, (ULONG64)dir_entry.fx_dir_entry_log_sector,
    ...
    status = _fx_utility_logical_sector_read(media_ptr, ((ULONG64)0), media_ptr->fx_media_memory_buffer, ((ULONG)1),
                                             FX_DATA_SECTOR);
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_INTERNAL_IO_DRIVER_BOOT_READ, media_ptr, media_ptr->fx_media_memory_buffer, 0, 0,
