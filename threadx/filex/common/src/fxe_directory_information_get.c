...
UINT _fxe_directory_information_get(FX_MEDIA *media_ptr, CHAR *directory_name, UINT *attributes, ULONG *size,
    ...
    if ((media_ptr == FX_NULL) ||
        ((attributes == FX_NULL) && (size == FX_NULL) && (year == FX_NULL) && (month == FX_NULL) && (day == FX_NULL) &&
         (hour == FX_NULL) && (minute == FX_NULL) && (second == FX_NULL))) {
        return (FX_PTR_ERROR);
    }
    ...
    status = _fx_directory_information_get(media_ptr, directory_name, attributes, size, year, month, day, hour, minute,
                                           second);
