...
#define IS_PWR_WAKEUP_PIN_PULL(PULL)                                                                                   \
    (((PULL) == PWR_PIN_NO_PULL) || ((PULL) == PWR_PIN_PULL_UP) || ((PULL) == PWR_PIN_PULL_DOWN))

/* Check wake up flag parameter */
#define IS_PWR_WAKEUP_FLAG(FLAG)                                                                                       \
    (((FLAG) == PWR_WAKEUP_FLAG1) || ((FLAG) == PWR_WAKEUP_FLAG2) || ((FLAG) == PWR_WAKEUP_FLAG3) ||                   \
     ((FLAG) == PWR_WAKEUP_FLAG4) || ((FLAG) == PWR_WAKEUP_FLAG5) || ((FLAG) == PWR_WAKEUP_FLAG6) ||                   \
     ((FLAG) == PWR_WAKEUP_FLAG_ALL))

/* Voltage scaling range check macro */
#define IS_PWR_VOLTAGE_SCALING_RANGE(RANGE)                                                                            \
    (((RANGE) == PWR_REGULATOR_VOLTAGE_SCALE0) || ((RANGE) == PWR_REGULATOR_VOLTAGE_SCALE1) ||                         \
     ((RANGE) == PWR_REGULATOR_VOLTAGE_SCALE2) || ((RANGE) == PWR_REGULATOR_VOLTAGE_SCALE3))

/* Check PWR regulator configuration in STOP mode parameter */
#define IS_PWR_STOP_MODE_REGULATOR_VOLTAGE(VOLTAGE)                                                                    \
    (((VOLTAGE) == PWR_REGULATOR_SVOS_SCALE3) || ((VOLTAGE) == PWR_REGULATOR_SVOS_SCALE4) ||                           \
...
#define IS_PWR_MEMORY_BLOCK(BLOCK)                                                                                     \
    (((BLOCK) == PWR_ETHERNET_MEMORY_BLOCK) || ((BLOCK) == PWR_RAM3_MEMORY_BLOCK) ||                                   \
     ((BLOCK) == PWR_RAM2_16_MEMORY_BLOCK) || ((BLOCK) == PWR_RAM2_48_MEMORY_BLOCK) ||                                 \
     ((BLOCK) == PWR_RAM1_MEMORY_BLOCK))
#elif defined(PWR_PMCR_SRAM2_16LSO)
#define IS_PWR_MEMORY_BLOCK(BLOCK)                                                                                     \
    (((BLOCK) == PWR_RAM3_MEMORY_BLOCK) || ((BLOCK) == PWR_RAM2_LOW_16_MEMORY_BLOCK) ||                                \
     ((BLOCK) == PWR_RAM2_HIGH_16_MEMORY_BLOCK) || ((BLOCK) == PWR_RAM2_48_MEMORY_BLOCK) ||                            \
     ((BLOCK) == PWR_RAM1_MEMORY_BLOCK))
#else
#define IS_PWR_MEMORY_BLOCK(BLOCK) (((BLOCK) == PWR_RAM2_MEMORY_BLOCK) || ((BLOCK) == PWR_RAM1_MEMORY_BLOCK))
#endif /* PWR_PMCR_SRAM2_16SO */

/* Check wake up flag parameter */
#define IS_PWR_AVD_LEVEL(LEVEL)                                                                                        \
    (((LEVEL) == PWR_AVDLEVEL_0) || ((LEVEL) == PWR_AVDLEVEL_1) || ((LEVEL) == PWR_AVDLEVEL_2) ||                      \
