...
#define ETH_MACL4CR_MASK                                                                                               \
    (ETH_MACL3L4CR_L4PEN | ETH_MACL3L4CR_L4SPM | ETH_MACL3L4CR_L4SPIM | ETH_MACL3L4CR_L4DPM | ETH_MACL3L4CR_L4DPIM)

#define ETH_MACL3CR_MASK                                                                                               \
    (ETH_MACL3L4CR_L3PEN | ETH_MACL3L4CR_L3SAM | ETH_MACL3L4CR_L3SAIM | ETH_MACL3L4CR_L3DAM | ETH_MACL3L4CR_L3DAIM |   \
     ETH_MACL3L4CR_L3HSBM | ETH_MACL3L4CR_L3HDBM)

#define ETH_MACRXVLAN_MASK                                                                                             \
    (ETH_MACVTR_EIVLRXS | ETH_MACVTR_EIVLS | ETH_MACVTR_ERIVLT | ETH_MACVTR_EDVLP | ETH_MACVTR_VTHM |                  \
     ETH_MACVTR_EVLRXS | ETH_MACVTR_EVLS | ETH_MACVTR_DOVLTC | ETH_MACVTR_ERSVLM | ETH_MACVTR_ESVL | ETH_MACVTR_VTIM | \
     ETH_MACVTR_ETV)
...
void HAL_ETHEx_EnableARPOffload(ETH_HandleTypeDef *heth) { SET_BIT(heth->Instance->MACCR, ETH_MACCR_ARP); }
 ...
 * @retval None
 */
void HAL_ETHEx_DisableARPOffload(ETH_HandleTypeDef *heth) { CLEAR_BIT(heth->Instance->MACCR, ETH_MACCR_ARP); }
...
HAL_StatusTypeDef HAL_ETHEx_SetL4FilterConfig(ETH_HandleTypeDef *heth, uint32_t Filter,
    ...
    if (Filter == ETH_L4_FILTER_0) {
        ...
        MODIFY_REG(
            heth->Instance->MACL3L4C0R, ETH_MACL4CR_MASK,
            (pL4FilterConfig->Protocol | pL4FilterConfig->SrcPortFilterMatch | pL4FilterConfig->DestPortFilterMatch));
    ...
    } else /* Filter == ETH_L4_FILTER_1 */
    {
        ...
        MODIFY_REG(
            heth->Instance->MACL3L4C1R, ETH_MACL4CR_MASK,
            (pL4FilterConfig->Protocol | pL4FilterConfig->SrcPortFilterMatch | pL4FilterConfig->DestPortFilterMatch));
...
HAL_StatusTypeDef HAL_ETHEx_GetL4FilterConfig(const ETH_HandleTypeDef *heth, uint32_t Filter,
    ...
    if (Filter == ETH_L4_FILTER_0) {
        ...
        pL4FilterConfig->DestPortFilterMatch =
            READ_BIT(heth->Instance->MACL3L4C0R, (ETH_MACL3L4CR_L4DPM | ETH_MACL3L4CR_L4DPIM));
        pL4FilterConfig->SrcPortFilterMatch =
            READ_BIT(heth->Instance->MACL3L4C0R, (ETH_MACL3L4CR_L4SPM | ETH_MACL3L4CR_L4SPIM));
    ...
    } else /* Filter == ETH_L4_FILTER_1 */
    {
        ...
        pL4FilterConfig->DestPortFilterMatch =
            READ_BIT(heth->Instance->MACL3L4C1R, (ETH_MACL3L4CR_L4DPM | ETH_MACL3L4CR_L4DPIM));
        pL4FilterConfig->SrcPortFilterMatch =
            READ_BIT(heth->Instance->MACL3L4C1R, (ETH_MACL3L4CR_L4SPM | ETH_MACL3L4CR_L4SPIM));
    ...
    if (Filter == ETH_L3_FILTER_0) {
        ...
        MODIFY_REG(heth->Instance->MACL3L4C0R, ETH_MACL3CR_MASK,
                   (pL3FilterConfig->Protocol | pL3FilterConfig->SrcAddrFilterMatch |
                    pL3FilterConfig->DestAddrFilterMatch | (pL3FilterConfig->SrcAddrHigherBitsMatch << 6) |
...
HAL_StatusTypeDef HAL_ETHEx_GetL3FilterConfig(const ETH_HandleTypeDef *heth, uint32_t Filter,
    ...
    pL3FilterConfig->Protocol =
        READ_BIT(*((__IO uint32_t *)(&(heth->Instance->MACL3L4C0R) + Filter)), ETH_MACL3L4CR_L3PEN);
    ...
    pL3FilterConfig->SrcAddrHigherBitsMatch =
        (READ_BIT(*((__IO uint32_t *)(&(heth->Instance->MACL3L4C0R) + Filter)), ETH_MACL3L4CR_L3HSBM) >> 6);
    pL3FilterConfig->DestAddrHigherBitsMatch =
        (READ_BIT(*((__IO uint32_t *)(&(heth->Instance->MACL3L4C0R) + Filter)), ETH_MACL3L4CR_L3HDBM) >> 11);
...
HAL_StatusTypeDef HAL_ETHEx_GetRxVLANConfig(const ETH_HandleTypeDef *heth, ETH_RxVLANConfigTypeDef *pVlanConfig) {
    ...
    pVlanConfig->InnerVLANTagInStatus =
        ((READ_BIT(heth->Instance->MACVTR, ETH_MACVTR_EIVLRXS) >> 31) == 0U) ? DISABLE : ENABLE;
    ...
    pVlanConfig->DoubleVLANProcessing =
        ((READ_BIT(heth->Instance->MACVTR, ETH_MACVTR_EDVLP) >> 26) == 0U) ? DISABLE : ENABLE;
    pVlanConfig->VLANTagHashTableMatch =
        ((READ_BIT(heth->Instance->MACVTR, ETH_MACVTR_VTHM) >> 25) == 0U) ? DISABLE : ENABLE;
    pVlanConfig->VLANTagInStatus =
        ((READ_BIT(heth->Instance->MACVTR, ETH_MACVTR_EVLRXS) >> 24) == 0U) ? DISABLE : ENABLE;
    pVlanConfig->StripVLANTag = READ_BIT(heth->Instance->MACVTR, ETH_MACVTR_EVLS);
    pVlanConfig->VLANTypeCheck =
        READ_BIT(heth->Instance->MACVTR, (ETH_MACVTR_DOVLTC | ETH_MACVTR_ERSVLM | ETH_MACVTR_ESVL));
    pVlanConfig->VLANTagInverceMatch =
        ((READ_BIT(heth->Instance->MACVTR, ETH_MACVTR_VTIM) >> 17) == 0U) ? DISABLE : ENABLE;
...
HAL_StatusTypeDef HAL_ETHEx_SetRxVLANConfig(ETH_HandleTypeDef *heth, ETH_RxVLANConfigTypeDef *pVlanConfig) {
    ...
    MODIFY_REG(heth->Instance->MACVTR, ETH_MACRXVLAN_MASK,
               (((uint32_t)pVlanConfig->InnerVLANTagInStatus << 31) | pVlanConfig->StripInnerVLANTag |
                ((uint32_t)pVlanConfig->InnerVLANTag << 27) | ((uint32_t)pVlanConfig->DoubleVLANProcessing << 26) |
                ((uint32_t)pVlanConfig->VLANTagHashTableMatch << 25) | ((uint32_t)pVlanConfig->VLANTagInStatus << 24) |
...
HAL_StatusTypeDef HAL_ETHEx_GetTxVLANConfig(const ETH_HandleTypeDef *heth, uint32_t VLANTag,
    ...
    if (VLANTag == ETH_INNER_TX_VLANTAG) {
        pVlanConfig->SourceTxDesc =
            ((READ_BIT(heth->Instance->MACIVIR, ETH_MACVIR_VLTI) >> 20) == 0U) ? DISABLE : ENABLE;
        ...
        pVlanConfig->SourceTxDesc =
            ((READ_BIT(heth->Instance->MACVIR, ETH_MACVIR_VLTI) >> 20) == 0U) ? DISABLE : ENABLE;
...
HAL_StatusTypeDef HAL_ETHEx_SetTxVLANConfig(ETH_HandleTypeDef *heth, uint32_t VLANTag,
    ...
    if (VLANTag == ETH_INNER_TX_VLANTAG) {
        MODIFY_REG(heth->Instance->MACIVIR, ETH_MACTXVLAN_MASK,
                   (((uint32_t)pVlanConfig->SourceTxDesc << 20) | ((uint32_t)pVlanConfig->SVLANType << 19) |
...
void HAL_ETHEx_EnterLPIMode(ETH_HandleTypeDef *heth, FunctionalState TxAutomate, FunctionalState TxClockStop) {
    ...
    MODIFY_REG(heth->Instance->MACLCSR, (ETH_MACLCSR_LPIEN | ETH_MACLCSR_LPITXA | ETH_MACLCSR_LPITCSE),
               (((uint32_t)TxAutomate << 19) | ((uint32_t)TxClockStop << 21) | ETH_MACLCSR_LPIEN));
}
 ...
 */
uint32_t HAL_ETHEx_GetMACLPIEvent(const ETH_HandleTypeDef *heth) { return heth->MACLPIEvent; }
