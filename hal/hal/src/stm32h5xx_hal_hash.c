...
void HAL_HASH_Suspend(HASH_HandleTypeDef *hhash, uint8_t *pMemBuffer) {
    ...
    *(uint32_t *)(mem_ptr) = READ_BIT(hhash->Instance->CR, HASH_CR_DMAE | HASH_CR_DATATYPE | HASH_CR_MODE |
...
HAL_StatusTypeDef HAL_HASH_ProcessSuspend(HASH_HandleTypeDef *hhash) {
    ...
    if (__HAL_HASH_GET_FLAG(hhash, HASH_FLAG_DMAS) != RESET) {
        if (hhash->State == HAL_HASH_STATE_READY) {
            ...
            remainingwords = ((((DMA_Channel_TypeDef *)hhash->hdmain->Instance)->CBR1) & DMA_CBR1_BNDT) / 4U;
            remainingwords +=
            ...
            */
            ...
            sizeinwords = (((hhash->Size % 4U) != 0U) ? ((hhash->Size + 3U) / 4U) : (hhash->Size / 4U));
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_HMAC_Accumulate_IT(HASH_HandleTypeDef *hhash, const uint8_t *const pInBuffer,
                                              uint32_t Size) {
    HAL_StatusTypeDef status;
    uint32_t blocksize; /* Block size in bytes */
...
HAL_StatusTypeDef HAL_HASH_HMAC_Start_DMA(HASH_HandleTypeDef *hhash, const uint8_t *const pInBuffer, uint32_t Size,
    ...
    if ((hhash->hdmain->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
        ...
        status = HAL_DMA_Start_IT(
            hhash->hdmain, (uint32_t)(hhash->pHashInBuffPtr), (uint32_t)&hhash->Instance->DIN,
            ((((hhash->Size) % 4U) != 0U) ? ((hhash->Size) + (4U - ((hhash->Size) % 4U))) : ((hhash->Size))));
    }
    if (status != HAL_OK) {
        /* DMA error code field */
 ...
 */
HAL_HASH_StateTypeDef HAL_HASH_GetState(const HASH_HandleTypeDef *hhash) { return hhash->State; }
