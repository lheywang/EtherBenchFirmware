...
HAL_StatusTypeDef HAL_SAI_Init(SAI_HandleTypeDef *hsai) {
    ...
    hsai->Instance->CR1 &= ~(SAI_xCR1_MODE | SAI_xCR1_PRTCFG | SAI_xCR1_DS | SAI_xCR1_LSBFIRST | SAI_xCR1_CKSTR |
                             SAI_xCR1_SYNCEN | SAI_xCR1_MONO | SAI_xCR1_OUTDRIV | SAI_xCR1_DMAEN | SAI_xCR1_NODIV |
                             SAI_xCR1_MCKDIV | SAI_xCR1_OSR | SAI_xCR1_MCKEN);

    hsai->Instance->CR1 |=
        (hsai->Init.AudioMode | hsai->Init.Protocol | hsai->Init.DataSize | hsai->Init.FirstBit | ckstr_bits |
         syncen_bits | hsai->Init.MonoStereoMode | hsai->Init.OutputDrive | hsai->Init.NoDivider |
         (hsai->Init.Mckdiv << 20) | hsai->Init.MckOverSampling | hsai->Init.MckOutput);
    ...
    if (hsai->Instance == SAI1_Block_A) {
        ...
        if (hsai->Init.PdmInit.Activation == ENABLE) {
            ...
            SAI1->PDMCR =
                (hsai->Init.PdmInit.ClockEnable | ((hsai->Init.PdmInit.MicPairsNbr - 1U) << SAI_PDMCR_MICNBR_Pos));
            SAI1->PDMCR |= SAI_PDMCR_PDMEN;
        }
    }
    ...
    if (hsai->State == HAL_SAI_STATE_READY) {
        ...
        if ((hsai->hdmatx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
            if (hsai->hdmatx->LinkedListQueue != NULL) {
                ...
                hsai->hdmatx->LinkedListQueue->Head->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] =
            ...
            status =
                HAL_DMA_Start_IT(hsai->hdmatx, (uint32_t)hsai->pBuffPtr, (uint32_t)&hsai->Instance->DR, dmaSrcSize);
        }
    ...
    if (hsai->State == HAL_SAI_STATE_READY) {
        ...
        if ((hsai->hdmarx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
            if (hsai->hdmarx->LinkedListQueue != NULL) {
                ...
                /* Set DMA source address */
                hsai->hdmarx->LinkedListQueue->Head->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] =
                    (uint32_t)&hsai->Instance->DR;
            ...
            status =
                HAL_DMA_Start_IT(hsai->hdmarx, (uint32_t)&hsai->Instance->DR, (uint32_t)hsai->pBuffPtr, dmaSrcSize);
        }
