...
#define IS_LL_OPAMP_POWER_MODE(__POWER_MODE__)                                                                         \
    (((__POWER_MODE__) == LL_OPAMP_POWERMODE_NORMAL) || ((__POWER_MODE__) == LL_OPAMP_POWERMODE_HIGHSPEED))
...
#define IS_LL_OPAMP_FUNCTIONAL_MODE(__FUNCTIONAL_MODE__)                                                               \
    (((__FUNCTIONAL_MODE__) == LL_OPAMP_MODE_STANDALONE) || ((__FUNCTIONAL_MODE__) == LL_OPAMP_MODE_FOLLOWER) ||       \
     ((__FUNCTIONAL_MODE__) == LL_OPAMP_MODE_PGA) || ((__FUNCTIONAL_MODE__) == LL_OPAMP_MODE_PGA_IO0) ||               \
...
#define IS_LL_OPAMP_INPUT_INVERTING(__INPUT_INVERTING__)                                                               \
    (((__INPUT_INVERTING__) == LL_OPAMP_INPUT_INVERT_IO0) || ((__INPUT_INVERTING__) == LL_OPAMP_INPUT_INVERT_IO1) ||   \
...
ErrorStatus LL_OPAMP_Init(OPAMP_TypeDef *OPAMPx, const LL_OPAMP_InitTypeDef *OPAMP_InitStruct) {
    ...
    if (OPAMP_InitStruct->FunctionalMode != LL_OPAMP_MODE_FOLLOWER) {
        MODIFY_REG(OPAMPx->CSR,
                   OPAMP_CSR_OPAHSM | OPAMP_CSR_CALON | OPAMP_CSR_VMSEL | OPAMP_CSR_VPSEL | OPAMP_CSR_PGGAIN_2 |
                       OPAMP_CSR_PGGAIN_1,
                   (OPAMP_InitStruct->PowerMode & OPAMP_POWERMODE_CSR_BIT_MASK) | OPAMP_InitStruct->FunctionalMode |
                       OPAMP_InitStruct->InputNonInverting | OPAMP_InitStruct->InputInverting);
    } else {
        MODIFY_REG(OPAMPx->CSR,
                   OPAMP_CSR_OPAHSM | OPAMP_CSR_CALON | OPAMP_CSR_VMSEL | OPAMP_CSR_VPSEL | OPAMP_CSR_PGGAIN_2 |
                       OPAMP_CSR_PGGAIN_1,
                   (OPAMP_InitStruct->PowerMode & OPAMP_POWERMODE_CSR_BIT_MASK) | LL_OPAMP_MODE_FOLLOWER |
