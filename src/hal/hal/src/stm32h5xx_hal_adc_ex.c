...
#define ADC_JSQR_FIELDS                                                                                                \
    ((ADC_JSQR_JL | ADC_JSQR_JEXTSEL | ADC_JSQR_JEXTEN | ADC_JSQR_JSQ1 | ADC_JSQR_JSQ2 | ADC_JSQR_JSQ3 |               \
      ADC_JSQR_JSQ4)) /*!< ADC_JSQR fields of                                                                          \
    ...
    if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) != 0UL) {
        ...
        if (tmp_hal_status == HAL_OK) {
            ...
            if ((hadc->DMA_Handle->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
                ...
                tmp_hal_status =
                    HAL_DMA_Start_IT(hadc->DMA_Handle, (uint32_t)&tmpADC_Common->CDR, (uint32_t)pData, length_bytes);
            }
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef *hadc,
    ...
    if ((hadc->Init.ScanConvMode == ADC_SCAN_DISABLE) || (pConfigInjected->InjectedNbrOfConversion == 1U)) {
    ...
    if ((tmp_adc_is_conversion_on_going_regular == 0UL) && (tmp_adc_is_conversion_on_going_injected == 0UL)) {
        ...
        if (pConfigInjected->InjectedOffsetNumber != ADC_OFFSET_NONE) {
            ...
            LL_ADC_SetOffsetSign(hadc->Instance, pConfigInjected->InjectedOffsetNumber,
