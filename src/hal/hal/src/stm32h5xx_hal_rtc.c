    ...
    if (hrtc != NULL) {
        ...
        if (__HAL_RTC_IS_CALENDAR_INITIALIZED(hrtc) == 0U) {
            ...
            if (status == HAL_OK) {
                ...
                if (status == HAL_OK) {
#if defined(RTC_CR_OUT2EN)
                    MODIFY_REG(RTC->CR, RTC_CR_TAMPALRM_PU | RTC_CR_TAMPALRM_TYPE | RTC_CR_OUT2EN,
    ...
    if (status == HAL_OK) {
        ...
        if (READ_BIT(RTC->ICSR, RTC_ICSR_BIN) != RTC_BINARY_ONLY) {
            if (Format == RTC_FORMAT_BIN) {
                ...
                tmpreg =
                    (((uint32_t)(sTime->Hours) << RTC_TR_HU_Pos) | ((uint32_t)(sTime->Minutes) << RTC_TR_MNU_Pos) |
                     ((uint32_t)(sTime->Seconds) << RTC_TR_SU_Pos) | ((uint32_t)(sTime->TimeFormat) << RTC_TR_PM_Pos));
            }
...
HAL_StatusTypeDef HAL_RTC_SetDate(RTC_HandleTypeDef *hrtc, RTC_DateTypeDef *sDate, uint32_t Format) {
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        datetmpreg = ((((uint32_t)sDate->Year) << RTC_DR_YU_Pos) | (((uint32_t)sDate->Month) << RTC_DR_MU_Pos) |
                      (((uint32_t)sDate->Date) << RTC_DR_DU_Pos) | (((uint32_t)sDate->WeekDay) << RTC_DR_WDU_Pos));
    }
