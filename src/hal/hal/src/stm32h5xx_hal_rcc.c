    ...
    if ((pOscInitStruct->PLL.PLLState) != RCC_PLL_NONE) {
        ...
        if (temp_sysclksrc != RCC_SYSCLKSOURCE_STATUS_PLLCLK) {
            if ((pOscInitStruct->PLL.PLLState) == RCC_PLL_ON) {
                 ...
                 */
                __HAL_RCC_PLL1_CONFIG(pOscInitStruct->PLL.PLLSource, pOscInitStruct->PLL.PLLM, pOscInitStruct->PLL.PLLN,
                                      pOscInitStruct->PLL.PLLP, pOscInitStruct->PLL.PLLQ, pOscInitStruct->PLL.PLLR);
...
uint32_t HAL_RCC_GetHCLKFreq(void) {
    ...
    SystemCoreClock =
        HAL_RCC_GetSysClockFreq() >> (AHBPrescTable[(RCC->CFGR2 & RCC_CFGR2_HPRE) >> RCC_CFGR2_HPRE_Pos] & 0x1FU);
