 ...
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef HAL_FMAC_ConfigFilterOutputBuffer(FMAC_HandleTypeDef *hfmac, int16_t *pOutput,
                                                    uint16_t *pOutputSize) {
    HAL_StatusTypeDef status;
...
static HAL_StatusTypeDef FMAC_FilterConfig(FMAC_HandleTypeDef *hfmac, FMAC_FilterConfigTypeDef *pConfig,
    ...
    if (pConfig->InputThreshold != FMAC_THRESHOLD_NO_VALUE) {
        /* Check the parameter */
        assert_param(
            IS_FMAC_THRESHOLD_APPLICABLE(FMAC_GET_X1_SIZE(hfmac), pConfig->InputThreshold, pConfig->InputAccess));
    ...
    if (pConfig->OutputThreshold != FMAC_THRESHOLD_NO_VALUE) {
        /* Check the parameter */
        assert_param(
            IS_FMAC_THRESHOLD_APPLICABLE(FMAC_GET_Y_SIZE(hfmac), pConfig->OutputThreshold, pConfig->OutputAccess));
    ...
    if ((pConfig->pCoeffB != NULL) && (pConfig->CoeffBSize != 0U)) {
        ...
        assert_param(
            ((pConfig->Filter == FMAC_FUNC_CONVO_FIR) && (pConfig->pCoeffA == NULL) && (pConfig->CoeffASize == 0U)) ||
        ...
        WRITE_REG(hfmac->Instance->PARAM,
                  (((uint32_t)(pConfig->CoeffBSize) << FMAC_PARAM_P_Pos) |
                   ((uint32_t)(pConfig->CoeffASize) << FMAC_PARAM_Q_Pos) | FMAC_FUNC_LOAD_X2 | FMAC_PARAM_START));
    ...
    if ((pInput != NULL) && (InputSize != 0U)) {
        ...
        if (PreloadAccess == PRELOAD_ACCESS_POLLING) {
            ...
            if ((hfmac->hdmaPreload->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
                ...
                return (HAL_DMA_Start_IT(hfmac->hdmaPreload, (uint32_t)pInput, (uint32_t)&hfmac->Instance->WDATA,
    ...
    if ((pOutput != NULL) && (OutputSize != 0U)) {
        ...
        if (PreloadAccess == PRELOAD_ACCESS_POLLING) {
            ...
            if ((hfmac->hdmaPreload->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
                ...
                return (HAL_DMA_Start_IT(hfmac->hdmaPreload, (uint32_t)pOutput, (uint32_t)&hfmac->Instance->WDATA,
...
static void FMAC_DMAFilterConfig(DMA_HandleTypeDef *hdma) {
    ...
    if (hfmac->pInput != NULL) {
        ...
        if ((hfmac->hdmaPreload->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
            ...
            status = HAL_DMA_Start_IT(hfmac->hdmaPreload, (uint32_t)hfmac->pInput, (uint32_t)&hfmac->Instance->WDATA,
