...
UINT _nxe_sntp_client_create(
    ...
    VOID(random_number_generator)(
        struct NX_SNTP_CLIENT_STRUCT *client_ptr, ULONG *rand)) {

    UINT status;

    /* Check for invalid input pointers.  */
    if ((ip_ptr == NX_NULL) || (ip_ptr->nx_ip_id != NX_IP_ID) ||
        (client_ptr == NX_NULL) || (packet_pool_ptr == NX_NULL)) {
...
UINT _nx_sntp_client_create(
    ...
    if (packet_pool_ptr->nx_packet_pool_payload_size <
        (sizeof(NX_IPV4_HEADER) + sizeof(NX_UDP_HEADER) +
    ...
    /* This only assures IPv4 packets will work. We do not know yet if the Client expects
       to receive SNTP packets over ipv6 networks. When sending packets, the SNTP Client
       will have set its SNTP server from which the IP type will be determined.  However,
       for receiving packets, it is up to the host to ensure its packet payload will be
       large enough for IPv6 packets:

             sizeof(NX_IPV6_HEADER) + sizeof(NX_UDP_HEADER) +
       NX_SNTP_CLIENT_PACKET_DATA_SIZE) 40 bytes                   8 bytes 48 bytes
    */
    ...
    status = nx_udp_socket_create(
        client_ptr->nx_sntp_client_ip_ptr,
        &(client_ptr->nx_sntp_client_udp_socket),
    ...
    status = nx_udp_socket_receive_notify(
        &(client_ptr->nx_sntp_client_udp_socket), _nx_sntp_client_receive_notify);
    ...
    status = tx_timer_create(
        &client_ptr->nx_sntp_update_timer,
        "SNTP Client Update Timer",
        _nx_sntp_client_update_timeout_entry,
        (ULONG)client_ptr,
    ...
    status = tx_mutex_create(
        &(client_ptr->nx_sntp_client_mutex), "NetX SNTP Client Mutex", TX_NO_INHERIT);
    ...
    status = tx_thread_create(
        &(client_ptr->nx_sntp_client_thread),
        "NetX SNTP Client",
        _nx_sntp_client_thread_entry,
        (ULONG)client_ptr,
        client_ptr->nx_sntp_client_thread_stack,
...
VOID _nx_sntp_client_update_timeout_entry(ULONG info) {
     ...
     */
    if (client_ptr->nx_sntp_update_time_remaining >=
        (NX_IP_PERIODIC_RATE * NX_SNTP_UPDATE_TIMEOUT_INTERVAL)) {

        /* Yes; ok to decrement time remaining.  */
        client_ptr->nx_sntp_update_time_remaining -=
            (NX_IP_PERIODIC_RATE * NX_SNTP_UPDATE_TIMEOUT_INTERVAL);
    } else {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_create_time_request_packet(
    NX_SNTP_CLIENT *client_ptr,
    NX_PACKET *packet_ptr,
    NX_SNTP_TIME_MESSAGE *time_message_ptr) {
    NX_PARAMETER_NOT_USED(client_ptr);

    if (40u > ((ULONG)(packet_ptr->nx_packet_data_end) -
    ...
    *((ULONG *)(packet_ptr->nx_packet_append_ptr)) =
        (ULONG)(time_message_ptr->transmit_time.seconds);
    *((ULONG *)(packet_ptr->nx_packet_append_ptr + 4)) =
        (ULONG)(time_message_ptr->transmit_time.fraction);
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_sntp_client_initialize_unicast(
    NX_SNTP_CLIENT *client_ptr, NXD_ADDRESS *unicast_time_server) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_sntp_client_initialize_unicast(
    NX_SNTP_CLIENT *client_ptr, NXD_ADDRESS *unicast_time_server) {
    ...
    client_ptr->nx_sntp_unicast_time_server.nxd_ip_address.v4 =
        unicast_time_server->nxd_ip_address.v4;

    /* Set as the Client's current SNTP server.  */
    client_ptr->nx_sntp_server_ip_address.nxd_ip_version = NX_IP_VERSION_V4;
    client_ptr->nx_sntp_server_ip_address.nxd_ip_address.v4 =
        unicast_time_server->nxd_ip_address.v4;
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_sntp_client_initialize_unicast(
    NX_SNTP_CLIENT *client_ptr, ULONG unicast_time_server) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_initialize_unicast(
    NX_SNTP_CLIENT *client_ptr, ULONG unicast_time_server) {
...
UINT _nx_sntp_client_run_unicast(NX_SNTP_CLIENT *client_ptr) {
    ...
    if ((NX_SNTP_CLIENT_RANDOMIZE_ON_STARTUP == NX_TRUE) &&
        (client_ptr->random_number_generator)) {
    ...
    client_ptr->nx_sntp_update_time_remaining =
        (NX_SNTP_CLIENT_MAX_TIME_LAPSE * NX_IP_PERIODIC_RATE);
...
VOID _nx_sntp_client_process_unicast(NX_SNTP_CLIENT *client_ptr) {
    ...
    if (status == NX_SUCCESS) {
        ...
        if (status == NX_SUCCESS) {
            ...
            /* Indicate the client has received at least one valid time update with the
             * current server. */
    ...
    if (client_ptr->nx_sntp_client_unicast_poll_interval == 0) {
        ...
        /* Save the last server message before the next update.  We need this for
         * comparing time data. */
        ...
        if ((NX_SNTP_CLIENT_MAX_TIME_LAPSE * NX_IP_PERIODIC_RATE -
             client_ptr->nx_sntp_update_time_remaining) >
            (NX_SNTP_CLIENT_UNICAST_POLL_INTERVAL * NX_IP_PERIODIC_RATE)) {

            /* No we have not. Increase the count of times we've increased the back off
             * rate. */
            client_ptr->nx_sntp_client_backoff_count++;

            client_ptr->nx_sntp_client_unicast_poll_interval =
                NX_IP_PERIODIC_RATE * NX_SNTP_CLIENT_UNICAST_POLL_INTERVAL *
        ...
        /* Check that the poll interval does not exceed the maximum lapse without a valid
         * update. */
        if (client_ptr->nx_sntp_client_unicast_poll_interval >
            (NX_SNTP_CLIENT_MAX_TIME_LAPSE * NX_IP_PERIODIC_RATE)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxde_sntp_client_initialize_broadcast(
    NX_SNTP_CLIENT *client_ptr,
    ...
    if ((broadcast_server_address != NX_NULL) &&
        (broadcast_server_address->nxd_ip_version == NX_IP_VERSION_V6)) {
    ...
    status = _nxd_sntp_client_initialize_broadcast(
        client_ptr, multicast_server_address, broadcast_server_address);
...
/*                                                                        */
/**************************************************************************/
UINT _nxd_sntp_client_initialize_broadcast(
    NX_SNTP_CLIENT *client_ptr,
    NXD_ADDRESS *multicast_server_address,
    NXD_ADDRESS *broadcast_server_address) {
    ...
    if (multicast_server_address != NX_NULL) {
        ...
        COPY_NXD_ADDRESS(
            multicast_server_address, &client_ptr->nx_sntp_multicast_server_address);

        /* Set this as the Client's current SNTP server. */
        COPY_NXD_ADDRESS(
            multicast_server_address, &client_ptr->nx_sntp_server_ip_address);
#else
        /* Set the Client multicast server.  */
        client_ptr->nx_sntp_multicast_server_address.nxd_ip_version = NX_IP_VERSION_V4;
        client_ptr->nx_sntp_multicast_server_address.nxd_ip_address.v4 =
            multicast_server_address->nxd_ip_address.v4;
    ...
    /* No multicast address supplied, so was a broadcast address specified?  This must be
       an IPv4 address since IPv6 does not support broadcast. */
    else if (
        (broadcast_server_address != NX_NULL) &&
        (broadcast_server_address->nxd_ip_version == NX_IP_VERSION_V4)) {

#ifdef FEATURE_NX_IPV6
        /* Set the Client broadcast server.  */
        COPY_NXD_ADDRESS(
            broadcast_server_address, &client_ptr->nx_sntp_broadcast_time_server);

        /* Set this as the Client's current SNTP server. */
        COPY_NXD_ADDRESS(
            broadcast_server_address, &client_ptr->nx_sntp_server_ip_address);
#else
        /* Set the Client broadcast server.  */
        client_ptr->nx_sntp_broadcast_time_server.nxd_ip_version = NX_IP_VERSION_V4;
        client_ptr->nx_sntp_broadcast_time_server.nxd_ip_address.v4 =
            broadcast_server_address->nxd_ip_address.v4;
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_sntp_client_initialize_broadcast(
    NX_SNTP_CLIENT *client_ptr,
    ULONG multicast_server_address,
    ULONG broadcast_server_address) {
    ...
    /* Call the actual initialize client for broadcast time update service.  */
    status = _nx_sntp_client_initialize_broadcast(
        client_ptr, multicast_server_address, broadcast_server_address);
...
UINT _nx_sntp_client_initialize_broadcast(
    ...
    if (multicast_server_address != 0x0) {
        ...
        status =
            _nxd_sntp_client_initialize_broadcast(client_ptr, &server_address, NX_NULL);
    }
    ...
    /* Was a broadcast SNTP server specified?  */
        ...
        server_address.nxd_ip_version = NX_IP_VERSION_V4;
        status =
            _nxd_sntp_client_initialize_broadcast(client_ptr, NX_NULL, &server_address);
    } else
        return NX_INVALID_PARAMETERS;
...
UINT _nx_sntp_client_stop(NX_SNTP_CLIENT *client_ptr) {
    ...
    memset(
        ...
        0,
        sizeof(NX_SNTP_TIME_MESSAGE));

    /* Get rid of the current server message if there is one.  */
    memset(
        &(client_ptr->nx_sntp_current_server_time_message),
        0,
        sizeof(NX_SNTP_TIME_MESSAGE));
    ...
    if (status == NX_SUCCESS) {
        ...
        if (status == NX_SUCCESS) {
            ...
            if (status != NX_SUCCESS) {
                ...
                /* Have we exceeded the max number of invalid or missed updates from this
                 * server? */
                if (client_ptr->nx_sntp_client_invalid_time_updates >
            ...
            /* Reset the Client timeout for receiving broadcast updates, zero out invalid
             * update count.  */
            client_ptr->nx_sntp_update_time_remaining =
                (NX_SNTP_CLIENT_MAX_TIME_LAPSE * NX_IP_PERIODIC_RATE);
...
UINT _nx_sntp_client_send_unicast_request(NX_SNTP_CLIENT *client_ptr) {
    ...
    status = nx_packet_allocate(
        client_ptr->nx_sntp_client_packet_pool_ptr,
        &packet_ptr,
        NX_UDP_PACKET,
    ...
    _nx_sntp_client_utility_convert_time_to_UCHAR(
        &local_time, unicast_request, TRANSMIT_TIME);

    /* Create the request packet with the unicast request message.  */
    status = _nx_sntp_client_create_time_request_packet(
        client_ptr, packet_ptr, unicast_request);
    ...
    status = nxd_udp_socket_send(
        &(client_ptr->nx_sntp_client_udp_socket),
        packet_ptr,
        &client_ptr->nx_sntp_server_ip_address,
...
UINT _nx_sntp_client_receive_time_update(NX_SNTP_CLIENT *client_ptr, ULONG timeout) {
    ...
    for (;;) {
        ...
        /* Indicate the SNTP Client can be stopped while the SNTP client waits for a
         * packet.  */
        ...
        udp_header_ptr = (NX_UDP_HEADER *)(receive_packet->nx_packet_prepend_ptr -
                                           sizeof(NX_UDP_HEADER));
        ...
        /* If this an IPv6 packet? An SNTP client with an IPv6 SNTP server will only
         * accept IPv6 packets.   */
        if ((receive_packet->nx_packet_ip_version == NX_IP_VERSION_V6) &&
            ...
            ipv6_header_ptr =
                (NX_IPV6_HEADER *)(receive_packet->nx_packet_prepend_ptr -
                                   sizeof(NX_UDP_HEADER) - sizeof(NX_IPV6_HEADER));

            /* Copy sender address into a local variable. */
            COPY_IPV6_ADDRESS(
                ipv6_header_ptr->nx_ip_header_source_ip,
                &(source_ip_address.nxd_ip_address.v6[0]));
            source_ip_address.nxd_ip_version = NX_IP_VERSION_V6;

            /* Copy destination address into a local variable. */
            COPY_IPV6_ADDRESS(
                ipv6_header_ptr->nx_ip_header_destination_ip,
                &(destination_ip_address.nxd_ip_address.v6[0]));
            ...
            if (client_ptr->nx_sntp_client_protocol_mode == UNICAST_MODE) {
                ...
                if (!CHECK_IPV6_ADDRESSES_SAME(
                        &client_ptr->nx_sntp_server_ip_address.nxd_ip_address.v6[0],
                        &source_ip_address.nxd_ip_address.v6[0])) {

                    /* No further need for the receive packet. Release back to the client
                     * pool.  */
                    nx_packet_release(receive_packet);
            ...
            } else /* Multicast mode */
            {
                ...
                /* Does the destination address match the expected server address (all
                 * hosts address FF02::1)?  */
                if (!CHECK_IPV6_ADDRESSES_SAME(
                        &destination_ip_address.nxd_ip_address.v6[0],
                        &All_Hosts_Multicast.nxd_ip_address.v6[0])) {
        ...
        /* Otherwise is this an IPv4 packet and is the SNTP client set for IPv4? */
            ...
            (receive_packet->nx_packet_ip_version == NX_IP_VERSION_V4) &&
            (client_ptr->nx_sntp_server_ip_address.nxd_ip_version == NX_IP_VERSION_V4)) {
#ifdef NX_DISABLE_IPV4
            nx_packet_release(receive_packet);
            continue;
            ...
            ipv4_header_ptr =
                (NX_IPV4_HEADER *)(receive_packet->nx_packet_prepend_ptr -
            ...
            /* Copy into a local variable. */
            destination_ip_address.nxd_ip_address.v4 =
                ipv4_header_ptr->nx_ip_header_destination_ip;
            destination_ip_address.nxd_ip_version = NX_IP_VERSION_V4;

            if (client_ptr->nx_sntp_client_protocol_mode == UNICAST_MODE) {
                /* Compare the sender address with the Client's current sntp server. */
                if (source_ip_address.nxd_ip_address.v4 !=
                    client_ptr->nx_sntp_server_ip_address.nxd_ip_address.v4) {
                    /* This is an untrusted server or the client had a broadcast server
                       already, reject this packet and return an error condition.  */
            ...
            } else /* BROADCAST_MODE */
            {
                ...
                network_mask =
                    client_ptr->nx_sntp_client_ip_ptr->nx_ip_interface[iface_index]
                        .nx_interface_ip_network_mask;

                /* Now verify this is a broadcast packet. */
                if ((destination_ip_address.nxd_ip_address.v4 & ~network_mask) !=
                    ~network_mask) {
        ...
        if ((length < NX_SNTP_TIME_MESSAGE_MIN_SIZE) ||
            (length > NX_SNTP_TIME_MESSAGE_MAX_SIZE)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_extract_time_message_from_packet(
    NX_SNTP_CLIENT *client_ptr, NX_PACKET *packet_ptr) {
    ...
    ntp_word_0++;
    memcpy(
        time_message_ptr->reference_clock_id,
        ntp_word_0,
        4); /* Use case of memcpy is verified. */

    /* Advance to the next field (64 bit field) and extract the reference time stamp
     * field.  */
    ntp_word_0++;
    NX_CHANGE_ULONG_ENDIAN(*ntp_word_0);
    ...
    if (_nx_sntp_client_utility_is_zero_data(
            (UCHAR *)(time_message_ptr->reference_clock_update_time_stamp), 8) ==
        NX_FALSE) {

        time_message_ptr->reference_clock_update_time.seconds =
            time_message_ptr->reference_clock_update_time_stamp[0];

        time_message_ptr->reference_clock_update_time.fraction =
            time_message_ptr->reference_clock_update_time_stamp[1];
    }
    ...
    if (_nx_sntp_client_utility_is_zero_data(
            (UCHAR *)(time_message_ptr->originate_time_stamp), 8) == NX_FALSE) {

        time_message_ptr->originate_time.seconds =
            time_message_ptr->originate_time_stamp[0];

        time_message_ptr->originate_time.fraction =
            time_message_ptr->originate_time_stamp[1];
    }
    ...
    if (_nx_sntp_client_utility_is_zero_data(
            (UCHAR *)(time_message_ptr->receive_time_stamp), 8) == NX_FALSE) {
    ...
    if (_nx_sntp_client_utility_is_zero_data(
            (UCHAR *)(time_message_ptr->transmit_time_stamp), 8) == NX_FALSE) {

        time_message_ptr->transmit_time.seconds =
            time_message_ptr->transmit_time_stamp[0];

        time_message_ptr->transmit_time.fraction =
            time_message_ptr->transmit_time_stamp[1];
    }
...
UINT _nx_sntp_client_reset_current_time_message(NX_SNTP_CLIENT *client_ptr) {
    ...
    if (client_ptr->nx_sntp_current_server_time_message.flags) {
        ...
        memcpy(
            &client_ptr->nx_sntp_previous_server_time_message,
            &client_ptr->nx_sntp_current_server_time_message,
            sizeof(NX_SNTP_TIME_MESSAGE)); /* Use case of memcpy is verified. */
    ...
    memset(
        &(client_ptr->nx_sntp_current_time_message_request),
        0,
        sizeof(NX_SNTP_TIME_MESSAGE));
...
UINT _nx_sntp_client_process_update_packet(NX_SNTP_CLIENT *client_ptr) {
    ...
    if (client_ptr->nx_sntp_client_time_update_notify) {
        ...
        client_ptr->nx_sntp_client_time_update_notify(
            &(client_ptr->nx_sntp_current_server_time_message),
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_duplicate_update_check(
    NX_SNTP_TIME_MESSAGE *timeA_msg_ptr,
    ...
    *is_a_dupe = NX_FALSE;
    ...
    if ((!memcmp(
            timeA_msg_ptr->transmit_time_stamp, timeB_msg_ptr->transmit_time_stamp, 8)) &&
        (!memcmp(
            timeA_msg_ptr->receive_time_stamp, timeB_msg_ptr->receive_time_stamp, 8)) &&
        (!memcmp(
            timeA_msg_ptr->originate_time_stamp,
            timeB_msg_ptr->originate_time_stamp,
            8)) &&
        (!memcmp(
            timeA_msg_ptr->reference_clock_update_time_stamp,
            timeB_msg_ptr->reference_clock_update_time_stamp,
            8)) &&
        (!memcmp(
            timeA_msg_ptr->reference_clock_id, timeB_msg_ptr->reference_clock_id, 4)) &&
...
UINT _nx_sntp_client_apply_sanity_checks(NX_SNTP_CLIENT *client_ptr) {
    ...
    if (client_ptr->nx_sntp_previous_server_time_message.flags) {
        ...
        status = _nx_sntp_client_duplicate_update_check(
            &(client_ptr->nx_sntp_current_server_time_message),
            &(client_ptr->nx_sntp_previous_server_time_message),
            &is_a_dupe_check);
        /* Check for error.  */
    ...
    if ((mode != PROTOCOL_MODE_SERVER_UNICAST) &&
        (client_ptr->nx_sntp_client_protocol_mode == UNICAST_MODE)) {
    ...
    /* Is the server of the correct association for a broadcast client?  */
        ...
        (mode != PROTOCOL_MODE_SERVER_BROADCAST) &&
        (client_ptr->nx_sntp_client_protocol_mode == BROADCAST_MODE)) {
    ...
    if ((server_ntp_version != NX_SNTP_CLIENT_NTP_VERSION) &&
        (client_ptr->nx_sntp_client_protocol_mode == UNICAST_MODE)) {
    ...
    /* If broadcast, verify the server SNTP version is compatible with the Client.  */
        ...
        (server_ntp_version < NX_SNTP_CLIENT_MIN_NTP_VERSION) &&
        (client_ptr->nx_sntp_client_protocol_mode == BROADCAST_MODE)) {
    ...
    if (server_time_msg_ptr->peer_clock_stratum == 0) {
        ...
        if (client_ptr->kiss_of_death_handler) {
            ...
            _nx_sntp_client_utility_convert_refID_KOD_code(
                server_time_msg_ptr->reference_clock_id, &code_id);
    ...
    /* Does the server stratum acceptable to the Client?  */
        ...
        server_time_msg_ptr->peer_clock_stratum > NX_SNTP_CLIENT_MIN_SERVER_STRATUM) {
    ...
    if (client_ptr->nx_sntp_client_protocol_mode == UNICAST_MODE) {
        ...
        /* Special case: the client has not set its local time. Only check server's
         * receive and transmit time. */
        if (client_ptr->nx_sntp_client_local_ntp_time.seconds == 0) {
            if ((_nx_sntp_client_utility_is_zero_data(
                     (UCHAR *)(server_time_msg_ptr->receive_time_stamp), 8) == NX_TRUE) ||
                (_nx_sntp_client_utility_is_zero_data(
                     (UCHAR *)(server_time_msg_ptr->transmit_time_stamp), 8) ==
                 NX_TRUE)) {
            ...
            if ((_nx_sntp_client_utility_is_zero_data(
                     (UCHAR *)(server_time_msg_ptr->originate_time_stamp), 8) ==
                 NX_TRUE) ||
                (_nx_sntp_client_utility_is_zero_data(
                     (UCHAR *)(server_time_msg_ptr->receive_time_stamp), 8) == NX_TRUE) ||
                (_nx_sntp_client_utility_is_zero_data(
        ...
        status = _nx_sntp_client_utility_get_msec_diff(
            &(server_time_msg_ptr->receive_time),
    ...
    /* Client is in broadcast mode.  */
        ...
        if (_nx_sntp_client_utility_is_zero_data(
                (UCHAR *)(server_time_msg_ptr->transmit_time_stamp), 8) == NX_TRUE)
...
UINT _nx_sntp_client_check_server_clock_dispersion(NX_SNTP_CLIENT *client_ptr) {
    ...
    dispersion_usecs =
        (client_ptr->nx_sntp_current_server_time_message.clock_dispersion >> 16) *
        1000000;

    /* Compare seconds in the most significant 16 bits against the Client dispersion
     * tolerance.  */
    ...
    /* Compare fraction from least significant 16 bits; sum of (1 / 2 ^ bit field) of
     * lower 16 bits.  */
...
VOID _nx_sntp_client_receive_notify(NX_UDP_SOCKET *socket_ptr) {
    ...
    /* Wakeup all threads that are attempting to perform a receive or that had their
     * select satisfied.  */
...
UINT _nx_sntp_client_process_time_data(NX_SNTP_CLIENT *client_ptr) {
    ...
    memcpy(
        &client_ptr->nx_sntp_server_update_time,
        &client_ptr->nx_sntp_current_server_time_message.transmit_time,
        sizeof(NX_SNTP_TIME)); /* Use case of memcpy is verified. */
    ...
    if (NX_SNTP_CLIENT_RTT_REQUIRED == NX_TRUE) {
        ...
        _nx_sntp_client_calculate_roundtrip(
            &(client_ptr->nx_sntp_client_roundtrip_time_msec));
        ...
        if (client_ptr->nx_sntp_client_roundtrip_time_msec) {
            ...
            status = _nx_sntp_client_utility_add_msecs_to_ntp_time(
                &(client_ptr->nx_sntp_current_server_time_message.transmit_time),
    ...
    /* If not ignoring the max adjustment, deal with time difference between client and
     * server. */
    if (ignore_max_adjustment_limit == NX_FALSE) {
        UINT pos_diff = NX_TRUE;

        /* Compute difference of server update packet minus the client's local time. It is
           reasonable to assume that the Client time is 'behind' the server time because
           it is updated by the server time.  */
        local_time.seconds = client_ptr->nx_sntp_client_local_ntp_time.seconds +
                             client_ptr->nx_sntp_client_local_ntp_time_elapsed;
        local_time.fraction = client_ptr->nx_sntp_client_local_ntp_time.fraction;
        status = _nx_sntp_client_utility_get_msec_diff(
        ...
        if (elapsed_msecs_difference > NX_SNTP_CLIENT_MIN_TIME_ADJUSTMENT) {
            ...
            /* Set the local clock to the server time only if 1)we are ignoring the
               maximum adjustment on startup, or 2) the difference is within the Client's
               max time adjustment.  */
    ...
    /* Apply a correction to server's time for internal SNTP Client delays e.g. periodic
     * task intervals. */
    adjustment = ((process_timerticks - receive_timerticks) * 1000) / NX_IP_PERIODIC_RATE;

    status = _nx_sntp_client_utility_add_msecs_to_ntp_time(
        &(client_ptr->nx_sntp_client_local_ntp_time), (LONG)adjustment);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_sntp_client_get_local_time(
    NX_SNTP_CLIENT *client_ptr, ULONG *seconds, ULONG *fraction, CHAR *buffer) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_get_local_time(
    NX_SNTP_CLIENT *client_ptr, ULONG *seconds, ULONG *fraction, CHAR *buffer) {

    UINT status;

    status = _nx_sntp_client_get_local_time_extended(
        client_ptr, seconds, fraction, buffer, NX_MAX_STRING_LENGTH);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_sntp_client_get_local_time_extended(
    NX_SNTP_CLIENT *client_ptr,
    ULONG *seconds,
    ULONG *fraction,
    CHAR *buffer,
    ...
    /* Call the actual service.  */
    status = _nx_sntp_client_get_local_time_extended(
        client_ptr, seconds, fraction, buffer, buffer_size);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_get_local_time_extended(
    NX_SNTP_CLIENT *client_ptr,
    ULONG *seconds,
    ULONG *fraction,
    CHAR *buffer,
    ...
    *fraction = client_ptr->nx_sntp_client_local_ntp_time.fraction;
    ...
    if (buffer != NX_NULL) {
        ...
        _nx_sntp_client_utility_fraction_to_usecs(
            client_ptr->nx_sntp_client_local_ntp_time.fraction, &usecs);
        ...
        buffer[offset++] = ':';
        buffer[offset++] = ' ';
        length = _nx_utility_uint_to_string(
        ...
        buffer[offset++] = '.';
        length =
            _nx_utility_uint_to_string(usecs, 10, &buffer[offset], buffer_size - offset);
        if (length == 0) {
            return (NX_SIZE_ERROR);
        }
        ...
        if (length < 6) {
            ...
            memmove(
                ...
                &buffer[offset],
                length); /* Use case of memmove is verified.  */
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_utility_convert_time_to_UCHAR(
    NX_SNTP_TIME *time_ptr, NX_SNTP_TIME_MESSAGE *time_message_ptr, UINT which_stamp) {
...
/* The conventional civil timescale used in most parts of the world is based on
   Coordinated Universal Time (UTC), which replaced Greenwich Mean Time (GMT) many years
   ago. UTC is based on International Atomic Time (TAI), which is derived from hundreds of
   cesium oscillators in the national standards laboratories of many regions. Deviations
   of UTC from TAI are implemented in the form of leap seconds, which occur at intervals
   from a few months to serveral years. */
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_utility_convert_seconds_to_date(
    NX_SNTP_TIME *current_NTP_time_ptr,
    UINT current_year,
    ...
    do {
        ...
        /* Compute number of seconds into the current year e.g. as of 01/01 at midnite by
           subtracting the total number of seconds since 1/1/1999 up to the end of the
           previous year.  Remember to compute the leapyear seconds at a different rate.
         */
        seconds_into_currentyear =
            seconds_diff - ((years_diff - leapyears_diff) * SECONDS_PER_NONLEAPYEAR) -
    ...
    _nx_sntp_client_utility_convert_fraction_to_msecs(
        (ULONG *)(&(current_date_time_ptr->millisecond)), current_NTP_time_ptr);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_sntp_client_utility_display_date_time(
    NX_SNTP_CLIENT *client_ptr, CHAR *buffer, UINT length) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_utility_display_date_time(
    NX_SNTP_CLIENT *client_ptr, CHAR *buffer, UINT length) {
    ...
    status = _nx_sntp_client_utility_convert_seconds_to_date(
        &(client_ptr->nx_sntp_client_local_ntp_time), NX_SNTP_CURRENT_YEAR, &DisplayTime);
    if (status != NX_SUCCESS) {
        return status;
    }
    ...
    return_length =
        _nx_utility_uint_to_string(DisplayTime.day, 10, &buffer[offset], length - offset);
    ...
    buffer[offset++] = ',';
    buffer[offset++] = ' ';
    return_length = _nx_utility_uint_to_string(
        DisplayTime.year, 10, &buffer[offset], length - offset);
    offset += return_length;
    ...
    buffer[offset++] = ' ';
    return_length = _nx_utility_uint_to_string(
        DisplayTime.hour, 10, &buffer[offset], length - offset);
    offset += return_length;
    if ((return_length == 0) || ((length - offset) < 1)) {
        return NX_SNTP_ERROR_CONVERTING_DATETIME;
    }
    buffer[offset++] = ':';
    return_length = _nx_utility_uint_to_string(
        DisplayTime.minute, 10, &buffer[offset], length - offset);
    offset += return_length;
    if ((return_length == 0) || ((length - offset) < 1)) {
        return NX_SNTP_ERROR_CONVERTING_DATETIME;
    }
    buffer[offset++] = ':';
    return_length = _nx_utility_uint_to_string(
        DisplayTime.second, 10, &buffer[offset], length - offset);
    offset += return_length;
    if ((return_length == 0) || ((length - offset) < 1)) {
        return NX_SNTP_ERROR_CONVERTING_DATETIME;
    }
    buffer[offset++] = '.';
    return_length = _nx_utility_uint_to_string(
        DisplayTime.millisecond, 10, &buffer[offset], length - offset);
    offset += return_length;
...
UINT _nx_sntp_client_utility_add_msecs_to_ntp_time(
    NX_SNTP_TIME *timeA_ptr, LONG msecs_to_add) {
    ...
    if (msecs_to_add > 0) {
        ...
        status = _nx_sntp_client_utility_addition_overflow_check(
    ...
    /* Check if a negative number larger than the timeA operand is being added (creates
       negative time!)*/
    ...
    status =
        _nx_sntp_client_utility_usecs_to_fraction((ULONG)usecs, &(timeA_ptr->fraction));
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_sntp_client_receiving_updates(
    NX_SNTP_CLIENT *client_ptr, UINT *receive_status) {
...
UINT _nx_sntp_client_receiving_updates(NX_SNTP_CLIENT *client_ptr, UINT *receive_status) {
    ...
    /* Verify the client's SNTP server is valid, and the Client has received at least one
     * valid udpate from it. */
    *receive_status =
        ((client_ptr->nx_sntp_valid_server_status == NX_TRUE) &&
         (client_ptr->nx_sntp_client_first_update_pending == NX_FALSE));
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_sntp_client_set_local_time(
    NX_SNTP_CLIENT *client_ptr, ULONG seconds, ULONG fraction) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_set_local_time(
    NX_SNTP_CLIENT *client_ptr, ULONG seconds, ULONG fraction) {
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_sntp_client_set_time_update_notify(
    NX_SNTP_CLIENT *client_ptr,
    VOID(time_update_cb)(
        NX_SNTP_TIME_MESSAGE *time_update_ptr, NX_SNTP_TIME *local_time)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_set_time_update_notify(
    NX_SNTP_CLIENT *client_ptr,
    VOID(time_update_cb)(
        NX_SNTP_TIME_MESSAGE *time_update_ptr, NX_SNTP_TIME *local_time)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_utility_get_msec_diff(
    NX_SNTP_TIME *timeReceived_ptr,
    NX_SNTP_TIME *timeTransmit_ptr,
    ULONG *total_difference_msecs,
    UINT *pos_diff) {
    ...
    if (timeReceived_ptr->seconds == timeTransmit_ptr->seconds) {
        ...
        if (timeTransmit_ptr->seconds > timeReceived_ptr->seconds) {
            ...
            if (usecsTransmit >= usecsReceived) {
                ...
                *total_difference_msecs =
                    seconds_difference_in_msecs + (msecsTransmit - msecsReceived);
            } else /* (usecsReceived > usecsTransmit) */
            {

                /* This will subtract from the total milliseconds' difference . */
                *total_difference_msecs =
                    seconds_difference_in_msecs - (msecsReceived - msecsTransmit);
            }
        }
...
/*                                                                        */
/**************************************************************************/
UINT _nx_sntp_client_utility_convert_fraction_to_msecs(
    ULONG *milliseconds, NX_SNTP_TIME *time_ptr) {
...
UINT _nx_sntp_client_utility_convert_refID_KOD_code(UCHAR *reference_id, UINT *code_id) {
    ...
    /* Compare the reference_id to each known reference_id and if it matches return the
     * reference_id ID.  */
