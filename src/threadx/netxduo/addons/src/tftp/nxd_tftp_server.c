...
UINT _nxde_tftp_server_create(
    ...
    /* Determine if the packet payload is large enough (must include 512 bytes plus packet
     * headers).  */
    ...
    status = _nxd_tftp_server_create(
        tftp_server_ptr,
        tftp_server_name,
        ip_ptr,
        media_ptr,
...
UINT _nxd_tftp_server_create(
    ...
    nx_udp_socket_receive_notify(
        &(tftp_server_ptr->nx_tftp_server_socket), _nx_tftp_server_data_present);
    ...
    status = nx_udp_socket_bind(
        &(tftp_server_ptr->nx_tftp_server_socket), NX_TFTP_SERVER_PORT, NX_NO_WAIT);
    ...
    status = tx_thread_create(
        &(tftp_server_ptr->nx_tftp_server_thread),
        "TFTP Server Thread",
        _nx_tftp_server_thread_entry,
        (ULONG)tftp_server_ptr,
        stack_ptr,
        stack_size,
        NX_TFTP_SERVER_PRIORITY,
        NX_TFTP_SERVER_PRIORITY,
        NX_TFTP_SERVER_TIME_SLICE,
        TX_DONT_START);
    ...
    /* Create the ThreadX event flags.  These will be used to drive the TFTP server
     * thread.  */
    status = tx_event_flags_create(
        &(tftp_server_ptr->nx_tftp_server_event_flags), "TFTP Server Thread Events");
    ...
    /* Create the ThreadX retransmit timeout timer.  This will be used to retransmit TFTP
       server packets if the Client has not responded or sends a duplicate (old) ACK or
       data packet.  */
    status = tx_timer_create(
        &(tftp_server_ptr->nx_tftp_server_timer),
        "TFTP Server Timer",
        _nx_tftp_server_timer_entry,
        (ULONG)tftp_server_ptr,
...
UINT _nxd_tftp_server_delete(NX_TFTP_SERVER *tftp_server_ptr) {
    ...
    while (i < NX_TFTP_MAX_CLIENTS) {
        ...
        if (client_request_ptr->nx_tftp_client_request_ip_address.nxd_ip_version ==
            NX_IP_VERSION_V4) {

#ifndef NX_DISABLE_IPV4
            /* This is an IPv4 client. Is this slot empty? */
            if ((client_request_ptr->nx_tftp_client_request_port != 0) &&
                (client_request_ptr->nx_tftp_client_request_ip_address.nxd_ip_address
                     .v4) != 0) {
            ...
            client_request_ptr->nx_tftp_client_request_ip_address.nxd_ip_version ==
            NX_IP_VERSION_V6) {
...
VOID _nx_tftp_server_data_present(NX_UDP_SOCKET *socket_ptr) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_tftp_server_event_flags), NX_TFTP_SERVER_RECEIVE_EVENT, TX_OR);
}
...
VOID _nx_tftp_server_timer_entry(ULONG tftp_server_address) {
    ...
    /* Set the data event flag.  */
    tx_event_flags_set(
        &(server_ptr->nx_tftp_server_event_flags), NX_TFTP_SERVER_TIMER_EVENT, TX_OR);
    ...
    for (i = 0; i < NX_TFTP_MAX_CLIENTS; i++) {
        ...
        if (client_request_ptr->nx_tftp_client_retransmit_timeout) {
            ...
            if (client_request_ptr->nx_tftp_client_retransmit_timeout >=
                ...
                client_request_ptr->nx_tftp_client_retransmit_timeout -=
                    NX_TFTP_SERVER_TIMEOUT_PERIOD;
            } else {
            ...
            if (client_request_ptr->nx_tftp_client_retransmit_timeout == 0) {
                ...
                if (client_request_ptr->nx_tftp_client_retransmit_retries <
                    NX_TFTP_SERVER_MAX_RETRIES) {
                    ...
                    if (client_request_ptr->nx_tftp_client_request_open_type ==
                        NX_TFTP_STATE_WRITE_OPEN) {
                        ...
                        _nx_tftp_server_send_data(
                            server_ptr, client_request_ptr, NX_TRUE);
                    }
                } else {

                    /* The session has timed out. Send error and close. */
                    _nx_tftp_server_send_error(
                        server_ptr,
                        &client_request_ptr->nx_tftp_client_request_ip_address,
                        client_request_ptr->nx_tftp_client_request_port,
                        NX_TFTP_SESSION_TIMED_OUT,
                        "NetX TFTP Server: Session timed out");
...
VOID _nx_tftp_server_thread_entry(ULONG tftp_server) {
    ...
    while (1) {
        ...
        status = tx_event_flags_get(
            &(server_ptr->nx_tftp_server_event_flags),
            NX_SERVER_TFTP_ANY_EVENT,
            TX_OR_CLEAR,
            &events,
            TX_WAIT_FOREVER);
...
VOID _nx_tftp_server_process_received_data(NX_TFTP_SERVER *server_ptr) {
    ...
    status = nx_udp_socket_receive(
        &(server_ptr->nx_tftp_server_socket), &packet_ptr, NX_NO_WAIT);
    ...
    /* Check for valid packet length (The minimum TFTP header size is ACK packet, four
     * bytes).  */
    ...
    status = nx_packet_data_extract_offset(
        packet_ptr, 0, request_code, sizeof(request_code), &bytes_copyed);
...
/*                                                                        */
/**************************************************************************/
void _nx_tftp_server_open_for_read_process(
    NX_TFTP_SERVER *server_ptr, NX_PACKET *packet_ptr) {
    ...
    client_request_ptr =
        _nx_tftp_server_find_client_request(server_ptr, port, &ip_address);
    ...
    if (client_request_ptr == NX_NULL) {
        ...
        _nx_tftp_server_send_error(
            ...
            &ip_address,
            port,
            NX_TFTP_ERROR_NOT_DEFINED,
            "NetX TFTP Server: Too Many Clients");
    ...
    */
    ...
    status = fx_directory_information_get(
        server_ptr->nx_tftp_server_media_ptr,
        (CHAR *)buffer_ptr,
        NX_NULL,
        &file_size,
        NX_NULL,
    ...
    if (status != FX_SUCCESS) {
        ...
        _nx_tftp_server_send_error(
            ...
            &ip_address,
            port,
            NX_TFTP_ERROR_FILE_NOT_FOUND,
            "NetX TFTP Server: File Not Found");
    ...
    status = fx_file_open(
        server_ptr->nx_tftp_server_media_ptr,
        &(client_request_ptr->nx_tftp_client_request_file),
        (CHAR *)buffer_ptr,
        FX_OPEN_FOR_READ);
    ...
    if (status != FX_SUCCESS) {
        ...
        _nx_tftp_server_send_error(
            ...
            port,
            NX_TFTP_ERROR_FILE_NOT_FOUND,
            "NetX TFTP Server: File Open Failed");
    ...
    if (ip_address.nxd_ip_version == NX_IP_VERSION_V4) {
        status = nx_packet_allocate(
            server_ptr->nx_tftp_server_packet_pool_ptr,
            &new_packet,
            NX_IPv4_UDP_PACKET,
            NX_WAIT_FOREVER);
    ...
    if (4u + NX_TFTP_FILE_TRANSFER_MAX > ((ULONG)(new_packet->nx_packet_data_end) -
                                          (ULONG)(new_packet->nx_packet_append_ptr))) {
    ...
    if (ip_address.nxd_ip_version == NX_IP_VERSION_V4) {
        ...
        client_request_ptr->nx_tftp_client_request_ip_address.nxd_ip_version =
            NX_IP_VERSION_V4;
        client_request_ptr->nx_tftp_client_request_ip_address.nxd_ip_address.v4 =
        ...
        COPY_NXD_ADDRESS(
            &ip_address, &(client_request_ptr->nx_tftp_client_request_ip_address));
    } else
#endif
    {
        /* Release the original packet.  */
    ...
    if (file_size) {
        ...
        status = fx_file_read(
            &(client_request_ptr->nx_tftp_client_request_file),
            new_packet->nx_packet_prepend_ptr + 4,
            NX_TFTP_FILE_TRANSFER_MAX,
            &actual_size);

        /* Check for successful file read.  */
        if ((status != NX_SUCCESS) ||
            ((file_size > NX_TFTP_FILE_TRANSFER_MAX) &&
             (actual_size != NX_TFTP_FILE_TRANSFER_MAX)) ||
            ...
            _nx_tftp_server_send_error(
                ...
                port,
                NX_TFTP_ERROR_NOT_DEFINED,
                "NetX TFTP Server: File Read Error");
    ...
    if (file_size % NX_TFTP_FILE_TRANSFER_MAX) {
        ...
        /* Yes, the file size happens to be evenly divisible by the TFTP transfer size. In
           this case, we need to send a zero-length data packet at the end of file to let
           the other side know we are at the end.  */
        client_request_ptr->nx_tftp_client_request_exact_fit = NX_TRUE;
    }

    /* Move the TFTP data code and block number into the payload before sending it to the
     * client.  */
    ...
    *buffer_ptr++ = 1; /* First block number of file.  */
    ...
    new_packet->nx_packet_append_ptr =
        new_packet->nx_packet_prepend_ptr + new_packet->nx_packet_length;

    /* Send the data packet out.  */
    status = nxd_udp_socket_send(
        &(server_ptr->nx_tftp_server_socket), new_packet, &ip_address, port);
...
/*                                                                        */
/**************************************************************************/
VOID _nx_tftp_server_open_for_write_process(
    NX_TFTP_SERVER *server_ptr, NX_PACKET *packet_ptr) {
    NXD_ADDRESS ip_address;
    UINT port;
    UCHAR *buffer_ptr;
    ...
    */
    ...
    /* Perform a file create. This will fail if the file is already present, which we
     * don't care about at this point.  */
    ...
    if (status != NX_SUCCESS) {
        ...
        if (status == FX_ACCESS_ERROR)
            _nx_tftp_server_send_error(
                ...
                &ip_address,
                port,
                NX_TFTP_ERROR_ACCESS_VIOLATION,
                "NetX TFTP Server: File Access Error");
        else
            _nx_tftp_server_send_error(
                server_ptr,
                &ip_address,
    ...
    if (4u > ((ULONG)(new_packet->nx_packet_data_end) -
              (ULONG)(new_packet->nx_packet_append_ptr))) {
        nx_packet_release(new_packet);
...
VOID _nx_tftp_server_data_process(NX_TFTP_SERVER *server_ptr, NX_PACKET *packet_ptr) {
    ...
    if (client_request_ptr == NX_NULL) {
        ...
        _nx_tftp_server_send_error(
            ...
            &ip_address,
            port,
            NX_TFTP_ERROR_NO_SUCH_USER,
            "NetX TFTP Server: Unknown connection");
    ...
    status = nx_packet_data_extract_offset(
        packet_ptr, 2, (UCHAR *)&block_number, sizeof(block_number), &bytes_copyed);
    ...
    if (client_request_ptr->nx_tftp_client_request_block_number != block_number) {
        ...
        /* Check if it matches our previous ACK e.g. it could be a retransmit from the
         * other side. */
        if (client_request_ptr->nx_tftp_client_request_block_number ==
            (USHORT)(block_number + 1)) {
            ...
            if (client_request_ptr->nx_tftp_client_request_retransmits <=
        ...
        _nx_tftp_server_send_error(
            ...
            "NetX TFTP Server: Bad block number");
    ...
    if (packet_ptr->nx_packet_length > 4) {
        ...
        if (packet_ptr->nx_packet_next) {
            ...
            /* Yes, the packet is chained.  We have to copy the receive packet into a
               packet with the a payload of at least 560 bytes so the write request can be
               supplied with just the payload pointer.  */
            ...
            /* Successful packet copy.  Release the original packet and reassign the
             * packet pointer variable.  */
            nx_packet_release(packet_ptr);
            packet_ptr = temp_ptr;
        }
#endif /* NX_DISABLE_PACKET_CHAIN */
        ...
        if (status != NX_SUCCESS) {
            ...
            _nx_tftp_server_send_error(
                ...
                port,
                NX_TFTP_ERROR_NOT_DEFINED,
                "NetX TFTP Server: File Write Error");
    ...
    if (status == NX_SUCCESS) {
        ...
        server_ptr->nx_tftp_server_total_bytes_received +=
            (packet_ptr->nx_packet_length - 4);
...
VOID _nx_tftp_server_ack_process(NX_TFTP_SERVER *server_ptr, NX_PACKET *packet_ptr) {
    ...
    if (client_request_ptr->nx_tftp_client_request_block_number != block_number) {
        ...
        /* Check if this is a retransmitted ACK e.g. our previous data packet was dropped
           our delayed. */
        if (client_request_ptr->nx_tftp_client_request_block_number ==
            (USHORT)(block_number + 1)) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_tftp_server_send_data(
    NX_TFTP_SERVER *server_ptr,
    NX_TFTP_CLIENT_REQUEST *client_request_ptr,
    UINT retransmit) {
    ...
    if (client_request_ptr->nx_tftp_client_request_remaining_bytes) {
        ...
        if (retransmit) {
            ...
            status =
                fx_file_seek(&(client_request_ptr->nx_tftp_client_request_file), index);
        }
        ...
        /* Clear the exact fit flag since the only way we can be here is if the TFTP
           transfer size evenly divided into the file size and we need to send a zero
           length data buffer to signal the end of the file.  */
        client_request_ptr->nx_tftp_client_request_exact_fit = NX_FALSE;
    ...
    *buffer_ptr++ =
        (UCHAR)(client_request_ptr->nx_tftp_client_request_block_number & 0xFF);
    ...
    /* Send the data packet out.  */
    status = nxd_udp_socket_send(
        &(server_ptr->nx_tftp_server_socket),
        new_packet,
        &client_request_ptr->nx_tftp_client_request_ip_address,
        client_request_ptr->nx_tftp_client_request_port);
...
UINT _nx_tftp_server_send_ack(
    ...
    *buffer_ptr++ = NX_TFTP_CODE_ACK;
    ...
    if (retransmit) {
        ...
        *buffer_ptr++ =
            (UCHAR)((client_request_ptr->nx_tftp_client_request_block_number - 1) >> 8);
        *buffer_ptr++ =
            (UCHAR)((client_request_ptr->nx_tftp_client_request_block_number - 1) & 0xFF);
    } else {

        *buffer_ptr++ =
            (UCHAR)((client_request_ptr->nx_tftp_client_request_block_number) >> 8);
        *buffer_ptr++ =
            (UCHAR)(client_request_ptr->nx_tftp_client_request_block_number & 0xFF);
    }
...
VOID _nx_tftp_server_error_process(NX_TFTP_SERVER *server_ptr, NX_PACKET *packet_ptr) {
    ...
    server_ptr->nx_tftp_server_error_string
        [sizeof(server_ptr->nx_tftp_server_error_string) - 1] = NX_NULL;
...
/*                                                                        */
/**************************************************************************/
NX_TFTP_CLIENT_REQUEST *_nx_tftp_server_find_client_request(
    NX_TFTP_SERVER *server_ptr, UINT port, NXD_ADDRESS *ip_address) {
    ...
    for (i = 0; i < NX_TFTP_MAX_CLIENTS; i++) {
        ...
        if (port == 0) {
            ...
            if (client_request_ptr->nx_tftp_client_request_port == 0) {
                ...
                /* If there is no port number, this is an empty slot. But let's clear the
                 * slot anyway. */
                memset(
                    &(server_ptr->nx_tftp_server_client_list[i]),
                    0,
                    sizeof(NX_TFTP_CLIENT_REQUEST));
        ...
        /* This is a valid address. Does it match the current entry?  */
            ...
            if (ip_address->nxd_ip_version == NX_IP_VERSION_V4) {
                ...
                if ((client_request_ptr->nx_tftp_client_request_port == port) &&
                    (client_request_ptr->nx_tftp_client_request_ip_address.nxd_ip_address
                         .v4 == ip_address->nxd_ip_address.v4)) {
...
/*                                                                        */
/**************************************************************************/
VOID _nx_tftp_server_send_error(
    NX_TFTP_SERVER *server_ptr,
    NXD_ADDRESS *ip_address,
    UINT port,
    UINT error,
    ...
    if (6u > ((ULONG)(new_packet->nx_packet_data_end) -
              (ULONG)(new_packet->nx_packet_append_ptr))) {
        nx_packet_release(new_packet);
        return;
    }
    ...
    /* Move the TFTP error code and message into the payload before sending it to the
     * client.  */
    buffer_ptr = new_packet->nx_packet_prepend_ptr;
    *buffer_ptr++ = 0;
    ...
    *buffer_ptr++ = NX_NULL;
    ...
    /* Setup the packet pointers appropriately.  */
    new_packet->nx_packet_length =
        (ULONG)(buffer_ptr - new_packet->nx_packet_prepend_ptr);
