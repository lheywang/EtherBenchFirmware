...
/*                                                                        */
/**************************************************************************/
UINT _nxe_telnet_server_create(
    ...
    ULONG stack_size,
    void (*new_connection)(
        struct NX_TELNET_SERVER_STRUCT *telnet_server_ptr, UINT logical_connection),
    void (*receive_data)(
    ...
    if ((ip_ptr == NX_NULL) || (ip_ptr->nx_ip_id != NX_IP_ID) ||
        ...
        (stack_ptr == NX_NULL) || (new_connection == NX_NULL) ||
        (receive_data == NX_NULL) || (connection_end == NX_NULL))
        return (NX_PTR_ERROR);

    /* Call actual server create function.  */
    status = _nx_telnet_server_create(
        server_ptr,
        server_name,
        ip_ptr,
        stack_ptr,
...
UINT _nx_telnet_server_create(
    ...
    status = tx_thread_create(
        &(server_ptr->nx_telnet_server_thread),
        "TELNET Server Thread",
        _nx_telnet_server_thread_entry,
        (ULONG)server_ptr,
        stack_ptr,
        stack_size,
        NX_TELNET_SERVER_PRIORITY,
        NX_TELNET_SERVER_PRIORITY,
        TX_NO_TIME_SLICE,
        TX_DONT_START);
    ...
    /* Create the ThreadX event flags.  These will be used to driver the TELNET server
     * thread.  */
    status = tx_event_flags_create(
        &(server_ptr->nx_telnet_server_event_flags), "TELNET Server Thread Events");
    ...
    status = tx_timer_create(
        &(server_ptr->nx_telnet_server_timer),
        "TELNET Server Timer",
        _nx_telnet_server_timeout,
        (ULONG)server_ptr,
    ...
    for (i = 0; i < NX_TELNET_MAX_CLIENTS; i++) {
        ...
        server_ptr->nx_telnet_server_client_list[i].nx_telnet_client_request_connection =
            i;

        /* Create an TELNET client socket.  */
        status += nx_tcp_socket_create(
            ip_ptr,
            &(server_ptr->nx_telnet_server_client_list[i]
                  .nx_telnet_client_request_socket),
            "TELNET Server Control Socket",
            NX_TELNET_TOS,
            NX_TELNET_FRAGMENT_OPTION,
            NX_TELNET_TIME_TO_LIVE,
            NX_TELNET_SERVER_WINDOW_SIZE,
            NX_NULL,
            _nx_telnet_server_disconnect_present);
        ...
        server_ptr->nx_telnet_server_client_list[i]
            .nx_telnet_client_request_socket.nx_tcp_socket_reserved_ptr = server_ptr;
    }
    ...
    if (status != NX_SUCCESS) {
        ...
        for (i = 0; i < NX_TELNET_MAX_CLIENTS; i++) {
            ...
            nx_tcp_socket_delete(&(server_ptr->nx_telnet_server_client_list[i]
                                       .nx_telnet_client_request_socket));
        }
    ...
    status = nx_packet_pool_create(
        &(server_ptr->nx_telnet_server_packet_pool),
        "Telnet Server Options",
        NX_TELNET_SERVER_PACKET_PAYLOAD,
        &server_ptr->nx_telnet_server_pool_area,
    ...
    server_ptr->nx_telnet_server_packet_pool_ptr =
        &(server_ptr->nx_telnet_server_packet_pool);
...
UINT _nx_telnet_server_delete(NX_TELNET_SERVER *server_ptr) {
    ...
    while (i < NX_TELNET_MAX_CLIENTS) {
        ...
        nx_tcp_socket_disconnect(
            &(client_request_ptr->nx_telnet_client_request_socket), NX_NO_WAIT);

        /* Unaccept the socket.  */
        nx_tcp_server_socket_unaccept(
            &(client_request_ptr->nx_telnet_client_request_socket));
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_telnet_server_disconnect(
    NX_TELNET_SERVER *server_ptr, UINT logical_connection) {
...
UINT _nx_telnet_server_disconnect(NX_TELNET_SERVER *server_ptr, UINT logical_connection) {
    ...
    if (client_ptr->nx_telnet_client_request_socket.nx_tcp_socket_state >=
        NX_TCP_ESTABLISHED) {

        /* Disconnect the socket.  */
        nx_tcp_socket_disconnect(
            &(client_ptr->nx_telnet_client_request_socket), NX_TELNET_SERVER_TIMEOUT);
        ...
        if (server_ptr->nx_telnet_connection_end) {
            ...
            (server_ptr->nx_telnet_connection_end)(
                server_ptr, client_ptr->nx_telnet_client_request_connection);
        }
    } else {
    ...
    for (i = 0; i < NX_TELNET_MAX_CLIENTS; i++) {
        ...
        if (client_ptr->nx_telnet_client_request_socket.nx_tcp_socket_state ==
            ...
            status = nx_tcp_server_socket_relisten(
                server_ptr->nx_telnet_server_ip_ptr,
                NX_TELNET_SERVER_PORT,
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_telnet_server_packet_send(
    NX_TELNET_SERVER *server_ptr,
    UINT logical_connection,
    NX_PACKET *packet_ptr,
    ULONG wait_option) {
    ...
    status = _nx_telnet_server_packet_send(
        server_ptr, logical_connection, packet_ptr, wait_option);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_telnet_server_packet_send(
    NX_TELNET_SERVER *server_ptr,
    UINT logical_connection,
    NX_PACKET *packet_ptr,
    ULONG wait_option) {
    ...
    status = nx_tcp_socket_send(
        &(client_ptr->nx_telnet_client_request_socket), packet_ptr, wait_option);
...
UINT _nx_telnet_server_start(NX_TELNET_SERVER *server_ptr) {
    ...
    status = nx_tcp_server_socket_listen(
        ...
        _nx_telnet_server_connection_present);
...
UINT _nx_telnet_server_stop(NX_TELNET_SERVER *server_ptr) {
    ...
    /* Suspend the TELNET server thread.  */
    tx_event_flags_set(
        &(server_ptr->nx_telnet_server_event_flags), NX_TELNET_STOP_EVENT, TX_OR);
...
/*                                                                        */
/**************************************************************************/
UINT _nxe_telnet_server_get_open_connection_count(
    NX_TELNET_SERVER *server_ptr, UINT *current_connections) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_telnet_server_get_open_connection_count(
    NX_TELNET_SERVER *server_ptr, UINT *current_connections) {
...
VOID _nx_telnet_server_connect_process(NX_TELNET_SERVER *server_ptr) {
    ...
    for (i = 0; i < NX_TELNET_MAX_CLIENTS; i++) {
        ...
        if ((client_req_ptr->nx_telnet_client_request_socket.nx_tcp_socket_state >
             NX_TCP_CLOSED) &&
            (client_req_ptr->nx_telnet_client_request_socket.nx_tcp_socket_state <
             NX_TCP_ESTABLISHED) &&
            ...
            if (status) {
                ...
                client_req_ptr->nx_telnet_client_request_activity_timeout =
                ...
                if (server_ptr->nx_telnet_new_connection) {
                    ...
                    (server_ptr->nx_telnet_new_connection)(
                        server_ptr, client_req_ptr->nx_telnet_client_request_connection);
                }

                /* Disable remote echo by default. */
                if (server_ptr->nx_telnet_set_echo)
                    server_ptr->nx_telnet_set_echo(
                        server_ptr,
                        client_req_ptr->nx_telnet_client_request_connection,
                        NX_FALSE);
                ...
                status =
                    _nx_telnet_server_send_option_requests(server_ptr, client_req_ptr);
                if (status != NX_SUCCESS)
                    return;
#endif /* NX_TELNET_SERVER_OPTION_DISABLE */
            }
...
VOID _nx_telnet_server_connection_present(NX_TCP_SOCKET *socket_ptr, UINT port) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_telnet_server_event_flags), NX_TELNET_SERVER_CONNECT, TX_OR);
}
...
VOID _nx_telnet_server_disconnect_present(NX_TCP_SOCKET *socket_ptr) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_telnet_server_event_flags), NX_TELNET_SERVER_DISCONNECT, TX_OR);
}
...
VOID _nx_telnet_server_disconnect_process(NX_TELNET_SERVER *server_ptr) {
    ...
    for (i = 0; i < NX_TELNET_MAX_CLIENTS; i++) {
        ...
        /* Has the socket received a RST packet? If so NetX will put it in a CLOSED or
           LISTEN state and the socket activity timeout has not been reset yet.  */
        if (client_req_ptr->nx_telnet_client_request_socket.nx_tcp_socket_state <
            NX_TCP_SYN_SENT) {
            ...
            while (client_req_ptr->nx_telnet_client_request_socket.nx_tcp_socket_state >
                   NX_TCP_ESTABLISHED) {

                /* Yes, a disconnect is present, which signals an end of session for
                 * TELNET request.  */
        ...
        /* If this connection is closed, update the telnet data and notify the application
         * of a disconnect. */
...
VOID _nx_telnet_server_data_present(NX_TCP_SOCKET *socket_ptr) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_telnet_server_event_flags), NX_TELNET_SERVER_DATA, TX_OR);
}
...
VOID _nx_telnet_server_data_process(NX_TELNET_SERVER *server_ptr) {
    ...
    for (i = 0; i < NX_TELNET_MAX_CLIENTS; i++) {
        ...
        while (client_req_ptr->nx_telnet_client_request_socket
            ...
            if (*packet_ptr->nx_packet_prepend_ptr == NX_TELNET_IAC) {
                ...
                /* We will use an offset to mark the beginning of each telnet option, if
                there is more than one, in the packet payload. */
                offset = 0;
                ...
                if ((data_char >= NX_TELNET_WILL) && (data_char <= NX_TELNET_DONT)) {
                    ...
                    /* Process the entire packet for telnet options, ensuring we don't go
                     * off the end of the payload. */
                    while ((offset < packet_ptr->nx_packet_length) &&
                           (*(packet_ptr->nx_packet_prepend_ptr + offset) ==
                            NX_TELNET_IAC)) {

                        /* Process this telnet option. The offset will be updated to the
                        location of the next option (if there is one) on return of this
                        function. */
                        _nx_telnet_server_process_option(
                            server_ptr, packet_ptr, &offset, client_req_ptr);
                    }
                }

                /* Are there any data left? */
                if ((offset < packet_ptr->nx_packet_length) &&
                    (server_ptr->nx_telnet_receive_data)) {
                ...
                if ((client_req_ptr->nx_telnet_client_agree_server_will_SGA_success ==
                     NX_TRUE) &&
                    (client_req_ptr->nx_telnet_client_agree_server_will_echo_success ==
                     NX_TRUE)) {
    ...
    for (i = 0; i < NX_TELNET_MAX_CLIENTS; i++) {
        ...
        if (client_req_ptr->nx_telnet_client_request_activity_timeout) {
            ...
            if (client_req_ptr->nx_telnet_client_request_activity_timeout == 0) {
                ...
                /* Yes, the activity timeout has been exceeded.  Tear down and clean up
                   the entire client request structure.  */
                ...
                nx_tcp_socket_disconnect(
                    &(client_req_ptr->nx_telnet_client_request_socket), NX_NO_WAIT);
...
UINT _nxe_telnet_server_packet_pool_set(
    NX_TELNET_SERVER *server_ptr, NX_PACKET_POOL *pool_ptr) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_telnet_server_packet_pool_set(
    NX_TELNET_SERVER *server_ptr, NX_PACKET_POOL *pool_ptr) {
...
VOID _nx_telnet_server_create_option_packet(
    UCHAR option_message_type, UCHAR option_id, UCHAR *stream) {
...
/*                                                                        */
/**************************************************************************/
UINT _nx_telnet_server_send_option_requests(
    NX_TELNET_SERVER *server_ptr, NX_TELNET_CLIENT_REQUEST *client_req_ptr) {
    UINT status;
    NX_PACKET *packet_ptr;
    UINT option_length;
    ...
    _nx_telnet_server_create_option_packet(
        NX_TELNET_WILL, NX_TELNET_ECHO, &option_stream[0]);

    /* Yes, create dont echo request (3 bytes). */
    _nx_telnet_server_create_option_packet(
        NX_TELNET_DONT, NX_TELNET_ECHO, &option_stream[3]);

    /* Yes, create will SGA request (3 bytes). */
    _nx_telnet_server_create_option_packet(
        NX_TELNET_WILL, NX_TELNET_SGA, &option_stream[6]);
...
VOID _nx_telnet_server_process_option(
    NX_TELNET_SERVER *server_ptr,
    NX_PACKET *packet_ptr,
    UINT *offset,
    NX_TELNET_CLIENT_REQUEST *client_req_ptr) {
    ...
    if (data_char == NX_TELNET_ECHO) {
        ...
        /* Check whether the client replies with echo negotiation of the local echo
         * disabled.  */
        work_ptr--;
        data_char = *work_ptr;
