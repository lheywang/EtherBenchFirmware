...
/* If FileX is not used in this application, define this option and define the FileX
services declared in filex_stub.h elsewhere. #define      NX_FTP_NO_FILEX
*/
...
#define NX_FTP_CODE_RESTART_MARKER                                                                 \
...
#define NX_FTP_CODE_BAD_FILE                                                             \
    "550" /* Requested action not taken. File unavailable (e.g., file not found, no      \
             access).  */
#define NX_FTP_CODE_BAD_PAGE_TYPE                                                        \
    "551"                          /* Requested action aborted: page type unknown.  */
#define NX_FTP_CODE_NO_SPACE "552" /* Requested file action aborted, no space.  */
#define NX_FTP_CODE_BAD_NAME                                                             \
    "553" /* Requested action not taken, File name not allowed.  */

static VOID _nx_ftp_server_number_to_ascii(
    UCHAR *buffer_ptr, UINT buffer_size, UINT number, UCHAR pad);
...
UINT _nxe_ftp_server_create(
    ...
    if ((ip_ptr == NX_NULL) || (ip_ptr->nx_ip_id != NX_IP_ID) ||
        (ftp_server_ptr == NX_NULL) || (ftp_logout == NX_NULL) ||
        (ftp_login == NX_NULL) || (stack_ptr == NX_NULL) || (pool_ptr == NX_NULL))
        return (NX_PTR_ERROR);

    /* Call actual server create function.  */
    status = _nx_ftp_server_create(
        ftp_server_ptr,
        ftp_server_name,
        ip_ptr,
        media_ptr,
...
UINT _nx_ftp_server_create(
    ...
    /* Call actual server create function but set the ftp login and logout arguments to
     * NULL.  */
    status = _nx_ftp_server_create_internal(
        ftp_server_ptr,
        ftp_server_name,
        ip_ptr,
        media_ptr,
...
UINT _nxde_ftp_server_create(
    ...
    UINT (*ftp_login)(
        struct NX_FTP_SERVER_STRUCT *ftp_server_ptr,
        NXD_ADDRESS *client_ipduo_address,
        UINT client_port,
        CHAR *name,
        CHAR *password,
        CHAR *extra_info),
    ...
    /* Check for invalid input pointers.  */
    if ((ip_ptr == NX_NULL) || (ip_ptr->nx_ip_id != NX_IP_ID) ||
        (ftp_server_ptr == NX_NULL) ||
        (ftp_server_ptr->nx_ftp_server_id == NXD_FTP_SERVER_ID) ||
        (ftp_logout == NX_NULL) || (ftp_login == NX_NULL) || (stack_ptr == NX_NULL) ||
        (pool_ptr == NX_NULL))
        return (NX_PTR_ERROR);

    /* Call actual server create function.  */
    status = _nxd_ftp_server_create(
        ftp_server_ptr,
        ftp_server_name,
        ip_ptr,
        media_ptr,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_server_create_internal(
    NX_FTP_SERVER *ftp_server_ptr,
    CHAR *ftp_server_name,
    NX_IP *ip_ptr,
    FX_MEDIA *media_ptr,
    ...
    /* Check the supplied packet pool for minimum required payload length
       (NX_FTP_SERVER_MIN_PACKET_PAYLOAD). This configurable option is explained in more
       detail in the header file.  */
    ...
    status = tx_thread_create(
        &(ftp_server_ptr->nx_ftp_server_thread),
        "FTP Server Thread",
        _nx_ftp_server_thread_entry,
        (ULONG)ftp_server_ptr,
        stack_ptr,
        stack_size,
        NX_FTP_SERVER_PRIORITY,
        NX_FTP_SERVER_PRIORITY,
        NX_FTP_SERVER_TIME_SLICE,
        TX_DONT_START);
    ...
    /* Create the ThreadX event flags.  These will be used to driver the FTP server
     * thread.  */
    status = tx_event_flags_create(
        &(ftp_server_ptr->nx_ftp_server_event_flags), "FTP Server Thread Events");
    ...
    status = tx_timer_create(
        &(ftp_server_ptr->nx_ftp_server_timer),
        "FTP Server Timer",
        _nx_ftp_server_timeout,
        (ULONG)ftp_server_ptr,
        (NX_IP_PERIODIC_RATE * NX_FTP_TIMEOUT_PERIOD),
        (NX_IP_PERIODIC_RATE * NX_FTP_TIMEOUT_PERIOD),
        TX_NO_ACTIVATE);
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        ftp_server_ptr->nx_ftp_server_client_list[i]
            .nx_ftp_client_request_control_socket.nx_tcp_socket_reserved_ptr =
            ftp_server_ptr;
    }
    ...
    if (status) {
        ...
        for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
            ...
            nx_tcp_socket_delete(&(ftp_server_ptr->nx_ftp_server_client_list[i]
                                       .nx_ftp_client_request_control_socket));
        }
...
UINT _nx_ftp_server_delete(NX_FTP_SERVER *ftp_server_ptr) {
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        client_request_ptr->nx_ftp_client_request_transfer_mode =
            NX_FTP_TRANSFER_MODE_STREAM;
        ...
        nx_tcp_socket_disconnect(
            &(client_request_ptr->nx_ftp_client_request_control_socket), NX_NO_WAIT);

        /* Unaccept the control socket.  */
        nx_tcp_server_socket_unaccept(
            &(client_request_ptr->nx_ftp_client_request_control_socket));
    ...
    nx_tcp_server_socket_unlisten(
        ftp_server_ptr->nx_ftp_server_ip_ptr, NX_FTP_SERVER_CONTROL_PORT);
...
UINT _nx_ftp_server_start(NX_FTP_SERVER *ftp_server_ptr) {
    ...
    status = nx_tcp_server_socket_listen(
        ftp_server_ptr->nx_ftp_server_ip_ptr,
        NX_FTP_SERVER_CONTROL_PORT,
        &(ftp_server_ptr->nx_ftp_server_client_list[0]
              .nx_ftp_client_request_control_socket),
        NX_FTP_MAX_CLIENTS,
        _nx_ftp_server_connection_present);
    ...
    /* Clear stop event. */
    tx_event_flags_get(
        &(ftp_server_ptr->nx_ftp_server_event_flags),
        NX_FTP_STOP_EVENT,
        TX_OR_CLEAR,
        &events,
        TX_NO_WAIT);
...
UINT _nx_ftp_server_stop(NX_FTP_SERVER *ftp_server_ptr) {
    ...
    /* Suspend the FTP server thread.  */
    tx_event_flags_set(
        &(ftp_server_ptr->nx_ftp_server_event_flags), NX_FTP_STOP_EVENT, TX_OR);
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        if (client_request_ptr->nx_ftp_client_request_data_socket.nx_tcp_socket_id) {
            ...
            nx_tcp_socket_disconnect(
                &(client_request_ptr->nx_ftp_client_request_data_socket), NX_NO_WAIT);

            /* Unbind/unaccept the data socket.  */
            if (client_request_ptr->nx_ftp_client_request_passive_transfer_enabled ==
                NX_TRUE) {
                nx_tcp_server_socket_unaccept(
                    &(client_request_ptr->nx_ftp_client_request_data_socket));
                nx_tcp_server_socket_unlisten(
                    ftp_server_ptr->nx_ftp_server_ip_ptr,
                    client_request_ptr->nx_ftp_client_request_data_socket
                        .nx_tcp_socket_port);
...
/*                                                                        */
/**************************************************************************/
VOID _nx_ftp_server_response(
    NX_TCP_SOCKET *socket, NX_PACKET *packet_ptr, CHAR *reply_code, CHAR *message) {
    ...
    while ((*reply_code) &&
           (buffer_ptr < (packet_ptr->nx_packet_data_end - NX_PHYSICAL_TRAILER))) {
...
/*                                                                        */
/**************************************************************************/
VOID _nx_ftp_server_directory_response(
...
VOID _nx_ftp_server_thread_entry(ULONG ftp_server_address) {
    ...
    while (1) {
        ...
        status = tx_event_flags_get(
            &(server_ptr->nx_ftp_server_event_flags),
            NX_FTP_ANY_EVENT,
            TX_OR_CLEAR,
            &events,
            TX_WAIT_FOREVER);
...
VOID _nx_ftp_server_command_process(NX_FTP_SERVER *ftp_server_ptr) {
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        if (client_req_ptr->nx_ftp_client_request_control_socket.nx_tcp_socket_connect_ip
            ...
            connect_ip4_address = client_req_ptr->nx_ftp_client_request_control_socket
                                      .nx_tcp_socket_connect_ip.nxd_ip_address.v4;
        }
#endif /* NX_DISABLE_IPV4 */

        /* Now see if this socket has data.  */
        if (client_req_ptr->nx_ftp_client_request_control_socket
            ...
            for (;;) {
                ...
                /* Now, parse the command in the packet.  Note that the parse command
                adjusts the packet pointer such that it is positioned at the next token in
                the buffer.  */
                ...
                if ((client_req_ptr->nx_ftp_client_request_authenticated == NX_FALSE) &&
                    ...
                    _nx_ftp_server_response(
                        &(client_req_ptr->nx_ftp_client_request_control_socket),
                        packet_ptr,
                        NX_FTP_CODE_UNAUTHORIZED,
                        "Not logged in");
                 ...
                 */
                    ...
                    (client_req_ptr->nx_ftp_client_request_read_only == NX_TRUE) &&
                    ((ftp_command == NX_FTP_STOR) || (ftp_command == NX_FTP_RNFR) ||
                     (ftp_command == NX_FTP_RNTO) || (ftp_command == NX_FTP_DELE) ||
                     (ftp_command == NX_FTP_RMD) || (ftp_command == NX_FTP_MKD))) {
                ...
                case NX_FTP_USER: {
                    ...
                    for (j = 0; j < (NX_FTP_USERNAME_SIZE - 1) &&
                                (j < packet_ptr->nx_packet_length);
                         j++) {

                        /* Copy a character.  */
                        client_req_ptr->nx_ftp_client_request_username[j] =
                            (CHAR)buffer_ptr[j];

                        /* Determine if a CR/LF is present.  */
                        if ((buffer_ptr[j] == 13) || (buffer_ptr[j] == 10) ||
                            (buffer_ptr[j] == 0))
                            break;
                    }
                ...
                case NX_FTP_PASS: {
                    ...
                    for (j = 0; j < (NX_FTP_PASSWORD_SIZE - 1) &&
                                (j < packet_ptr->nx_packet_length);
                         j++) {

                        /* Copy a character.  */
                        client_req_ptr->nx_ftp_client_request_password[j] =
                            (CHAR)buffer_ptr[j];
                    ...
                    if (ftp_server_ptr->nx_ftp_login) {
                        ...
                        /* Now call the user's login callback routine to see if the
                         * username,password is valid.  */
                        status = (ftp_server_ptr->nx_ftp_login)(
                            ftp_server_ptr,
                            &(client_req_ptr->nx_ftp_client_request_control_socket
                        ...
                        if (client_req_ptr->nx_ftp_client_request_ip_type ==
                            NX_IP_VERSION_V4) {
                            ...
                            if (ftp_server_ptr->nx_ftp_login_ipv4) {
                                ...
                                /* Yes; Now call the user's login callback routine to see
                ...
                case NX_FTP_QUIT: {
                    ...
                    if (client_req_ptr->nx_ftp_client_request_login) {
                        ...
                        if (ftp_server_ptr->nx_ftp_logout_ipv4) {
                            ...
                            (ftp_server_ptr->nx_ftp_logout_ipv4)(
                                ftp_server_ptr,
                                client_req_ptr->nx_ftp_client_request_control_socket
                                    .nx_tcp_socket_connect_ip.nxd_ip_address.v4,
                                client_req_ptr->nx_ftp_client_request_control_socket
                        ...
                        if (ftp_server_ptr->nx_ftp_logout) {
                            ...
                            /* Call the 'duo' logout function which takes IPv6 or IPv4 IP
                             * addresses. */
                            (ftp_server_ptr->nx_ftp_logout)(
                                ftp_server_ptr,
                                &(client_req_ptr->nx_ftp_client_request_control_socket
                                      .nx_tcp_socket_connect_ip),
                                client_req_ptr->nx_ftp_client_request_control_socket
                    ...
                    if (client_req_ptr->nx_ftp_client_request_data_socket
                            .nx_tcp_socket_id) {

                        /* Clean up the client socket.  */
                        _nx_ftp_server_data_socket_cleanup(
                            ftp_server_ptr, client_req_ptr);
                    }
                    ...
                    /* Relisten on this socket. This will probably fail, but it is needed
                    just in case all available clients were in use at the time of the last
                    relisten.  */
                ...
                case NX_FTP_RETR: {
                    ...
                    if (client_req_ptr->nx_ftp_client_request_transfer_type != 'I') {
                        ...
                        _nx_ftp_server_response(
                            &(client_req_ptr->nx_ftp_client_request_control_socket),
                            packet_ptr,
                            NX_FTP_CODE_BAD_TYPE,
                            "Only Image transfer allowed");
                    ...
                    status = fx_file_open(
                        ftp_server_ptr->nx_ftp_server_media_ptr,
                        &(client_req_ptr->nx_ftp_client_request_file),
                        (CHAR *)buffer_ptr,
                        FX_OPEN_FOR_READ);
                    ...
                    if (status == FX_SUCCESS) {
                        ...
                        if (client_req_ptr
                            ...
                            status = nx_tcp_socket_state_wait(
                        ...
                        } else if (client_req_ptr->nx_ftp_client_request_data_socket
                                       .nx_tcp_socket_id) {
                            ...
                            /* If no error is present, register the receive notify
                             * function.  */
                            if (status == NX_SUCCESS) {

                                /* Make sure each socket points to the corresponding FTP
                                 * server.  */
                                client_req_ptr->nx_ftp_client_request_data_socket
                                    .nx_tcp_socket_reserved_ptr = ftp_server_ptr;

                                /* Bind the socket to the FTP server data port.  */
                                status = nx_tcp_client_socket_bind(
                                    &(client_req_ptr->nx_ftp_client_request_data_socket),
                                    NX_FTP_SERVER_DATA_PORT,
                                    NX_NO_WAIT);
                                ...
                                if (status) {
                                    ...
                                    nx_tcp_client_socket_bind(
                                        &(client_req_ptr
                                              ->nx_ftp_client_request_data_socket),
                                        NX_ANY_PORT,
                                        NX_NO_WAIT);
                                }

                                /* Now attempt to connect the data port to the client's
                                 * data port.  */
                                ...
                                if (status) {
                                    ...
                                    /* Yes, a connect error is present.  Tear everything
                                     * down.  */
                                    nx_tcp_client_socket_unbind(
                                        &(client_req_ptr
                                              ->nx_ftp_client_request_data_socket));
                                    ...
                                    /* Setup the data port with a specific packet transmit
                                     * retry logic.  */
                    ...
                    if (status == NX_SUCCESS) {
                        ...
                        client_req_ptr->nx_ftp_client_request_open_type =
                            NX_FTP_OPEN_FOR_READ;

                        /* Set the total bytes field to files size.  */
                        client_req_ptr->nx_ftp_client_request_total_bytes =
                            (ULONG)client_req_ptr->nx_ftp_client_request_file
                        ...
                        _nx_ftp_server_response(
                            &(client_req_ptr->nx_ftp_client_request_control_socket),
                            packet_ptr,
                            NX_FTP_CODE_START_XFER,
                            "File Opened");

                        /* Determine if the block mode is enabled.  */
                        if ((client_req_ptr->nx_ftp_client_request_transfer_mode ==
                             NX_FTP_TRANSFER_MODE_BLOCK) &&
                            (client_req_ptr->nx_ftp_client_request_total_bytes)) {

                            /* Send start block header for file size.  */
                            status = _nx_ftp_server_block_header_send(
                                ftp_server_ptr->nx_ftp_server_packet_pool_ptr,
                                client_req_ptr,
                                client_req_ptr->nx_ftp_client_request_total_bytes);
                        }
                        ...
                        while (status == NX_SUCCESS) {
                            ...
                            length = ((ULONG)(packet_ptr->nx_packet_data_end -
                                              packet_ptr->nx_packet_prepend_ptr)) -
                                     NX_PHYSICAL_TRAILER;

                            /* Determine if the length is greater than the connected MSS.
                             */
                            if (length > client_req_ptr->nx_ftp_client_request_data_socket
                                             .nx_tcp_socket_connect_mss) {

                                /* Yes, reduce the length to match the MSS.  */
                                length = client_req_ptr->nx_ftp_client_request_data_socket
                                             .nx_tcp_socket_connect_mss;
                            }
                            ...
                            if (status == FX_SUCCESS) {
                                ...
                                if (status) {
                                    ...
                                    client_req_ptr->nx_ftp_client_request_total_bytes =
                                        client_req_ptr
                                            ->nx_ftp_client_request_total_bytes -
                                        length;

                                    /* Increment the number of bytes sent.  */
                                    ftp_server_ptr->nx_ftp_server_total_bytes_sent +=
                                        length;
                                }
                            } else {
                ...
                case NX_FTP_STOR: {
                    ...
                    if (status != FX_SUCCESS) {
                        ...
                        status = fx_file_create(
                            ftp_server_ptr->nx_ftp_server_media_ptr, (CHAR *)buffer_ptr);
                    ...
                    status += fx_file_truncate(
                        &(client_req_ptr->nx_ftp_client_request_file), 0);
                        ...
                        if (client_req_ptr
                            ...
                            if (client_req_ptr->nx_ftp_client_request_data_socket
                                ...
                                if (status == NX_SUCCESS) {
                                    ...
                                    /* Make sure each socket points to the corresponding
                                     * FTP server.  */
                                    client_req_ptr->nx_ftp_client_request_data_socket
                                        .nx_tcp_socket_reserved_ptr = ftp_server_ptr;
                                    ...
                                    if (status) {
                                        ...
                                        /* FTP server data port is busy, use any data
                                         * port. */
                                        nx_tcp_client_socket_bind(
                                            &(client_req_ptr
                                                  ->nx_ftp_client_request_data_socket),
                                    ...
                                    /* Now attempt to connect the data port to the
                                     * client's data port.  */
                                    status = nxd_tcp_client_socket_connect(
                                        &(client_req_ptr
                                              ->nx_ftp_client_request_data_socket),
                                    ...
                                    /* Check for connect error.  */
                                    if (status) {
                                        /* Yes, a connect error is present.  Tear
                                         * everything down.  */
                                        nx_tcp_client_socket_unbind(
                                            &(client_req_ptr
                                                  ->nx_ftp_client_request_data_socket));
                                        ...
                                        /* Setup the data port with a specific packet
                ...
                case NX_FTP_RNFR: {
                    ...
                    status = fx_file_attributes_read(
                        ftp_server_ptr->nx_ftp_server_media_ptr, (CHAR *)buffer_ptr, &j);

                    /* If not a file, read the directory attributes.  */
                    if (status == FX_NOT_A_FILE)
                        status = fx_directory_attributes_read(
                            ftp_server_ptr->nx_ftp_server_media_ptr,
                            (CHAR *)buffer_ptr,
                            &j);
                ...
                case NX_FTP_RNTO: {
                    ...
                    /* Rename the file.  */
                    status = fx_file_rename(
                        ftp_server_ptr->nx_ftp_server_media_ptr,
                        (CHAR *)(client_req_ptr->nx_ftp_client_request_packet)
                ...
                case NX_FTP_MKD: {
                    ...
                    if (status == NX_SUCCESS) {
                        ...
                        /* Change the path to the new directory, using a temporary
                         * directory structure */
                        status = fx_directory_local_path_set(
                            ftp_server_ptr->nx_ftp_server_media_ptr,
                            &temporary_path,
                            (CHAR *)buffer_ptr);
                        ...
                        if (status == NX_SUCCESS) {
                            ...
                            fx_directory_local_path_get(
                                ftp_server_ptr->nx_ftp_server_media_ptr, &local_dir);

                            /* Now send a successful response to the client.  */
                            _nx_ftp_server_directory_response(
                                &(client_req_ptr->nx_ftp_client_request_control_socket),
                                packet_ptr,
                                NX_FTP_CODE_CMD_OK,
                                "Directory Created",
                ...
                case NX_FTP_NLST: {
                    ...
                    if (status == NX_SUCCESS) {
                        ...
                        if (client_req_ptr->nx_ftp_client_request_transfer_mode ==
                            ...
                            _nx_ftp_server_block_size_get(
                                ftp_server_ptr, ftp_command, filename, &block_size);
                        ...
                        /* Allocate a new packet.  */
                        status = _nx_ftp_packet_allocate(
                            ftp_server_ptr->nx_ftp_server_packet_pool_ptr,
                            client_req_ptr,
                            &packet_ptr,
                            NX_TCP_PACKET,
                        ...
                        /* Now generate the full directory listing and send the contents
                         * to the client.  */
                        j = 0;
                        while (status == NX_SUCCESS) {
                            ...
                            if (status == NX_SUCCESS) {
                                ...
                                if ((length + 2) > remaining_length) {
                                    ...
                                    /* Calculate the remaining length.  */
                                    remaining_length =
                                        (ULONG)((packet_ptr->nx_packet_data_end -
                                                 packet_ptr->nx_packet_append_ptr) -
                                                NX_PHYSICAL_TRAILER);
                        ...
                        if ((status == FX_NO_MORE_ENTRIES) && (packet_ptr) &&
                            (packet_ptr->nx_packet_length)) {
                        ...
                        /* Now determine if the directory listing was a success (e..g runs
                         * until no more entries found).  */
                ...
                case NX_FTP_LIST: {
                    ...
                    /* Change to the default directory of this connection.  */
                    status = fx_directory_local_path_restore(
                        ftp_server_ptr->nx_ftp_server_media_ptr,
                        &(client_req_ptr->nx_ftp_client_local_path));
                    ...
                    if (j) {
                        ...
                        if (status) {
                            ...
                            if ((status == FX_SUCCESS) &&
                                ((attributes & FX_DIRECTORY) == 0)) {

                                /* Yes, a file name was supplied. Set the single file flag
                                 * for the processing below.  */
                                single_file = NX_TRUE;

                                /* Advance to first character of the filename.  */
                                k++;

                                /* Copy the file name from the last slash into the
                                 * filename buffer.  */
                                j = 0;
                                while ((buffer_ptr[(UINT)k + j]) &&
                                       (j < FX_MAX_LONG_NAME_LEN - 1)) {
                    ...
                    if (status == NX_SUCCESS) {
                        ...
                        while (status == NX_SUCCESS) {
                            ...
                            if (single_file == NX_FALSE) {
                                ...
                                /* The parameter to the LIST command is a single file.
                                Simply return the information already gathered above for
                                this one file instead of traversing the entire list.  */

                                /* Is this the first pass through the loop?  */
                                if (j) {

                                    /* End the loop, since the single file has already
                                     * been sent.  */
                            ...
                            if (status == NX_SUCCESS) {
                                ...
                                /* Make sure there is enough space for the data plus the
                                file info. File Info is 10 chars for permissions, 15 chars
                                for owner and group, 11 chars for size (for file size up
                                to 4gB), 14 for date, 2 chars for cr lf.  */
                                ...
                                if (attributes & FX_READ_ONLY) {
                                    memcpy(
                                        &buffer_ptr[1],
                                        "r--r--r--",
                                        9); /* Use case of memcpy is verified. */
                                } else {
                                    memcpy(
                                        &buffer_ptr[1],
                                        "rw-rw-rw-",
                                ...
                                _nx_ftp_server_number_to_ascii(
                                    &buffer_ptr[26], 10, size, ' ');
                                buffer_ptr[36] = ' ';
                                ...
                                buffer_ptr[40] = ' ';
                                _nx_ftp_server_number_to_ascii(
                                    &buffer_ptr[41], 2, day, '0');
                                buffer_ptr[43] = ' ';
                                _nx_ftp_server_number_to_ascii(
                                    &buffer_ptr[44], 2, hour, '0');
                                buffer_ptr[46] = ':';
                                _nx_ftp_server_number_to_ascii(
                                    &buffer_ptr[47], 2, minute, '0');
                                buffer_ptr[49] = ' ';
                                memcpy(
                                    &buffer_ptr[50],
                                    filename,
                ...
                case NX_FTP_PORT: {
                    ...
                    if (client_req_ptr->nx_ftp_client_request_control_socket
                            .nx_tcp_socket_connect_ip.nxd_ip_version ==
                        NX_IP_VERSION_V6) {
                        ...
                        _nx_ftp_server_response(
                            &(client_req_ptr->nx_ftp_client_request_control_socket),
                            packet_ptr,
                            NX_FTP_CODE_NOT_IMPLEMENTED,
                            "PORT illegal in IPv6");
                    ...
                    if ((buffer_ptr[j] != 13) || (commas != 5) || (ip_address == 0) ||
                        (port == 0) || (ip_address != connect_ip4_address)) {
                ...
                case NX_FTP_PASV: {
                    ...
                    status = nx_tcp_free_port_find(
                        ftp_server_ptr->nx_ftp_server_ip_ptr,
                        ftp_server_ptr->nx_ftp_server_data_port++,
                        &port);
                    ...
                    nx_tcp_server_socket_accept(
                        &(client_req_ptr->nx_ftp_client_request_data_socket), NX_NO_WAIT);

                    /* Pickup the IPv4 address of this IP instance.  */
                    ip_address =
                        client_req_ptr->nx_ftp_client_request_control_socket
                            .nx_tcp_socket_connect_interface->nx_interface_ip_address;

                    /* Reset the packet prepend pointer for alignment.  */
                    packet_ptr->nx_packet_prepend_ptr =
                        packet_ptr->nx_packet_data_start + NX_TCP_PACKET;

                    /* Setup pointer to packet buffer area.  */
                    buffer_ptr = packet_ptr->nx_packet_prepend_ptr;

                    /* Now build PASV response. "227 Entering Passive Mode
                     * (h1,h2,h3,h4,p1,p2)."  */

                    /* Verify packet payload. The max size of this message is 54. */
                    if ((packet_ptr->nx_packet_data_end -
                         packet_ptr->nx_packet_prepend_ptr) < 54) {

                        /* Delete data socket.  */
                        nx_tcp_server_socket_unlisten(
                            ftp_server_ptr->nx_ftp_server_ip_ptr, port);
                        nx_tcp_socket_delete(
                            &(client_req_ptr->nx_ftp_client_request_data_socket));
                    ...
                    memcpy(
                        ...
                        26); /* Use case of memcpy is verified. */

                    /* Build the IP address and port.  */
                    j = 26;
                    buffer_ptr[j++] = '(';
                    j += _nx_utility_uint_to_string(
                        (ip_address >> 24), 10, (CHAR *)(buffer_ptr + j), 54 - j);
                    buffer_ptr[j++] = ',';
                    j += _nx_utility_uint_to_string(
                        ((ip_address >> 16) & 0xFF),
                        10,
                    ...
                    buffer_ptr[j++] = ',';
                    j += _nx_utility_uint_to_string(
                        ((ip_address >> 8) & 0xFF), 10, (CHAR *)(buffer_ptr + j), 54 - j);
                    buffer_ptr[j++] = ',';
                    j += _nx_utility_uint_to_string(
                        (ip_address & 0xFF), 10, (CHAR *)(buffer_ptr + j), 54 - j);
                    buffer_ptr[j++] = ',';
                    j += _nx_utility_uint_to_string(
                        (port >> 8), 10, (CHAR *)(buffer_ptr + j), 54 - j);
                    buffer_ptr[j++] = ',';
                    j += _nx_utility_uint_to_string(
                        (port & 0XFF), 10, (CHAR *)(buffer_ptr + j), 54 - j);
                    buffer_ptr[j++] = ')';
                    buffer_ptr[j++] = '.';
                    ...
                    client_req_ptr->nx_ftp_client_request_passive_transfer_enabled =
                        NX_TRUE;
                ...
                case NX_FTP_EPRT: {
                    ...
                    status = _nx_ftp_utility_parse_IPv6_address(
                        (CHAR *)buffer_ptr, packet_ptr->nx_packet_length, &ipduo_address);
                    ...
                    /* Now pickup the port number.  */
                    status = _nx_ftp_utility_parse_port_number(
                        (CHAR *)buffer_ptr, packet_ptr->nx_packet_length, &port);
                ...
                case NX_FTP_EPSV: {
                    ...
                    /* Now build EPSV response. "229 Entering Extended Passive Mode
                     * (|||6446|)."  */

                    /* Get port size.  */
                    port_size =
                        _nx_ftp_server_utility_fill_port_number(temp_buffer, port);

                    /* Verify packet payload.  */
                    if ((ULONG)(packet_ptr->nx_packet_data_end -
                                packet_ptr->nx_packet_prepend_ptr) < (43 + port_size)) {
                    ...
                    memcpy(
                        ...
                        35); /* Use case of memcpy is verified. */
                    ...
                    memcpy(
                        &buffer_ptr[39],
                        temp_buffer,
                        port_size); /* Use case of memcpy is verified. */
                ...
                case NX_FTP_CWD: {
                    ...
                    if (ftp_command == NX_FTP_CDUP) {
                        ...
                        /* Move the pointer to make sure there is enough memory to store
                         * the data.  */
                        buffer_ptr -= 3;
                        buffer_ptr[0] = '.';
                        buffer_ptr[1] = '.';
                ...
                case NX_FTP_MODE: {
                    ...
                    if (buffer_ptr[0] == 'S') {
                        ...
                        client_req_ptr->nx_ftp_client_request_transfer_mode =
                            NX_FTP_TRANSFER_MODE_STREAM;
...
VOID _nx_ftp_server_connect_process(NX_FTP_SERVER *ftp_server_ptr) {
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        if ((client_req_ptr->nx_ftp_client_request_control_socket.nx_tcp_socket_state >
             NX_TCP_CLOSED) &&
            (client_req_ptr->nx_ftp_client_request_control_socket.nx_tcp_socket_state <
             NX_TCP_ESTABLISHED) &&
            (client_req_ptr->nx_ftp_client_request_control_socket
                 .nx_tcp_socket_connect_port)) {
            ...
            if (status) {
                ...
                if (client_req_ptr->nx_ftp_client_request_control_socket
                        .nx_tcp_socket_connect_ip.nxd_ip_version == NX_IP_VERSION_V6) {
                ...
                /* Send a connection ACK back to the client, indicating a successful
...
VOID _nx_ftp_server_command_present(NX_TCP_SOCKET *control_socket_ptr) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_ftp_server_event_flags), NX_FTP_SERVER_COMMAND, TX_OR);
}
...
VOID _nx_ftp_server_connection_present(NX_TCP_SOCKET *control_socket_ptr, UINT port) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_ftp_server_event_flags), NX_FTP_SERVER_CONNECT, TX_OR);
...
VOID _nx_ftp_server_data_disconnect(NX_TCP_SOCKET *data_socket_ptr) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_ftp_server_event_flags), NX_FTP_SERVER_DATA_DISCONNECT, TX_OR);
}
...
VOID _nx_ftp_server_data_disconnect_process(NX_FTP_SERVER *ftp_server_ptr) {
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        while (client_req_ptr->nx_ftp_client_request_data_socket.nx_tcp_socket_state >
               NX_TCP_ESTABLISHED) {

            /* Yes, a disconnect is present, which signals an end of file of a client FTP
             * write request.  */
            ...
            if (client_req_ptr->nx_ftp_client_request_transfer_mode ==
                ...
                if (client_req_ptr->nx_ftp_client_request_total_bytes !=
                    client_req_ptr->nx_ftp_client_request_block_bytes)
            ...
            if (client_req_ptr->nx_ftp_client_request_open_type ==
                NX_FTP_OPEN_FOR_WRITE) {

                /* Pickup the file length.  */
                length = (ULONG)client_req_ptr->nx_ftp_client_request_file
                             .fx_file_current_file_size;
                ...
                if ((length == client_req_ptr->nx_ftp_client_request_total_bytes) &&
                    (block_status == NX_TRUE)) {
...
VOID _nx_ftp_server_data_present(NX_TCP_SOCKET *data_socket_ptr) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_ftp_server_event_flags), NX_FTP_SERVER_DATA, TX_OR);
}
...
VOID _nx_ftp_server_data_process(NX_FTP_SERVER *ftp_server_ptr) {
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        while (client_req_ptr->nx_ftp_client_request_data_socket
                   .nx_tcp_socket_receive_queue_count) {
            ...
            /* Update the total bytes for this file.  This will be checked on close to see
            ...
            do {
                ...
                status = fx_file_write(
                    ...
                    (ULONG)((
                        next_packet_ptr->nx_packet_append_ptr -
                        next_packet_ptr->nx_packet_prepend_ptr)));
                ...
                /* If not successful, keep retrieving packets from the socket receive
                   queue so we can free up the packets. */

                /* Update the remaining length to write.  */
                length = length - (ULONG)(next_packet_ptr->nx_packet_append_ptr -
                                          next_packet_ptr->nx_packet_prepend_ptr);
...
UINT _nx_ftp_server_parse_command(NX_PACKET *packet_ptr) {
     ...
     */
    i = 0;
    while ((buffer_ptr[i] != ' ') && (buffer_ptr[i] != (char)13) &&
           (i < packet_ptr->nx_packet_length - 1)) {
...
VOID _nx_ftp_server_timeout(ULONG ftp_server_address) {
    ...
    /* Set the data event flag.  */
    tx_event_flags_set(
        &(server_ptr->nx_ftp_server_event_flags), NX_FTP_SERVER_ACTIVITY_TIMEOUT, TX_OR);
}
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        if (client_req_ptr->nx_ftp_client_request_activity_timeout) {
            ...
            if (client_req_ptr->nx_ftp_client_request_activity_timeout == 0) {
                ...
                /* Now disconnect the command socket.  */
                nx_tcp_socket_disconnect(
                    &(client_req_ptr->nx_ftp_client_request_control_socket), NX_NO_WAIT);
                ...
                /* Relisten on this socket. This will probably fail, but it is needed just
                   in case all available clients were in use at the time of the last
                   relisten.  */
                nx_tcp_server_socket_relisten(
                    ftp_server_ptr->nx_ftp_server_ip_ptr,
                    NX_FTP_SERVER_CONTROL_PORT,
...
VOID _nx_ftp_server_control_disconnect(NX_TCP_SOCKET *control_socket_ptr) {
    ...
    tx_event_flags_set(
        &(server_ptr->nx_ftp_server_event_flags),
        NX_FTP_SERVER_CONTROL_DISCONNECT,
        TX_OR);
}
...
VOID _nx_ftp_server_control_disconnect_processing(NX_FTP_SERVER *ftp_server_ptr) {
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++) {
        ...
        if ((client_req_ptr->nx_ftp_client_request_control_socket.nx_tcp_socket_state >
            ...
            /* Relisten on this socket. This will probably fail, but it is needed just in
               case all available clients were in use at the time of the last relisten. */
            nx_tcp_server_socket_relisten(
                ftp_server_ptr->nx_ftp_server_ip_ptr,
                NX_FTP_SERVER_CONTROL_PORT,
                &(client_req_ptr->nx_ftp_client_request_control_socket));
...
UINT _nx_ftp_packet_allocate(
    ...
    if (client_request_ptr->nx_ftp_client_request_ip_type == NX_IP_VERSION_V6) {
        ...
        status =
            nx_packet_allocate(pool_ptr, packet_ptr, NX_IPv6_TCP_PACKET, wait_option);
    } else {

        /* This is an IPv4 connection.  */
        status =
            nx_packet_allocate(pool_ptr, packet_ptr, NX_IPv4_TCP_PACKET, wait_option);
    }
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_utility_parse_IPv6_address(
    CHAR *buffer_ptr, UINT buffer_length, NXD_ADDRESS *ipduo_address) {
    ...
    if ((buffer_ptr[0] == 'E' || buffer_ptr[0] == 'e') &&
        ...
        (buffer_ptr[3] == 'T' || buffer_ptr[3] == 't') && (buffer_ptr[4] == ' ')) {
    ...
    while (j < buffer_length) {
        ...
        /* Determine if a CR/LF is present.  */
            ...
            /*  Whether this is a prematurely terminated IPv6 string, abort any further
             * parsing. */
            break;
        }
        ...
        /* Determine if a colon is present.  */
            ...
            if (colons % 2 == 1) {
                ...
                ipduo_address->nxd_ip_address.v6[k] =
                    ipduo_address->nxd_ip_address.v6[k] & 0xFFFF0000;
            ...
            if (buffer_ptr[j + 1] == ':') {
                ...
                /* Compute the number of zeros to insert into the NXD_ADDRESS IPv6 ULONG
                 * array. */
                number_consecutive_zeros_to_add = remaining_colons - actual_colons_left;

                /* Add the zeroes until its time to start parsing rest of the IPv6 address
                 * string. */
                while (number_consecutive_zeros_to_add) {
                    ...
                    if (colons % 2 == 1) {
                        ...
                        /* Yes, shift the bits in v6[k] left, and insert a zero at the
        ...
        /* Have we hit the end of the IPv6 address? */
            ...
            /* Clear the temporary ULONG so we know we applied it to our IPv6 address
             * array. */
            temp = 0;
    ...
    if (temp) {
        ...
        /* Mask out the lower 16 bits. */
        ipduo_address->nxd_ip_address.v6[3] =
            ipduo_address->nxd_ip_address.v6[3] & 0xFFFF0000;
...
UINT _nx_ftp_utility_parse_port_number(
    CHAR *buffer_ptr, UINT buffer_length, UINT *portnumber) {
    ...
    while ((INT)j >= 0) {
        ...
        if (buffer_ptr[j] == '|') {
            ...
            while (j < buffer_length - 2) {
                ...
                if ((buffer_ptr[j] == ':')) {
                    /* Yes, apparently this string is missing the separator and port
                     * number. */
                    return NX_FTP_INVALID_NUMBER;
                }
                ...
                if ((buffer_ptr[j] >= '0') && (buffer_ptr[j] <= '9')) {
                    ...
                    *portnumber = (*portnumber * 10) + (ULONG)(buffer_ptr[j] - '0');
                    ...
                    if ((buffer_ptr[j + 1] == '|') || (buffer_ptr[j + 1] == 13) ||
                        (buffer_ptr[j + 2] == 10)) {
                        return NX_SUCCESS;
                    }
...
/*                                                                        */
/**************************************************************************/
VOID _nx_ftp_server_block_size_get(
    NX_FTP_SERVER *ftp_server_ptr, UINT ftp_command, CHAR *filename, ULONG *block_size) {
    ...
    do {
        ...
        if (status == NX_SUCCESS) {
            ...
            if (_nx_utility_string_length_check(
                    filename, &length, FX_MAX_LONG_NAME_LEN - 1)) {
                return;
            }
            ...
            if (ftp_command == NX_FTP_NLST) {
                ...
                /* LIST. Add extra file info (52 bytes).
                   ...
                   11 chars for size (for file size up to 4gB), 14 for date, 2 chars for
                   cr lf.  */
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_server_block_header_send(
    NX_PACKET_POOL *pool_ptr,
    NX_FTP_CLIENT_REQUEST *client_request_ptr,
    ULONG block_size) {
    ...
    /* Allocate the packet.  */
    status = _nx_ftp_packet_allocate(
        pool_ptr, client_request_ptr, &packet_ptr, NX_TCP_PACKET, NX_FTP_SERVER_TIMEOUT);
...
/*                                                                        */
/**************************************************************************/
UINT _nx_ftp_server_block_header_retrieve(
    NX_FTP_CLIENT_REQUEST *ftp_client_ptr, NX_PACKET *packet_ptr) {
    ...
    if (ftp_client_ptr->nx_ftp_client_request_block_bytes == 0) {
        ...
        ftp_client_ptr->nx_ftp_client_request_block_bytes =
            (ULONG)((buffer_ptr[1] << 8) | buffer_ptr[2]);
    ...
    if (remaining_bytes < packet_ptr->nx_packet_length) {
        ...
        while (delta) {
            ...
            if (((ULONG)(last_packet->nx_packet_append_ptr -
                ...
                /* At this point, we need to release the last packet and adjust the other
                   packet pointers.  */
...
/*                                                                        */
/**************************************************************************/
VOID _nx_ftp_server_data_socket_cleanup(
    NX_FTP_SERVER *ftp_server_ptr, NX_FTP_CLIENT_REQUEST *client_req_ptr) {

    /* First, cleanup this data socket.  */
    nx_tcp_socket_disconnect(
        &(client_req_ptr->nx_ftp_client_request_data_socket), NX_FTP_SERVER_TIMEOUT);
