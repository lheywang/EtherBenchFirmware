...
/* Set a size of the Server option list requested by the Client. Since the NetX DHCP
   Server does not as yet support the complete set of options in DHCP, this number can be
   optimized down to a smaller size that the largest possible number of options a Client
   could request.
*/
...
/* At the very least, a DHCP server should supply the
   Client's subnet mask, router (default gateway) address and dns server. This list must
   be space separated. The combined required and optional server options should be less
   than NX_DHCP_CLIENT_OPTIONS_MAX.
*/
...
#define NX_DHCP_SERVER_OPTION_LIST_SIZE                                                  \
...
#define NX_DHCP_SERVER_OPTION_LIST                                                       \
    NX_DHCP_REQUIRED_SERVER_OPTION_LIST                                                  \
...
/*
   ...
   at least a 548 byte DHCP Client message as per RFC 2131 and allow room for Ethernet,
   UDP and IP headers and 4 byte alignment.
*/
#ifndef NX_DHCP_MINIMUM_PACKET_PAYLOAD
#define NX_DHCP_MINIMUM_PACKET_PAYLOAD                                                   \
    (NX_BOOT_BUFFER_SIZE + NX_PHYSICAL_HEADER + sizeof(NX_IPV4_HEADER) +                 \
     sizeof(NX_UDP_HEADER))
#endif /* NX_DHCP_MINIMUM_PACKET_PAYLOAD */

/* Define the DHCP Message Area Offsets.  The DHCP message format is identical to that of
   BootP, except for the Vendor options that start at the offset specified by
   NX_DHCP_OFFSET_OPTIONS.  */

#define NX_DHCP_OFFSET_OP 0 /* 1 BootP Operation 1=req, 2=reply */
#define NX_DHCP_OFFSET_HTYPE                                                             \
    1 /* 1 Hardware type 1 = Ethernet                             */
#define NX_DHCP_OFFSET_HLEN                                                              \
    2 /* 1 Hardware address length, 6 for Ethernet                */
#define NX_DHCP_OFFSET_HOPS                                                              \
    3 /* 1 Number of hops, usually 0                              */
#define NX_DHCP_OFFSET_XID                                                               \
    4 /* 4 Transaction ID, pseudo random number                   */
#define NX_DHCP_OFFSET_SECS                                                              \
    8 /* 2 Seconds since boot                                     */
#define NX_DHCP_OFFSET_FLAGS                                                             \
    10 /* 2 Flags, 0x80 = Broadcast response, 0 = unicast response */
#define NX_DHCP_OFFSET_CLIENT_IP                                                         \
    12 /* 4 Initial client IP, used as dest for unicast response   */
#define NX_DHCP_OFFSET_YOUR_IP                                                           \
    16 /* 4 Assigned IP, initialized to 0.0.0.0                    */
#define NX_DHCP_OFFSET_SERVER_IP                                                         \
    20 /* 4 Server IP, usually initialized to 0.0.0.0              */
#define NX_DHCP_OFFSET_RELAY_IP                                                          \
    24 /* 4 Relay IP, usually 0.0.0.0, only for DHCP               */
#define NX_DHCP_OFFSET_CLIENT_HW                                                         \
    28 /* 16 Client hardware address                               */
#define NX_DHCP_OFFSET_SERVER_NM                                                         \
    44 /* 64 Server name, nulls if unused                          */
#define NX_DHCP_OFFSET_BOOT_FILE                                                         \
    108 /* 128 Boot file name, null if unused                       */
#define NX_DHCP_OFFSET_VENDOR                                                            \
    236 /* 64 Vendor options, set first 4 bytes to a magic number   */
#define NX_DHCP_OFFSET_OPTIONS                                                           \
    240 /* First variable vendor option                             */
#define NX_DHCP_OFFSET_END                                                               \
    300 /* End of BOOTP buffer                                      */
...
#define NX_DHCP_STATE_BOOT                                                               \
    1 /* Started with a previous address                          */
#define NX_DHCP_STATE_INIT                                                               \
    2 /* Started with no previous address                         */
#define NX_DHCP_STATE_SELECTING                                                          \
    3 /* Waiting to identify a DHCP server                        */
#define NX_DHCP_STATE_REQUESTING                                                         \
    4 /* Address requested, waiting for the Ack                   */
#define NX_DHCP_STATE_BOUND                                                              \
    5 /* Address established, no time outs                        */
#define NX_DHCP_STATE_RENEWING                                                           \
    6 /* Address established, renewal time out                    */
#define NX_DHCP_STATE_REBINDING                                                          \
    7 /* Address established, renewal and rebind time out         */
#define NX_DHCP_STATE_FORCERENEW                                                         \
    8 /* Address established, force renewal                       */

/* Define error codes from DHCP Server operation.  */

#define NX_DHCP_SERVER_ALREADY_STARTED 0x90 /* DHCP Server already started      */
#define NX_DHCP_SERVER_NOT_STARTED                                                       \
    0x91                             /* DHCP Server not started when stop was issued   */
#define NX_DHCP_PARAMETER_ERROR 0x92 /* Invalid non pointer input   */
#define NX_DHCP_INADEQUATE_PACKET_POOL_PAYLOAD                                           \
    0x93 /* DHCP Server packet pool has inadequate packet payload for DHCP messages. */
#define NX_DHCP_BAD_OPTION_LIST_ERROR                                                    \
    0x94 /* Server default option list has invalid characters. */
#define NX_DHCP_INTERNAL_OPTION_PARSE_ERROR                                              \
    0x95 /* Internal error parsing options from string to ULONG. */
#define NX_DHCP_NO_SERVER_OPTION_LIST                                                    \
    0x96 /* Server default option list is empty when server is configured to use it. */
#define NX_DHCP_IMPROPERLY_TERMINATED_OPTION                                             \
    0x97 /* Improperly formatted client option data e.g missing terminating symbol 0xFF. \
          */
#define NX_DHCP_ADD_OPTION_ERROR                                                         \
    0x98 /* Unable to add option to buffer of server DHCP response packet */
#define NX_DHCP_INVALID_IP_ADDRESS_LIST                                                  \
    0x99 /* Invalid start or end IP address list parameter for creating assignable IP    \
            address list.   */
#define NX_DHCP_NO_AVAILABLE_IP_ADDRESSES                                                \
    0x9A /* Server has no available IP addresses for DHCP Clients  */
#define NX_DHCP_INVALID_IP_ADDRESS                                                       \
    0x9B /* Invalid IP address e.g. null address received or obtained from client        \
            record. */
#define NX_DHCP_IP_ADDRESS_NOT_FOUND 0x9C /* No match found for IP address search */
#define NX_DHCP_IP_ADDRESS_ASSIGNED_TO_OTHER                                             \
    0x9D /* IP address not assigned to the current client */
#define NX_DHCP_INVALID_UPDATE_ADDRESS_CMD                                               \
    0x9E /* Unable to ascertain assigned ip address status (e.g. assign, mark available  \
            etc) */
#define NX_DHCP_CLIENT_RECORD_NOT_FOUND                                                  \
    0x9F /* Unable to find client in Client records table. */
#define NX_DHCP_CLIENT_TABLE_FULL                                                        \
    0xA0 /* Server unable to add record to client table; table is full. */
#define NX_DHCP_SERVER_BAD_INTERFACE_INDEX                                               \
    0xA1 /* Interface index exceeds the NX_MAX_PHYSICAL_INTERFACES limit.  */
#define NX_DHCP_INVALID_HW_ADDRESS                                                       \
    0xA2 /* Invalid hardware address e.g. null or exceeding 6 bytes. */
#define NX_DHCP_INVALID_NETWORK_PARAMETERS                                               \
    0xA3 /* Invalid network parameters (subnet, dns server, default gateway/router). */
...
#define NX_DHCP_ADDRESS_STATUS_SERVER_ASSIGN                                             \
    0x02 /* Server will assign IP address to client. */
#define NX_DHCP_ADDRESS_STATUS_ASSIGNED_EXT                                              \
    0x03 /* Client assigned IP address elesewhere, mark client as owner and IP as not    \
            available. */
#define NX_DHCP_ADDRESS_STATUS_MARK_AVAILABLE                                            \
    0x04 /* Mark client's IP address as available and clear owner field. */
#define NX_DHCP_ADDRESS_STATUS_ASSIGNED_OTHER                                            \
    0x05 /* Mark client's IP address as assigned already (owner unknown). */
...
/* Define the DHCP structure that contains DHCP client information during DHCP session.
   Note this is not the same control block as the NX_DHCP_STRUCT in nx_dhcp.h for the NetX
   DHCP Client package.  */

typedef struct NX_DHCP_CLIENT_STRUCT {

    UINT nx_dhcp_client_state;  /* Client DHCP state: e.g. INIT, BOOT, SELECTING, RENEWING
                                   etc */
    UCHAR nx_dhcp_message_type; /* DHCP message type (DISCOVER, REQUEST etc) received from
                                   Client. */
    ...
    ULONG nx_dhcp_clientip_address; /* "Client IP address" (sometimes called "ci-addr"
                                       field). */
    ULONG nx_dhcp_your_ip_address;  /* "Your IP address" field in client DHCP message. */
    ULONG
    nx_dhcp_requested_ip_address; /* IP address requested in client message option. */
    ULONG nx_dhcp_requested_lease_time; /* IP address lease time requested in client
                                           message option. */
    ULONG nx_dhcp_assigned_ip_address; /* IP address the Server offers/assigns the client.
                                        */
    UINT nx_dhcp_client_iface_index; /* Index into the server interface table matching the
                                        DHCP client's packet interface. */
    ULONG nx_dhcp_clientrec_server_ip; /* Next Server IP Address (may be another DHCP
                                          server) for advanced DHCP features */
    ULONG nx_dhcp_server_id; /* Requested/assigned Server ID (usually set to DHCP server
                                IP address) */
    ULONG
    nx_dhcp_router_ip_address;    /* Requested/assigned router on DHCP Client network. */
    ULONG nx_dhcp_dns_ip_address; /* Requested/assigned DNS IP address; usually set to
                                     zero. */
    ULONG nx_dhcp_relay_ip_address; /* Requested/assigned Relay IP address; usually set to
                                       zero. */
...
typedef struct NX_DHCP_INTERFACE_TABLE_STRUCT {
    ...
    ULONG nx_dhcp_dns_ip_address;    /* DHCP server DNS Server Address in message to DHCP
                                        Client..  */
    ULONG nx_dhcp_subnet_mask;       /* DHCP server interface subnet mask. */
    ULONG nx_dhcp_subnet;            /* DHCP server interface subnet. */
    ULONG nx_dhcp_router_ip_address; /* The router IP Address for DHCP client
                                        configuration  */
    UINT nx_dhcp_address_list_size;  /* Actual number of assignable addresses for this
                                        interface. */
...
/* Define the DHCP structure that contains all the server information necessary for this
   DHCP instance.  */
...
typedef struct NX_DHCP_SERVER_STRUCT {
    ...
    UINT nx_dhcp_number_clients; /* Number of clients currently assigned IP address by
                                    this server. */
    NX_DHCP_CLIENT
    client_records[NX_DHCP_CLIENT_RECORD_TABLE_SIZE]; /* Table of DHCP clients.*/
    /* List of IP addresses server can assign to DHCP Clients */
    NX_UDP_SOCKET nx_dhcp_socket; /* DHCP server socket to receive DHCP messages on its
                                     interfaces. */
    UINT nx_dhcp_server_options[NX_DHCP_SERVER_OPTION_LIST_SIZE];
    /* List of max number of options in supply data to Client */
    UINT nx_dhcp_server_option_count; /* Actual number of options the server is supplying
                                         back to client. */
...
#define nx_dhcp_set_interface_network_parameters                                         \
    _nxe_dhcp_set_interface_network_parameters
