...
/* If the host application wishes to specify a source port, define it here. The default is
   to let NetX choose the TFTP Client source port. */
...
#define NX_TFTP_FILE_TRANSFER_MAX                                                        \
    512 /* 512 byte maximum file transfer                      */

/* Define open types.  */

#define NX_TFTP_OPEN_FOR_READ                                                            \
    0x01 /* TFTP open for reading                                */
#define NX_TFTP_OPEN_FOR_WRITE                                                           \
    0x02 /* TFTP open for writing                                */
...
#define NX_TFTP_ERROR_NOT_DEFINED                                                        \
    0x00 /* TFTP not defined error code, see error string        */
#define NX_TFTP_ERROR_FILE_NOT_FOUND                                                     \
    0x01 /* TFTP file not found error code                       */
#define NX_TFTP_ERROR_ACCESS_VIOLATION                                                   \
    0x02 /* TFTP file access violation error code                */
#define NX_TFTP_ERROR_DISK_FULL                                                          \
    0x03 /* TFTP disk full error code                            */
#define NX_TFTP_ERROR_ILLEGAL_OPERATION                                                  \
    0x04 /* TFTP illegal operation error code                    */
#define NX_TFTP_CODE_ERROR                                                               \
    0x05 /* TFTP client request received error code from server                          \
          */
#define NX_TFTP_ERROR_FILE_EXISTS                                                        \
    0x06 /* TFTP file already exists error code                  */
#define NX_TFTP_ERROR_NO_SUCH_USER                                                       \
    0x07 /* TFTP no such user error code                         */
#define NX_TFTP_INVALID_SERVER_ADDRESS                                                   \
    0x08 /* Invalid TFTP server IP extracted from received packet*/
#define NX_TFTP_NO_ACK_RECEIVED                                                          \
    0x09 /* Did not receive TFTP server ACK response             */
#define NX_TFTP_INVALID_BLOCK_NUMBER                                                     \
    0x0A /* Invalid block number received from Server response   */
#define NX_TFTP_INVALID_INTERFACE                                                        \
    0x0B /* Invalid interface for TFTP Client                    */
         /* (or multihome not supported)                         */
#define NX_TFTP_INVALID_IP_VERSION                                                       \
    0x0C /* Invalid or unsupported IP version specified          */

/* Define offsets into the TFTP message buffer.  */

#define NX_TFTP_CODE_OFFSET 0 /* Offset to TFTP code in buffer                        */
#define NX_TFTP_FILENAME_OFFSET                                                          \
    2 /* Offset to TFTP filename in message                   */
#define NX_TFTP_BLOCK_NUMBER_OFFSET                                                      \
    2                         /* Offset to TFTP block number in buffer                */
#define NX_TFTP_DATA_OFFSET 4 /* Offset to TFTP data in buffer                        */
#define NX_TFTP_ERROR_CODE_OFFSET                                                        \
    2 /* Offset to TFTP error code                            */
#define NX_TFTP_ERROR_STRING_OFFSET                                                      \
    4 /* Offset to TFPT error string                          */
...
#define NX_TFTP_END_OF_FILE                                                              \
    0xC5                        /* TFTP end of file error                               */
...
#define NX_TFTP_STATE_NOT_OPEN                                                           \
    0                        /* TFTP connection not open                             */
#define NX_TFTP_STATE_OPEN 1 /* TFTP connection open                                 */
#define NX_TFTP_STATE_WRITE_OPEN                                                         \
    2 /* TFTP connection open for writing                     */
#define NX_TFTP_STATE_END_OF_FILE                                                        \
    3                         /* TFTP connection at end of file                       */
#define NX_TFTP_STATE_ERROR 4 /* TFTP error condition                                 */
#define NX_TFTP_STATE_FINISHED                                                           \
    5 /* TFTP finished writing condition                      */
...
typedef struct NX_TFTP_CLIENT_STRUCT {
    ...
    NX_PACKET_POOL
    *nx_tftp_client_packet_pool_ptr;      /* Pointer to TFTP client packet pool   */
...
#define nx_tftp_client_create(a, b, c, d)                                                \
    _nxd_tftp_client_create(a, b, c, d, NX_IP_VERSION_V4)
#define nx_tftp_client_packet_allocate(a, b, c)                                          \
    _nxd_tftp_client_packet_allocate(a, b, c, NX_IP_VERSION_V4)
...
#define nx_tftp_client_file_read(a, b, c)                                                \
    _nxd_tftp_client_file_read(a, b, c, NX_IP_VERSION_V4)
#define nx_tftp_client_file_write(a, b, c)                                               \
    _nxd_tftp_client_file_write(a, b, c, NX_IP_VERSION_V4)
...
/* For NetX TFTP applications, map the NetX TFTP service to the equivalent NetX Duo TFTP
 * service.  */
#define nx_tftp_client_create(a, b, c, d)                                                \
    _nxde_tftp_client_create(a, b, c, d, NX_IP_VERSION_V4)
#define nx_tftp_client_packet_allocate(a, b, c)                                          \
    _nxde_tftp_client_packet_allocate(a, b, c, NX_IP_VERSION_V4)
...
#define nx_tftp_client_file_read(a, b, c)                                                \
    _nxde_tftp_client_file_read(a, b, c, NX_IP_VERSION_V4)
#define nx_tftp_client_file_write(a, b, c)                                               \
    _nxde_tftp_client_file_write(a, b, c, NX_IP_VERSION_V4)
...
UINT nxd_tftp_client_error_info_get(
    NX_TFTP_CLIENT *tftp_client_ptr, UINT *error_code, CHAR **error_string);
...
UINT nxd_tftp_client_packet_allocate(
    NX_PACKET_POOL *pool_ptr, NX_PACKET **packet_ptr, ULONG wait_option, UINT ip_type);
...
UINT _nxe_tftp_client_file_open(
    NX_TFTP_CLIENT *tftp_client_ptr,
    CHAR *file_name,
    ULONG server_ip_address,
    UINT open_type,
    ULONG wait_option);
UINT _nx_tftp_client_file_open(
    NX_TFTP_CLIENT *tftp_client_ptr,
    CHAR *file_name,
    ULONG server_ip_address,
    UINT open_type,
...
UINT _nxde_tftp_client_create(
    NX_TFTP_CLIENT *tftp_client_ptr,
    CHAR *tftp_client_name,
    NX_IP *ip_ptr,
    NX_PACKET_POOL *pool_ptr,
    UINT ip_type);
UINT _nxd_tftp_client_create(
    NX_TFTP_CLIENT *tftp_client_ptr,
    CHAR *tftp_client_name,
    NX_IP *ip_ptr,
    NX_PACKET_POOL *pool_ptr,
...
UINT _nxde_tftp_client_error_info_get(
    NX_TFTP_CLIENT *tftp_client_ptr, UINT *error_code, CHAR **error_string);
UINT _nxd_tftp_client_error_info_get(
    NX_TFTP_CLIENT *tftp_client_ptr, UINT *error_code, CHAR **error_string);
...
UINT _nxde_tftp_client_file_open(
    NX_TFTP_CLIENT *tftp_client_ptr,
    CHAR *file_name,
    NXD_ADDRESS *server_ip_address,
    UINT open_type,
    ULONG wait_option,
    UINT ip_type);
UINT _nxd_tftp_client_file_open(
    NX_TFTP_CLIENT *tftp_client_ptr,
    CHAR *file_name,
    NXD_ADDRESS *server_ip_address,
    UINT open_type,
    ULONG wait_option,
    UINT ip_type);
UINT _nxde_tftp_client_file_read(
    NX_TFTP_CLIENT *tftp_client_ptr,
    NX_PACKET **packet_ptr,
    ULONG wait_option,
    UINT ip_type);
UINT _nxd_tftp_client_file_read(
    NX_TFTP_CLIENT *tftp_client_ptr,
    NX_PACKET **packet_ptr,
    ULONG wait_option,
    UINT ip_type);
UINT _nxde_tftp_client_file_write(
    NX_TFTP_CLIENT *tftp_client_ptr,
    NX_PACKET *packet_ptr,
    ULONG wait_option,
    UINT ip_type);
UINT _nxd_tftp_client_file_write(
    NX_TFTP_CLIENT *tftp_client_ptr,
    NX_PACKET *packet_ptr,
    ULONG wait_option,
    UINT ip_type);
UINT _nxde_tftp_client_packet_allocate(
    NX_PACKET_POOL *pool_ptr, NX_PACKET **packet_ptr, ULONG wait_option, UINT ip_type);
UINT _nxd_tftp_client_packet_allocate(
    NX_PACKET_POOL *pool_ptr, NX_PACKET **packet_ptr, ULONG wait_option, UINT ip_type);
...
/* Internal functions. */
UINT _nx_tftp_client_file_open_internal(
    NX_TFTP_CLIENT *tftp_client_ptr,
    CHAR *file_name,
    NXD_ADDRESS *server_ip_address,
    UINT open_type,
