...
#define NX_HTTP_SERVER_RETRY_SECONDS                                                     \
    2 /* 2 second initial timeout                            */
#endif

#ifndef NX_HTTP_SERVER_TRANSMIT_QUEUE_DEPTH
#define NX_HTTP_SERVER_TRANSMIT_QUEUE_DEPTH                                              \
    20 /* Maximum of 20 queued transmit packets               */
#endif

#ifndef NX_HTTP_SERVER_RETRY_MAX
#define NX_HTTP_SERVER_RETRY_MAX                                                         \
    10 /* Maximum of 10 retries per packet                    */
#endif

#ifndef NX_HTTP_SERVER_RETRY_SHIFT
#define NX_HTTP_SERVER_RETRY_SHIFT                                                       \
    1 /* Every retry is twice as long                        */
#endif

#ifndef NX_HTTP_SERVER_NONCE_SIZE
#define NX_HTTP_SERVER_NONCE_SIZE                                                        \
    32 /* The size of nonce for digest authtentication        */
#endif

#ifndef NX_HTTP_SERVER_NONCE_MAX
#define NX_HTTP_SERVER_NONCE_MAX                                                         \
    2 /* The size of nonce for digest authtentication        */
#endif
...
/* NX_HTTP_MAX_STRING is base64 of "name:password" and plus 1 if an extra conversion is
 * needed and plus 2 pad if needed. */
...
#define NX_HTTP_DONT_AUTHENTICATE                                                        \
    0xE3 /* HTTP authentication not needed                       */
#define NX_HTTP_BASIC_AUTHENTICATE                                                       \
    0xE4 /* HTTP basic authentication requested                  */
#define NX_HTTP_DIGEST_AUTHENTICATE                                                      \
    0xE5                       /* HTTP digest authentication requested                 */
#define NX_HTTP_NOT_FOUND 0xE6 /* HTTP request not found */
#define NX_HTTP_DATA_END 0xE7  /* HTTP end of content area                             */
#define NX_HTTP_CALLBACK_COMPLETED                                                       \
    0xE8                        /* HTTP user callback completed the processing          */
...
#define NX_HTTP_AUTHENTICATION_ERROR                                                     \
    0xEB                      /* HTTP client authentication failed                    */
#define NX_HTTP_GET_DONE 0xEC /* HTTP client get is complete                          */
#define NX_HTTP_BAD_PACKET_LENGTH                                                        \
    0xED /* Invalid packet received - length incorrect           */
#define NX_HTTP_REQUEST_UNSUCCESSFUL_CODE                                                \
    0xEE /* Received an error code instead of 2xx from server    */
#define NX_HTTP_INCOMPLETE_PUT_ERROR                                                     \
    0xEF /* Server responds before PUT is complete               */
#define NX_HTTP_PASSWORD_TOO_LONG                                                        \
    0xF0 /* Password exceeded expected length                    */
#define NX_HTTP_USERNAME_TOO_LONG                                                        \
    0xF1 /* Username exceeded expected length                    */
#define NX_HTTP_NO_QUERY_PARSED                                                          \
    0xF2 /* Server unable to find query in client request        */
#define NX_HTTP_IMPROPERLY_TERMINATED_PARAM                                              \
    0xF3 /* Client request parameter not properly terminated     */
#define NX_HTTP_BOUNDARY_ALREADY_FOUND                                                   \
    0xF4 /* Boundary is already found.                           */
...
typedef struct NX_HTTP_SERVER_STRUCT {
    ...
    NX_PACKET_POOL
    *nx_http_server_packet_pool_ptr;           /* Pointer to HTTP Server packet pool   */
    ...
    *nx_http_server_mime_maps_additional; /* Additional HTTP MIME maps            */
    UINT
        nx_http_server_mime_maps_additional_num; /* Number of additional HTTP MIME maps */
    /* Define the user supplied routines that are used to inform the application of
     * particular server requests.  */

    UINT (*nx_http_server_authentication_check)(
        struct NX_HTTP_SERVER_STRUCT *server_ptr,
        UINT request_type,
        CHAR *resource,
        CHAR **name,
        CHAR **password,
        CHAR **realm);
    UINT (*nx_http_server_authentication_check_extended)(
        struct NX_HTTP_SERVER_STRUCT *server_ptr,
        UINT request_type,
        CHAR *resource,
        CHAR **name,
        ...
        CHAR **realm,
        UINT *realm_length);
    UINT (*nx_http_server_request_notify)(
        struct NX_HTTP_SERVER_STRUCT *server_ptr,
        UINT request_type,
        CHAR *resource,
        NX_PACKET *packet_ptr);
    UINT (*nx_http_server_invalid_username_password_callback)(
        CHAR *resource, NXD_ADDRESS *client_nxd_address, UINT request_type);
    VOID (*nx_http_server_gmt_get)(NX_HTTP_SERVER_DATE *date);
    UINT (*nx_http_server_cache_info_get)(
        CHAR *resource, UINT *max_age, NX_HTTP_SERVER_DATE *last_modified);
#ifdef NX_HTTP_DIGEST_ENABLE
    UINT (*nx_http_server_digest_authenticate_callback)(
        struct NX_HTTP_SERVER_STRUCT *server_ptr,
        CHAR *name_ptr,
        CHAR *realm_ptr,
        CHAR *password_ptr,
...
#define nx_http_server_callback_response_send_extended                                   \
...
#define nx_http_server_content_length_get_extended                                       \
...
#define nx_http_server_callback_generate_response_header                                 \
    _nx_http_server_callback_generate_response_header
#define nx_http_server_callback_generate_response_header_extended                        \
...
#define nx_http_server_invalid_userpassword_notify_set                                   \
    _nx_http_server_invalid_userpassword_notify_set
#define nx_http_server_digest_authenticate_notify_set                                    \
...
#define nx_http_server_create(p, n, i, m, sp, ss, pp, a, r)                              \
    _nxe_http_server_create(p, n, i, m, sp, ss, pp, a, r, sizeof(NX_HTTP_SERVER))
...
#define nx_http_server_content_length_get_extended                                       \
    _nxe_http_server_content_length_get_extended
...
#define nx_http_server_invalid_userpassword_notify_set                                   \
    _nxe_http_server_invalid_userpassword_notify_set
#define nx_http_server_digest_authenticate_notify_set                                    \
