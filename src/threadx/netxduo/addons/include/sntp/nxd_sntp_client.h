...
#define NX_SNTP_CLIENT_RECEIVE_EVENT                                                     \
    ((ULONG)0x00000001) /* Event flag to signal a receive packet event */
...
#define RATE                                                                             \
    "RATE" /* Rate exceeded.  The server temporarily denied access; client exceeded rate \
              threshold.  */
#define RMOT                                                                             \
    "RMOT" /* Somebody is tinkering with association from remote host running ntpdc.  OK \
              unless they've stolen your keys.  */
#define STEP                                                                             \
    "STEP" /* A step change in system time has occurred, but association has not yet     \
              resynchronized.  */
...
/* Define SNTP protocol modes. SNTP is limited to primarily Client, server and broadcast
 * modes.  */
...
/* Set size of SNTP data in bytes, not including IP and UDP header fields or
 * authentication.  */
#define NX_SNTP_CLIENT_PACKET_DATA_SIZE 48

/* To display date in year/month/date format, set the current year (same year as in NTP
   time being evaluated) e.g 2015. Otherwise set to zero. */
...
/* Define the number of seconds from 01-01-1990 00:00:00 to 01-01-1999 00:00:00 (last
   occurrance of a leap second) to be able to define time in year, month, date. If zero,
...
#define NX_SNTP_CLIENT_NOT_INITIALIZED                                                   \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x01) /* Client not properly initialized to receive time data.  */
#define NX_SNTP_OVER_LIMIT_ON_SERVERS                                                    \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x02) /* Cannot accept server because client list has reached max # servers.  */
#define NX_SNTP_INVALID_DOMAIN                                                           \
    (NX_SNTP_ERROR_CONSTANT | 0x03) /* Invalid domain such as bad IP format or empty     \
                                       string. Applicable to broadcast mode.  */
#define NX_SNTP_NO_AVAILABLE_SERVERS                                                     \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x04) /* Client has no available time servers to contact.  */
#define NX_SNTP_INVALID_LOCAL_TIME                                                       \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x05) /* Client local time has not been set or is invalid.  */
#define NX_SNTP_OUT_OF_DOMAIN_SERVER                                                     \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x06) /* Broadcast server does not belong to client broadcast domain.  */
#define NX_SNTP_INVALID_DATETIME_BUFFER                                                  \
    (NX_SNTP_ERROR_CONSTANT | 0x07) /* Insufficient or invalid buffer for writing human  \
                                       readable time date string.  */
#define NX_SNTP_ERROR_CONVERTING_DATETIME                                                \
    (NX_SNTP_ERROR_CONSTANT | 0x08) /* An internal error has occurred converting NTP     \
                                       time to mm-dd-yy time format.  */
#define NX_SNTP_UNABLE_TO_CONVERT_DATETIME                                               \
    (NX_SNTP_ERROR_CONSTANT | 0x09) /* An internal error has occurred converting NTP     \
                                       time to mm-dd-yy time format.  */
#define NX_SNTP_INVALID_SERVER_ADDRESS                                                   \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x0A) /* Invalid server type e.g. IPv4 or IPv6 incompatible. */
#define NX_SNTP_CLIENT_NOT_STARTED                                                       \
    (NX_SNTP_ERROR_CONSTANT | 0x0B) /* SNTP Client task is not running */
#define NX_SNTP_CLIENT_ALREADY_STARTED                                                   \
    (NX_SNTP_ERROR_CONSTANT | 0x0C) /* SNTP Client task is already running */
#define NX_SNTP_PARAM_ERROR                                                              \
    (NX_SNTP_ERROR_CONSTANT | 0x0D) /* Invalid non pointer parameter.  */

/* Server side errors */
#define NX_SNTP_SERVER_NOT_AVAILABLE                                                     \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x10) /* Client will not get any time update service from current server.  */
#define NX_SNTP_NO_UNICAST_FROM_SERVER                                                   \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x11) /* Client did not receive a valid unicast response from server.  */
#define NX_SNTP_SERVER_CLOCK_NOT_SYNC                                                    \
    (NX_SNTP_ERROR_CONSTANT | 0x12) /* Server clock not synchronized.  */
#define NX_SNTP_KOD_SERVER_NOT_AVAILABLE                                                 \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x13) /* Server sent a KOD packet indicating service temporarily not available.  */
#define NX_SNTP_KOD_REMOVE_SERVER                                                        \
    (NX_SNTP_ERROR_CONSTANT | 0x14) /* Server sent a KOD packet indicating service is    \
                                       not available to client (ever).  */
#define NX_SNTP_SERVER_AUTH_FAIL                                                         \
    (NX_SNTP_ERROR_CONSTANT | 0x15) /* Server rejects Client packet on basis of missing  \
                                       or invalid authorization data.  */

/* Bad packet and time update errors */
#define NX_SNTP_INVALID_TIME_PACKET                                                      \
    (NX_SNTP_ERROR_CONSTANT | 0x20) /* Invalid packet (length or data incorrect).   */
#define NX_SNTP_INVALID_NTP_VERSION                                                      \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x21) /* Server NTP/SNTP version not incompatible with client.  */
#define NX_SNTP_INVALID_SERVER_MODE                                                      \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x22) /* Server association invalid (out of protocol with client).  */
#define NX_SNTP_INVALID_SERVER_PORT                                                      \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x23) /* Server port does not match what the client expects.  */
#define NX_SNTP_INVALID_IP_ADDRESS                                                       \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x24) /* Server IP address does not match what the client expects.  */
#define NX_SNTP_INVALID_SERVER_STRATUM                                                   \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x25) /* Server stratum is invalid or below client stratum.  */
#define NX_SNTP_BAD_SERVER_ROOT_DISPERSION                                               \
    (NX_SNTP_ERROR_CONSTANT | 0x26) /* Server root dispersion (clock precision) is too   \
                                       high or invalid value (0) reported.  */
#define NX_SNTP_OVER_INVALID_LIMIT                                                       \
    (NX_SNTP_ERROR_CONSTANT | 0x27) /* Client over the limit on consecutive server       \
                                       updates with bad data received.  */
#define NX_SNTP_DUPE_SERVER_PACKET                                                       \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x28) /* Client has received duplicate packets from server.  */
#define NX_SNTP_INVALID_TIMESTAMP                                                        \
    (NX_SNTP_ERROR_CONSTANT | 0x29) /* Server time packet has one or more invalid time   \
                                       stamps in update message.  */
#define NX_SNTP_INSUFFICIENT_PACKET_PAYLOAD                                              \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x2A) /* Packet payload not large enough for SNTP message.  */
#define NX_SNTP_INVALID_SNTP_PACKET                                                      \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x2B) /* Server IP version does not match what the client expects.  */

/* Arithmetic errors or invalid results */
#define NX_SNTP_INVALID_TIME                                                             \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x30) /* Invalid time resulting from arithmetic operation.  */
#define NX_SNTP_INVALID_RTT_TIME                                                         \
    (NX_SNTP_ERROR_CONSTANT | 0x31) /* Round trip time correction to server time yields  \
                                       invalid time (e.g. <0).  */
#define NX_SNTP_OVERFLOW_ERROR                                                           \
    (NX_SNTP_ERROR_CONSTANT | 0x32) /* Overflow error resulting from multiplying/adding  \
                                       two 32 bit (timestamp) numbers.  */
#define NX_SNTP_SIGN_ERROR                                                               \
    (NX_SNTP_ERROR_CONSTANT |                                                            \
     0x33) /* Loss of sign error resulting from multiplying/adding two 32 bit            \
              (timestamp) numbers.  */

/* Time out errors */
#define NX_SNTP_TIMED_OUT_ON_SERVER                                                      \
    (NX_SNTP_ERROR_CONSTANT | 0x44) /* Client did not receive update from the current    \
                                       server within specified timeout.  */
#define NX_SNTP_MAX_TIME_LAPSE_EXCEEDED                                                  \
    (NX_SNTP_ERROR_CONSTANT | 0x45) /* Client has not received update from any server    \
                                       within the max allowed time lapse.  */
...
typedef struct NX_SNTP_TIME_STRUCT {
    ...
    ULONG fraction; /* Fraction of a second, in the 32 bit fraction field of an NTP time
                       data.  */

} NX_SNTP_TIME;

/* Define the NetX SNTP Time Message structure.  The Client only uses the flags field and
   the transmit_time_stamp field in time requests it sends to its time server.  */
...
typedef struct NX_SNTP_TIME_MESSAGE_STRUCT {
    ...
    UINT peer_clock_stratum; /* Level of precision in the NTP network. Applicable only in
                                server NTP messages.  */
    UINT peer_poll_interval; /* Frequency at which an NTP host polls its NTP peer.
                                Applicable only in server NTP messages.  */
    UINT peer_clock_precision; /* Precision of the NTP server clock. Applicable only in
                                  server NTP messages.  */

    /* These are represented as 32 bit data fields in the time message format*/
    ULONG root_delay; /* Round trip time from NTP Server to primary reference source.
                         Applicable only in server NTP messages.  */
    ULONG clock_dispersion;      /* Server reference clock type (but if stratum is zero,
                                    indicates server status when not able to send time updates.
                                  */
    UCHAR reference_clock_id[4]; /* Maximum error in server clock based to the clock
                                    frequency tolerance. Applicable only in server NTP
                                    messages.  */

    /* These are represented as 64 bit data fields in the time message format*/
    ULONG reference_clock_update_time_stamp[2]; /* Time at which the server clock was last
                                                   set or corrected in a server time
                                                   message.  */
    ULONG originate_time_stamp[2]; /* Time at which the Client update request left the
                                      Client in a server time message.  */
    ULONG receive_time_stamp[2];   /* Time at which the server received the Client request
                                      in a server time message.  */
    ULONG transmit_time_stamp[2]; /* Time at which the server transmitted its reply to the
                                     Client in a server time message (or the time client
                                     request was sent in the client request message).  */

    /* Optional authenticator fields.  */
    UCHAR KeyIdentifier[4]; /* Key Identifier and Message Digest fields contain the...  */
    UCHAR
    MessageDigest[16]; /* ...message authentication code (MAC) information defined. */

    /* These fields are used internally for 'convert' UCHAR data in NX_SNTP_TIME data e.g.
     * seconds and fractions.  */
    NX_SNTP_TIME
    reference_clock_update_time; /* Time at which the server clock was last set or
                                    corrected in a server time message.  */
    NX_SNTP_TIME originate_time; /* Time at which the Client update request left the
                                    Client in a server time message.  */
    NX_SNTP_TIME receive_time;  /* Time at which the server received the Client request in
                                   a server time message.  */
    NX_SNTP_TIME transmit_time; /* Time at which the server transmitted its reply to the
                                   Client in a server time message (or the time client
                                   request was sent in the client request message).  */
...
typedef struct NX_SNTP_CLIENT_STRUCT {
    ...
    NX_PACKET_POOL
    *nx_sntp_client_packet_pool_ptr; /* Pointer to the Client packet pool.  */
    UINT nx_sntp_client_sleep_flag;  /* The flag indicating the SNTP Client thread is
                                        sleeping          */
    ...
    UCHAR nx_sntp_client_thread_stack
        [NX_SNTP_CLIENT_THREAD_STACK_SIZE]; /* Stack size for SNTP client thread        */
    NXD_ADDRESS nx_sntp_server_ip_address; /* Client's current time server IP address.  */
    NX_UDP_SOCKET nx_sntp_client_udp_socket;  /* Client UDP socket for sending and
                                                 receiving time updates.  */
    UINT nx_sntp_client_first_update_pending; /* First SNTP update not yet received with
                                                 current server   */
    UINT nx_sntp_client_time_start_wait; /* Initial time at start of receiving broadcast
                                            SNTP updates */
    UINT nx_sntp_client_sent_initial_unicast; /* Status on initial unicast transmittal for
                                                 clients in broadcast mode */
    UINT nx_sntp_client_invalid_time_updates; /* Number of invalid SNTP messages received
                                               */
    UINT nx_sntp_valid_server_status; /* Server status; if receiving valid updates, status
                                         is TRUE */
    UINT
        nx_sntp_client_protocol_mode; /* Mode of operation, either unicast or broadcast */
    UINT nx_sntp_client_broadcast_initialized; /* Client task is ready to receive
                                                  broadcast time data.  */
    NXD_ADDRESS nx_sntp_broadcast_time_server; /* Client's broadcast SNTP server */
    NXD_ADDRESS
    nx_sntp_multicast_server_address; /* IP address Client should listen on to receive
                                         broadcasts from a multicast server.  */
    UINT nx_sntp_client_unicast_initialized;    /* Client task is ready to receive unicast
                                                   time data.  */
    NXD_ADDRESS nx_sntp_unicast_time_server;    /* Client's unicast time server.  */
    ULONG nx_sntp_client_unicast_poll_interval; /* Unicast interval at which client is
                                                   polling the time server.  */
    UINT nx_sntp_client_backoff_count; /* Count of times the back off rate has been
                                          applied to the poll interval */
    TX_TIMER nx_sntp_update_timer; /* SNTP update timer; expires when no data received for
                                      specified time lapse.  */
    ULONG nx_sntp_update_time_remaining; /* Time (in seconds) remaining that the Client
                                            task can continue running without receiving a
                                            valid update.  */
    LONG nx_sntp_client_roundtrip_time_msec;     /* Round trip time (msec) for a packet to
                                                    travel to server and back to client. Does
                                                    not include server processing time.  */
    ULONG nx_sntp_client_local_ntp_time_elapsed; /* Seconds elapsed since local time is
                                                    updated last time. */
    NX_SNTP_TIME_MESSAGE
    nx_sntp_current_server_time_message; /* Time update which the Client has just
                                            received from its server.  */
    NX_SNTP_TIME_MESSAGE nx_sntp_current_time_message_request; /* Client request to send
                                                                  to its time server.  */
    NX_SNTP_TIME_MESSAGE
    nx_sntp_previous_server_time_message; /* Previous valid time update received from
                                             the Client time server.  */
    NX_SNTP_TIME nx_sntp_client_local_ntp_time; /* Client's notion of local time.  */
    NX_SNTP_TIME nx_sntp_server_update_time; /* Time (based on client local time) when the
                                                server update was received in response to
                                                the current request.  */
    UINT (*apply_custom_sanity_checks)(
        struct NX_SNTP_CLIENT_STRUCT *client_ptr,
        NX_SNTP_TIME_MESSAGE *client_time_msg_ptr,
        NX_SNTP_TIME_MESSAGE *server_time_msg_ptr);
    /* Pointer to callback service for  performing additional sanity checks on received
     * time data.  */
    ...
    /* Pointer to callback service for handling kiss of death packets received from
     * server.  */
    VOID (*random_number_generator)(
        struct NX_SNTP_CLIENT_STRUCT *client_ptr, ULONG *rand);
    /* Pointer to callback service for random number generator.  */

    VOID (*nx_sntp_client_time_update_notify)(
...
#define nx_sntp_client_utility_msecs_to_fraction                                         \
    _nxe_sntp_client_utility_msecs_to_fraction
#define nx_sntp_client_utility_usecs_to_fraction                                         \
    _nxe_sntp_client_utility_usecs_to_fraction
#define nx_sntp_client_utility_fraction_to_usecs                                         \
    _nxe_sntp_client_utility_fraction_to_usecs
...
UINT _nx_sntp_client_utility_get_msec_diff(
    NX_SNTP_TIME *timeA_ptr,
    NX_SNTP_TIME *timeB_ptr,
...
UINT _nx_sntp_client_utility_convert_time_to_UCHAR(
    NX_SNTP_TIME *time, NX_SNTP_TIME_MESSAGE *time_message_ptr, UINT which_stamp);
