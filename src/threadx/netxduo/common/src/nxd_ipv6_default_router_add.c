...
/*                                                                        */
/**************************************************************************/
UINT _nxd_ipv6_default_router_add(NX_IP *ip_ptr, NXD_ADDRESS *router_addr, ULONG router_lifetime,
                                  UINT interface_index) {
#ifdef FEATURE_NX_IPV6

    UINT status;

    NX_TRACE_IN_LINE_INSERT(NXD_TRACE_IPV6_DEFAULT_ROUTER_ADD, ip_ptr, router_addr->nxd_ip_address.v6[3],
                            router_lifetime, 0, NX_TRACE_IP_EVENTS, 0, 0);

    /* Obtain protection on this IP instance for access into the default router
     * table. */
    tx_mutex_get(&(ip_ptr->nx_ip_protection), TX_WAIT_FOREVER);

    status = _nxd_ipv6_default_router_add_internal(ip_ptr, router_addr->nxd_ip_address.v6, router_lifetime,
