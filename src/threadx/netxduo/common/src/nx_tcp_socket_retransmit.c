    ...
    if (socket_ptr->nx_tcp_socket_tx_window_advertised == 0) {
        ...
        if (packet_ptr) {
            ...
            socket_ptr->nx_tcp_socket_zero_window_probe_data =
                *(packet_ptr->nx_packet_prepend_ptr + ((header_ptr->nx_tcp_header_word_3 >> 28) << 2));
        ...
        */
        {
            ...
            checksum =
                _nx_ip_checksum_compute(packet_ptr, NX_PROTOCOL_TCP, packet_ptr->nx_packet_length, source_ip, dest_ip);
        ...
        NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_RETRY, ip_ptr, socket_ptr, packet_ptr,
                                socket_ptr->nx_tcp_socket_timeout_retries, NX_TRACE_INTERNAL_EVENTS, 0, 0);
        ...
        if ((next_ptr == (NX_PACKET *)NX_PACKET_ENQUEUED) || (socket_ptr->nx_tcp_socket_fast_recovery == NX_TRUE)) {
            break;
        } else {
            packet_ptr = next_ptr;
        }
