...
UINT _nx_arp_announce_send(NX_IP *ip_ptr, UINT interface_index) {
    ...
    if (_nx_packet_allocate(ip_ptr->nx_ip_auxiliary_packet_pool, &request_ptr,
                            (NX_PHYSICAL_HEADER + NX_ARP_MESSAGE_SIZE), NX_NO_WAIT)) {
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_ARP_REQUEST_SEND, ip_ptr, nx_interface->nx_interface_ip_address,
                            request_ptr, 0, NX_TRACE_INTERNAL_EVENTS, 0, 0);
    ...
    *(message_ptr + 1) = (ULONG)(NX_ARP_HARDWARE_SIZE << 24) | (NX_ARP_PROTOCOL_SIZE << 16) | NX_ARP_OPTION_REQUEST;
    ...
    *(message_ptr + 2) = (ULONG)(nx_interface->nx_interface_physical_address_msw << 16) |
     ...
     * not be NULL.  */
    *(message_ptr + 3) =
        (ULONG)(nx_interface->nx_interface_physical_address_lsw << 16) | (nx_interface->nx_interface_ip_address >> 16);
    /*lint -e{613} suppress possible use of null pointer, since nx_interface must
     * not be NULL.  */
    ...
    *(message_ptr + 6) = (ULONG)nx_interface->nx_interface_ip_address;
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_IO_DRIVER_ARP_SEND, ip_ptr, request_ptr, request_ptr->nx_packet_length, 0,
