...
VOID _nx_udp_packet_receive(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
    ...
    if (packet_ptr->nx_packet_length <
        (((udp_header_ptr->nx_udp_header_word_1) >> NX_SHIFT_BY_16) & NX_LOWER_16_MASK)) {
    ...
    if (!socket_ptr) {
        ...
        if ((packet_ptr->nx_packet_ip_version == NX_IP_VERSION_V4) && (ip_ptr->nx_ip_icmpv4_packet_process)) {
    ...
    do {
        ...
        if (socket_ptr->nx_udp_socket_port == port) {
            ...
            NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_UDP_RECEIVE, ip_ptr, socket_ptr, packet_ptr,
    ...
    if (thread_ptr) {
        ...
        *((NX_PACKET **)thread_ptr->tx_thread_additional_suspend_info) = packet_ptr;
        ...
        if (socket_ptr->nx_udp_socket_receive_head) {
            ...
            if (socket_ptr->nx_udp_socket_receive_count > socket_ptr->nx_udp_socket_queue_maximum) {
