...
UINT _nx_packet_data_adjust(NX_PACKET *packet_ptr, ULONG header_size) {
    ...
    available_size =
        (ULONG)(((ALIGN_TYPE)packet_ptr->nx_packet_prepend_ptr - (ALIGN_TYPE)packet_ptr->nx_packet_data_start) +
    ...
    if (available_size >= header_size) {
        ...
        shift_size =
            (ULONG)((ALIGN_TYPE)packet_ptr->nx_packet_append_ptr - (ALIGN_TYPE)packet_ptr->nx_packet_prepend_ptr);

        /* Move the data, */
        memmove(packet_ptr->nx_packet_data_start + header_size, packet_ptr->nx_packet_prepend_ptr,
