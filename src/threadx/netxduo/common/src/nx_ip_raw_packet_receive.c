...
UINT _nx_ip_raw_packet_receive(NX_IP *ip_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_IP_RAW_PACKET_RECEIVE, ip_ptr, 0, wait_option, 0, NX_TRACE_IP_EVENTS, &trace_event,
                            &trace_timestamp);
        ...
        *packet_ptr = work_ptr;
        ...
        if (wait_option) {
            ...
            if (ip_ptr->nx_ip_raw_packet_suspension_list) {
                ...
                thread_ptr->tx_thread_suspended_next = ip_ptr->nx_ip_raw_packet_suspension_list;
