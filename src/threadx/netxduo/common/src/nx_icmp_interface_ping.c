...
/*                                                                        */
/**************************************************************************/
UINT _nx_icmp_interface_ping(NX_IP *ip_ptr, ULONG ip_address, NX_INTERFACE *interface_ptr, ULONG next_hop_address,
                             CHAR *data_ptr, ULONG data_size, NX_PACKET **response_ptr, ULONG wait_option) {
    ...
    *response_ptr = NX_NULL;
    ...
    status = _nx_packet_data_append(request_ptr, (VOID *)data_ptr, data_size, ip_ptr->nx_ip_default_packet_pool,
                                    wait_option);
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_ICMP_SEND, ip_ptr, ip_address, request_ptr,
                            (((ULONG)NX_ICMP_ECHO_REQUEST_TYPE) << 24), NX_TRACE_INTERNAL_EVENTS, 0, 0);
    ...
    header_ptr->nx_icmp_header_word_1 =
        (ULONG)(request_ptr->nx_packet_address.nx_packet_interface_ptr->nx_interface_ip_address << 16) | sequence;
    ...
    if (request_ptr->nx_packet_address.nx_packet_interface_ptr->nx_interface_capability_flag &
        ...
        */
    {
        ...
        checksum = _nx_ip_checksum_compute(request_ptr, NX_IP_ICMP, (UINT)request_ptr->nx_packet_length,
    ...
    if (wait_option) {
        ...
        if (ip_ptr->nx_ip_icmp_ping_suspension_list) {
            ...
            thread_ptr->tx_thread_suspended_next = ip_ptr->nx_ip_icmp_ping_suspension_list;
    ...
    _nx_ip_packet_send(ip_ptr, request_ptr, ip_address, NX_IP_NORMAL, NX_IP_TIME_TO_LIVE, NX_IP_ICMP, NX_FRAGMENT_OKAY,
                       next_hop_address);
