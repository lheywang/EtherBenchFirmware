...
VOID _nx_tcp_packet_send_syn(NX_TCP_SOCKET *socket_ptr, ULONG tx_sequence) {
    ...
    if (socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_version == NX_IP_VERSION_V4) {
        mss = (ULONG)((socket_ptr->nx_tcp_socket_connect_interface->nx_interface_ip_mtu_size - sizeof(NX_IPV4_HEADER)) -
...
#ifdef FEATURE_NX_IPV6
    if (socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_version == NX_IP_VERSION_V6) {
        mss = (ULONG)((socket_ptr->nx_tcp_socket_connect_interface->nx_interface_ip_mtu_size - sizeof(NX_IPV6_HEADER)) -
                      sizeof(NX_TCP_HEADER));
    ...
    if (socket_ptr->nx_tcp_socket_state == NX_TCP_SYN_SENT) {
        _nx_tcp_packet_send_control(socket_ptr, NX_TCP_SYN_BIT, tx_sequence, 0, option_word_1, option_word_2, NX_NULL);
    } else {
