...
/*                                                                        */
/**************************************************************************/
UINT _nxe_http_proxy_client_enable(NX_IP *ip_ptr, NXD_ADDRESS *proxy_server_ip, UINT proxy_server_port, UCHAR *username,
                                   UINT username_length, UCHAR *password, UINT password_length) {
    UINT status;

    /* Check for invalid input pointers.  */
    if ((ip_ptr == NX_NULL) || (ip_ptr->nx_ip_id != NX_IP_ID) || (proxy_server_ip == NX_NULL) ||
        (proxy_server_port == 0) || ((username == NX_NULL) && (username_length != 0)) ||
        ((password == NX_NULL) && (password_length != 0))) {
        return (NX_PTR_ERROR);
    }

    /* Check the length of username and password.  */
    if ((username_length > NX_HTTP_PROXY_MAX_USERNAME) || (password_length > NX_HTTP_PROXY_MAX_PASSWORD)) {
    ...
    status = _nx_http_proxy_client_enable(ip_ptr, proxy_server_ip, proxy_server_port, username, username_length,
...
/*                                                                        */
/**************************************************************************/
UINT _nx_http_proxy_client_enable(NX_IP *ip_ptr, NXD_ADDRESS *proxy_server_ip, UINT proxy_server_port, UCHAR *username,
                                  UINT username_length, UCHAR *password, UINT password_length) {
    ...
    if (proxy_server_ip->nxd_ip_version == NX_IP_VERSION_V6) {
        ...
        COPY_IPV6_ADDRESS(proxy_server_ip->nxd_ip_address.v6, ip_ptr->nx_ip_http_proxy_ip_address.nxd_ip_address.v6);
    }
    ...
    if ((username_length) && (password_length)) {
        ...
        status = _nx_utility_base64_encode(
            (UCHAR *)string, username_length + password_length + 1, (UCHAR *)ip_ptr->nx_ip_http_proxy_authentication,
            sizeof(ip_ptr->nx_ip_http_proxy_authentication), &(ip_ptr->nx_ip_http_proxy_authentication_length));
...
UINT _nx_http_proxy_client_connect(NX_TCP_SOCKET *socket_ptr) {
    ...
    ip_string_len =
        _nx_utility_uint_to_string((UCHAR)(server_ip->nxd_ip_address.v4 >> 24), 10, ip_string_ptr, temp_length);
    ip_string_ptr[ip_string_len++] = '.';
    ip_string_len += _nx_utility_uint_to_string((UCHAR)(server_ip->nxd_ip_address.v4 >> 16), 10,
                                                ip_string_ptr + ip_string_len, temp_length - ip_string_len);
    ip_string_ptr[ip_string_len++] = '.';
    ip_string_len += _nx_utility_uint_to_string((UCHAR)(server_ip->nxd_ip_address.v4 >> 8), 10,
                                                ip_string_ptr + ip_string_len, temp_length - ip_string_len);
    ip_string_ptr[ip_string_len++] = '.';
    ip_string_len += _nx_utility_uint_to_string((UCHAR)(server_ip->nxd_ip_address.v4), 10,
                                                ip_string_ptr + ip_string_len, temp_length - ip_string_len);
...
UINT _nx_http_proxy_client_connect_response_process(NX_TCP_SOCKET *socket_ptr) {
    ...
    do {
        ...
        while ((buffer_ptr < (CHAR *)work_ptr->nx_packet_append_ptr) && (crlf_found < 4)) {
