...
VOID _nx_tcp_socket_state_data_trim(NX_PACKET *packet_ptr, ULONG amount) {
    ...
    while (work_ptr) {
        ...
        if ((INT)(work_ptr->nx_packet_append_ptr - work_ptr->nx_packet_prepend_ptr) < (INT)bytes_to_keep) {
            ...
            bytes_to_keep -=
                (ULONG)((ALIGN_TYPE)work_ptr->nx_packet_append_ptr - (ALIGN_TYPE)work_ptr->nx_packet_prepend_ptr);
...
UINT _nx_tcp_socket_state_data_check(NX_TCP_SOCKET *socket_ptr, NX_PACKET *packet_ptr) {
    ...
    if (packet_data_length
        ...
        if (((INT)((packet_end_sequence - socket_ptr->nx_tcp_socket_rx_sequence) -
            ...
            /* Calculate the data length that out of window.  */
            trim_data_length = packet_end_sequence -
                               (socket_ptr->nx_tcp_socket_rx_sequence + socket_ptr->nx_tcp_socket_rx_window_current);
    ...
    if (tcp_header_ptr->nx_tcp_header_word_3 & NX_TCP_FIN_BIT) {
        ...
        NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_FIN_RECEIVE, ip_ptr, socket_ptr, packet_ptr,
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_DATA_RECEIVE, ip_ptr, socket_ptr, packet_ptr,
                            tcp_header_ptr->nx_tcp_sequence_number, NX_TRACE_INTERNAL_EVENTS, 0, 0);
     ...
     * watermark? */
#ifdef NX_ENABLE_LOW_WATERMARK
    if (((socket_ptr->nx_tcp_socket_receive_queue_count >= socket_ptr->nx_tcp_socket_receive_queue_maximum) ||
    ...
    if (((tcp_header_ptr->nx_tcp_sequence_number == socket_ptr->nx_tcp_socket_rx_sequence) &&
        ...
        if (drop_packet) {
            ...
            socket_ptr->nx_tcp_socket_receive_queue_tail->nx_packet_union_next.nx_packet_tcp_queue_next =
    ...
    if ((socket_ptr->nx_tcp_socket_receive_queue_count - acked_packets) > NX_TCP_MAX_OUT_OF_ORDER_PACKETS) {
