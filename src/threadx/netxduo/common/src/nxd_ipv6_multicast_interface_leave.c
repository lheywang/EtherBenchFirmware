...
UINT _nxd_ipv6_multicast_interface_leave(NX_IP *ip_ptr, NXD_ADDRESS *group_address, UINT interface_index) {
    ...
    for (i = 0; i < NX_MAX_MULTICAST_GROUPS; i++) {
        ...
        if ((nx_interface == ip_ptr->nx_ipv6_multicast_entry[i].nx_ip_mld_join_interface_list) &&
            (CHECK_IPV6_ADDRESSES_SAME(ip_ptr->nx_ipv6_multicast_entry[i].nx_ip_mld_join_list,
            ...
            if (ip_ptr->nx_ipv6_multicast_entry[i].nx_ip_mld_join_count == 0) {
                ...
                (ip_ptr->nx_ipv6_multicast_entry[i].nx_ip_mld_join_interface_list->nx_interface_link_driver_entry)(
                    &driver_request);
