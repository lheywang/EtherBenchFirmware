...
/*                                                                        */
/**************************************************************************/
ULONG _nx_ip_route_find(NX_IP *ip_ptr, ULONG destination_address, NX_INTERFACE **ip_interface_ptr,
    ...
    *next_hop_address = 0;
    ...
    for (i = 0; i < NX_MAX_PHYSICAL_INTERFACES; i++) {
        ...
        if ((interface_ptr->nx_interface_valid) && (interface_ptr->nx_interface_link_up) &&
            ...
            ((*ip_interface_ptr == NX_NULL) || (*ip_interface_ptr == interface_ptr))) {
    ...
    if ((destination_address & 0xFFFF0000) == 0xA9FE0000) {
        ...
        if (*ip_interface_ptr) {
            ...
            if (((*ip_interface_ptr)->nx_interface_valid) && ((*ip_interface_ptr)->nx_interface_link_up)) {
    ...
    if ((ip_ptr->nx_ip_gateway_address) && (ip_ptr->nx_ip_gateway_interface) &&
        ...
        if (interface_ptr->nx_interface_ip_network !=
            (ip_ptr->nx_ip_gateway_address & interface_ptr->nx_interface_ip_network_mask)) {
            return (NX_IP_ADDRESS_ERROR);
        }
