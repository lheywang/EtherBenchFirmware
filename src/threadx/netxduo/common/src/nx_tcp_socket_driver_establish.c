...
UINT _nx_tcp_socket_driver_establish(NX_TCP_SOCKET *socket_ptr, NX_INTERFACE *interface_ptr, UINT remote_port) {
    ...
    if (socket_ptr->nx_tcp_socket_client_type == NX_FALSE) {
        ...
        status = interface_ptr->nx_interface_tcpip_offload_handler(
            ip_ptr, interface_ptr, socket_ptr, NX_TCPIP_OFFLOAD_TCP_SERVER_SOCKET_ACCEPT, NX_NULL, NX_NULL, &ip_address,
            port, &remote_port, NX_NO_WAIT);
        ...
        if (ip_address.nxd_ip_version == NX_IP_VERSION_V6) {
            COPY_IPV6_ADDRESS(ip_address.nxd_ip_address.v6, socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_address.v6);
            socket_ptr->nx_tcp_socket_mss =
