...
INT _nxd_ipv6_search_onlink(NX_IP *ip_ptr, ULONG *dest_addr) {
    ...
    while (prefix_entry) {
        ...
        if (CHECK_IP_ADDRESSES_BY_PREFIX(dest_addr, prefix_entry->nx_ipv6_prefix_entry_network_address,
    ...
    for (addr_index = 0; addr_index < NX_MAX_IPV6_ADDRESSES; addr_index++) {
        ...
        /* Check whether or not the destination address is matched. */
        if (CHECK_IP_ADDRESSES_BY_PREFIX(dest_addr, ipv6_address->nxd_ipv6_address,
