...
UINT _nx_udp_socket_receive(NX_UDP_SOCKET *socket_ptr, NX_PACKET **packet_ptr, ULONG wait_option) {
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_UDP_SOCKET_RECEIVE, socket_ptr->nx_udp_socket_ip_ptr, socket_ptr, 0, 0,
    ...
    *packet_ptr = NX_NULL;
    ...
    for (;;) {
            ...
            *packet_ptr = socket_ptr->nx_udp_socket_receive_head;
            ...
            if (wait_option) {
                ...
                if (socket_ptr->nx_udp_socket_receive_suspension_list) {
                    ...
                    thread_ptr->tx_thread_suspended_next = socket_ptr->nx_udp_socket_receive_suspension_list;
        ...
        if (((*packet_ptr)->nx_packet_ipsec_sa_ptr != NX_NULL) &&
            (((NX_IPSEC_SA *)((*packet_ptr)->nx_packet_ipsec_sa_ptr))->nx_ipsec_sa_encryption_method !=
             NX_CRYPTO_NONE)) {
            compute_checksum = 1;
        }
#endif /* NX_IPSEC_ENABLE */
        ...
        */
        {
            ...
            if ((!socket_ptr->nx_udp_socket_disable_checksum &&
                 (*(temp_ptr + 1) & NX_LOWER_16_MASK)) ||                  /* per-socket checksum is not disabled, and
                ...
                ((*packet_ptr)->nx_packet_ip_version == NX_IP_VERSION_V6)) /* It is IPv6 packet */
            {
                ...
                checksum = _nx_ip_checksum_compute(current_ptr, NX_PROTOCOL_UDP, (UINT)current_ptr->nx_packet_length,
    ...
    NX_TRACE_EVENT_UPDATE(trace_event, trace_timestamp, NX_TRACE_UDP_SOCKET_RECEIVE, 0, 0, *packet_ptr,
