...
UINT _nx_icmpv6_send_rs(NX_IP *ip_ptr, UINT if_index) {
    ...
    if (_nx_packet_allocate(ip_ptr->nx_ip_auxiliary_packet_pool, &pkt_ptr, (NX_ICMP_PACKET + sizeof(NX_ICMPV6_RS) + 8),
                            NX_NO_WAIT)) {
...
#endif /* NX_ENABLE_DUAL_PACKET_POOL */
        {
            if (_nx_packet_allocate(ip_ptr->nx_ip_default_packet_pool, &pkt_ptr,
                                    (NX_ICMP_PACKET + sizeof(NX_ICMPV6_RS) + 8), NX_NO_WAIT)) {
    ...
    mac_addr[1] = (USHORT)((ip_ptr->nx_ip_interface[if_index].nx_interface_physical_address_lsw & 0xFFFF0000) >>
                           16); /* lgtm[cpp/overflow-buffer] */
    mac_addr[2] = (USHORT)(ip_ptr->nx_ip_interface[if_index].nx_interface_physical_address_lsw &
        ...
        */
    {
        ...
        checksum = _nx_ip_checksum_compute(pkt_ptr, NX_PROTOCOL_ICMPV6, (UINT)pkt_ptr->nx_packet_length,
