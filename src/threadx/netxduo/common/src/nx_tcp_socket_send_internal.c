...
static UINT _nx_tcp_socket_driver_send(NX_TCP_SOCKET *socket_ptr, NX_PACKET *packet_ptr, ULONG wait_option) {
    ...
    if (ip_ptr->nx_ip_packet_filter || ip_ptr->nx_ip_packet_filter_extended) {
        ...
        if (socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_version == NX_IP_VERSION_V4) {
            _nx_ip_header_add(ip_ptr, packet_ptr, socket_ptr->nx_tcp_socket_connect_interface->nx_interface_ip_address,
                              socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_address.v4,
                              socket_ptr->nx_tcp_socket_type_of_service, socket_ptr->nx_tcp_socket_time_to_live,
                              NX_IP_TCP, socket_ptr->nx_tcp_socket_fragment_enable);
        }
#endif /* !NX_DISABLE_IPV4  */

#ifdef FEATURE_NX_IPV6
        if (socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_version == NX_IP_VERSION_V6) {
            if (_nx_ipv6_header_add(ip_ptr, &packet_ptr, NX_PROTOCOL_TCP, packet_ptr->nx_packet_length,
                                    ip_ptr->nx_ipv6_hop_limit, socket_ptr->nx_tcp_socket_ipv6_addr->nxd_ipv6_address,
                                    socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_address.v6, NX_NULL)) {
        ...
        if (ip_ptr->nx_ip_packet_filter) {
            if (ip_ptr->nx_ip_packet_filter((VOID *)(packet_ptr->nx_packet_prepend_ptr), NX_IP_PACKET_OUT) !=
                NX_SUCCESS) {
    ...
    status = interface_ptr->nx_interface_tcpip_offload_handler(ip_ptr, interface_ptr, socket_ptr,
                                                               NX_TCPIP_OFFLOAD_TCP_SOCKET_SEND, packet_ptr, NX_NULL,
                                                               NX_NULL, 0, NX_NULL, wait_option);
...
UINT _nx_tcp_socket_send_internal(NX_TCP_SOCKET *socket_ptr, NX_PACKET *packet_ptr, ULONG wait_option) {
    ...
    NX_TRACE_IN_LINE_INSERT(NX_TRACE_TCP_SOCKET_SEND, socket_ptr, packet_ptr, packet_ptr->nx_packet_length,
                            socket_ptr->nx_tcp_socket_tx_sequence, NX_TRACE_TCP_EVENTS, 0, 0);
    ...
    for (;;) {
        ...
        if ((tx_window_current != 0) &&
            ...
            if (adjust_packet) {
                ...
                ret = _nx_packet_allocate(pool_ptr, &send_packet, data_offset, wait_option);
                ...
                while (remaining_bytes) {
                    ...
                    /* Append data. */
                    ret = _nx_packet_data_append(send_packet, current_ptr, copy_size, pool_ptr, wait_option);
            ...
            if (socket_ptr->nx_tcp_socket_connect_interface->nx_interface_capability_flag &
            ...
            if ((send_packet->nx_packet_ipsec_sa_ptr != NX_NULL) &&
                (((NX_IPSEC_SA *)(send_packet->nx_packet_ipsec_sa_ptr))->nx_ipsec_sa_encryption_method !=
                 NX_CRYPTO_NONE)) {
                compute_checksum = 1;
            }
#endif /* NX_IPSEC_ENABLE */
        ...
        */
            {
                ...
                checksum = _nx_ip_checksum_compute(send_packet, NX_PROTOCOL_TCP, (UINT)send_packet->nx_packet_length,
                                                   source_ip, dest_ip);
            ...
            socket_ptr->nx_tcp_socket_tx_sequence =
                socket_ptr->nx_tcp_socket_tx_sequence + (send_packet->nx_packet_length - (ULONG)sizeof(NX_TCP_HEADER));
            ...
            if (socket_ptr->nx_tcp_socket_transmit_sent_head) {
                 ...
                 * tail.  */
                (socket_ptr->nx_tcp_socket_transmit_sent_tail)->nx_packet_union_next.nx_packet_tcp_queue_next =
                    send_packet;
            ...
            /* If trace is enabled, insert this event into the trace buffer.  */
            NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_TCP_DATA_SEND, ip_ptr, socket_ptr, send_packet,
                                    socket_ptr->nx_tcp_socket_tx_sequence -
            ...
            if (socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_version == NX_IP_VERSION_V4) {
                ...
                _nx_ip_packet_send(ip_ptr, send_packet, socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_address.v4,
                                   socket_ptr->nx_tcp_socket_type_of_service, socket_ptr->nx_tcp_socket_time_to_live,
                                   NX_IP_TCP, socket_ptr->nx_tcp_socket_fragment_enable,
            ...
            if (socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_version == NX_IP_VERSION_V6) {
                ...
                _nx_ipv6_packet_send(ip_ptr, send_packet, NX_PROTOCOL_TCP, send_packet->nx_packet_length,
                                     ip_ptr->nx_ipv6_hop_limit, socket_ptr->nx_tcp_socket_ipv6_addr->nxd_ipv6_address,
            ...
            if (data_left == 0) {
                ...
                while (remaining_bytes >
                    ...
                    /*lint -e{923} suppress cast of pointer to ULONG.  */
                    packet_ptr->nx_packet_length -= (ULONG)((ALIGN_TYPE)current_packet->nx_packet_append_ptr -
                                                            (ALIGN_TYPE)current_packet->nx_packet_prepend_ptr);

                    /*lint -e{923} suppress cast of pointer to ULONG.  */
                    remaining_bytes -= (ULONG)((ALIGN_TYPE)current_packet->nx_packet_append_ptr -
                                               (ALIGN_TYPE)current_packet->nx_packet_prepend_ptr);

                    /*lint -e{923} suppress cast between ULONG and pointer.  */
                    current_packet->nx_packet_append_ptr =
                        (UCHAR *)(((ALIGN_TYPE)current_packet->nx_packet_append_ptr) & (ALIGN_TYPE)(~3));
                    current_packet->nx_packet_prepend_ptr = current_packet->nx_packet_append_ptr;
            ...
            _nx_tcp_socket_thread_suspend(&(socket_ptr->nx_tcp_socket_transmit_suspension_list),
