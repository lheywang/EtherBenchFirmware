...
VOID _nx_icmpv6_process_ns(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
    ...
    if (!error) {
        ...
        while (interface_addr != NX_NULL) {
            ...
            /* Does the current address in the IP interface list match the one in the
               ND message header? */
            if ((CHECK_IPV6_ADDRESSES_SAME(interface_addr->nxd_ipv6_address, nd_ptr->nx_icmpv6_nd_targetAddress))) {
    ...
    while (option_length > 0) {
        ...
        if (option_ptr->nx_icmpv6_option_type == ICMPV6_OPTION_TYPE_SRC_LINK_ADDR) {
            ...
            if (status != NX_SUCCESS) {
                 ...
                 */
                _nx_nd_cache_add(ip_ptr, ipv6_header->nx_ip_header_source_ip, interface_addr->nxd_ipv6_address_attached,
                                 (CHAR *)&option_ptr->nx_icmpv6_option_data, 0, ND_CACHE_STATE_STALE, interface_addr,
                                 &nd_entry);
            } else {
                ...
                new_lsw = ((ULONG)(new_mac[2]) << 24) | ((ULONG)(new_mac[3]) << 16) | ((ULONG)(new_mac[4]) << 8) |
                          new_mac[5];                             /* lgtm[cpp/overflow-buffer] */
                if ((mac_msw != new_msw) || (mac_lsw != new_lsw)) /* If the new MAC is different from what we
                                                                     have in the table. */
                {
                ...
                if (nd_entry->nx_nd_cache_packet_waiting_head) /* There are packets waiting
                                                                  to be transmitted */
                {
     ...
     * field (8 bytes).
     */
    packet_ptr->nx_packet_append_ptr = packet_ptr->nx_packet_prepend_ptr + sizeof(NX_ICMPV6_ND) + 8;
    ...
    mac_addr[1] =
        (USHORT)((interface_addr->nxd_ipv6_address_attached->nx_interface_physical_address_lsw & 0xFFFF0000) >>
                 16); /* lgtm[cpp/overflow-buffer] */
    mac_addr[2] = (USHORT)(interface_addr->nxd_ipv6_address_attached->nx_interface_physical_address_lsw &
        ...
        */
    {
        ...
        header_ptr->nx_icmpv6_header_checksum =
            _nx_ip_checksum_compute(packet_ptr, NX_PROTOCOL_ICMPV6, (UINT)packet_ptr->nx_packet_length,
    ...
    _nx_ipv6_packet_send(ip_ptr, packet_ptr, NX_PROTOCOL_ICMPV6, packet_ptr->nx_packet_length,
                         255 /* NA message must have hop limit 255 */, interface_addr->nxd_ipv6_address, dest_address);
