...
VOID _nx_udp_socket_driver_packet_receive(NX_UDP_SOCKET *socket_ptr, NX_PACKET *packet_ptr, NXD_ADDRESS *local_ip,
    ...
    if (remote_ip->nxd_ip_version == NX_IP_VERSION_V4) {
        _nx_ip_header_add(ip_ptr, packet_ptr, remote_ip->nxd_ip_address.v4, local_ip->nxd_ip_address.v4,
                          socket_ptr->nx_udp_socket_type_of_service, socket_ptr->nx_udp_socket_time_to_live, NX_IP_UDP,
    ...
    if (remote_ip->nxd_ip_version == NX_IP_VERSION_V6) {
        if (_nx_ipv6_header_add(ip_ptr, &packet_ptr, NX_PROTOCOL_UDP, packet_ptr->nx_packet_length,
                                ip_ptr->nx_ipv6_hop_limit, remote_ip->nxd_ip_address.v6, local_ip->nxd_ip_address.v6,
                                NX_NULL)) {
