...
VOID _nx_tcp_socket_driver_packet_receive(NX_TCP_SOCKET *socket_ptr, NX_PACKET *packet_ptr) {
    ...
    header_ptr->nx_tcp_header_word_0 =
        (((ULONG)(socket_ptr->nx_tcp_socket_connect_port)) << NX_SHIFT_BY_16) | (ULONG)socket_ptr->nx_tcp_socket_port;
    ...
    if (socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_version == NX_IP_VERSION_V6) {
        if (_nx_ipv6_header_add(ip_ptr, &packet_ptr, NX_PROTOCOL_TCP, packet_ptr->nx_packet_length,
                                ip_ptr->nx_ipv6_hop_limit, socket_ptr->nx_tcp_socket_connect_ip.nxd_ip_address.v6,
