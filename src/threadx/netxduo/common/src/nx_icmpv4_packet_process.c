...
VOID _nx_icmpv4_packet_process(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
        ...
        */
    {
        ...
        checksum = _nx_ip_checksum_compute(packet_ptr, NX_IP_ICMP, (UINT)packet_ptr->nx_packet_length,
    ...
    if (header_ptr->nx_icmpv4_header_type == NX_ICMP_ECHO_REPLY_TYPE) {
        ...
        NX_TRACE_IN_LINE_INSERT(NX_TRACE_INTERNAL_ICMP_RECEIVE, ip_ptr, ip_header_ptr->nx_ip_header_source_ip,
                                packet_ptr, 0, NX_TRACE_INTERNAL_EVENTS, 0, 0);
#endif /* TX_ENABLE_EVENT_TRACE  */
