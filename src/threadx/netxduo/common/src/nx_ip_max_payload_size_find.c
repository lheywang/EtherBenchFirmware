...

{
    ...
    if (dest_address->nxd_ip_version == NX_IP_VERSION_V6) {
        ...
        /* Get the mtu size.  */
        payload_length =
            ip_ptr->nx_ipv6_address[if_index].nxd_ipv6_address_attached->nx_interface_ip_mtu_size + NX_PHYSICAL_HEADER;
    }
#endif /* FEATURE_NX_IPV6  */
...
#endif /* FEATURE_NX_IPV6  */
            ret = ip_ptr->nx_ip_packet_egress_sa_lookup(ip_ptr, &src_address, dest_address, (UCHAR)protocol, src_port,
    ...
    NX_TRACE_IN_LINE_INSERT(NXD_TRACE_IP_MAX_PAYLOAD_SIZE_FIND, src_address_lsw, dst_address_lsw, payload_length,
                            start_offset, NX_TRACE_PACKET_EVENTS, 0, 0);
