...
VOID _nx_icmpv6_process_redirect(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
        ...
        while (packet_length) {
            ...
            if (option_ptr->nx_icmpv6_option_type == ICMPV6_OPTION_TYPE_TRG_LINK_ADDR) {
                ...
                if (status != NX_SUCCESS) {
                    ...
                    if (_nx_nd_cache_add(ip_ptr, redirect_ptr->nx_icmpv6_redirect_target_address, interface_ptr,
                        ...
                        if (!CHECK_IPV6_ADDRESSES_SAME(redirect_ptr->nx_icmpv6_redirect_target_address,
                            ...
                            _nxd_ipv6_default_router_add_internal(
                                ip_ptr, redirect_ptr->nx_icmpv6_redirect_target_address, /* TLLA in Redirect
                                                                                            ICMPv6 options */
                                0, interface_ptr, NX_IPV6_ROUTE_TYPE_UNSOLICITATED, &rt_entry);
    ...
    /* Find/update existing entry or add a new one for this router in the
       ...
       timeout values.  */
    status = _nx_icmpv6_dest_table_add(ip_ptr, redirect_ptr->nx_icmpv6_redirect_destination_address, &dest_entry_ptr,
                                       redirect_ptr->nx_icmpv6_redirect_target_address, mtu, mtu_timeout,
                                       packet_ptr->nx_packet_address.nx_packet_ipv6_address_ptr);
    if (status) {
