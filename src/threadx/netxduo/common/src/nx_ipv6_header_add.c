...
UINT _nx_ipv6_header_add(NX_IP *ip_ptr, NX_PACKET **packet_pptr, ULONG protocol, ULONG payload_size, ULONG hop_limit,
    ...
    if (!is_hw_processed) {
        ...
        packet_ptr->nx_packet_ip_header_length =
            (UCHAR)(packet_ptr->nx_packet_ip_header_length + sizeof(NX_IPV6_HEADER));
        ...
        if (packet_ptr->nx_packet_address.nx_packet_ipv6_address_ptr->nxd_ipv6_address_state !=
            ...
            NX_ICMPV6_HEADER *icmpv6_header =
                (NX_ICMPV6_HEADER *)(packet_ptr->nx_packet_prepend_ptr + packet_ptr->nx_packet_ip_header_length);
            ...
            if (!((protocol == NX_PROTOCOL_ICMPV6) &&
                  (packet_ptr->nx_packet_address.nx_packet_ipv6_address_ptr->nxd_ipv6_address_state ==
    ...
    if (packet_ptr->nx_packet_ipsec_sa_ptr && packet_ptr->nx_packet_ipsec_state != NX_IPSEC_ESP_PACKET &&
        ...
        status = _nx_ipsec_ip_output_packet_process(ip_ptr, &packet_ptr, NX_PROTOCOL_IPV6, (ULONG)payload_size,
                                                    (ULONG *)(&payload_size));
