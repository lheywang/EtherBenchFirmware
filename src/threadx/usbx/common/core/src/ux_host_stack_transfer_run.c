...
UINT _ux_host_stack_transfer_run(UX_TRANSFER *transfer_request) {
    ...
    case UX_STATE_RESET:
        ...
        if (_ux_host_stack_transfer_locate(transfer_request, UX_NULL) < UX_HOST_STACK_TRANSFER_AT_LIST_HEAD) {
            transfer_request->ux_transfer_request_next_pending = _ux_system_host->ux_system_host_pending_transfers;
    ...
    case UX_STATE_WAIT:
        ...
        if (transfer_request->ux_transfer_request_timeout_value != UX_WAIT_FOREVER) {
            if (transfer_request->ux_transfer_request_timeout_value <
                _ux_utility_time_elapsed(transfer_request->ux_transfer_request_time_start, _ux_utility_time_get())) {
...
static inline void _ux_host_stack_transfer_retire(UX_TRANSFER *transfer) {
    ...
    case UX_HOST_STACK_TRANSFER_AT_LIST_HEAD:
        ...
        _ux_system_host->ux_system_host_pending_transfers = transfer->ux_transfer_request_next_pending;
        break;
    case UX_HOST_STACK_TRANSFER_IN_LIST:

        /* Unlink from pending transfer list.  */
        previous->ux_transfer_request_next_pending = transfer->ux_transfer_request_next_pending;
        break;
    ...
    transfer->ux_transfer_request_flags &= ~(UX_TRANSFER_FLAG_LOCK | UX_TRANSFER_FLAG_AUTO_DEVICE_UNLOCK);
    if (flags & UX_TRANSFER_FLAG_AUTO_DEVICE_UNLOCK) {
        transfer->ux_transfer_request_endpoint->ux_endpoint_device->ux_device_flags &= ~UX_DEVICE_FLAG_LOCK;
    }
}
