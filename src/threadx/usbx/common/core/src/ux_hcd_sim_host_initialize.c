...
UINT _ux_hcd_sim_host_initialize(UX_HCD *hcd) {
    ...
    if (_ux_system_host->ux_system_host_max_ed != 0) {
        hcd_sim_host->ux_hcd_sim_host_ed_list = _ux_utility_memory_allocate(
            UX_ALIGN_16, UX_REGULAR_MEMORY, (ULONG)sizeof(UX_HCD_SIM_HOST_ED) * _ux_system_host->ux_system_host_max_ed);
    ...
    if ((status == UX_SUCCESS) && (_ux_system_host->ux_system_host_max_td != 0)) {
        hcd_sim_host->ux_hcd_sim_host_td_list = _ux_utility_memory_allocate(
            UX_ALIGN_32, UX_REGULAR_MEMORY, (ULONG)sizeof(UX_HCD_SIM_HOST_TD) * _ux_system_host->ux_system_host_max_td);
    ...
    if ((status == UX_SUCCESS) && (_ux_system_host->ux_system_host_max_iso_td != 0)) {
        hcd_sim_host->ux_hcd_sim_host_iso_td_list = _ux_utility_memory_allocate(
            UX_ALIGN_32, UX_REGULAR_MEMORY,
            (ULONG)sizeof(UX_HCD_SIM_HOST_ISO_TD) * _ux_system_host->ux_system_host_max_iso_td);
    ...
    if (status == UX_SUCCESS) {
        ...
        status = _ux_host_timer_create(&hcd_sim_host->ux_hcd_sim_host_timer, "USBX Simulation Timer",
                                       _ux_hcd_sim_host_timer_function, (ULONG)(ALIGN_TYPE)hcd_sim_host, 1, 1,
                                       UX_AUTO_ACTIVATE);
    }
