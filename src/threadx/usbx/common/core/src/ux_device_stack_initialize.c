...
UX_COMPILE_TIME_ASSERT(!UX_OVERFLOW_CHECK_MULC_ULONG(sizeof(UX_SLAVE_CLASS), UX_MAX_SLAVE_CLASS_DRIVER),
...
UINT _ux_device_stack_initialize(UCHAR *device_framework_high_speed, ULONG device_framework_length_high_speed,
     ...
     */
    memory =
        _ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY, sizeof(UX_SLAVE_CLASS) * UX_MAX_SLAVE_CLASS_DRIVER);
    if (memory == UX_NULL)
    ...
    if (status == UX_SUCCESS) {
        ...
        if (interfaces_found == 0) {
            ...
            /* If trace is enabled, insert this event into the trace buffer.  */
            UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_DESCRIPTOR_CORRUPTED, device_framework, 0, 0, UX_TRACE_ERRORS, 0,
                                    0)
        ...
        if (status == UX_SUCCESS && max_interface_number >= UX_MAX_SLAVE_INTERFACES) {

            /* Error trap. */
            _ux_system_error_handler(UX_SYSTEM_LEVEL_THREAD, UX_SYSTEM_CONTEXT_INIT, UX_MEMORY_INSUFFICIENT);

            /* If trace is enabled, insert this event into the trace buffer.  */
            UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_MEMORY_INSUFFICIENT, device_framework, 0, 0, UX_TRACE_ERRORS, 0,
                                    0)
    ...
    if (status == UX_SUCCESS) {
        ...
        interfaces_pool = _ux_utility_memory_allocate_mulc_safe(UX_NO_ALIGN, UX_REGULAR_MEMORY, interfaces_found,
    ...
    if (endpoints_found != 0 && status == UX_SUCCESS) {
        ...
        endpoints_pool = _ux_utility_memory_allocate_mulc_safe(UX_NO_ALIGN, UX_REGULAR_MEMORY, endpoints_found,
        ...
        if (endpoints_pool == UX_NULL)
            ...
            while (endpoints_pool < (device->ux_slave_device_endpoints_pool + endpoints_found)) {
                ...
                status = _ux_device_semaphore_create(
                    &endpoints_pool->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_semaphore,
    ...
    if (device->ux_slave_device_control_endpoint.ux_slave_endpoint_transfer_request
        ...
        _ux_utility_memory_free(device->ux_slave_device_control_endpoint.ux_slave_endpoint_transfer_request
                                    .ux_slave_transfer_request_data_pointer);
...
UINT _uxe_device_stack_initialize(UCHAR *device_framework_high_speed, ULONG device_framework_length_high_speed,
    ...
    if (((device_framework_high_speed == UX_NULL) && (device_framework_length_high_speed != 0)) ||
        ...
        ((string_framework == UX_NULL) && (string_framework_length != 0)) || (language_id_framework == UX_NULL) ||
        (language_id_framework_length == 0))
