...
UINT _ux_host_stack_device_resources_free(UX_DEVICE *device) {
    ...
    if (device->ux_device_flags & UX_DEVICE_FLAG_ENUM) {
        ...
        if (device->ux_device_enum_trans && device->ux_device_enum_trans->ux_transfer_request_data_pointer) {
            _ux_utility_memory_free(device->ux_device_enum_trans->ux_transfer_request_data_pointer);
        }
        ...
        if ((device->ux_device_enum_state == UX_HOST_STACK_ENUM_CONFIG_DESCR_PARSE) ||
            ...
            if (device->ux_device_enum_inst.configuration &&
                device->ux_device_enum_inst.configuration->ux_configuration_device == UX_NULL) {
    ...
    _ux_host_semaphore_delete(
        &device->ux_device_control_endpoint.ux_endpoint_transfer_request.ux_transfer_request_semaphore);
