...
#define UX_HOST_STACK_ENUM_TRANS_ERROR(t)                                                                              \
    ((t)->ux_transfer_request_completion_code != UX_SUCCESS ||                                                         \
     (t)->ux_transfer_request_actual_length != (t)->ux_transfer_request_requested_length)
...
UINT _ux_host_stack_tasks_run(VOID) {
    ...
    for (hcd_index = 0; hcd_index < UX_SYSTEM_HOST_MAX_HCD_GET(); hcd_index++) {
        ...
        if (hcd_inst->ux_hcd_status != UX_HCD_STATUS_OPERATIONAL || hcd_inst->ux_hcd_entry_function == UX_NULL)
            continue;
    ...
    for (class_index = 0; class_index < UX_SYSTEM_HOST_MAX_CLASS_GET(); class_index++) {
        ...
        if ((class_inst->ux_host_class_status == UX_UNUSED) || (class_inst->ux_host_class_task_function == UX_NULL))
            continue;
    ...
    if (_ux_system_host->ux_system_host_change_function) {
        _ux_system_host->ux_system_host_change_function(UX_STANDALONE_WAIT_BACKGROUND_TASK, UX_NULL, UX_NULL);
    }
...
static inline VOID _ux_host_stack_enum_address_sent(UX_DEVICE *device) {
    ...
    device->ux_device_address =
        device->ux_device_control_endpoint.ux_endpoint_transfer_request.ux_transfer_request_value & 0x7F;
    device->ux_device_state = UX_DEVICE_ADDRESSED;

    /* Unlock enumeration.  */
    _ux_system_host->ux_system_host_enum_lock = UX_NULL;
}
static inline VOID _ux_host_stack_enum_device_descriptor_parse0(UX_DEVICE *device, UCHAR *descriptor) {

    /* Endpoint descriptor updated.  */
    device->ux_device_descriptor.bMaxPacketSize0 = descriptor[7];

    /* Update EP0 wMaxPacketSize.  */
    device->ux_device_control_endpoint.ux_endpoint_descriptor.wMaxPacketSize = descriptor[7];
}
static inline VOID _ux_host_stack_enum_configuration_descriptor_parse0(UX_DEVICE *device,
...
static inline UINT _ux_host_stack_enum_configuration_read(UX_DEVICE *device) {
    ...
    transfer_request->ux_transfer_request_value =
        (UINT)device->ux_device_enum_index | (UINT)(UX_CONFIGURATION_DESCRIPTOR_ITEM << 8);
...
static inline VOID _ux_host_stack_configuration_parsed(UX_DEVICE *device) {
    ...
    if (status == UX_SUCCESS) {
        ...
        if (status == UX_SUCCESS) {
            ...
            device->ux_device_enum_inst.configuration = device->ux_device_first_configuration;
        }
    }
}
...
static inline VOID _ux_host_stack_device_enumerated(UX_DEVICE *device) {
    ...
    UX_TRACE_OBJECT_REGISTER(UX_TRACE_HOST_OBJECT_TYPE_DEVICE, UX_DEVICE_HCD_GET(device), UX_DEVICE_PARENT_GET(device),
                             UX_DEVICE_PORT_LOCATION_GET(device), 0);

    /* Check if there is unnecessary resource to free.  */
    if (device->ux_device_packed_configuration && device->ux_device_packed_configuration_keep_count == 0) {
    ...
    /* Unlock device EP0.  */
    device->ux_device_control_endpoint.ux_endpoint_transfer_request.ux_transfer_request_flags &= ~UX_TRANSFER_FLAG_LOCK;
    ...
    if (UX_DEVICE_HCD_GET(device)->ux_hcd_status != UX_HCD_STATUS_OPERATIONAL ||
        ...
        _ux_host_stack_device_remove(UX_DEVICE_HCD_GET(device), UX_DEVICE_PARENT_GET(device),
    ...
    if (UX_DEVICE_PARENT_IS_ROOTHUB(device)) {
        UX_DEVICE_HCD_GET(device)->ux_hcd_rh_device_connection |= (ULONG)(1 << UX_DEVICE_PORT_LOCATION_GET(device));
    }
...
static inline UINT _ux_host_stack_interface_activate_wait(UX_DEVICE *device, UX_HOST_CLASS_COMMAND *command) {
    UINT status;
    ...
    if (status == UX_FUNCTION_NOT_SUPPORTED || status == UX_STATE_NEXT) {
        ...
        if (device->ux_device_enum_index == 0xFF) {
            ...
            while (interface_inst != UX_NULL && interface_inst->ux_interface_descriptor.bAlternateSetting != 0) {
...
static inline VOID _ux_host_stack_device_enum_run(UX_DEVICE *device) {
    ...
    UX_DISABLE
    if (_ux_system_host->ux_system_host_enum_lock && device->ux_device_address == 0) {
    ...
    while (immediate_state) {
        ...
        case UX_HOST_STACK_ENUM_PORT_ENABLE:
            ...
            device->ux_device_enum_wait_ms = UX_MS_TO_TICK_NON_ZERO(UX_RH_ENUMERATION_RETRY_DELAY);
            continue;
        ...
        case UX_HOST_STACK_ENUM_PORT_RESET_WAIT:
            ...
            if (status == UX_STATE_NEXT) {
                ...
                if (device->ux_device_address) {
                    ...
                    /* Free the address.  */
                    UX_DEVICE_HCD_GET(device)->ux_hcd_address[(device->ux_device_address - 1) >> 3] &=
        ...
        case UX_HOST_STACK_ENUM_DEVICE_DESCR_PARSE:
            ...
            _ux_utility_descriptor_parse(buffer, _ux_system_device_descriptor_structure, UX_DEVICE_DESCRIPTOR_ENTRIES,
        ...
        case UX_HOST_STACK_ENUM_CONFIG_DESCR_PARSE:
            ...
            if (trans->ux_transfer_request_actual_length == UX_CONFIGURATION_DESCRIPTOR_LENGTH) {
                ...
                if (configuration->ux_configuration_descriptor.wTotalLength == UX_CONFIGURATION_DESCRIPTOR_LENGTH) {
                    device->ux_device_enum_state = UX_HOST_STACK_ENUM_CONFIG_DESCR_NEXT;
                    continue;
                }

                /* Allocate new buffer for total configuration.  */
                buffer = _ux_utility_memory_allocate(UX_SAFE_ALIGN, UX_CACHE_SAFE_MEMORY,
                ...
                trans->ux_transfer_request_requested_length = configuration->ux_configuration_descriptor.wTotalLength;
        ...
        case UX_HOST_STACK_ENUM_CONFIG_DESCR_NEXT:
            ...
            if (device->ux_device_enum_index < device->ux_device_descriptor.bNumConfigurations) {
        ...
        case UX_HOST_STACK_ENUM_CONFIG_SET:
            ...
            /* The device is now in the unconfigured state. We need to deal
               ...
               current fault. */
            configuration = device->ux_device_enum_inst.configuration;
            if (((configuration->ux_configuration_descriptor.bmAttributes & UX_CONFIGURATION_DEVICE_SELF_POWERED) ==
                 0) &&
                (configuration->ux_configuration_descriptor.MaxPower > UX_DEVICE_MAX_POWER_GET(device))) {

                /* Error trap. */
                _ux_system_error_handler(UX_SYSTEM_LEVEL_THREAD, UX_SYSTEM_CONTEXT_ENUMERATOR,
                                         UX_OVER_CURRENT_CONDITION);

                /* If trace is enabled, insert this event into the trace buffer.  */
                UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_OVER_CURRENT_CONDITION, configuration, 0, 0, UX_TRACE_ERRORS,
                                        0, 0)
        ...
        case UX_HOST_STACK_ENUM_CONFIG_ACTIVATE:
            ...
            if (configuration->ux_configuration_first_interface) {
                ...
                /* Activate classes.  */
                device->ux_device_enum_index = 0;
                device->ux_device_enum_inst.interface = configuration->ux_configuration_first_interface;
            } else {
        ...
        case UX_HOST_STACK_ENUM_ACTIVATE:
            ...
            if (class_command.ux_host_class_command_class_ptr != UX_NULL) {
                status = class_command.ux_host_class_command_class_ptr->ux_host_class_entry_function(&class_command);
                if (status != UX_SUCCESS) {
                    device->ux_device_enum_state = UX_HOST_STACK_ENUM_RETRY;
                    continue;
                }
        ...
        case UX_HOST_STACK_ENUM_WAIT:
            ...
            elapsed_ms = _ux_utility_time_elapsed(current_ms, device->ux_device_enum_wait_start);
        ...
        case UX_HOST_STACK_ENUM_FAIL:
            ...
            /* If trace is enabled, insert this event into the trace buffer.  */
            UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_DEVICE_ENUMERATION_FAILURE, UX_DEVICE_PORT_LOCATION_GET(device),
