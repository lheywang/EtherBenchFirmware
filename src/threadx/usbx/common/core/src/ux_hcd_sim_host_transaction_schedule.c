...
UINT _ux_hcd_sim_host_transaction_schedule(UX_HCD_SIM_HOST *hcd_sim_host, UX_HCD_SIM_HOST_ED *ed) {
    ...
    if ((slave_ed->ux_sim_slave_ed_status &
         (UX_DCD_SIM_SLAVE_ED_STATUS_TRANSFER | UX_DCD_SIM_SLAVE_ED_STATUS_STALLED)) == 0)
        return (UX_ERROR);
    ...
    if (td->ux_sim_host_td_status & UX_HCD_SIM_HOST_TD_SETUP_PHASE) {
        ...
        if (((*slave_transfer_request->ux_slave_transfer_request_setup & UX_REQUEST_IN) == 0) &&
            ...
            slave_transfer_request->ux_slave_transfer_request_requested_length =
                _ux_utility_short_get(slave_transfer_request->ux_slave_transfer_request_setup + 6);

            /* Avoid buffer overflow.  */
            if (slave_transfer_request->ux_slave_transfer_request_requested_length >
            ...
            slave_transfer_request->ux_slave_transfer_request_current_data_pointer =
                slave_transfer_request->ux_slave_transfer_request_data_pointer;
        ...
        if ((*slave_transfer_request->ux_slave_transfer_request_setup & UX_REQUEST_IN) == 0) {
            ...
            if (slave_ed->ux_sim_slave_ed_status & UX_DCD_SIM_SLAVE_ED_STATUS_STALLED) {
                ...
                UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_TRANSFER_STALLED, transfer_request, 0, 0, UX_TRACE_ERRORS, 0,
                                        0)
            } else
        ...
        if (slave_ed->ux_sim_slave_ed_status & UX_DCD_SIM_SLAVE_ED_STATUS_STALLED) {
            ...
            if (slave_transfer_request->ux_slave_transfer_request_requested_length != 0)
                slave_transfer_remaining = slave_transfer_request->ux_slave_transfer_request_requested_length -
                                           slave_transfer_request->ux_slave_transfer_request_actual_length;
            ...
            if (slave_endpoint->ux_slave_endpoint_descriptor.wMaxPacketSize ==
                0) /* Special for tests (avoid DIV0/MOD0) */
            {
