...
UINT _ux_host_stack_rh_device_insertion(UX_HCD *hcd, UINT port_index) {
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_HOST_STACK_RH_DEVICE_INSERTION, hcd, port_index, 0, 0, UX_TRACE_HOST_STACK_EVENTS,
                            0, 0)
    ...
    status =
        _ux_host_stack_new_device_create(hcd, UX_NULL, port_index, UX_FULL_SPEED_DEVICE, UX_MAX_SELF_POWER, &device);
    ...
    for (index_loop = 0; index_loop < UX_RH_ENUMERATION_RETRY; index_loop++) {
        ...
        if (port_status == UX_SUCCESS) {
            ...
            if (port_status == UX_PORT_INDEX_UNKNOWN) {
                ...
                /* Error trap. */
                _ux_system_error_handler(UX_SYSTEM_LEVEL_THREAD, UX_SYSTEM_CONTEXT_ROOT_HUB,
                                         UX_DEVICE_ENUMERATION_FAILURE);

                /* If trace is enabled, insert this event into the trace buffer.  */
                UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_DEVICE_ENUMERATION_FAILURE, port_index, 0, 0,
                                        UX_TRACE_ERRORS, 0, 0)
            ...
            /* Ask the USB stack to enumerate this device. A root hub is considered self
               powered. */
            status =
                _ux_host_stack_new_device_create(hcd, UX_NULL, port_index, device_speed, UX_MAX_SELF_POWER, &device);
