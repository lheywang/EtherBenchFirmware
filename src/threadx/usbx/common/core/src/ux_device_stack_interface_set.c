...
UINT _ux_device_stack_interface_set(UCHAR *device_framework, ULONG device_framework_length,
    ...
    while (device_framework_length != 0) {
        ...
        case UX_ENDPOINT_DESCRIPTOR_ITEM:
            ...
            /* Parse the descriptor in something more readable.  */
            _ux_utility_descriptor_parse(device_framework, _ux_system_endpoint_descriptor_structure,
            ...
            max_transfer_length = endpoint->ux_slave_endpoint_descriptor.wMaxPacketSize & UX_MAX_PACKET_SIZE_MASK;
            if ((_ux_system_slave->ux_system_slave_speed == UX_HIGH_SPEED_DEVICE) &&
                (endpoint->ux_slave_endpoint_descriptor.bmAttributes & 0x1u)) {
                n_trans = endpoint->ux_slave_endpoint_descriptor.wMaxPacketSize & UX_MAX_NUMBER_OF_TRANSACTIONS_MASK;
                if (n_trans) {
