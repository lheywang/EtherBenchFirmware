...
/*                                                                        */
/**************************************************************************/
UINT _ux_device_class_audio20_control_process(UX_DEVICE_CLASS_AUDIO *audio, UX_SLAVE_TRANSFER *transfer,
        ...
        if (unit_id == control->ux_device_class_audio20_control_cs_id) {
            ...
            if ((request_type & UX_REQUEST_DIRECTION) == UX_REQUEST_OUT &&
                ...
                case UX_DEVICE_CLASS_AUDIO20_CUR:
                    ...
                    /* Get first RES.  */
                    res =
                        _ux_utility_long_get(control->ux_device_class_audio20_control_sampling_frequency_range + 2 + 8);
                    ...
                    for (pos = 2; pos < (2 + n_sub * 12); pos += 12) {
                        min = _ux_utility_long_get(control->ux_device_class_audio20_control_sampling_frequency_range +
                                                   pos);
                        max = _ux_utility_long_get(control->ux_device_class_audio20_control_sampling_frequency_range +
                                                   pos + 4);
                        if (freq >= min && freq <= max) {
                            ...
                            if (control->ux_device_class_audio20_control_sampling_frequency_cur != freq) {
                                ...
                                control->ux_device_class_audio20_control_changed =
                                    UX_DEVICE_CLASS_AUDIO20_CONTROL_FREQUENCY_CHANGED;
                            }
                            return (UX_SUCCESS);
                        }
                    }
            ...
            if ((request_type & UX_REQUEST_DIRECTION) == UX_REQUEST_IN &&
                ...
                case UX_DEVICE_CLASS_AUDIO20_CUR:
                    ...
                    if (control->ux_device_class_audio20_control_sampling_frequency)
                        _ux_utility_long_put(transfer->ux_slave_transfer_request_data_pointer,
                                             control->ux_device_class_audio20_control_sampling_frequency);
                    else
                        _ux_utility_long_put(transfer->ux_slave_transfer_request_data_pointer,
                ...
                case UX_DEVICE_CLASS_AUDIO20_RANGE:
                    ...
                    if (control->ux_device_class_audio20_control_sampling_frequency == 0) {
                        ...
                        /* Get wNumSubRanges.  */
                        n_sub =
                            _ux_utility_short_get(control->ux_device_class_audio20_control_sampling_frequency_range);
                        UX_ASSERT(n_sub > 0);
                        ...
                        _ux_utility_long_put(transfer->ux_slave_transfer_request_data_pointer + 2,
                                             control->ux_device_class_audio20_control_sampling_frequency);
                        _ux_utility_long_put(transfer->ux_slave_transfer_request_data_pointer + 6,
                                             control->ux_device_class_audio20_control_sampling_frequency);
            ...
            case UX_DEVICE_CLASS_AUDIO20_FU_MUTE_CONTROL:
                ...
                case UX_DEVICE_CLASS_AUDIO20_CUR:
                    ...
                    if ((request_type & UX_REQUEST_DIRECTION) == UX_REQUEST_IN) {
                        ...
                        /* Send mute.  */
                        transfer->ux_slave_transfer_request_data_pointer[0] =
                            (UCHAR)control->ux_device_class_audio20_control_mute[0];
                        _ux_device_stack_transfer_request(transfer, 1, request_length);
                    } else if (control->ux_device_class_audio20_control_mute[0] !=
                               transfer->ux_slave_transfer_request_data_pointer[0]) {

                        /* Update mute.  */
                        control->ux_device_class_audio20_control_changed = UX_DEVICE_CLASS_AUDIO20_CONTROL_MUTE_CHANGED;
                        control->ux_device_class_audio20_control_mute[0] =
                            transfer->ux_slave_transfer_request_data_pointer[0];
                    }
                    /* Done success.  */
                    return (UX_SUCCESS);
            ...
            case UX_DEVICE_CLASS_AUDIO20_FU_VOLUME_CONTROL:
                ...
                case UX_DEVICE_CLASS_AUDIO20_CUR:
                    ...
                    if ((request_type & UX_REQUEST_DIRECTION) == UX_REQUEST_IN) {
                        ...
                        /* Send volume.  */
                        _ux_utility_short_put(transfer->ux_slave_transfer_request_data_pointer,
                                              (USHORT)control->ux_device_class_audio20_control_volume[0]);
                        _ux_device_stack_transfer_request(transfer, 2, request_length);
                    } else if (control->ux_device_class_audio20_control_volume[0] !=
                               (SHORT)_ux_utility_short_get(transfer->ux_slave_transfer_request_data_pointer)) {

                        /* Update volume.  */
                        control->ux_device_class_audio20_control_changed =
                            UX_DEVICE_CLASS_AUDIO20_CONTROL_VOLUME_CHANGED;
                        control->ux_device_class_audio20_control_volume[0] =
                            (SHORT)_ux_utility_short_get(transfer->ux_slave_transfer_request_data_pointer);
                    }
                ...
                case UX_DEVICE_CLASS_AUDIO20_RANGE:
                    ...
                    _ux_utility_short_put(transfer->ux_slave_transfer_request_data_pointer + 2,
                                          (USHORT)control->ux_device_class_audio20_control_volume_min[0]);
                    _ux_utility_short_put(transfer->ux_slave_transfer_request_data_pointer + 4,
                                          (USHORT)control->ux_device_class_audio20_control_volume_max[0]);
                    _ux_utility_short_put(transfer->ux_slave_transfer_request_data_pointer + 6,
                                          (USHORT)UX_MAX(1, control->ux_device_class_audio20_control_volume_res[0]));
...
/*                                                                        */
/**************************************************************************/
UINT _uxe_device_class_audio20_control_process(UX_DEVICE_CLASS_AUDIO *audio, UX_SLAVE_TRANSFER *transfer,
                                               UX_DEVICE_CLASS_AUDIO20_CONTROL_GROUP *group) {
