...
UINT _ux_device_class_cdc_acm_initialize(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    cdc_acm->ux_device_class_cdc_acm_endpoint_buffer =
        _ux_utility_memory_allocate(UX_NO_ALIGN, UX_CACHE_SAFE_MEMORY, UX_DEVICE_CLASS_CDC_ACM_ENDPOINT_BUFFER_SIZE);
    ...
    status =
        _ux_utility_mutex_create(&cdc_acm->ux_slave_class_cdc_acm_endpoint_in_mutex, "ux_slave_class_cdc_acm_in_mutex");
    ...
    if (status == UX_SUCCESS) {
        ...
        status = _ux_utility_event_flags_create(&cdc_acm->ux_slave_class_cdc_acm_event_flags_group,
    ...
    if (status == UX_SUCCESS) {
        ...
        status = _ux_utility_thread_create(
            &cdc_acm->ux_slave_class_cdc_acm_bulkin_thread, "ux_slave_class_cdc_acm_bulkin_thread",
            _ux_device_class_cdc_acm_bulkin_thread, (ULONG)(ALIGN_TYPE)cdc_acm,
            (VOID *)cdc_acm->ux_slave_class_cdc_acm_bulkin_thread_stack, UX_THREAD_STACK_SIZE, UX_THREAD_PRIORITY_CLASS,
    ...
    if (status == UX_SUCCESS) {
        ...
        /* Bulk endpoint treatment needs to be running in a different thread. So
           ...
           class. */
        status = _ux_utility_thread_create(
            &cdc_acm->ux_slave_class_cdc_acm_bulkout_thread, "ux_slave_class_cdc_acm_bulkout_thread",
            _ux_device_class_cdc_acm_bulkout_thread, (ULONG)(ALIGN_TYPE)cdc_acm,
            (VOID *)cdc_acm->ux_slave_class_cdc_acm_bulkout_thread_stack, UX_THREAD_STACK_SIZE,
