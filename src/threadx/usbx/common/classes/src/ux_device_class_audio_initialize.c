...
UINT _ux_device_class_audio_initialize(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    UX_UTILITY_MULC_SAFE(audio_parameter->ux_device_class_audio_parameter_streams_nb,
                         (ULONG)sizeof(UX_DEVICE_CLASS_AUDIO_STREAM), streams_size, status);
    ...
    audio->ux_device_class_audio_status_size = audio_parameter->ux_device_class_audio_parameter_status_size;

    /* Calculate queue size in bytes.  */
    if (UX_OVERFLOW_CHECK_MULV_ULONG(audio_parameter->ux_device_class_audio_parameter_status_queue_size,
    ...
    if (status == UX_SUCCESS) {
        status = _ux_device_thread_create(&audio_class->ux_slave_class_thread, "ux_device_class_audio_status_thread",
                                          _ux_device_class_audio_interrupt_thread_entry, (ULONG)(ALIGN_TYPE)audio,
                                          audio_class->ux_slave_class_thread_stack,
                                          UX_DEVICE_CLASS_AUDIO_INTERRUPT_THREAD_STACK_SIZE, UX_THREAD_PRIORITY_CLASS,
    ...
    audio->ux_device_class_audio_status_queue =
        (UCHAR *)audio_class->ux_slave_class_thread_stack + UX_DEVICE_CLASS_AUDIO_INTERRUPT_THREAD_STACK_SIZE;
#else
    audio->ux_device_class_audio_status_queue =
        _ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY, memory_size);
    ...
    audio->ux_device_class_audio_interrupt_buffer = _ux_utility_memory_allocate(
        UX_NO_ALIGN, UX_CACHE_SAFE_MEMORY, audio_parameter->ux_device_class_audio_parameter_status_size);
    ...
    /* Save streams.  */
    if (streams_size) {
        audio->ux_device_class_audio_streams =
            (UX_DEVICE_CLASS_AUDIO_STREAM *)((UCHAR *)audio + sizeof(UX_DEVICE_CLASS_AUDIO));
    ...
    for (i = 0; i < audio->ux_device_class_audio_streams_nb; i++) {
        ...
        stream->ux_device_class_audio_stream_feedback_buffer =
            _ux_utility_memory_allocate(UX_NO_ALIGN, UX_CACHE_SAFE_MEMORY, UX_DEVICE_CLASS_AUDIO_FEEDBACK_BUFFER_SIZE);
...
#if UX_DEVICE_ENDPOINT_BUFFER_OWNER == 1
        stream->ux_device_class_audio_stream_buffer =
            (UCHAR *)_ux_utility_memory_allocate(UX_NO_ALIGN, UX_CACHE_SAFE_MEMORY, memory_size);
#else
        stream->ux_device_class_audio_stream_buffer =
            (UCHAR *)_ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY, memory_size);
#endif
        ...
        stream->ux_device_class_audio_stream_transfer_pos =
            (UX_DEVICE_CLASS_AUDIO_FRAME *)stream->ux_device_class_audio_stream_buffer;
        ...
        stream->ux_device_class_audio_stream_thread_stack =
            _ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY, memory_size);
        ...
        status = _ux_device_thread_create(
            &stream->ux_device_class_audio_stream_thread, "ux_device_class_audio_stream_thread",
            stream_parameter->ux_device_class_audio_stream_parameter_thread_entry, (ULONG)(ALIGN_TYPE)stream,
            (VOID *)stream->ux_device_class_audio_stream_thread_stack, memory_size, UX_THREAD_PRIORITY_CLASS,
            UX_THREAD_PRIORITY_CLASS, UX_NO_TIME_SLICE, UX_DONT_START);
        ...
        stream->ux_device_class_audio_stream_task_function =
            stream_parameter->ux_device_class_audio_stream_parameter_task_function;
#endif
        ...
        if (stream_parameter->ux_device_class_audio_stream_parameter_feedback_thread_entry) {
            ...
            stream->ux_device_class_audio_stream_feedback_thread_stack =
                _ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY, memory_size);
            ...
            status = _ux_utility_thread_create(
                &stream->ux_device_class_audio_stream_feedback_thread, "ux_device_class_audio_stream_feedback_thread",
                ...
                memory_size, UX_THREAD_PRIORITY_CLASS, UX_THREAD_PRIORITY_CLASS, UX_NO_TIME_SLICE, UX_DONT_START);
        ...
        if (stream_parameter->ux_device_class_audio_stream_parameter_feedback_task_function) {
            ...
            /* Save task function for streaming.  */
            stream->ux_device_class_audio_stream_feedback_task_function =
                stream_parameter->ux_device_class_audio_stream_parameter_feedback_task_function;
        }
#endif
#endif
...
UINT _uxe_device_class_audio_initialize(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    for (i = 0; i < audio_parameter->ux_device_class_audio_parameter_streams_nb; i++) {
        if ((audio_parameter->ux_device_class_audio_parameter_streams[i]
                 .ux_device_class_audio_stream_parameter_max_frame_buffer_size == 0) ||
            (audio_parameter->ux_device_class_audio_parameter_streams[i]
                 .ux_device_class_audio_stream_parameter_max_frame_buffer_nb == 0))
            return (UX_INVALID_PARAMETER);
    }
