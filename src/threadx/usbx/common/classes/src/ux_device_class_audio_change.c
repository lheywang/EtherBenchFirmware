...
UINT _ux_device_class_audio_change(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    if (interface_ptr->ux_slave_interface_descriptor.bAlternateSetting != 0) {
        ...
        endpoint_dir = (stream->ux_device_class_audio_stream_task_function == _ux_device_class_audio_read_task_function)
                           ? UX_ENDPOINT_OUT
                           : UX_ENDPOINT_IN;
#else

        endpoint_dir =
            (stream->ux_device_class_audio_stream_thread.tx_thread_entry == _ux_device_class_audio_read_thread_entry)
                ? UX_ENDPOINT_OUT
                : UX_ENDPOINT_IN;
#endif
        ...
        while (endpoint != UX_NULL) {
            ...
            if ((endpoint->ux_slave_endpoint_descriptor.bmAttributes & UX_DEVICE_CLASS_AUDIO_EP_TRANSFER_TYPE_MASK) ==
                UX_ISOCHRONOUS_ENDPOINT) {

                /* Check the endpoint direction.  */
                if ((endpoint->ux_slave_endpoint_descriptor.bEndpointAddress & UX_ENDPOINT_DIRECTION) == endpoint_dir) {
                    ...
                    endpoint->ux_slave_endpoint_transfer_request.ux_slave_transfer_request_requested_length =
                        endpoint_dir == UX_ENDPOINT_OUT
                            ...
                            : ((_ux_system_slave->ux_system_slave_speed == UX_HIGH_SPEED_DEVICE)
                                   ? UX_FEEDBACK_SIZE_HIGH_SPEED
                                   : UX_FEEDBACK_SIZE_FULL_SPEED);
        ...
        /*
        _ux_device_stack_transfer_all_request_abort(stream -> ux_device_class_audio_stream_endpoint,
    ...
    if (stream->ux_device_class_audio_stream_callbacks.ux_device_class_audio_stream_change)
        stream->ux_device_class_audio_stream_callbacks.ux_device_class_audio_stream_change(
            stream, interface_ptr->ux_slave_interface_descriptor.bAlternateSetting);
