...
VOID _ux_device_class_ccid_thread_entry(ULONG ccid_inst) {
    ...
    while (1) {
        ...
        status = _ux_device_stack_transfer_request(transfer_cmd, parameter->ux_device_class_ccid_max_transfer_length,
                                                   parameter->ux_device_class_ccid_max_transfer_length);

        /* Access to CCID command message header.  */
        cmd = (UX_DEVICE_CLASS_CCID_MESSAGE_HEADER *)transfer_cmd->ux_slave_transfer_request_data_pointer;

        /* Check the completion code and message length. */
        if ((status != UX_SUCCESS) || (transfer_cmd->ux_slave_transfer_request_completion_code != UX_SUCCESS) ||
            (transfer_cmd->ux_slave_transfer_request_actual_length < UX_DEVICE_CLASS_CCID_MESSAGE_HEADER_LENGTH) ||
        ...
        for (cmd_index = 0; cmd_index < UX_DEVICE_CLASS_CCID_N_COMMANDS;) {
            if (cmd->bMessageType == cmd_sett->ux_device_class_ccid_command_sett_command_type)
                break;
        ...
        /* Initialize response.  */
        rsp = (UX_DEVICE_CLASS_CCID_RDR_TO_PC_SLOT_STATUS_HEADER *)ccid->ux_device_class_ccid_header;
        _ux_utility_memory_set(rsp, 0,
                               UX_DEVICE_CLASS_CCID_MESSAGE_HEADER_LENGTH); /* Use case of memset is verified. */
        ...
        rsp->bStatus = UX_DEVICE_CLASS_CCID_SLOT_STATUS(slot->ux_device_class_ccid_slot_icc_status, 0);
        ...
        if (cmd->bMessageType != UX_DEVICE_CLASS_CCID_PC_TO_RDR_ABORT || !slot->ux_device_class_ccid_slot_aborting) {

            /* Check if slot is idle.  */
            if ((signed char)slot->ux_device_class_ccid_slot_runner < 0) {

                /* Slot is idle, check if free runner available.  */
                if (ccid->ux_device_class_ccid_n_busy < parameter->ux_device_class_ccid_max_n_busy_slots) {

                    /* Get a free runner for the command.  */
                    runner = ccid->ux_device_class_ccid_runners;
                    for (i = 0; i < parameter->ux_device_class_ccid_max_n_busy_slots; i++) {
                    ...
                    _ux_utility_memory_copy(
                        runner->ux_device_class_ccid_runner_command, cmd,
                        transfer_cmd->ux_slave_transfer_request_actual_length); /* Use case of memcpy is verified. */
                    _ux_utility_memory_copy(
                        runner->ux_device_class_ccid_runner_response, rsp,
                        UX_DEVICE_CLASS_CCID_MESSAGE_HEADER_LENGTH); /* Use case of memcpy is verified. */
                    ...
                    _ux_device_event_flags_set(&ccid->ux_device_class_ccid_events, 1u << i, UX_OR);
                    continue;
                }
            ...
            rsp->bStatus = UX_DEVICE_CLASS_CCID_SLOT_STATUS(slot->ux_device_class_ccid_slot_icc_status, 1);
        ...
        if (cmd->bMessageType == UX_DEVICE_CLASS_CCID_PC_TO_RDR_ABORT) {
            ...
            if (cmd->bSeq != slot->ux_device_class_ccid_slot_aborting_seq) {
                ...
                if (slot->ux_device_class_ccid_slot_aborting) {
                    ...
                    parameter->ux_device_class_ccid_handles->ux_device_class_ccid_handles_abort(cmd->bSlot, &messages);
