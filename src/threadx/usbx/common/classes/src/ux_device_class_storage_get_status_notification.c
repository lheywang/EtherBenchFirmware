...
UINT _ux_device_class_storage_get_status_notification(UX_SLAVE_CLASS_STORAGE *storage, ULONG lun,
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_STORAGE_READ_CAPACITY, storage, lun, 0, 0,
    ...
    status = storage->ux_slave_class_storage_lun[lun].ux_slave_class_storage_media_notification(
        storage, lun, storage->ux_slave_class_storage_lun[lun].ux_slave_class_storage_media_id, notification_class,
        &media_notification, &media_notification_length);
    ...
    if (status != UX_SUCCESS) {
        ...
        _ux_utility_memory_copy(transfer_request->ux_slave_transfer_request_data_pointer + sizeof(USHORT),
                                media_notification, media_notification_length); /* Use case of memcpy is verified. */
        ...
        _ux_device_stack_transfer_request(transfer_request, media_notification_length, media_notification_length);
#else
        /* Next: Transfer (DATA).  */
