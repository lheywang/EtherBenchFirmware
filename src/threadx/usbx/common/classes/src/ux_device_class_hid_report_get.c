...
UINT _ux_device_class_hid_report_get(UX_SLAVE_CLASS_HID *hid, ULONG descriptor_type, ULONG request_index,
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_HID_REPORT_GET, hid, descriptor_type, request_index, 0,
    ...
    if (report_type == UX_DEVICE_CLASS_HID_REPORT_TYPE_INPUT && hid->ux_device_class_hid_report_id != UX_TRUE)
    ...
    if (status == UX_SUCCESS) {
        ...
        _ux_utility_memory_copy(buffer, hid_event.ux_device_class_hid_event_buffer,
                                hid_event_length); /* Use case of memcpy is verified. */
    } else {
