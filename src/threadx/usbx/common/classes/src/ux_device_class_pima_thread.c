...
VOID _ux_device_class_pima_thread(ULONG pima_class) {
    ...
    status =
        _ux_device_thread_create(&pima->ux_device_class_pima_interrupt_thread, "ux_slave_class_thread_pima_interrupt",
                                 _ux_device_class_pima_interrupt_thread, (ULONG)(ALIGN_TYPE)pima,
                                 (VOID *)pima->ux_device_class_pima_interrupt_thread_stack, UX_THREAD_STACK_SIZE,
                                 UX_THREAD_PRIORITY_CLASS, UX_THREAD_PRIORITY_CLASS, UX_NO_TIME_SLICE, UX_AUTO_START);
    ...
    while (1) {
        ...
        while (device->ux_slave_device_state == UX_DEVICE_CONFIGURED) {
            ...
            if (status == UX_SUCCESS) {
                ...
                if (_ux_utility_short_get(pima_command + UX_DEVICE_CLASS_PIMA_COMMAND_HEADER_TYPE) ==
                    UX_DEVICE_CLASS_PIMA_CT_COMMAND_BLOCK) {

                    /* Save the transaction ID.  */
                    pima->ux_device_class_pima_transaction_id =
                        _ux_utility_long_get(pima_command + UX_DEVICE_CLASS_PIMA_COMMAND_HEADER_TRANSACTION_ID);
                    ...
                    pima_parameter_1 =
                        _ux_utility_long_get(pima_command + UX_DEVICE_CLASS_PIMA_COMMAND_HEADER_PARAMETER_1);

                    /* Retrieve the parameter 2.  */
                    pima_parameter_2 =
                        _ux_utility_long_get(pima_command + UX_DEVICE_CLASS_PIMA_COMMAND_HEADER_PARAMETER_2);

                    /* Retrieve the parameter 3.  */
                    pima_parameter_3 =
                        _ux_utility_long_get(pima_command + UX_DEVICE_CLASS_PIMA_COMMAND_HEADER_PARAMETER_3);
                    ...
                    case UX_DEVICE_CLASS_PIMA_OC_OPEN_SESSION:
                        ...
                        /* If the first parameter is 0x00000000,
                           ...
                           The response Session_Already_Open should also be used if the device supports multiple
                           sessions, but a session with that ID is already open. If the device supports multiple
                           sessions, and the maximum number of sessions are open, the device should respond with
                           Device_Busy  */
                        if (pima_parameter_1 == 0) {
                            _ux_device_class_pima_response_send(pima, UX_DEVICE_CLASS_PIMA_RC_INVALID_PARAMETER, 0, 0,
                                                                0, 0);
                            break;
                        }
                        /* Check if session is already opened.  */
                        if (pima->ux_device_class_pima_session_id != 0) {
                            _ux_device_class_pima_response_send(pima, UX_DEVICE_CLASS_PIMA_RC_SESSION_ALREADY_OPENED,
                    ...
                    default:
                        ...
                        if (pima->ux_device_class_pima_session_id == 0) {
                            ...
                            _ux_device_class_pima_response_send(pima, UX_DEVICE_CLASS_PIMA_RC_SESSION_NOT_OPEN, 0, 0, 0,
                                                                0);
                        } else {
                            ...
                            case UX_DEVICE_CLASS_PIMA_OC_GET_NUM_OBJECTS:
                                ...
                                status = _ux_device_class_pima_objects_number_send(pima, pima_parameter_1,
                                                                                   pima_parameter_2, pima_parameter_3);
                                break;

                            case UX_DEVICE_CLASS_PIMA_OC_GET_OBJECT_HANDLES:

                                /* Return the object handles found in the system.  */
                                status = _ux_device_class_pima_object_handles_send(pima, pima_parameter_1,
                                                                                   pima_parameter_2, pima_parameter_3);
                            ...
                            case UX_DEVICE_CLASS_PIMA_OC_SEND_OBJECT_INFO:
                                ...
                                status =
                                    _ux_device_class_pima_object_info_send(pima, pima_parameter_1, pima_parameter_2);
                                break;
                            ...
                            case UX_DEVICE_CLASS_PIMA_OC_GET_PARTIAL_OBJECT:
                                ...
                                status = _ux_device_class_pima_partial_object_data_get(
                                    pima, pima_parameter_1, pima_parameter_2, pima_parameter_3);
                                break;

                            case UX_DEVICE_CLASS_PIMA_OC_FORMAT_STORE:

                                /* Format the storage device. This calls the application to reset all object handles
                                   stored on the media.   */
                            ...
                            case UX_DEVICE_CLASS_PIMA_OC_RESET_DEVICE:
                                ...
                                /* Reset the device. This calls the application to reset the device. The session is
                                   closed but all objects retain their properties.  */
                                status = _ux_device_class_pima_device_reset(pima);
                                break;

                            case UX_DEVICE_CLASS_PIMA_OC_GET_OBJECT_PROPS_SUPPORTED:

                                /* Return an Object Property Code array of supported object properties for the object
                            ...
                            case UX_DEVICE_CLASS_PIMA_OC_GET_OBJECT_PROP_DESC:
                                ...
                                /* Returns the appropriate property that describes the dataset that is indicated in the
                                 * first parameter.  */
                            ...
                            case UX_DEVICE_CLASS_PIMA_OC_GET_OBJECT_PROP_VALUE:
                                ...
                                status = _ux_device_class_pima_object_prop_value_get(pima, pima_parameter_1,
                                                                                     pima_parameter_2);
                                break;
                            ...
                            case UX_DEVICE_CLASS_PIMA_OC_SET_OBJECT_PROP_VALUE:
                                ...
                                status = _ux_device_class_pima_object_prop_value_set(pima, pima_parameter_1,
                                                                                     pima_parameter_2);
                                break;
                            ...
                            case UX_DEVICE_CLASS_PIMA_OC_COPY_OBJECT:
                                ...
                                _ux_device_class_pima_response_send(
                                    pima, UX_DEVICE_CLASS_PIMA_RC_OPERATION_NOT_SUPPORTED, 0, 0, 0, 0);
