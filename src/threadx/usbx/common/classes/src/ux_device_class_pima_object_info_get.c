...
UINT _ux_device_class_pima_object_info_get(UX_SLAVE_CLASS_PIMA *pima, ULONG object_handle) {
    ...
    UX_TRACE_IN_LINE_INSERT(UX_TRACE_DEVICE_CLASS_PIMA_OBJECT_INFO_GET, pima, object_handle, 0, 0,
    ...
    if (status != UX_SUCCESS)
        ...
        object_info_length = UX_DEVICE_CLASS_PIMA_DATA_HEADER_SIZE + UX_DEVICE_CLASS_PIMA_OBJECT_VARIABLE_OFFSET +
                             file_name_length + capture_date_length + modification_date_length + keywords_length;
        ...
        _ux_utility_short_put(object_info + UX_DEVICE_CLASS_PIMA_DATA_HEADER_TYPE, UX_DEVICE_CLASS_PIMA_CT_DATA_BLOCK);
        ...
        _ux_utility_descriptor_pack((UCHAR *)object, _ux_system_class_pima_object_structure,
                                    UX_DEVICE_CLASS_PIMA_OBJECT_ENTRIES, object_info_pointer);
        ...
        _ux_utility_memory_copy(object_info_pointer, object->ux_device_class_pima_object_filename,
                                file_name_length); /* Use case of memcpy is verified. */

        /* Point to the next field.  */
        object_info_pointer += file_name_length;

        /* Copy that string into the capture date field.  */
        _ux_utility_memory_copy(object_info_pointer, object->ux_device_class_pima_object_capture_date,
                                capture_date_length); /* Use case of memcpy is verified. */

        /* Point to the next field.  */
        object_info_pointer += capture_date_length;

        /* Copy that string into the modification date field.  */
        _ux_utility_memory_copy(object_info_pointer, object->ux_device_class_pima_object_modification_date,
                                modification_date_length); /* Use case of memcpy is verified. */

        /* Point to the next field.  */
        object_info_pointer += modification_date_length;

        /* Copy that string into the keywords field.  */
        _ux_utility_memory_copy(object_info_pointer, object->ux_device_class_pima_object_keywords,
                                keywords_length); /* Use case of memcpy is verified. */

        /* Fill in the size of the response header.  */
        _ux_utility_long_put(object_info + UX_DEVICE_CLASS_PIMA_DATA_HEADER_LENGTH, object_info_length);
