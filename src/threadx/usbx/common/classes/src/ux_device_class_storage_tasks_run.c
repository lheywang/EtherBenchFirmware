...
static inline UINT _ux_device_class_storage_task_usb(UX_SLAVE_CLASS_STORAGE *storage) {
    ...
    while (immediate_state) {
        ...
        if (device->ux_slave_device_state != UX_DEVICE_CONFIGURED ||
            storage->ux_device_class_storage_ep_in == UX_NULL || storage->ux_device_class_storage_ep_out == UX_NULL) {
        ...
        case UX_DEVICE_CLASS_STORAGE_STATE_TRANS_WAIT:
            ...
            if (status < UX_STATE_ERROR) {
                ...
                if (storage->ux_device_class_storage_disk_state != UX_DEVICE_CLASS_STORAGE_DISK_IDLE) {
                    storage->ux_device_class_storage_disk_state = UX_DEVICE_CLASS_STORAGE_DISK_USB_ERROR;
                }
            ...
            if (status == UX_STATE_ERROR) {
                ...
                storage->ux_slave_class_storage_csw_residue =
                    storage->ux_slave_class_storage_host_length - storage->ux_device_class_storage_data_count;
                ...
                /* Issue CSW.  */
                _ux_device_class_storage_csw_send(storage, storage->ux_slave_class_storage_cbw_lun,
            ...
            if (status == UX_STATE_NEXT) {
                ...
                storage->ux_device_class_storage_data_count +=
                    storage->ux_device_class_storage_transfer->ux_slave_transfer_request_actual_length;
...
static inline VOID _ux_device_class_storage_cbw_process(UX_SLAVE_CLASS_STORAGE *storage) {
    ...
    if (storage->ux_slave_class_storage_csw_status) {
        ...
        if (storage->ux_slave_class_storage_host_length &&
            (storage->ux_slave_class_storage_cbw_flags & UX_DEVICE_CLASS_STORAGE_CBW_FLAG_DIR) == 0) {
            _ux_device_class_storage_halt_out(storage);
        } else {
            _ux_device_class_storage_halt_in(storage);
        }
...
static inline VOID _ux_device_class_storage_cmd_process(UX_SLAVE_CLASS_STORAGE *storage, UCHAR *cbw) {
    ...
    /* Get dCBWTag.  */
    storage->ux_slave_class_storage_scsi_tag = _ux_utility_long_get(cbw + UX_SLAVE_CLASS_STORAGE_CBW_TAG);
...
static inline VOID _ux_device_class_storage_data_cases_check(UX_SLAVE_CLASS_STORAGE *storage) {
    ...
    if (storage->ux_slave_class_storage_cbw_flags & UX_DEVICE_CLASS_STORAGE_CBW_FLAG_DIR) {
        ...
        /* Case (7).  */
        if (storage->ux_slave_class_storage_host_length < storage->ux_device_class_storage_data_length) {

            /* Part of data will be sent.  */
            storage->ux_device_class_storage_data_length = storage->ux_slave_class_storage_host_length;
        }
        ...
        /* Ho.  */
        /* Case (3), (10), (13).  */
        if (storage->ux_slave_class_storage_host_length < storage->ux_device_class_storage_device_length ||
        ...
        if (storage->ux_slave_class_storage_host_length != storage->ux_device_class_storage_device_length) {
            ...
            storage->ux_slave_class_storage_csw_residue =
                storage->ux_slave_class_storage_host_length - storage->ux_device_class_storage_device_length;
...
static inline UINT _ux_device_class_storage_data_next(UX_SLAVE_CLASS_STORAGE *storage) {
    ...
    case UX_SLAVE_CLASS_STORAGE_SCSI_READ32:
        ...
        if (storage->ux_device_class_storage_data_count >= storage->ux_device_class_storage_data_length) {
            ...
            if (storage->ux_slave_class_storage_host_length > storage->ux_device_class_storage_data_length) {
                ...
                /* Update dCSWDataResidue.  */
                storage->ux_slave_class_storage_csw_residue =
                    storage->ux_slave_class_storage_host_length - storage->ux_device_class_storage_data_length;
            }

            /* Set to fail if device expects more data.  */
            if (storage->ux_device_class_storage_device_length > storage->ux_device_class_storage_data_count) {
            ...
            /* Issue CSW.  */
            _ux_device_class_storage_csw_send(storage, storage->ux_slave_class_storage_cbw_lun,
                                              storage->ux_device_class_storage_ep_in, 0 /* Not used.  */);
        } else {
    ...
    default:
        ...
        /* Stall if host expects more data.  */
        if (storage->ux_slave_class_storage_host_length > storage->ux_device_class_storage_data_count) {
            _ux_device_class_storage_halt_trans(storage);
...
static inline VOID _ux_device_class_storage_trans_start(UX_SLAVE_CLASS_STORAGE *storage) {
    ...
    if (storage->ux_device_class_storage_data_buffer && device_length &&
        storage->ux_device_class_storage_cmd_state == UX_DEVICE_CLASS_STORAGE_CMD_READ) {
        _ux_utility_memory_copy(storage->ux_device_class_storage_transfer->ux_slave_transfer_request_data_pointer,
...
static inline VOID _ux_device_class_storage_task_disk(UX_SLAVE_CLASS_STORAGE *storage) {
    ...
    while (immediate_state) {
        ...
        case UX_DEVICE_CLASS_STORAGE_DISK_OP_START:
            ...
            storage->ux_device_class_storage_disk_state = UX_DEVICE_CLASS_STORAGE_DISK_OP_WAIT;
    ...
    case UX_SLAVE_CLASS_STORAGE_SCSI_READ32:
        return storage->ux_slave_class_storage_lun[storage->ux_slave_class_storage_cbw_lun]
            .ux_slave_class_storage_media_read(
                storage, storage->ux_slave_class_storage_cbw_lun,
                storage->ux_device_class_storage_buffer[storage->ux_device_class_storage_buffer_disk],
                storage->ux_device_class_storage_disk_n_lb, storage->ux_device_class_storage_cmd_lba,
    ...
    case UX_SLAVE_CLASS_STORAGE_SCSI_SYNCHRONIZE_CACHE:
        return storage->ux_slave_class_storage_lun[storage->ux_slave_class_storage_cbw_lun]
            .ux_slave_class_storage_media_flush(
                storage, storage->ux_slave_class_storage_cbw_lun, storage->ux_device_class_storage_disk_n_lb,
                storage->ux_device_class_storage_cmd_lba, &storage->ux_device_class_storage_media_status);
...
static inline VOID _ux_device_class_storage_disk_read_next(UX_SLAVE_CLASS_STORAGE *storage) {
    ...
    storage->ux_device_class_storage_buffer_state[storage->ux_device_class_storage_buffer_disk] =
        UX_DEVICE_CLASS_STORAGE_BUFFER_FULL;
    ...
    if (storage->ux_device_class_storage_cmd_n_lb == 0) {
        ...
        /* Update buffer index.  */
        storage->ux_device_class_storage_buffer_disk = !storage->ux_device_class_storage_buffer_disk;
...
static inline VOID _ux_device_class_storage_disk_error(UX_SLAVE_CLASS_STORAGE *storage) {
    ...
    case UX_SLAVE_CLASS_STORAGE_SCSI_READ16:
    case UX_SLAVE_CLASS_STORAGE_SCSI_READ32:
        storage->ux_slave_class_storage_lun[storage->ux_slave_class_storage_cbw_lun].ux_slave_class_storage_media_read(
            storage, storage->ux_slave_class_storage_cbw_lun, UX_NULL, 0, 0, UX_NULL);
        break;
    case UX_SLAVE_CLASS_STORAGE_SCSI_WRITE16:
    case UX_SLAVE_CLASS_STORAGE_SCSI_WRITE32:
        storage->ux_slave_class_storage_lun[storage->ux_slave_class_storage_cbw_lun].ux_slave_class_storage_media_write(
            storage, storage->ux_slave_class_storage_cbw_lun, UX_NULL, 0, 0, UX_NULL);
        break;
    default:
        break;
