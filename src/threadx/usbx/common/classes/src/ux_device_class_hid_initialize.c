...
UINT _ux_device_class_hid_initialize(UX_SLAVE_CLASS_COMMAND *command) {
    ...
    hid->ux_device_class_hid_endpoint_buffer =
        _ux_utility_memory_allocate(UX_NO_ALIGN, UX_CACHE_SAFE_MEMORY, UX_DEVICE_CLASS_HID_ENDPOINT_BUFFER_SIZE);
    ...
    if (status == UX_SUCCESS)
        status = _ux_device_thread_create(&class_ptr->ux_slave_class_thread, "ux_slave_hid_thread",
                                          _ux_device_class_hid_interrupt_thread, (ULONG)(ALIGN_TYPE)class_ptr,
                                          (VOID *)class_ptr->ux_slave_class_thread_stack,
    ...
    /* Set event buffer.  */
    hid->ux_device_class_hid_event.ux_device_class_hid_event_buffer = UX_DEVICE_CLASS_HID_INTERRUPTIN_BUFFER(hid);
#endif
    ...
    if (status == UX_SUCCESS) {
        ...
        UX_ASSERT(!UX_OVERFLOW_CHECK_MULC_ULONG(UX_DEVICE_CLASS_HID_EVENT_QUEUE_ITEM_SIZE(hid),
                                                UX_DEVICE_CLASS_HID_PARAM_EVENT_QUEUE_SIZE(hid_parameter)));
        array_memory_size =
            UX_DEVICE_CLASS_HID_EVENT_QUEUE_ITEM_SIZE(hid) * UX_DEVICE_CLASS_HID_PARAM_EVENT_QUEUE_SIZE(hid_parameter);
        hid->ux_device_class_hid_event_array =
            _ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY, array_memory_size);
        ...
        if (hid->ux_device_class_hid_event_array != UX_NULL) {
            ...
            hid->ux_device_class_hid_event_array_end =
                (UX_DEVICE_CLASS_HID_EVENT *)((UCHAR *)hid->ux_device_class_hid_event_array + array_memory_size);
            ...
            status = _ux_utility_event_flags_create(&hid->ux_device_class_hid_event_flags_group,
                                                    "ux_device_class_hid_event_flag");
...
#endif
            {
                ...
                status =
                    _ux_utility_mutex_create(&hid->ux_device_class_hid_read_mutex, "ux_device_class_hid_read_mutex");
                if (status == UX_SUCCESS) {
#endif
                    ...
                    if (hid_parameter->ux_device_class_hid_parameter_receiver_initialize) {
                        ...
                        status = hid_parameter->ux_device_class_hid_parameter_receiver_initialize(
                            hid, hid_parameter, &hid->ux_device_class_hid_receiver);
                    }
