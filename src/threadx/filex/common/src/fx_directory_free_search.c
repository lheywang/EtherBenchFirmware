...
UINT _fx_directory_free_search(FX_MEDIA *media_ptr, FX_DIR_ENTRY *directory_ptr, FX_DIR_ENTRY *entry_ptr) {
    ...
    for (i = 0, j = 0; entry_ptr->fx_dir_entry_name[i]; i++) {
        ...
        else if ((entry_ptr->fx_dir_entry_name[i] == '~') || (entry_ptr->fx_dir_entry_name[i] == '-') ||
                 (entry_ptr->fx_dir_entry_name[i] == '_') || (entry_ptr->fx_dir_entry_name[i] == '}') ||
                 (entry_ptr->fx_dir_entry_name[i] == '{') || (entry_ptr->fx_dir_entry_name[i] == '(') ||
                 (entry_ptr->fx_dir_entry_name[i] == ')') || (entry_ptr->fx_dir_entry_name[i] == '`') ||
                 (entry_ptr->fx_dir_entry_name[i] == '\'') || (entry_ptr->fx_dir_entry_name[i] == '!') ||
                 (entry_ptr->fx_dir_entry_name[i] == '#') || (entry_ptr->fx_dir_entry_name[i] == '$') ||
                 (entry_ptr->fx_dir_entry_name[i] == '&') || (entry_ptr->fx_dir_entry_name[i] == '@') ||
    ...
    do {
        ...
        if (directory_index >= directory_entries) {
            ...
            clusters_needed =
                (sectors + (media_ptr->fx_media_sectors_per_cluster - 1)) / media_ptr->fx_media_sectors_per_cluster;
            ...
            if (((search_dir_ptr) || (media_ptr->fx_media_32_bit_FAT)) &&
                ...
                while (clusters) {
                    ...
                    logical_sector =
                        ...
                        ((((ULONG)FAT_index) - FX_FAT_ENTRY_START) * ((ULONG)media_ptr->fx_media_sectors_per_cluster));
                    ...
                    if (sectors > 1) {
                        ...
                        status = _fx_utility_logical_sector_flush(media_ptr, (ULONG64)(logical_sector + 1),
                ...
                directory_entries = directory_entries + ((clusters_needed * media_ptr->fx_media_sectors_per_cluster) *
                                                         media_ptr->fx_media_bytes_per_sector) /
                                                            FX_DIR_ENTRY_SIZE;
