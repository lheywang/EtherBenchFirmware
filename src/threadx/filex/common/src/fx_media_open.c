...
UINT _fx_media_open(FX_MEDIA *media_ptr, CHAR *media_name, VOID (*media_driver)(FX_MEDIA *), VOID *driver_info_ptr,
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_MEDIA_OPEN, media_ptr, media_driver, memory_ptr, memory_size,
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_INTERNAL_IO_DRIVER_BOOT_READ, media_ptr, memory_ptr, 0, 0,
    ...
    FX_TRACE_OBJECT_REGISTER(FX_TRACE_OBJECT_TYPE_MEDIA, media_ptr, media_name, FX_MAX_FAT_CACHE,
    ...
    if (media_ptr->fx_media_FAT_type != FX_exFAT) {
        ...
        media_ptr->fx_media_root_sector_start =
            ...
            (media_ptr->fx_media_number_of_FATs * media_ptr->fx_media_sectors_per_FAT);

        /* Calculate the number of directory sectors.  */
        media_ptr->fx_media_root_sectors = ((media_ptr->fx_media_root_directory_entries * FX_DIR_ENTRY_SIZE) +
                                            media_ptr->fx_media_bytes_per_sector - 1) /
                                           media_ptr->fx_media_bytes_per_sector;

        /* Calculate the starting data sector.  */
        media_ptr->fx_media_data_sector_start =
            media_ptr->fx_media_root_sector_start + media_ptr->fx_media_root_sectors;

        /* Calculate the total number of clusters.  */
        media_ptr->fx_media_total_clusters =
            (ULONG)((media_ptr->fx_media_total_sectors - media_ptr->fx_media_data_sector_start) /
...
#ifdef FX_ENABLE_EXFAT
    if (media_ptr->fx_media_32_bit_FAT == FX_TRUE || (media_ptr->fx_media_FAT_type == FX_exFAT))
#else
...
#endif /* FX_ENABLE_EXFAT */
    {
        ...
        if (media_ptr->fx_media_32_bit_FAT == FX_TRUE) {
            ...
            media_ptr->fx_media_root_sector_start =
                media_ptr->fx_media_data_sector_start +
                (media_ptr->fx_media_root_cluster_32 - FX_FAT_ENTRY_START) * media_ptr->fx_media_sectors_per_cluster;
#ifdef FX_ENABLE_EXFAT
        }
#endif /* FX_ENABLE_EXFAT */
        ...
        for (cluster_number = media_ptr->fx_media_root_cluster_32;;) {
            ...
            if (status != FX_SUCCESS) {
                ...
                /* If trace is enabled, insert this event into the trace buffer.  */
                FX_TRACE_IN_LINE_INSERT(FX_TRACE_INTERNAL_IO_DRIVER_UNINIT, media_ptr, 0, 0, 0,
                                        FX_TRACE_INTERNAL_EVENTS, 0, 0)
        ...
        media_ptr->fx_media_root_directory_entries =
            (i * media_ptr->fx_media_sectors_per_cluster * media_ptr->fx_media_bytes_per_sector) / FX_DIR_ENTRY_SIZE;
    }
    ...
    if (media_ptr->fx_media_12_bit_FAT) {
        ...
        for (cluster_number = FX_FAT_ENTRY_START;
             cluster_number < (media_ptr->fx_media_total_clusters) + FX_FAT_ENTRY_START; cluster_number++) {
...
#ifdef FX_ENABLE_EXFAT
    else if ((media_ptr->fx_media_available_clusters == 0) && (media_ptr->fx_media_FAT_type != FX_exFAT))
#else
...
#endif /* FX_ENABLE_EXFAT */
    {
        ...
        _fx_utility_logical_sector_flush(media_ptr, ((ULONG64)1), (ULONG64)(media_ptr->fx_media_total_sectors),
                                         FX_TRUE);
        ...
        for (i = 0; i < media_ptr->fx_media_sectors_per_FAT; i++) {
            ...
            /* If trace is enabled, insert this event into the trace buffer.  */
            FX_TRACE_IN_LINE_INSERT(FX_TRACE_INTERNAL_IO_DRIVER_READ, media_ptr, FAT_sector, FAT_read_sectors,
                                    media_ptr->fx_media_memory_buffer, FX_TRACE_INTERNAL_EVENTS, 0, 0)
