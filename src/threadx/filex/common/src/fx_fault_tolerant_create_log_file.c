...
UINT _fx_fault_tolerant_create_log_file(FX_MEDIA *media_ptr) {
    ...
    if (media_ptr->fx_media_FAT_type == FX_exFAT) {
        ...
        /* Mark the cluster as used.  */
        status =
            _fx_utility_exFAT_cluster_state_set(media_ptr, FAT_index + clusters - 1, FX_EXFAT_BITMAP_CLUSTER_OCCUPIED);
    ...
    _fx_utility_32_unsigned_write(media_ptr->fx_media_fault_tolerant_memory_buffer + FX_FAULT_TOLERANT_BOOT_INDEX,
                                  FAT_index);
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_INTERNAL_IO_DRIVER_BOOT_WRITE, media_ptr,
                            media_ptr->fx_media_fault_tolerant_memory_buffer, 0, 0, FX_TRACE_INTERNAL_EVENTS, 0, 0)
