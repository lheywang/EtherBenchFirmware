...
UINT _fx_fault_tolerant_enable(FX_MEDIA *media_ptr, VOID *memory_buffer, UINT memory_size) {
    ...
    /* If trace is enabled, insert this event into the trace buffer.  */
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_INTERNAL_IO_DRIVER_BOOT_READ, media_ptr, memory_buffer, 0, 0,
    ...
    if (start_cluster != 0) {
        ...
        if ((start_cluster >= FX_FAT_ENTRY_START) && (start_cluster < media_ptr->fx_media_fat_reserved)) {
            ...
            if (start_cluster) {
                ...
                /* Verify ID field. */
                if (_fx_utility_32_unsigned_read((UCHAR *)&log_header->fx_fault_tolerant_log_header_id) ==
                    FX_FAULT_TOLERANT_ID) {

                    /* Calculate checksum of log header. */
                    checksum =
                        _fx_fault_tolerant_calculate_checksum((UCHAR *)log_header, FX_FAULT_TOLERANT_LOG_HEADER_SIZE);
                    ...
                    if (checksum == 0) {
                        ...
                        total_size =
                            _fx_utility_16_unsigned_read((UCHAR *)&log_header->fx_fault_tolerant_log_header_total_size);
                        media_ptr->fx_media_fault_tolerant_file_size = total_size;

                        /* Verify the checksum of the FAT chain. */
                        checksum =
                            _fx_fault_tolerant_calculate_checksum((UCHAR *)FAT_chain, FX_FAULT_TOLERANT_FAT_CHAIN_SIZE);
                        ...
                        if (checksum == 0) {
                            ...
                            if (total_size >
                                (FX_FAULT_TOLERANT_LOG_CONTENT_OFFSET + FX_FAULT_TOLERANT_LOG_HEADER_SIZE)) {
