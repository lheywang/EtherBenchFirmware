...
UINT _fx_utility_exFAT_system_area_format(FX_MEDIA *media_ptr, ULONG boundary_unit, ULONG *system_area_checksum_ptr,
    ...
    for (counter = 0; counter < (FX_EXFAT_FAT_OEM_PARAM_OFFSET - FX_EXFAT_FAT_EXT_BOOT_SECTOR_OFFSET); counter++) {
        ...
        for (i = 0; i < media_ptr->fx_media_bytes_per_sector; i++) {
            ...
            *system_area_checksum_ptr =
                ((*system_area_checksum_ptr >> 1) | (*system_area_checksum_ptr << 31)) + (ULONG)system_area_buffer[i];
        }
        ...
        status = _fx_utility_exFAT_system_sector_write(
            ...
            (ULONG64)(FX_EXFAT_FAT_EXT_BOOT_SECTOR_OFFSET + counter + FX_EXFAT_FAT_MAIN_SYSTEM_AREA_SIZE), ((ULONG)1),
            FX_BOOT_SECTOR);
    ...
    status = _fx_utility_exFAT_system_sector_write(media_ptr, system_area_buffer, FX_EXFAT_FAT_OEM_PARAM_OFFSET, 1,
                                                   FX_BOOT_SECTOR);
    ...
    for (counter = 0; counter < media_ptr->fx_media_bytes_per_sector; counter++) {
        ...
        /* Right rotate the checksum by one bit position and add the data.  */
        *system_area_checksum_ptr =
            ((*system_area_checksum_ptr >> 1) | (*system_area_checksum_ptr << 31)) + (ULONG)system_area_buffer[counter];
    }
    ...
    status = _fx_utility_exFAT_system_sector_write(media_ptr, system_area_buffer, FX_EXFAT_FAT_OEM_PARAM_OFFSET + 1, 1,
                                                   FX_BOOT_SECTOR);
    ...
    status = _fx_utility_exFAT_system_sector_write(
        media_ptr, system_area_buffer, FX_EXFAT_FAT_OEM_PARAM_OFFSET + 1 + FX_EXFAT_FAT_MAIN_SYSTEM_AREA_SIZE, 1,
        FX_BOOT_SECTOR);
