...
UINT _fx_file_write(FX_FILE *file_ptr, VOID *buffer_ptr, ULONG size) {
...
#ifdef FX_ENABLE_FAULT_TOLERANT
    UCHAR data_append = FX_FALSE;      /* Whether or not new data would extend beyond the size of the
...
#ifdef FX_ENABLE_EXFAT
    if ((media_ptr->fx_media_FAT_type != FX_exFAT) && (file_ptr->fx_file_current_file_offset + size > 0xFFFFFFFFULL))
#else
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_FILE_WRITE, file_ptr, buffer_ptr, size, 0, FX_TRACE_FILE_EVENTS, &trace_event,
                            &trace_timestamp)
    ...
    bytes_per_cluster =
        ((ULONG)media_ptr->fx_media_bytes_per_sector) * ((ULONG)media_ptr->fx_media_sectors_per_cluster);
    ...
    if (media_ptr->fx_media_fault_tolerant_enabled) {
        ...
        if (file_ptr->fx_file_current_file_offset == file_ptr->fx_file_current_file_size) {
            ...
            replace_clusters =
                (UINT)((file_ptr->fx_file_current_file_offset + size + bytes_per_cluster - 1) / bytes_per_cluster -
    ...
    if (((file_ptr->fx_file_current_available_size - file_ptr->fx_file_current_file_offset) < size)
            ...
            if (replace_clusters > 0) {
                ...
                if ((media_ptr->fx_media_FAT_type == FX_exFAT) &&
                    ...
                    if (data_append == FX_FALSE) {
                        copy_tail_cluster = file_ptr->fx_file_first_physical_cluster +
        ...
        if (replace_clusters > 0) {
            ...
            if ((file_ptr->fx_file_dir_entry.fx_dir_entry_dont_use_fat & 1) &&
                ...
                /* Build FAT chain.  */
                for (i = file_ptr->fx_file_dir_entry.fx_dir_entry_cluster; i < file_ptr->fx_file_last_physical_cluster;
                     ++i) {
        ...
        while (clusters) {
                ...
                if (media_ptr->fx_media_fault_tolerant_enabled) {
                    ...
                    if (copy_head_cluster == 0) {
                        status = _fx_fault_tolerant_set_FAT_chain(
                            media_ptr, dont_use_fat_old, file_ptr->fx_file_current_physical_cluster, first_new_cluster,
                        ...
                        status = _fx_fault_tolerant_set_FAT_chain(media_ptr, dont_use_fat_old, insertion_front,
                                                                  first_new_cluster, copy_head_cluster,
                                                                  file_ptr->fx_file_last_physical_cluster + 1);
                    }
#endif /* FX_ENABLE_EXFAT */
                    else {

                        status = _fx_fault_tolerant_set_FAT_chain(media_ptr, dont_use_fat_old, insertion_front,
                                                                  first_new_cluster, copy_head_cluster, insertion_back);
                    }
            ...
            if (last_cluster) {
                ...
                if ((file_ptr->fx_file_current_relative_sector >= (media_ptr->fx_media_sectors_per_cluster - 1)) &&
                ...
                file_ptr->fx_file_current_logical_sector =
                    ...
                    (((ULONG64)(FAT_index - FX_FAT_ENTRY_START)) * ((ULONG)media_ptr->fx_media_sectors_per_cluster));
#ifdef FX_ENABLE_FAULT_TOLERANT
        ...
        if (file_ptr->fx_file_last_physical_cluster) {
            ...
            if (!(file_ptr->fx_file_dir_entry.fx_dir_entry_dont_use_fat & 1)) {
                ...
                if (insertion_front) {
                    ...
                    status = _fx_utility_FAT_entry_write(media_ptr, file_ptr->fx_file_last_physical_cluster,
                                                         first_new_cluster);
                }
#else
    ...
    /* Check for a need to increment to the next sector within a previously
       allocated cluster.  */
    if (file_ptr->fx_file_current_logical_offset >= media_ptr->fx_media_bytes_per_sector) {
    ...
    if (replace_clusters > 0) {
        ...
        file_ptr->fx_file_current_logical_sector =
            ((ULONG)media_ptr->fx_media_data_sector_start) +
            (((ULONG64)(first_new_cluster - FX_FAT_ENTRY_START)) * ((ULONG)media_ptr->fx_media_sectors_per_cluster)) +
            file_ptr->fx_file_current_relative_sector;
    ...
    while (bytes_remaining) {
        ...
        if ((file_ptr->fx_file_current_logical_offset) || (bytes_remaining < media_ptr->fx_media_bytes_per_sector)) {
            ...
            if (replace_clusters > 0) {
                if (file_ptr->fx_file_current_physical_cluster == first_new_cluster) {
                    ...
                    /* It's at beginning. */
                    status =
                        _fx_utility_logical_sector_read(media_ptr,
                                                        ((ULONG)media_ptr->fx_media_data_sector_start) +
                                                            (((ULONG)(copy_head_cluster - FX_FAT_ENTRY_START)) *
                                                             ((ULONG)media_ptr->fx_media_sectors_per_cluster)) +
                                                            file_ptr->fx_file_current_relative_sector,
                    ...
                    /* It's at ending. */
                    status =
                        _fx_utility_logical_sector_read(media_ptr, file_ptr->fx_file_current_logical_sector,
                                                        media_ptr->fx_media_memory_buffer, ((ULONG)1), FX_DATA_SECTOR);
                }
            } else
#endif /* FX_ENABLE_FAULT_TOLERANT */
            ...
            copy_bytes = media_ptr->fx_media_bytes_per_sector - file_ptr->fx_file_current_logical_offset;
            ...
            _fx_utility_memory_copy(source_ptr,
                                    ((UCHAR *)media_ptr->fx_media_memory_buffer) + /* Use case of memcpy is verified. */
            ...
            file_ptr->fx_file_current_logical_offset = file_ptr->fx_file_current_logical_offset + copy_bytes;
            ...
            for (i = (media_ptr->fx_media_sectors_per_cluster - file_ptr->fx_file_current_relative_sector); i < sectors;
            ...
            status = _fx_utility_logical_sector_write(media_ptr, file_ptr->fx_file_current_logical_sector, source_ptr,
            ...
            file_ptr->fx_file_current_logical_sector = file_ptr->fx_file_current_logical_sector + (sectors - 1);

            /* Move the relative cluster and sector as well.  */
            file_ptr->fx_file_current_relative_cluster =
                file_ptr->fx_file_current_relative_cluster +
                (file_ptr->fx_file_current_relative_sector + (sectors - 1)) / media_ptr->fx_media_sectors_per_cluster;
            ...
            bytes_remaining = bytes_remaining - (((ULONG)media_ptr->fx_media_bytes_per_sector) * sectors);

            /* Adjust the pointer to the source buffer.  */
            source_ptr = source_ptr + (((ULONG)media_ptr->fx_media_bytes_per_sector) * sectors);
        }
        ...
        if (file_ptr->fx_file_current_logical_offset >= media_ptr->fx_media_bytes_per_sector) {
            ...
            if ((bytes_remaining == 0) &&
                ((file_ptr->fx_file_current_file_offset + size) >= file_ptr->fx_file_current_available_size)) {
            ...
            if (file_ptr->fx_file_current_relative_sector >= media_ptr->fx_media_sectors_per_cluster) {
                ...
                if (file_ptr->fx_file_dir_entry.fx_dir_entry_dont_use_fat & 1) {
                    ...
                    /* Read the FAT entry of the current cluster to find
                       the next cluster.  */
                    status = _fx_utility_FAT_entry_read(media_ptr, file_ptr->fx_file_current_physical_cluster,
                                                        &next_cluster);
    ...
    file_ptr->fx_file_current_file_offset = file_ptr->fx_file_current_file_offset + size;
