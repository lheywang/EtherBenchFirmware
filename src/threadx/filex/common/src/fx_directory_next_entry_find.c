...
UINT _fx_directory_next_entry_find(FX_MEDIA *media_ptr, CHAR *directory_name) {
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_DIRECTORY_NEXT_ENTRY_FIND, media_ptr, directory_name, 0, 0,
    ...
    while (path_ptr->fx_path_current_entry < directory_size) {
        ...
        temp_status = _fx_directory_entry_read(media_ptr, search_dir_ptr, &(path_ptr->fx_path_current_entry), &entry);
