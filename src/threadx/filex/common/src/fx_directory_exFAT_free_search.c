...
UINT _fx_directory_exFAT_free_search(FX_MEDIA *media_ptr, FX_DIR_ENTRY *directory_ptr, FX_DIR_ENTRY *entry_ptr) {
    ...
    if (directory_entries > 0) {
        do {
            ...
            status = _fx_directory_exFAT_entry_read(media_ptr, search_dir_ptr, &i, entry_ptr, 0, FX_TRUE, NULL, NULL);
    ...
    if (media_ptr->fx_media_available_clusters) {
        ...
        status = _fx_utility_logical_sector_read(media_ptr, logical_sector, media_ptr->fx_media_memory_buffer,
