...
UINT _fx_directory_next_full_entry_find(FX_MEDIA *media_ptr, CHAR *directory_name, UINT *attributes, ULONG *size,
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_DIRECTORY_NEXT_FULL_ENTRY_FIND, media_ptr, directory_name, 0, 0,
...
#ifdef FX_ENABLE_EXFAT
        if (media_ptr->fx_media_FAT_type == FX_exFAT) {
            temp_status = _fx_directory_exFAT_entry_read(media_ptr, search_dir_ptr, &(path_ptr->fx_path_current_entry),
                                                         &entry, 0, FX_FALSE, unicode_name, &unicode_len);
        } else {
#endif /* FX_ENABLE_EXFAT */
            temp_status =
                _fx_directory_entry_read(media_ptr, search_dir_ptr, &(path_ptr->fx_path_current_entry), &entry);
#ifdef FX_ENABLE_EXFAT
        }
#endif /* FX_ENABLE_EXFAT */
