...
UINT _fx_file_extended_truncate(FX_FILE *file_ptr, ULONG64 size) {
    ...
    FX_TRACE_IN_LINE_INSERT(FX_TRACE_FILE_TRUNCATE, file_ptr, size, file_ptr->fx_file_current_file_size, 0,
    ...
    if (size < file_ptr->fx_file_current_file_size) {
        ...
        /* Update the trace event with the truncated size.  */
        FX_TRACE_EVENT_UPDATE(trace_event, trace_timestamp, FX_TRACE_FILE_TRUNCATE, 0, 0, 0,
