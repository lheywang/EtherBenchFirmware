...
UINT _fx_directory_exFAT_entry_read(FX_MEDIA *media_ptr, FX_DIR_ENTRY *source_dir, ULONG *entry_ptr,
    ...
    if (destination_ptr->fx_dir_entry_type != FX_EXFAT_DIR_ENTRY_TYPE_FILE_DIRECTORY) {
        ...
        if ((destination_ptr->fx_dir_entry_type &
             (FX_EXFAT_ENTRY_TYPE_IMPORTANCE_MASK | FX_EXFAT_ENTRY_TYPE_CATEGORY_MASK)) == 0) {
    ...
    destination_ptr->fx_dir_entry_attributes =
        (UCHAR)_fx_utility_16_unsigned_read(read_ptr); /* The same order as FAT32 but more unused bits.  */
    ...
    for (i = 0; i < secondary_count; ++i) {
        ...
        if (byte_offset + FX_DIR_ENTRY_SIZE >= media_ptr->fx_media_bytes_per_sector) {
            ...
            /* Read the new sector.  */
            status =
                _fx_utility_logical_sector_read(media_ptr, ((ULONG64)logical_sector), media_ptr->fx_media_memory_buffer,
                                                ((ULONG)1), FX_DIRECTORY_SECTOR);
        ...
        if (i == 0) {
            ...
            if (source_dir) {
                ...
                dont_use_fat =
                    (UCHAR)(dont_use_fat | ((source_dir->fx_dir_entry_dont_use_fat & 1) << 1)); /* 1st bit parent */
            }
            ...
            } else if (((*read_ptr & (FX_EXFAT_ENTRY_TYPE_IN_USE_MASK | FX_EXFAT_ENTRY_TYPE_IMPORTANCE_MASK |
