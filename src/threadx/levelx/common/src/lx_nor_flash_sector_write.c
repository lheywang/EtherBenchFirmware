...
UINT _lx_nor_flash_sector_write(LX_NOR_FLASH *nor_flash, ULONG logical_sector, VOID *buffer) {
    ...
    if (new_mapping_address) {
         ...
         */
        new_mapping_entry = ((ULONG)LX_NOR_PHYSICAL_SECTOR_VALID) | ((ULONG)LX_NOR_PHYSICAL_SECTOR_SUPERCEDED) |
        ...
        if (old_mapping_address) {
            ...
            block = (ULONG)(old_mapping_address - nor_flash->lx_nor_flash_base_address) /
