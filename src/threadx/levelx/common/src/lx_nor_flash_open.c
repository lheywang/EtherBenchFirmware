...
UINT _lx_nor_flash_open(LX_NOR_FLASH *nor_flash, CHAR *name, UINT (*nor_driver_initialize)(LX_NOR_FLASH *)) {
    ...
    LX_MEMSET(nor_flash, 0,
              (ULONG)((UCHAR *)&(nor_flash->lx_nor_flash_open_previous) - (UCHAR *)nor_flash) +
    ...
    if (min_erased_count == LX_ALL_ONES) {
        ...
        for (l = 0; l < nor_flash->lx_nor_flash_total_blocks; l++) {
            ...
            status = _lx_nor_flash_driver_write(
                nor_flash, block_word_ptr + (nor_flash->lx_nor_flash_block_free_bit_map_offset + (bit_map_words - 1)),
                &bit_map_mask, 1);
        ...
        for (l = 0; l < nor_flash->lx_nor_flash_total_blocks; l++) {
            ...
            if (((block_word & LX_BLOCK_ERASED) == LX_BLOCK_ERASED) || (block_word == LX_BLOCK_ERASE_STARTED)) {
                ...
                status = _lx_nor_flash_driver_write(
                    nor_flash,
                    block_word_ptr + (nor_flash->lx_nor_flash_block_free_bit_map_offset + (bit_map_words - 1)),
                    &bit_map_mask, 1);
                ...
                for (j = 0; j < bit_map_words; j++) {
                    ...
                    status = _lx_nor_flash_driver_read(
                        nor_flash, (block_word_ptr + nor_flash->lx_nor_flash_block_free_bit_map_offset + j),
                        &block_word, 1);
                ...
                for (j = 0; j < sectors_per_block; j++) {
...
#else
                    status = _lx_nor_flash_driver_read(
                        nor_flash, (block_word_ptr + nor_flash->lx_nor_flash_block_physical_sector_mapping_offset + j),
                        &block_word, 1);
                    ...
                    if (used_sectors) {
                        ...
                        if ((block_word & LX_NOR_LOGICAL_SECTOR_MASK) != LX_NOR_LOGICAL_SECTOR_MASK) {
                            ...
                            if ((block_word & LX_NOR_PHYSICAL_SECTOR_VALID) &&
                                ...
                                /* Yes, this block was about to become obsolete. Perform a
                                   ...
                                   set.  */
                                _lx_nor_flash_logical_sector_find(nor_flash, (block_word & LX_NOR_LOGICAL_SECTOR_MASK),
                                ...
                                if (new_map_entry) {
                                    ...
                                    status = _lx_nor_flash_driver_write(
                                        nor_flash,
                                        (block_word_ptr + nor_flash->lx_nor_flash_block_physical_sector_mapping_offset +
                                         j),
                                        &block_word, 1);
                        ...
                        /* Determine if the sector is free.  */
                            ...
                            status = _lx_nor_flash_driver_write(
                                nor_flash,
                                (block_word_ptr + nor_flash->lx_nor_flash_block_physical_sector_mapping_offset + j),
                                &block_word, 1);
                        ...
                        sector_word_ptr = block_word_ptr + (nor_flash->lx_nor_flash_block_physical_sector_offset) +
