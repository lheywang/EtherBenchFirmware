    ...
    if (block != LX_NAND_BLOCK_UNMAPPED) {
        ...
        if (block_status & LX_NAND_BLOCK_STATUS_NON_SEQUENTIAL) {
            ...
            for (page = (LONG)available_pages - 1; page >= 0; page--) {
                ...
                status = (nand_flash->lx_nand_flash_driver_pages_read)(nand_flash, block, (ULONG)page, LX_NULL,
                                                                       spare_buffer_ptr, 1);
#else
                status =
                    (nand_flash->lx_nand_flash_driver_pages_read)(block, (ULONG)page, LX_NULL, spare_buffer_ptr, 1);
#endif
                ...
                if ((LX_UTILITY_LONG_GET(&spare_buffer_ptr[nand_flash->lx_nand_flash_spare_data1_offset]) &
                    ...
                    if ((LX_UTILITY_LONG_GET(&spare_buffer_ptr[nand_flash->lx_nand_flash_spare_data1_offset]) &
                         (~LX_NAND_PAGE_TYPE_USER_DATA_MASK)) == (LX_NAND_PAGE_TYPE_USER_DATA)) {
        ...
        if (release_sector) {
            ...
            if (block_status & LX_NAND_BLOCK_STATUS_FULL) {
                ...
                status = _lx_nand_flash_data_page_copy(
                    nand_flash, logical_sector - (logical_sector % nand_flash->lx_nand_flash_pages_per_block), block,
                ...
                if (logical_sector % nand_flash->lx_nand_flash_pages_per_block <
                    ...
                    /* Copy valid sector to new block.  */
                    status = _lx_nand_flash_data_page_copy(
                        nand_flash, logical_sector + 1, block, block_status, new_block, &new_block_status,
                        (nand_flash->lx_nand_flash_pages_per_block - 1) -
                            (logical_sector % nand_flash->lx_nand_flash_pages_per_block));
                ...
                status = _lx_nand_flash_erase_count_set(
                    nand_flash, block, (UCHAR)(nand_flash->lx_nand_flash_erase_count_table[block] + 1));
                ...
                /* Write the page.  */
#ifdef LX_NAND_ENABLE_CONTROL_BLOCK_FOR_DRIVER_INTERFACE
                status = (nand_flash->lx_nand_flash_driver_pages_write)(nand_flash, block, available_pages,
                                                                        (UCHAR *)nand_flash->lx_nand_flash_page_buffer,
                                                                        spare_buffer_ptr, 1);
#else
                status = (nand_flash->lx_nand_flash_driver_pages_write)(
                    block, available_pages, (UCHAR *)nand_flash->lx_nand_flash_page_buffer, spare_buffer_ptr, 1);
#endif
