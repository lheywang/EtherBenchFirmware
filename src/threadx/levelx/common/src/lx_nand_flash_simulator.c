...
UINT _lx_nand_flash_simulator_extra_bytes_get(LX_NAND_FLASH *nand_flash, ULONG block, ULONG page, UCHAR *destination,
                                              UINT size);
UINT _lx_nand_flash_simulator_extra_bytes_set(LX_NAND_FLASH *nand_flash, ULONG block, ULONG page, UCHAR *source,
                                              UINT size);
UINT _lx_nand_flash_simulator_system_error(LX_NAND_FLASH *nand_flash, UINT error_code, ULONG block, ULONG page);

UINT _lx_nand_flash_simulator_pages_read(LX_NAND_FLASH *nand_flash, ULONG block, ULONG page, UCHAR *main_buffer,
                                         UCHAR *spare_buffer, ULONG pages);
UINT _lx_nand_flash_simulator_pages_write(LX_NAND_FLASH *nand_flash, ULONG block, ULONG page, UCHAR *main_buffer,
                                          UCHAR *spare_buffer, ULONG pages);
UINT _lx_nand_flash_simulator_pages_copy(LX_NAND_FLASH *nand_flash, ULONG source_block, ULONG source_page,
...
UINT _lx_nand_flash_simulator_pages_write(ULONG block, ULONG page, UCHAR *main_buffer, UCHAR *spare_buffer,
                                          ULONG pages);
UINT _lx_nand_flash_simulator_pages_copy(ULONG source_block, ULONG source_page, ULONG destination_block,
...
UINT _lx_nand_flash_simulator_page_ecc_check(ULONG block, ULONG page) {
    ...
    for (i = 0; i < BYTES_PER_PHYSICAL_PAGE; i += 256) {
        status = lx_nand_flash_256byte_ecc_check(
            (UCHAR *)&nand_memory_area[block].physical_pages[page].memory[i / sizeof(ULONG)],
...
#endif
{
    ...
    for (i = 0; i < pages; i++) {
...
#ifdef LX_NAND_ENABLE_CONTROL_BLOCK_FOR_DRIVER_INTERFACE
            status = _lx_nand_flash_simulator_read(nand_flash, block, page + i,
                                                   (ULONG *)(main_buffer + i * BYTES_PER_PHYSICAL_PAGE),
                                                   WORDS_PER_PHYSICAL_PAGE);
#else
            status = _lx_nand_flash_simulator_read(
                block, page + i, (ULONG *)(main_buffer + i * BYTES_PER_PHYSICAL_PAGE), WORDS_PER_PHYSICAL_PAGE);
#endif
        ...
        status = _lx_nand_flash_simulator_extra_bytes_get(
            nand_flash, block, page + i, spare_buffer + i * SPARE_BYTES_PER_PAGE, SPARE_BYTES_PER_PAGE);
#else
        status = _lx_nand_flash_simulator_extra_bytes_get(block, page + i, spare_buffer + i * SPARE_BYTES_PER_PAGE,
...
#endif
{
    ...
    for (i = 0; i < pages; i++) {
#ifdef LX_NAND_ENABLE_CONTROL_BLOCK_FOR_DRIVER_INTERFACE
        _lx_nand_flash_simulator_extra_bytes_set(nand_flash, block, page + i, spare_buffer + i * SPARE_BYTES_PER_PAGE,
                                                 SPARE_BYTES_PER_PAGE);
        status = _lx_nand_flash_simulator_write(
            nand_flash, block, page + i, (ULONG *)(main_buffer + i * BYTES_PER_PHYSICAL_PAGE), WORDS_PER_PHYSICAL_PAGE);
#else
        _lx_nand_flash_simulator_extra_bytes_set(block, page + i, spare_buffer + i * SPARE_BYTES_PER_PAGE,
                                                 SPARE_BYTES_PER_PAGE);
        status = _lx_nand_flash_simulator_write(block, page + i, (ULONG *)(main_buffer + i * BYTES_PER_PHYSICAL_PAGE),
                                                WORDS_PER_PHYSICAL_PAGE);
#endif
...
#endif
{
    ...
    for (i = 0; i < pages; i++) {
#ifdef LX_NAND_ENABLE_CONTROL_BLOCK_FOR_DRIVER_INTERFACE
        status = _lx_nand_flash_simulator_pages_read(nand_flash, source_block, source_page + i, data_buffer,
                                                     data_buffer + BYTES_PER_PHYSICAL_PAGE, 1);
#else
        status = _lx_nand_flash_simulator_pages_read(source_block, source_page + i, data_buffer,
                                                     data_buffer + BYTES_PER_PHYSICAL_PAGE, 1);
#endif
...
#ifdef LX_NAND_ENABLE_CONTROL_BLOCK_FOR_DRIVER_INTERFACE
        status = _lx_nand_flash_simulator_pages_write(nand_flash, destination_block, destination_page + i, data_buffer,
                                                      data_buffer + BYTES_PER_PHYSICAL_PAGE, 1);
#else
        status = _lx_nand_flash_simulator_pages_write(destination_block, destination_page + i, data_buffer,
                                                      data_buffer + BYTES_PER_PHYSICAL_PAGE, 1);
#endif
